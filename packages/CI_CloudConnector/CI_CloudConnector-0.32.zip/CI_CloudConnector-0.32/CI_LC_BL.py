...
def setClock(newDate):
    ...
    try:
        ...
        serverTime = datetime.strptime(newDate, '%Y-%m-%d %H:%M:%S')
        #print 'serverTime=' + str(serverTime)
        now = datetime.now()                
        diff = (now - serverTime).total_seconds()
        #print 'differencr from now=' + str(diff)
        if diff>3:
            ci_print('setting new system time to' + str(serverTime) + ' Currently' + str(now))
            os.system('sudo date --set="%s"' % newDate)
    except Exception as inst:
