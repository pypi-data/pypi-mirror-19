...
class DKCloudCommandRunner(object):
    ...
    def merge_kitchens_improved(dk_api, from_kitchen, to_kitchen):
        ...
        if merge_no_conflicts:
            ...
            if current_kitchen == to_kitchen:
                # We just merged into the kitchen in which we are located. Force an update of the files from the repo
                update_local_rc = DKCloudCommandRunner.update_local_files()
                msg += update_local_rc.get_message()
            md.set_message(msg)
        else:
            # Found conflicts
            recipe_name = DKRecipeDisk.find_recipe_name()
    ...
    @staticmethod
    def update_local_files(kitchen_dir=None):
        rc = DKReturnCode()
        rc.set(rc.DK_SUCCESS,
               'If this code was complete, I would have updated the local files in the kitchen at %s' % kitchen_dir)
        return rc
