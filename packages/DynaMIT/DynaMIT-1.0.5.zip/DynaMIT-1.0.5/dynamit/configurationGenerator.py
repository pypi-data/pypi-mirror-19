...
class DynamitConfigurationGenerator(tk.Frame):
	...
	def __createMenu(self):
		...
		self.newMenu.add_command(label="ChIP", command=lambda: self.newConfig("ChIP"))
	...
	def newConfig(self, configType):
		...
		if configType == "empty":
			...
			printerLine = "ResultsPrinter    dynamit.positionalDensityPrinter." \
										"PositionalDensityPrinter        "
			self.generatedConfigListBox.insert(tk.END, printerLine)
		elif configType == "ChIP":
			self.resetConfig()
			self.resetTextBoxes()

			# add a HOMER and MDscan Searchers.
			searcherLine = "MotifSearcher    dynamit.homerSearcher.HOMERSearcher    " + \
									"/home/dynamit/software/homer/bin    bg.fasta,"
			self.generatedConfigListBox.insert(tk.END, searcherLine)
			searcherLine = "MotifSearcher    dynamit.mdScanSearcher.MDscanSearcher" + \
									 "    /home/dynamit/software/MDscan    "
			self.generatedConfigListBox.insert(tk.END, searcherLine)
