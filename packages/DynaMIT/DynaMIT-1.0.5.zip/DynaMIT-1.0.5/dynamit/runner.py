...
class Runner(object):
	...
	def run(self, outputFolderName, sequencesFilename, nproc=1, polisherDist=-1):
		...
		if self.integrationResults != 1:
			...
			if (polisherDist > 0) and \
				...
				for i in xrange(0, len(self.searchesResults)):
					...
					foundOverlap = 0
					iRange = range(int(iMatch[4]), int(iMatch[5]))
					for j in xrange(0, len(self.searchesResults)):
						...
						if (i != j) and ("".join(iMatch) != "".join(jMatch)):
							jRange = range(int(jMatch[4]), int(jMatch[5]))
				...
				print "[FINISHED] Motif cluster polishing completed successfully.\n"
		else:
