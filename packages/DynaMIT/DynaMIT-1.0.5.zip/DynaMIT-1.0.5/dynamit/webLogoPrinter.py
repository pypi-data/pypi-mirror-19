	...
	def printResults(self, sequences, searchResults,
		...
		if (integrationResults.has_key("clustersMultipleAlignments") and
			...
			for i, multAlign in enumerate(integrationResults["clustersMultipleAlignments"]):
				...
				# generate the JASPAR-style matrix from the logo and save it into the
				# printer results folder
				self._printJASPARmatrixFromWebLogos(os.path.join(os.getcwd(),
											 "Cluster_" + str(i) + "_WebLogo.txt"),
											 os.path.join(os.getcwd(),
											 "Cluster_" + str(i) + "_PFM.txt"))

			# report printer completion and inform the user of output file existence.
			print "  [WebLogoPrinter] Printing completed. " + \
						"PDF and text weblogos and JASPAR-style PFM matrix are stored in \n    <" + \
						os.path.join(os.getcwd(), "Cluster_<number>_WebLogo.pdf|txt") + \
						"> and <" + \
						os.path.join(os.getcwd(), "Cluster_<number>_PFM.txt") + ">."
		...
		# alignments, so print individual clusters weblogos (by instances).
			...
			for cluster in list(set(integrationResults["clustering"])):
				...
				# printer results folder
				self._printJASPARmatrixFromWebLogos(os.path.join(os.getcwd(),
											 "Cluster_" + str(cluster) + "_instances_WebLogo.txt"),
											 os.path.join(os.getcwd(),
											 "Cluster_" + str(cluster) + "_instances_PFM.txt"))

			# report printer completion and inform the user of output file existence.
			print "  [WebLogoPrinter] Printing completed. " + \
						"PDF and text weblogos and JASPAR-style PFM matrix are stored in \n    <" + \
						os.path.join(os.getcwd(),
												 "Cluster_<number>_instances_WebLogo.pdf|txt") + \
						"> and <" + \
						os.path.join(os.getcwd(),
												 "Cluster_<number>_instances_PFM.txt") + ">."
		...
		# weblogos (by instances).
			...
			for motif in motifs:
				...
				# printer results folder
				self._printJASPARmatrixFromWebLogos(os.path.join(os.getcwd(),
											 motifInfo[0] + "_" + motifInfo[1] + "_instances_WebLogo.txt"),
											 os.path.join(os.getcwd(),
											 motifInfo[0] + "_" + motifInfo[1] + "_instances_PFM.txt"))

			# report printer completion and inform the user of output file existence.
			print "  [WebLogoPrinter] Printing completed. " + \
						"PDF and text weblogos and JASPAR-style PFM matrix are stored in \n    <" + \
						os.path.join(os.getcwd(),
												 "<motifName>_instances_WebLogo.pdf|txt") + \
						"> and <" + \
						os.path.join(os.getcwd(),
												 "<motifName>_instances_PFM.txt") + ">."

		return 0

	def _printJASPARmatrixFromWebLogos(self, textWebLogoFilename, outputMatrixFilename):
		""" Convert the WebLogo positional frequency matrix to the JASPAR format and save it to file.
		Args:
			textWebLogoFilename: input filename of the WebLogo matrix
			outputMatrixFilename: output filename for the JASPAR-style converted matrix.

		Returns:
			Returns 0 if everything went fine (details on results filenames, etc.,
			are printed to the console); returns 1 and an error message otherwise.
		"""
		with open(textWebLogoFilename) as inHandle, open(outputMatrixFilename, "wb") as outHandle:
			mat = [l.split(' \t')[1:5] for l in inHandle.readlines() if not l.startswith("#")]
			transposedMat = [["A"], ["C"], ["G"], ["T"]]
			for col in mat:
				transposedMat[0].append(col[0])
				transposedMat[1].append(col[1])
				transposedMat[2].append(col[2])
				transposedMat[3].append(col[3])
			for tMatRow in transposedMat:
				outHandle.write("\t".join([tMatRow[0], "[", "\t".join(tMatRow[1:]), "]"]) + "\n")
		return 0
