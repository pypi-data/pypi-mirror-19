...
class BranchOperator(object):
    ...
    def updateCurrentBranch(self):
        currentBranch = self.commandexecutor.getCurrentBranch()
        team = ''
        if 'feature' in currentBranch:

            if 'foxhound' in currentBranch:
                team = 'foxhound'
            elif 'demeter'in currentBranch:
                team = 'demeter'
            else:
                self.printer.showError('This branch name is not structured correctly for GBRestructure')
                sys.exit()

            self.commandexecutor.updateCurrentBranch(team, currentBranch)
        else: 
            self.printer.showError("Update only works on feature branches")
