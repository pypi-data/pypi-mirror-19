...
def _expand(rules, scope, settings):
    ...
    for element in scope:
        ...
        if not isinstance(element, int):
            rv['__src_%s' % element] = settings
            rv['__lnk_%s' % element] = settings
            continue
        ...
        if element & 0b0101 == 0b0101:
            for rule in rules:
                rv['__lnk_%s' % rule] = settings

        if element & 0b1001 == 0b1001:
            for rule in rules:
                rv['__src_%s' % rule] = settings

        if element & 0b0110 == 0b0110:
            rv['__dest'] = settings

    return tuple(rv.items())
    ...
    def __new__(mcs, name, bases, d):
        ...
        for name, func in inspect.getmembers(cls, inspect.isfunction):
            ...
            if name in HTTP_METHODS:
                available_methods.add(name.upper())

            scope, *settings = getattr(func, '__is_checkpoint', ((),))
            for scope, (priority, methods) in _expand(rules, scope, settings):
                checkpoints[scope].append((priority, name, methods))

            scope, *settings = getattr(func, '__is_filter', ((),))
            for scope, (key, requires) in _expand(rules, scope, settings):
                filters[scope].append((key, name, requires))

        # flatten and sort checkpoints = cp[scope] = [(func, methods), ...]
        cls._checkpoints = {
            s: tuple(sorted(set(cp))) for s, cp in checkpoints.items()}
        cls._filters = {s: tuple(set(f)) for s, f in filters.items()}
        cls.methods = cls.methods or tuple(available_methods)
...
class Provider(metaclass=_ProviderType):
    ...
    * Decorative method :
    ...
    def _execute_checkpoints(self, strategy, verb, match_info):
        ...
        for _, name, verbs in checkpoints:
