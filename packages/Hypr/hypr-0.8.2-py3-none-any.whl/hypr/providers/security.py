...
def filter(scope=ALWAYS, key=None, requires=None):
    """Decorate a provider method to make it a filter method."""
    if hasattr(scope, '__call__'):  # the decorator is used with default values
        func = scope
        setattr(func, '__is_filter', ((ALWAYS,), func.__name__, requires))
        return func
    ...
    def decorator(func):
        ...
        for desc in scope:
            ...
            # TODO: assess necessity of handle other scope values
            if not (desc % 4 and desc >> 2):
                raise ScopeError('Conflicting scope detected.', func)

        setattr(func, '__is_filter', (scope, key or func.__name__, requires))
        return func
