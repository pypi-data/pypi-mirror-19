...
class Repository(github.GithubObject.CompletableGithubObject):
    ...
    def get_release_asset(self, id):
        ...
        headers, resp_headers, data = self._requester.requestBlob(
            "GET",
    ...
    # todo: determine if these methods stay only on GitRelease
    # def get_release_assets(self, release_id):
    #     assert isinstance(release_id, (int)), release_id
    #
    #     return github.PaginatedList.PaginatedList(
    #         github.GitReleaseAsset.GitReleaseAsset,
    #         self._requester,
    #         self.url + "/releases/" + str(release_id) + "/assets",
    #         None
    #     )
...
if __name__ == "__main__":
    ...
    kssgit = Github("317ee7e6e88a9cedba48f771f14e9df66192fe20", base_url="https://kss-github.kaplan.com/api/v3")

    repo_name = "Kaplan/self-enrollment-SANDBOX"
    repo = kssgit.get_repo(repo_name)

    from uuid import uuid4

    release_name = "v1.chris." + str(uuid4())
    the_release = repo.create_git_tag_and_release(release_name, "Some tag message", "SEP " + release_name,
                                                  "Are these release notes?",
                                                  "c00b7656c4ae7b53c8f6e4b8c388955072b9d25c", "commit")
    print the_release.upload_url

    print the_release.title
    the_asset = the_release.upload_asset("/Users/chris/Archive.zip", label="Iamareleaseasset")
    print the_asset.name

    the_asset = the_asset.update_asset("archive-name-edited.zip",
                                       "i am an edited short description to add additional info beyond the file name.")
    print the_asset.name
    print the_asset.id

    r = repo.get_release(the_release.id)
    print r

    l = the_release.get_assets()
    for x in l:
        print x

    a = repo.get_release_asset(the_asset.id)
    print a

    is_deleted = the_asset.delete_asset()
    print is_deleted
