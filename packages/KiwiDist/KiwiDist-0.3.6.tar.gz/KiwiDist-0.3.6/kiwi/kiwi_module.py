...
def plot(gsn, gss, gsc='', gls='', pc=0.01, adj=True, gsz=10**6,
    ...
    import os
    import kiwi.classes as kiwiC
    import kiwi.functions as kiwiF
    import warnings
    ...
    content = np.genfromtxt(GSfile,dtype=str,delimiter='\t')
    header = content[0]
    ...
    G = kiwiC.Genome()
    if len(Gfile)>0:
        content = np.genfromtxt(Gfile,dtype=str,delimiter='\t')
        header = content[0]
