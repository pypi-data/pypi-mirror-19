...
def run_test(application, using, rds):
    ...
    if test_app != 'behave':
        ...
        for line in env_data:
            if not line or line.startswith("#") or line.startswith(" ") or line.startswith("DATABASE"):
                continue
        ...
        command = "docker run -d --privileged -e DATABASE_HOST={host} -e DATABASE_PORT={port} {args} lidocker_{name}".format(
                        host=host,
                        port=port,
                        args=env_args,
                        name=name)
...
def rebuild_docker(no_confirm):
    ...
    if no_confirm:
        ...
        resp = confirma(
            ...
            "que todas as alterações importantes estejam commitadas.\033[0m\n\n"
            u"Deseja continuar")
