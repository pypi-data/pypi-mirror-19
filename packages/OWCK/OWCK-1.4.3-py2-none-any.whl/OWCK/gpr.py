...
#        pdb.set_trace()
        ...
        if self.optimizer == 'fmin_cobyla':
            ...
            def fitnessfunc(theta):
                theta = theta.T
                return np.array([self.reduced_likelihood_function(theta=__)[0] \
                    for __ in theta])
            
            dim = self.X.shape[1]
            x_lb = np.atleast_2d(self.thetaL)
            x_ub = np.atleast_2d(self.thetaU)
            ...
            init_wcm = np.random.rand(1, dim) * (x_ub - x_lb) + x_lb
            
            optimizer = cma_es(dim, init_wcm, fitnessfunc, opt, 
                               is_minimize=False)
            xopt, fopt, evalcount, _ = optimizer.optimize()
            optimal_theta = xopt.flatten()
            optimal_rlf_value, optimal_par = \
                    self.reduced_likelihood_function(theta=optimal_theta)
        else:
