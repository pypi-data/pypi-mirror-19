...
class GaussianProcess_extra(GaussianProcess):
    ...
    def __init__(self, regr='constant', corr='squared_exponential', 
                 beta0=None, storage_mode='full', verbose=False, 
                 theta0=1e-1, thetaL=None, thetaU=None, 
                 optimizer='fmin_cobyla', random_start=1, normalize=True,
                 nugget=10. * MACHINE_EPSILON, random_state=None):
                     
        super(GaussianProcess_HAO, self).__init__(regr, corr, beta0,
                 storage_mode, verbose, theta0, thetaL, thetaU, optimizer,
    ...
    def corr_dx(self, x, X=None, theta=None, r=None, nu=1.5):
        ...
        if self.corr_type == 'squared_exponential':
            ...
            if nu == 0.5:
                ...
                grad = diff * theta / D
                grad *= c * exp(-c * D) - c * (1. + c*D) * exp(-c * D)
            elif nu == 2.5:
                pass
    ...
    def predict(self, X, eval_MSE=False, eval_cov=False, eval_rt=True,
                eval_r=True, batch_size=None):
        """
        ...
        if batch_size is None:
            ...
            if eval_MSE or eval_cov:
                ...
                res += [rt] if eval_rt else []
                res += [r] if eval_r else []
...
if __name__ == '__main__':
    ...
    model = GaussianProcess_HAO(corr='matern', theta0=[1e-1, 5*1e-1], normalize=False)
