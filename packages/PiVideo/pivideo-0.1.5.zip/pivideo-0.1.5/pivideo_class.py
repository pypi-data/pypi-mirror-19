...
#  Copyright(c) 2016 by Lintest Systems LLC
#
######################################################################
#
#  Revision History
...
#	version				Report the software version
#	id					Report the PiConfig hardware ID
#	update				Update the PiVideo processor software
...
# version
	...
	version = property(_get_version,_set_version, doc="""
		Software version of the video processor
		""" )
	...
	def _open_serial_port(self,port_name):
		for speed in self.serial_baudrates:
			try:
				...
				try:
					if ((self._read_serial_video_register(3) & 0x80) > 0):
						try:
	...
	def _read_serial_video_register(self,address):
		...
		while (retrycount < 4):
			try:
				...
				if (len(value)<1):
					raise PiVideoException("Video processor communication error (is something else using the serial port?)")	
				retrycount=10
				return ord(value)
			except IOError:
				retrycount+=1
