...
class ContentManagementApi(object):
    ...
    def get_workspaces_workspace_id_documents(self, workspace_id, **kwargs):
        """
        Get a list of documents.
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_workspaces_workspace_id_documents(workspace_id, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_workspaces_workspace_id_documents" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        # verify the required parameter 'workspace_id' is set
        if ('workspace_id' not in params) or (params['workspace_id'] is None):
            raise ValueError("Missing the required parameter `workspace_id` when calling `get_workspaces_workspace_id_documents`")
