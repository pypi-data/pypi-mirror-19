...
def test_manager():
    ...
    manager = p.Manager(p.SqliteDatabase)
    proxy = manager.proxy()
    ...
    with manager.using(dbfile):
        Person.create_table()
        Notification.create_table()

        assert Person.meta_database() == manager.connection(dbfile)
    ...
    analyser = manager.analyse(dbfile)
    ...
    assert len(analyser.statements) == 8

    for name, table, item in analyser.statements:
