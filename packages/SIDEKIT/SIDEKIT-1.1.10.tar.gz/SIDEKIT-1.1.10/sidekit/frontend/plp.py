...
def fft2barkmx(nfft, sr, nfilts=0, width=1., minfreq=0., maxfreq=8000):
    ...
    binbarks = hz2bark(numpy.arange(nfft / 2 + 1) * sr / nfft)
    ...
    for i in range(nfilts):
        ...
        #LIGNE A VERIFIER...
        wts[i, :nfft//2+1] = 10**(numpy.minimum(numpy.zeros_like(hif), numpy.minimum(hif, -2.5*lof) / width))
...
def audspec(pspectrum,
    ...
    """
    if nfilts is None:
        nfilts = int(numpy.ceil(hz2bark(sr / 2)) + 1)
    if not sr == 16000:
        maxfreq = min(sr / 2, maxfreq)
...
def dolpc(x, model_order=8):
    ...
    # REGARDER: http://thomas-cokelaer.info/software/spectrum/html/user/ref_others.html
    #           audiolazy
...
% modelorder is order of PLP model, defaults to 8.  0 -> no PLP
    ...
    nbands = aspectrum.shape[1]

    if rasta:
        # put in log domain
        nl_aspectrum = numpy.log(aspectrum)
    ...
    if model_order > 0:
        ...
        # LPC analysis
        lpcas = dolpc(postspectrum, model_order)[0]
