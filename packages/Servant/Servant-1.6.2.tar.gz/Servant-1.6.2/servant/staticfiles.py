...
class StaticFileRoute(Route):
    ...
    """
    def __init__(self, prefix, path, cache_control=None, route_keywords=None):
        """
        prefix
          The URL prefix.
        ...
        cache_control
           The value to set for the cache-control header.  If not provided the header
           is not added.
        """
    ...
    def __call__(self, match, ctx):
        ...
        e = USE_CACHE and self.cache.get(relpath)
        if not e:
            e = self._getfile(relpath)

            if USE_CACHE:
                self.cache[relpath] = e
...
def serve_prefix(prefix, path, cache_control=None, **route_keywords):
    """
    ...
    route = StaticFileRoute(prefix, path, cache_control=cache_control, route_keywords=route_keywords)
