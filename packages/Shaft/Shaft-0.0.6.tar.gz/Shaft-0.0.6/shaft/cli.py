...
def copy_resource(src, dest):
    ...
    if pkg_resources.resource_isdir("shaft", src):
...
def create_project(project_name, no_assets=True, no_installed_apps=True):
    ...
    app_tpl = pkg_resources.resource_string(__name__, '%s/application.py' % (SKELETON_DIR))
    cli_tpl = pkg_resources.resource_string(__name__, '%s/cli.py' % (SKELETON_DIR))
    ...
    application_file = "%s/application.py" % CWD
    cli_file = "%s/cli.py" % CWD
    ...
    files = [
        ("%s/__init__.py" % project_dir, "# Shaft"),
        (init_py, init_tpl),
        (cli_file, cli_tpl),
        (config_py, config_tpl),
        (model_py, model_tpl),
        (application_file, app_tpl.format(project_name=project_name)),
    ...
    copy_resource("%s/var/" % SKELETON_DIR, APPLICATION_DATA_DIR)
...
def create(app_name, no_assets, no_installed_apps):
    ...
    print("")
    print("- Your new Shaft app [ %s ] has been created" % app)
    print("- Location: [ apps/%s ]" % app)
    print("")
    print("> What's next?")
    print("- Edit the config [ apps/config.py ] ")
    print("- If necessary edit and run the command [ shaft setup ]")
    print("- Launch app on development mode, run [ app='%s' shaft serve ]" % app)
    print("")
    print("*" * 80)
...
def cmd():
    ...
    if os.path.isfile(os.path.join(os.path.join(CWD, "application.py"))):
        cwd_to_sys_path()
        application = import_string("application")
    else:
        print("-" * 80)
        print("** Missing << 'application.py' >> @ %s" % CWD)
        print("-" * 80)
