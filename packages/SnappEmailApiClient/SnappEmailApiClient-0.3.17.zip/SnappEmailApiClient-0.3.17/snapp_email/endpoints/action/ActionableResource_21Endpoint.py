...
import json
from snapp_email.datacontract.classes import ActionableResource_21
from snapp_email.datacontract.utils import export_dict, fill


class ActionableResource_21Endpoint:
    def __init__(self, api_client):
        self.api_client = api_client
    
    def options(self, impersonate_user_id=None, accept_type=None):
        """
        Retrieve options available for resource 'ActionableResource_21'.
        ...
        add_headers = {
            'Content-Type': 'application/vnd.4thoffice.actionable.resource-v5.17+json',
            'Accept': 'application/vnd.4thoffice.actionable.resource-v5.17+json' if accept_type is None else accept_type,
        }
        response = self.api_client.api_call('options', endpoint, url_parameters, add_headers, impersonate_user_id=impersonate_user_id)
        
        return fill(ActionableResource_21, response.json(), content_type=response.headers['Content-Type'])
    ...
    def create(self, obj, impersonate_user_id=None, accept_type=None):
        ...
        :type obj: ActionableResource_21
