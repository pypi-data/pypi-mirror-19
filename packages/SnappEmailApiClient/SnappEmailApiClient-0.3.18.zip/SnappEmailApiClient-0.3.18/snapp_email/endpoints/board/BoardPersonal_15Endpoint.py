...
import json
from snapp_email.datacontract.classes import BoardPersonal_15
from snapp_email.datacontract.utils import export_dict, fill


class BoardPersonal_15Endpoint:
    def __init__(self, api_client):
        self.api_client = api_client
    
    def options(self, impersonate_user_id=None, accept_type=None):
        """
        Retrieve options available for resource 'BoardPersonal_15'.
        ...
        add_headers = {
            'Content-Type': 'application/vnd.4thoffice.board.personal-5.0+json',
            'Accept': 'application/vnd.4thoffice.board.personal-5.0+json' if accept_type is None else accept_type,
        }
        response = self.api_client.api_call('options', endpoint, url_parameters, add_headers, impersonate_user_id=impersonate_user_id)
        
        return fill(BoardPersonal_15, response.json(), content_type=response.headers['Content-Type'])
