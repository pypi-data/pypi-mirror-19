...
import json
from snapp_email.datacontract.classes import ReminderMention_20
from snapp_email.datacontract.utils import export_dict, fill


class ReminderMention_20Endpoint:
    def __init__(self, api_client):
        self.api_client = api_client
    
    def options(self, impersonate_user_id=None, accept_type=None):
        """
        Retrieve options available for resource 'ReminderMention_20'.
        ...
        add_headers = {
            'Content-Type': 'application/vnd.4thoffice.reminder.mention-v5.15+json',
            'Accept': 'application/vnd.4thoffice.reminder.mention-v5.15+json' if accept_type is None else accept_type,
        }
        response = self.api_client.api_call('options', endpoint, url_parameters, add_headers, impersonate_user_id=impersonate_user_id)
        
        return fill(ReminderMention_20, response.json(), content_type=response.headers['Content-Type'])
