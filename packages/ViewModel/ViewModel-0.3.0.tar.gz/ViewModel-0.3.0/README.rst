...
city.  From the perpective of the address, the relationship is 'one-to-one' because
...
for the address together wtih the information for the city still leaves a single 'row' 
...
oppositive perspective has implications that can mean the correct implementation 
...
might have a relationship to serveral addresses. But rather than this being a 
...
will need to match potentially serveral (or many) people. The matching person cannot be 
...
Read an adress and we can read all people with our address in their address list. The
...
label from each perspecive.  Conside people relationships where Bob could be
'husband' to Alice, but the same relatioship from the other perspective could
be 'wife'.
...
The rows structure
++++++++++++++++++
The actual data is kept in a view list  called 'dbRows_',
which reflects the actual data being held in the underlying
database.
For each row of the view, there is one entry in dbRows_.

The list elements of 'dbRows_'
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
