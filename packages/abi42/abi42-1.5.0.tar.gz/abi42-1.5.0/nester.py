"""This module contains a function which prints lists which may/may not include nested lists"""
import sys

movies = [
    "The Holy Grail", 1975, "Terry JOnes & Terry Gilliam", 91,
    ["Graham Chapman",
     ["Michael Palin", "John Cleese", "Terry Gilliam", "Eric Idle", "Terry Jones"]]
]


def print_lol(the_list, indent=False, level=0, writedata='sys.stdout'):
    """This function takes a list argument and each item of the list is printed on the screen in its own line """
    for each_item in the_list:
        if isinstance(each_item, list):
            print_lol(each_item, indent, level + 1, writedata)
        else:
            if indent:
                for num in range(level):
                    print('\t', end='', file=writedata)
            print(each_item, file=writedata)
