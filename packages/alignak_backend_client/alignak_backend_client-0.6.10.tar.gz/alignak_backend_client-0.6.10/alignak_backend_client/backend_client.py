...
alignak-backend-cli command line interface::
    ...
    Hints and tips:
        ...
        To get all the services of an host, you can get the service list with a wildcard in
        the item name. For all the services of the host named 'passive-01', use 'passive-01/*'
        as in 'alignak-backend-cli get -l -t service passive-01/*'
            ...
            if len(rsp['_items']) > 0 and rsp['_status'] == 'OK':
                ...
                if not self.dry_run:
                    ...
                    filename = self.file_dump(response,
                                              'alignak-object-list-%ss.json' % (resource_name))
                    if filename:
        ...
        try:
            ...
            if len(response['_items']) > 0:
                ...
                for field in item_data.copy():
                    ...
                    if field in ['realm', 'command', 'timeperiod', 'host', 'service',
                                 'grafana', 'statsd']:
                        try:
                            int(item_data[field])
                        except ValueError:
                ...
                for field in item_data.copy():
                    ...
                    # Manage potential object link fields
                    if field in ['realm', 'command', 'timeperiod', 'host', 'service',
                                 'grafana', 'statsd'] and item_data[field] is not None:
                        try:
                            int(item_data[field])
                        except ValueError:
                            # Not an integer, consider an item name
