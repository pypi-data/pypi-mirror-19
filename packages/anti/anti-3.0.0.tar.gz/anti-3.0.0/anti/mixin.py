...
from .utils import get_normal_url, get_normal_quote, parse_yandex, parse_google, get_redis_key, check_redis_urls, \
    wait_redis_urls
...
class openYandexMixin():
    def load_yandex_cache_pos(self, phrase, lr=213, cache=True, depth=2):
        ...
        self.urls = [YANDEX_URL % (i, phrase_quote, lr) for i in xrange(depth)]
        if not check_redis_urls(self.rds, self.urls) or not cache:
    ...
    def get_yandex_cache_pos(self, phrase, lr=213, cache=True, depth=2, timeout=120):
        ...
        wait_redis_urls(self.rds, self.urls, timeout)
        res = []
        for url in self.urls:
            logging.debug(url)
    ...
    def get_google_cache_pos(self, phrase, cache=True, timeout=120):
        ...
        url = GOOGLE_URL % phrase_quote
        logging.debug(url)
        soup = self.get_soup(url, cache=cache, timeout=timeout)
        res = parse_google(soup, self.query)
        return res
