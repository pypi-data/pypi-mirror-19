...
from appdynamics.agent.services.transaction_monitor import TransactionMonitorService
...
class Agent(object):
    ...
    def __init__(self):
        ...
        self.active_bts = set()
        self.current_bts = weakref.WeakValueDictionary()
    ...
    def on_start_node_response(self, response):
        if self.config_svc is None:
            ...
            self.tx_monitor_svc = TransactionMonitorService(self.active_bts)
    ...
    def get_current_bt(self):
        ...
        """
        return self.enabled and self.current_bts.get(get_ident(), None)
    ...
    def start_exit_call(self, bt, start_frame, backend, **kwargs):
        """Start an exit call, forcing a sample if the BT is snapshotting.

        """
        exit_call = bt.start_exit_call(start_frame, backend, **kwargs)
        if exit_call and bt.snapshotting:
            bt.sample_data.take_leaf(start_frame, exit_call.timer.start_time_ms, bt.active_exit_calls)
        return exit_call

    def end_exit_call(self, bt, exit_call, end_frame, exc_info=None):
        """End an exit call.

        """
        bt.end_exit_call(exit_call, end_frame, exc_info=exc_info)
