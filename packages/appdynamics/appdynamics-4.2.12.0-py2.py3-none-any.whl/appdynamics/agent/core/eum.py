...
def eum_metadata_injection_enabled(eum_config, user_agent):
    ...
    user_agent : str
        The HTTP User-Agent header for the incoming request.
...
def _generate_eum_metadata(eum_config, bt):
    ...
    if bt.wait_for_bt_info_response():
        if bt.bt_info_response.HasField('averageResponseTimeForLastMinute'):
            metadata[bt_art_key(use_long_form)] = bt.bt_info_response.averageResponseTimeForLastMinute

        if bt.snapshot_guid or bt.bt_info_response.isSnapshotRequired:
            partial_snapshot = is_partial_snapshot(bt.snapshot_trigger)
...
def inject_eum_metadata(eum_config, bt, response_headers):
    ...
    """
    if not bt.eum_guid and eum_metadata_injection_enabled(eum_config, bt.request.user_agent):
