...
import logging
from appdynamics.lang import filter, range, values
from appdynamics.lib import is_internal_frame
from appdynamics.lang import zip
...
class SampleData(object):
    ...
    def take(self, frames, now, exit_calls):
        if self.greenlet_ref:
            ...
            if ctx:
                ...
                leaf = ctx.gr_frame or frames.get(self.thread_id, None)
                if leaf is None:
                    return
            else:
                return  # do nothing
            ...
            if leaf is None:
                return
        self.take_leaf(leaf, now, exit_calls)

    def take_leaf(self, leaf, now, exit_calls):
...
class FrameIterator(object):
    ...
    def __next__(self):  # Py3 iterator protocol
        ...
        f_hash = frame_hash(frame)
        self.next_frame = frame.f_back

        try:
            return self.frame_cache[f_hash]
        except KeyError:
            frame_info = FrameInfo(frame)
            self.frame_cache[f_hash] = frame_info
            return frame_info
