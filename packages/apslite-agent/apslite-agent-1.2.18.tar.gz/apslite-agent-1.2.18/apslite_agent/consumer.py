...
class OdinCloudConsumer(object):
    ...
    async def handle_message(self, msg, ws):
        ...
        if self.deferred_task:
            data = await self.run_task(self.deferred_task, ws)
        ...
        if msg.tp == aiohttp.MsgType.text:
            ...
            if data.get('type') == 'task':
                task_data = await self.run_task(data, ws)
    ...
    async def ws_message_handler(self, msg, ws):
        try:
            results = await self.handle_message(msg, ws)
            if results:
                for res in results:
    ...
    async def ws_handler(self, ws):
        ...
        async for msg in ws:
            loop.create_task(self.ws_message_handler(msg, ws))
