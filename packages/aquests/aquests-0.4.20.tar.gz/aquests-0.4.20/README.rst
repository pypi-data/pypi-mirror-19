...
- http2_constreams: if you making requests to single http2 host, how many concurrent streams per channel. BE CAREFUL, it is only useful for single http2 server, otherwise your workers will be increased to number of http2_constreams times than you really want. It might be useful for generating traffic load for testing your servers.
