...
intel_compile_args = ['-std=c++11', '-O3', '-g', '-xHost', '-fPIC']
...
if not is_making_distro:
    ...
    if not os.path.exists(versioninfo_s) or t > os.stat(versioninfo_s)[stat.ST_MTIME]:
        ...
        parallelcomp = "distutils with {0} {1}".format(mpi_compiler[1], 
                                                       " ".join([cfgDict['CFLAGS']]
                                                                +extra_compile_args))
        runcmd("python recordversion.py '"+version+"' 'serial' '"+serialcomp+"' '' > "+versioninfo_s)
        runcmd("python recordversion.py '"+version+"' 'parallel' '"+parallelcomp+"' '' > "+versioninfo_p)
