...
class FileSQLiteDB(DataFile):
    ...
    def init_default(self):
        ...
        messed by user)
        """
        pkg = ag.get_class_package(self.__class__)
        fullpath = ag.get_default_data_path(self.default_filename, module=pkg)
        self.load(fullpath)
        name, ext = os.path.splitext(self.default_filename)
        new = ag.new_filename(os.path.join("./", name), ext)
        self.save_as(new)
    ...
    def _get_table_names(self):
        return ag.get_table_names(self.__get_conn())
    ...
    def get_table_info(self, tablename):
        ...
        **Note** Fields "caption" and "tooltip" are added to rows using information in moldb.gui_info
        ...
        ret = ag.get_table_info(conn, tablename)
    ...
    def _conn_is_open(self):
        ...
        return ag.conn_is_open(self._conn)
