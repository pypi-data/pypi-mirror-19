...
class Spectrum(object):
    ...
    def xunit(self, value):
        if not isinstance(value, (u.UnitBase,)):  #(u.Unit, u.CompositeUnit, u.IrreducibleUnit)):
            raise TypeError("Spectrum x-unit must be pa astropy unit, not '{}'".format(str(value)))
    ...
    def yunit(self, value):
        if not isinstance(value, (u.UnitBase,)):  #, (u.Unit, u.CompositeUnit)):
            raise TypeError("Spectrum y-unit must be pa astropy unit, not '{}'".format(str(value)))
    ...
    def get_rgb(self, visible_range=None, method=0):
        ...
        if visible_range[1] <= visible_range[0]:
            raise RuntimeError("Second element of visible range ({0!s}) must be greater than first element".format(visible_range))
            ...
            try:
                ...
                if key in ("COMMENT", "HISTORY", ""):
                    if isinstance(value, str):
                        ...
                        # value is probably pa instance of fits.header._HeaderCommentaryCards
                        for _ in value:
                            hdu.header[key] = _
                else:
                    hdu.header[key] = value
            except:
                get_python_logger().exception("Error adding header['{0!s}'] = '{1!s}'".format(key, value))
                raise
        hdu.data = self.y
        return hdu
