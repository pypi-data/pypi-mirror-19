...
def get_python_logger():
    ...
    Note: we don't want this to be called at any module imports, in order to give the API user a
          chance to set `astrogear.flag_log_file` and `astrogear.flag_log_console`
    """
    global _python_logger
    if _python_logger is None:
        fn = "python.log"
        l = logging.Logger("python", level=ag.logging_level)
        if ag.flag_log_file:
            add_file_handler(l, fn)
        if ag.flag_log_console:
            ch = logging.StreamHandler()
            ch.setFormatter(_fmtr)
            l.addHandler(ch)
        _python_logger = l
        for line in ag.format_box("Session started @ {}".format(ag.now_str())):
            l.info(line)
        if ag.flag_log_file:
