    ...
    def get_summary(self):
        ...
        if not self.isfile:
            ...
            if self.flag_error:
                ...
                ll.append("<span style=\"font-weight: bold; color: {0!s}\">{1!s}</span>".format(ag.COLOR_ERROR, emsg))
            else:
...
class LoadThread(QThread):
    ...
    def run(self):
        for props in self.propss:
            ...
            f = None
            try:
                f = ag.load_any_file(props.filepath)
                props.f = f
            except Exception as e:
                # Suppresses exceptions
                props.error_message = str(e)
                ag.get_python_logger().exception("Loading file '{}'".format(props.filepath))
            finally:
                props.flag_scanned = True
                props.flag_error = f is None
                props.flag_text = ag.is_text_file(props.filepath)
...
class XExplorer(QMainWindow):
    ...
    def __init__(self, parent=None, dir_="."):
        ...
        self.__lock_propss = ag.MyLock(flag_verbose=False)
    ...
    def eventFilter(self, obj, event):
        ...
        if obj == self.tableWidget:
            if ag.check_return_space(event, self.on_load):
                return True
            ...
            return ag.check_return_space(event, self.__visualize)
    ...
    def on_cd2(self):
        ...
        if os.path.isdir(new_dir):
            ...
            ag.show_error("Invalid directory")
    ...
    def on_collect_errors(self, _=None):
        ...
        try:
            ...
            w = ag.XHTML(self, k.get_html(), "Errors in '{0!s}' and subdirectories".format(self.dir))
            w.show()
        except Exception as e:
            MSG = "Could not collect errors"
            ag.get_python_logger().exception(MSG)
            ag.show_error("{0!s}: {1!s}".format(MSG, str(e)))
        finally:
            self.__flag_loading = False
    ...
    def __update_info(self):
        ...
        if npp == 1:
            ...
            if p.flag_scanned:
                if isinstance(p.f, ag.DataFile):
                    classes.extend(ag.get_suitable_vis_classes(p.f))
                    if ag.VisPrint in classes:
                        classes.remove(ag.VisPrint)
                if p.flag_text:
                    # This is pa exception, since "txt" is not a Vis descendant.
            ...
            ff = [p.f for p in pp]
            flag_spectra = all([isinstance(f, ag.FileSpectrum) for f in ff])
            flag_mod = all([isinstance(f, ag.FileModBin) and len(f.records) > 1 for f in ff])
            if flag_spectra:
    ...
    def __visualize(self):
        ...
        try:
            ...
            if vis_class == "ovl":
                ...
                ag.plot_spectra_overlapped(spectra)
            elif vis_class == "sta":
                pp = self.__lock_get_current_propss()
                spectra = [p.f.spectrum for p in pp]
                ag.plot_spectra(spectra)
            elif vis_class == "modgrid":
                pp = self.__lock_get_current_propss()
                models = [p.f for p in pp]
                ag.plot_mod_grid(models)
            else:
                props = self.__lock_get_current_props()
                if vis_class == "txt":
                    w = ag.XText(self, open(props.filepath, "r").read(), props.filepath)
                    w.show()
                else:
                    vis_class().use(props.f)
        finally:
...
#        dir_ = [x for x in dir_ if os.path.isfile(x)]
        ...
        ag.get_python_logger().info("AUTO LOAD: max file size: {0:g}".format(max_size))
        pp = []
        with self.__lock_propss:
