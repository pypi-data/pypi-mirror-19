...
def plot_spectra_pieces_pdf(ss, aint=10, pdf_filename='pieces.pdf', setup=_default_setup):
    ...
    **Note** overrides setup.fmt_xlabel; leaves y-labell and title blank
    ...
    for h in range(num_pages):
        ...
        for i, s in enumerate(ss):
            s_cut = pa.cut_spectrum(s, lambda0, lambda1)
            ax = plt.gca()
    ...
    # for fig in xrange(1, figure().number): ## will open pa empty extra figure :(
    #     pdf.savefig( fig )
    pdf.close()
...
def _calc_max_min(ss):
    ...
    for s in ss:
        assert isinstance(s, pa.Spectrum)
