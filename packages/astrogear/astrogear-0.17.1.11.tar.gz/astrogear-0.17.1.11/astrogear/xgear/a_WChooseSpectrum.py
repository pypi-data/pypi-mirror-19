...
class WChooseSpectrum(QWidget):
    ...
    def load(self, x):
        assert isinstance(x, ag.FileSpectrum)
        self.sp = x
    ...
    def b_clicked(self):
        ...
        #                                  self.wilds[index]
        try:
            d = self.sp.filename if self.sp and self.sp.filename is not None else ag.FileSpectrumPfant.default_filename
            new_filename = QFileDialog.getOpenFileName(self, "Cube", d, "*.fits")
            if new_filename:
                sp = ag.load_spectrum(new_filename)
                if not sp:
