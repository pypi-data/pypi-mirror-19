...
from PyQt4.QtGui import QWidget, QVBoxLayout, QTabWidget, QTextEdit, QLabel, QMessageBox, \
    QFileDialog
...
class XFileMainWindow(XLogMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        ac = m.addAction(ag.get_icon("document-open"), "&Open...")
        ac.setShortcut("Ctrl+O")
        ac.triggered.connect(self.on_open)

        m.addSeparator()

        ac = m.addAction(ag.get_icon("document-save"), "&Save")
        ac.setShortcut("Ctrl+S")
        ac.triggered.connect(self.on_save)

        ac = m.addAction(ag.get_icon("document-save-as"), "Save &as...")
        ...
        ac = m.addAction(ag.get_icon("system-shutdown"), "&Quit")
    ...
    def add_log_error(self, x, flag_also_show=False):
        ...
        if len(x) == 0:
            ...
            tb.print_stack()
        x_ = x
        x = '<span style="color: {0!s}">{1!s}</span>'.format(ag.COLOR_ERROR, x)
    ...
    def __generic_save(self):
        ...
        if not editor.flag_valid:
            ag.show_error("Cannot save, {0!s} has error(s)!".format(f.description))
            return True
        if f.filename:
            try:
                f.save_as()
    ...
    def load_filename(self, filename, index=None):
        ...
        f = ag.load_with_classes(filename, possible_classes)
        if f is None:
