...
class PythonHighlighter (QSyntaxHighlighter):
    ...
    def __init__(self, document):
        ...
        self.tri_single = (QRegExp("'''"), 1, _STYLES['string2'])
        self.tri_double = (QRegExp('"""'), 2, _STYLES['string2'])

        rules = []

        # Keyword, operator, and brace rules
        rules += [(r'\b{0!s}\b'.format(w), 0, _STYLES['keyword'])
            for w in PythonHighlighter.keywords]
        rules += [(r'{0!s}'.format(o), 0, _STYLES['operator'])
            for o in PythonHighlighter.operators]
        rules += [(r'{0!s}'.format(b), 0, _STYLES['brace'])
