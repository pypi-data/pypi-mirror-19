...
if __name__ == "__main__":
    ...
    for x in ff:
        ...
        f = ag.load_with_classes(x, ag.classes_sp())
        if f is None:
            ag.print_error("... type not recognized, sorry")
        else:
            print("... successfully read using reader {0!s}.".format(f.__class__.__name__))
            ss.append(f.spectrum)

    if len(ss) == 0:
        ag.print_error("Nothing to plot!")
    else:
        setup = ag.PlotSpectrumSetup(ymin=ymin)

        if args.pieces:
            ag.plot_spectra_pieces_pdf(ss, aint=args.aint,
                                    pdf_filename=args.fn_output, setup=setup)
        elif args.pages:
            ag.plot_spectra_pages_pdf(ss, pdf_filename=args.fn_output, setup=setup)
        else:
            if args.ovl:
                ag.plot_spectra_overlapped(ss, "", setup=setup)
            else:
                ag.plot_spectra(ss, "", setup=setup, num_rows=num_rows)
