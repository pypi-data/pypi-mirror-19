...
from azuremlcli.cli_util import get_ice_key
...
def env_usage():
    ...
    print("aml env key      : view/set your Azure ML access key")
...
def env_describe():
    ...
    try:
        conf = read_config()
        if conf and 'api_key' in conf:
            print('AML API key            : {}'.format(conf['api_key']))
        else:
            print('AML API key            : None')
...
def env_cluster(args):
    ...
    if not acs_is_setup:
        ...
        if continue_without_acs != 'y' and continue_without_acs != 'yes':
            print("Aborting switch to cluster mode. Please run 'aml env setup' for more information on setting up your cluster.") #pylint: disable=line-too-long
            return
...
def env_key(args):
    """Helps set up an AML API key, or display a configured key."""

    parser = argparse.ArgumentParser(prog='aml env key')
    parser.add_argument('-u', '--update',
                        action='store_true',
                        help='Update your API key for accessing Azure ML services.')
    ...
    try:
        ...
        if 'api_key' not in conf or parsed_args.update:
            api_key = get_ice_key()
            if api_key:
                conf['api_key'] = api_key
                write_config(conf)
        else:
            print('API key: {}'.format(conf['api_key']))
            print("To update key, run 'aml env key -u'")
    except InvalidConfError:
...
def acs_setup():
    ...
    answer = input('Would you like to set up port forwarding to your ACS cluster (Y/n)? ')
    answer = answer.rstrip().lower()
    if answer != 'n' and answer != 'no':
        acs_url = input('ACS Master URL (default: {}): '.format(acs_master_url))
        ...
        acs_username = input('ACS username: ')
        if acs_username is None or acs_username == '':
            print('Error: no ACS username provided.')
            return False, -1
...
def realtime_service_deploy_local(image, verbose):
    ...
    if container_created:
        ...
        if dockerport < 0:
            print('[Local mode] Failed to start container. Please report this to azureml@microsoft.com with your image id: {}'.format(image)) #pylint: disable=line-too-long
            return
...
def realtime_service_delete(args):
    ...
    if acs_master_url is None:
        print("")
        print("Please set up your ACS cluster for AML. See 'aml env setup' for more information.")
        return
    ...
    try:
        ...
        print('For more information about setting up your environment, see: "aml env setup".')
        return
    except requests.ConnectionError:
...
def realtime_service_create(args):
    ...
    if not conf or 'api_key' not in conf:
        print("No API key found. Please set up a key first by running 'aml env key'.")
        return

    ice_key = conf['api_key']
    ...
    headers = {'Content-Type': 'application/json', 'Authorization': 'Basic {}'.format(ice_key)}
    ...
    try:
        ...
        print('Error: could not connect to Azure ML. Please try again later. If the problem persists, please contact azureml@microsoft.com') #pylint: disable=line-too-long
        return
    ...
    if ice_put_result.status_code == 401:
        ...
        print('Error connecting to Azure ML. Please contact azureml@microsoft.com with the stack below.')
...
def batch_service_create(args):
    ...
    print(get_success_and_resp_str(resp, response_obj=StaticStringWithTableReponse(
        usage, batch_create_service_header_to_fn_dict), verbose=verbose)[1])
