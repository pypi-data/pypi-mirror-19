...
def first_run():
    ...
    try:
        ...
        if conf:
            if 'api_key' not in conf:
                if 'next_key_check' in conf:
                    try:
                        check_again_at = datetime.strptime(conf['next_key_check'], '%Y-%m-%d')
                    except ValueError:
                        conf['next_key_check'] = (datetime.now() + timedelta(days=1)).strftime('%Y-%m-%d')
                        check_again_at = datetime.now() + timedelta(days=1)
                    if datetime.now() < check_again_at:
                        write_config(conf)
                        return
    ...
    if is_first_run or is_config_corrupt or not is_valid_api_key:
        conf['mode'] = 'local'
        answer = input('Would you like to configure your Azure ML API key (Y/n)? ')
        answer = answer.strip().lower()
        if answer == 'n' or answer == 'no':
            print("To set up your key later, please run 'aml env key'.")
            # Ask again about setting up key in a day
            conf['next_key_check'] = (datetime.now() + timedelta(days=1)).strftime('%Y-%m-%d')
            write_config(conf)
            return
...
def get_ice_key():
    ...
    Ask user to enter a key for ICE and validate it.
    :return: str - valid key if one was entered, None otherwise
    """
    attempts = 0
    candidate_key = None
    while not candidate_key and attempts < 3:
        candidate_key = input('Please enter your Azure ML API key: ')
    ...
    headers = {'Content-Type':'application/json', 'Authorization':'Basic {}'.format(candidate_key)}
    random_ice_url = ice_base_url + '/jobs/' + str(uuid.uuid4())
    try:
        ice_response = requests.get(random_ice_url, headers=headers, timeout=ice_connection_timeout)
    except (requests.ConnectTimeout, requests.ConnectionError, requests.exceptions.ReadTimeout):
        print('Error: failed to connect to Azure ML. Please try again later. If this error persists, please contact azureml@microsoft.com.') #pylint: disable=line-too-long
        return None

    if ice_response.status_code == 404:
        print('Success: validated key.')
