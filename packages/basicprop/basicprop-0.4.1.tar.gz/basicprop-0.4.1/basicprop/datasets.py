...
class Rects(object):
    def get_image(self, y, width=10):
        ...
        # Create Background
        x = np.full((28, 28), BG_PIXEL)
        
        # Create Foreground
        line1 = np.full((heights[y1], width), FG_PIXEL)
        x[3:3+heights[y1],x1offset:x1offset+width] = line1
        
        line2 = np.full((heights[y2], width), FG_PIXEL)
    ...
    def get_batch(self, batch_size):
        ...
        data = np.concatenate([np.expand_dims(x, axis=0) for x in data], axis=0).astype(np.uint8)
        return (data, labels)
...
class Line(object):
    def get_image(self, y, width=4):
        x = np.full((28, 28), BG_PIXEL)
        offset = y * 2
        line = np.full((28,width), FG_PIXEL)
