...
def _bgzip_from_fastq(data):
    ...
    if in_file.endswith(".gz") and not objectstore.is_remote(in_file):
        needs_bgzip, needs_gunzip = _check_gzipped_input(in_file, needs_convert)
...
def _check_gzipped_input(in_file, needs_convert):
...
def is_bgzf(filename):
    ...
    Thanks to Jason Weirather, modified from https://github.com/jason-weirather/Au-public
    """
    import struct
    import sys
    import StringIO
    def _write_msg(msg):
        sys.stderr.write("%s not bgzip file; problem: %s\n" % (filename, msg))
    with open(filename, 'rb') as inf:
        bytes1 = inf.read(12)
        if len(bytes1) != 12:
            _write_msg("File length")
            return False
        try:
        ...
        if compression_method != 8:
            _write_msg("Compression Method")
            return False
        if flag != 4:
            _write_msg("flg")
            return False
        if xlen < 6:
            _write_msg("no extra fields")
            return False
        ...
        while True:
            ...
            v = s.read(slen)
            if len(v) != slen:
                _write_msg("extra field read")
                return False
        if not has_id or not obsslen == 2:
            _write_msg("no proper extra header")
            return False
    return True
