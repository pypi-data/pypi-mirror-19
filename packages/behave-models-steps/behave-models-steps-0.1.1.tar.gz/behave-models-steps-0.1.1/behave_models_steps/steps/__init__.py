...
# from app.helpers.uuidgenerator import UUIDGenerator, UUID
...
def step_impl(context, model_type, _db):
    ...
    untouched_attrs = {k: v for k, v in untouched_attrs.items() if v != ''}  # want strict empty string rather than falsey
...
@given(u'I set this "(?P<model_type>.*)"\'s "(?P<attr_name>.*)" using method "(?P<set_method>.*)"')  # (?: to be value )?("(?P<set_value>.*)")?')
...
def step_impl(context, model_type, attr_name, get_value):
    ...
    if attr_name == 'uuid' and isinstance(get_value, LocalData.UUID):
        get_value = LocalData.UUIDGenerator.uuid_to_int(get_value.hex)
        context.getter_value = LocalData.UUIDGenerator.uuid_to_int(context.getter_value)
    ...
    # print "\nIS ATTR_NAME", attr_name, " IN:", ('active', 'groups') + NON_OWNER_RELATIONS
    if attr_name not in (('password', 'confirmed_at', 'active', 'groups', 'uuid') + LocalData.NON_OWNER_RELATIONS):
...
def step_impl(context, specific_, model, attr_name, set_method, attr_value):
    ...
    with LocalData.app.app_context():
        ...
        if not specific_:  # otherwise we assume we already have context.desired_<model>
            ...
            print "CONTEXT: ", context, ", MODEL: ", model, " GETATTR: ", getattr(context, 'desired_' + model)
    ...
    if not candidate:
        with LocalData.app.app_context():
            ...
            if attr_name in LocalData.MANY_TO_MANY_RELATIONS:
                ...
                filtr = {attr_name: make_attr(attr_value, context=context)}
                filtered = base_query.filter_by(not_(**filtr))
...
def step_impl(context, model, is_or_contains, attr_name, attr_value):
    ...
    attr_value = make_attr(attr_value)
    ...
    if not candidate:
        if attr_name in LocalData.NON_OWNER_RELATIONS:
            attr_klass = load_class('app.models.', singularize(attr_name))
            with LocalData.app.app_context():
                ...
                setattr(candidate, attr_name, [attr_klass.query.filter_by(client_id=client_id, name=rolename).one()]) # i.e. user.roles = []
                LocalData.db.session.commit()
                # print "RE-WORKED:", candidate.__dict__
        else:
            with LocalData.app.app_context():
                ...
                LocalData.db.session.commit()
                # print "RE-WORKED USER:", candidate.__dict__
    else:
...
def step_impl(context, importable, save_import, importable_args):
    ...
    if hasattr(LocalData, importable_args) or hasattr(LocalData, importable_args.split(".")[0]):
        importable_args = "LocalData." + importable_args

#    print "SAVE_IMPORT: ", save_import, ", importable_args: ", importable_args
    ...
    if class_load_reqd(checker_method):
        ...
        with LocalData.app.app_context():
            if method_has_args(method):
                generated_value = getattr(klass, method_name)(eval(margs[:-1]))  # since we never took care of dangling ')'
            else:
...
def make_loadable_args_for_clients(context, filepath, num):
    ...
    if clients and (isinstance(num, int) or re.match("\d+", num)):
        if int(num) > len(clients):  # we'll have to append some
            while int(num) > len(clients):
                clients.append({"name": faker.company()})
...
def load_class(base_namespace, model_type):
    ...
    try:
        ...
        module_name, class_name = str(base_namespace + pluralize(str(klass_name.lower())) + "." + klass_name).rsplit(".", 1)
    except AttributeError:
        module_name, class_name = str(base_namespace + pluralize(str(model_type.lower())) + "." + LocalData.word_to_class[model_type]).rsplit(".",1)
    ...
    try:
        ...
        return getattr(importlib.import_module(module_name, class_name), singularize(camelize(LocalData.relation_to_class[pluralize(model_type)])))
    except ImportError:
        if '_' in model_type:
        ...
        module_name, class_name = str(base_namespace + pluralize(str(klass_name).lower()) + "." + singularize(klass_name)).rsplit(".", 1)
        # return getattr(importlib.import_module(module_name), class_name)
        return getattr(importlib.import_module(module_name, class_name), singularize(class_name))
...
def load_model_relation(context, relation_name, model_type, attrs=None):
    ...
    class_name = load_class('app.models.', LocalData.relation_to_class[relation_name] if relation_name in LocalData.relation_to_class else relation_name)
...
def make_attribute_from_string(context, model, name, value, replacers={}):
    ...
    if single_value(value) and [rnd for rnd in ('rand', 'random', 'range') if rnd in value]:
        ...
        first_pass_list = recurse_list_to_make_attrs(list_arg, unpacked_args) if unpacked_args else list_arg
        ...
        with LocalData.app.app_context():
            ...
            print "LOOKING FOR PFORM ID"
            print LocalData.db.session.dirty, LocalData.db.session.new
            LocalData.db.session.flush()
...
def prep_fasttype_string_attr(context, model, name, value, replacers={}):
    ...
    import re
    if '[' in value:
        value = replace_list_in_string_attr(context, model, name, value, replacers)
    ...
    for loadable in pars:
        ...
        if 'None' in loadable_klass:
            ...
            loadable = [loadable_klass, value[value.find(loadable[1]) - 1:match_close], loadable_klass + value[value.find(loadable[1]) - 1:match_close]]
        ...
        loadable_args = [make_attr(l) if isinstance(l, list) or l not in parsed_pars else parsed_pars[l] for l in loadable_args]
            ...
            if db_object(evaled_class):
                with LocalData.app.app_context():
                    evaled_class, created = LocalData.ModelHelpers.get_one_or_create(LocalData.db.session, evaluable_klass,
                                                **dict(zip(inspect.getargspec(evaluable_klass.__init__).args[1:], loadable_args)))
                    LocalData.db.session.commit()
...
def prep_loadable(value, loadable):
    ...
    loadable_klass = ''.join(
        ...
        reversed(re.findall(r'[^\(\s]*[[A-Z]|(?=\()]', ''.join(reversed(temp_value[:temp_value.find(temp_loadable) - 1])))[0]))
    ...
    for i in range(len(opens)):
        ...
        while quotable_index_close < max(all_quotes):
            if quotable_index_close < 0:
                quotable_index = min([replaceable.find('"'), replaceable.find("'")])
            else:
                quotable_index = min([replaceable[quotable_index_close + 1:].find('"'),
            ...
            re_list += [make_attribute_from_string(None, None, None, replaceable[quotable_index + 1:quotable_index_close], replacers)]
            # re_list = re_list + '"' + replaceable[quotable_index + 1:quotable_index_close] + '",'

        replacers['LIST_%s' % str(i + 1)] = re_list
        # replacers['LIST_%s' % str(i + 1)] = re_list[:-1] + ']'
        # value = value.replace('LIST_%s' % str(i + 1), re_list[:-1] + ']')
        # replacers['LIST_%s' % str(i + 1)] = make_attribute_from_string(
        #     context, model, name, re_list)
        #     context, model, name, replaceable[quotable_index + 1:quotable_index_close])
...
def instance_from_string_value(context, value):
    ...
    # use this for calculating uuids as well
    value_type = LocalData.UUIDGenerator.int_to_uuid if value_type == 'uuid' else eval(value_type)
...
def recurse_list_to_make_attrs(list_arg, unpacked_args):
    """
    Recurse through a list and make attributes for each element

    Return LIST of made attributes
    """
    attr_args = [w if w not in unpacked_args else make_attr(w) for w in list_arg]
    return attr_args
...
def make_random_attr(attr_name, attr_value, model_type):
    ...
    if attr_name:
        ...
        if attr_name == 'uuid' or str(mytype).lower() == 'uuid':
            attr_value = LocalData.UUIDGenerator.int_to_uuid(random.randint(1, 1000)).hex
    ...
    if (not mytype or mytype in ('str', 'unicode', 'basestring', 'string')) and isinstance(attr_value, basestring) and attr_value.startswith('range'):
...
def create_non_primative(attr_name, attr_value, context=None):
    ...
    """
    model_name = LocalData.relation_to_class[attr_name] if attr_name in LocalData.RELATIONS_NOT_FIELDS or private_attr(attr_name) else attr_name
    ...
    #     klass_args = '(' + attr_value.split('(')[0]
        ...
        if set(set(required_args)-{'self'}).issubset(attr_value.keys()):
            ...
            raise AttributeError("Did not give required arguments %s for class %s" % (required_args, make_klass.__name__))
    ...
    try:
        ...
        if hasattr(context, model_type + "_persisted") and getattr(context, model_type + '_persisted'):
            ...
            try:
                ...
                if db_object(getattr(context, prepender + model_type)) and is_blobby(load_class('app.models.', model_type), method_name):
                    with LocalData.app.app_context():
                        model_dict = getattr(context, prepender + model_type).__dict__
                        model_dict.pop('_sa_instance_state')
                        refreshed_model, _ = LocalData.ModelHelpers.get_one_or_create(LocalData.db.session, load_class('app.models.', model_type), **model_dict)
