...
def step_impl(context, creatable_type, _a_type, extra_attr_type, extra_attr_value):
    with LocalData.app.app_context():
        if _a_type:
            ...
            if extra_attr_type in reencode:
...
@given(u'a(?:n)? (?P<owner_type>\w+) exists in the LocalData.db(?P<add_client> belonging to client "(?P<clientname>.*)")?')
    ...
    if _db:
        with LocalData.app.app_context():
            ...
            if getattr(context, model_type + "_persisted") and not hasattr(context, 'insert_error'):
                model_attrs = redo_attrs(context.model, {key: value for key, value in model_attrs.iteritems()
