...
# from app.helpers.LocalData.UUIDGenerator import LocalData.UUIDGenerator, LocalData.UUID
...
def step_impl(context, model_type, _db):
    with LocalData.app.app_context():
        try: LocalData.db.session.add(getattr(context, 'desired_' + model_type))
        except: print "ALREADY EXISTS IN SESSION"
        LocalData.db.session.commit()
        # from app import pp
...
def step_impl(context, model_type, attr_name, set_method):#, set_value):
    ...
    if set_method:
        ...
        if getattr(context, model_type + "_persisted"):
            with LocalData.app.app_context(): LocalData.db.session.commit()
...
def request_with_method_step(context, model_type, get_method, gargs):
    ...
    if hasattr(context, 'insert_error'):
        ...
        if child_of_method(get_method):
            ...
            if pickled: get_method = pickle.loads(get_method)
...
def step_impl(context, model_type, attr_name, get_value):
    ...
    if get_value and not is_primative(get_value) and not vanilla_object(get_value):
        if isinstance(get_value, list): pass
        elif isinstance(get_value, dict): pass
        elif hasattr(context, 'insert_error'): pass
        else: get_value = produce_comparable(context, get_value)
    # print '\n\t', type(get_value), "\n\t"
    # from app import pp
    # import pickle
    ...
    if getattr(context, model_type + "_persisted") and not hasattr(context, 'insert_error'):
        ...
        if isinstance(context.getter_value, str):
            ...
            except (KeyError,EOFError,IndexError,ImportError): pass
...
def step_impl(context, load_these, filepath):
    ...
    loaders = []
    if pluralize(load_these.lower()) == 'clients': loaders = ['clients']
    if pluralize(load_these.lower()) == 'groups': loaders = ['clients', 'groups']
    if pluralize(load_these.lower()) == 'roles': loaders = ['clients', 'groups', 'roles']
    if pluralize(load_these.lower()) == 'users': loaders = ['clients', 'groups', 'roles', 'users']
    ...
    with LocalData.app.app_context():
        if has_same_attrs(context.getter_value, get_value):
            ...
            for k in get_value.__dict__.keys():
                if k not in temp_value.__dict__: setattr(temp_value, k, getattr(get_value, k))
...
def has_same_attrs(persisted_item, incumbent):
    ...
    if isinstance(persisted_item, list):
        for item in persisted_item:
            if set(incumbent.__dict__.keys()) - set(item.__dict__.keys()): itdoes = False
    else:
        if set(incumbent.__dict__.keys()) - set(persisted_item.__dict__.keys()): itdoes = False
    return itdoes
...
def make_loadable_args_for_experiments(context, extra_args=None):
    ...
    """
    if extra_args: return extra_args
...
def class_load_reqd(loader):
    ...
    if '.' in loader and loader.split('.')[0].istitle(): return True
    return False
...
def make_attribute_from_string(context, model, name, value, replacers={}):
    ...
    if single_value(value) and [rnd for rnd in ('rand', 'random', 'range') if rnd in value]:
        ...
        if 'pickle' in value[5:-1]: import pickle
        value = value[5:-1]
...
def prep_fasttype_string_attr(context, model, name, value, replacers={}):
    ...
    for loadable in pars:
        ...
        if loadable[1]: loadable_args.append(loadable[1])
    ...
    for i in range(len(opens)):
        ...
        while quotable_index_close < max(all_quotes):
            if quotable_index_close < 0:
                ...
                quotable_index = min([m.start() for m in re.finditer("[\"\']", replaceable[quotable_index_close + 1])]) + \
                                quotable_index_close + 1
            ...
            re_list += [make_attribute_from_string(context, None, None, replaceable[quotable_index + 1:quotable_index_close], replacers)]
