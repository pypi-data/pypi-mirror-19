        ...
        if _one_of:  # may want to refactor this to capture the entirety of "match|contain one of"
            if must_or_not_at_all != "must":
                ...
                for thing in comparable:
                    at_key = unpack_unicode_dict_at(keyname, thing['attributes'] if 'attributes' in thing else thing)
                    if is_primative(at_key):
                        all_things.add(at_key)
                    else:
                        thethings = thing['attributes'][keyname] if 'attributes' in thing else thing[keyname]
                ...
                for thing in comparable:
                    at_key = unpack_unicode_dict_at(keyname, thing['attributes'] if 'attributes' in thing else thing)
                    # print "MY MATCHER:"
                    # pp.pprint(eval(matchable))
                    # print "COMPARABLE THING:"
                    # pp.pprint(thing)
                    if is_primative(at_key):
            ...
            if must_or_not_at_all == "must":
                matcher = eval(matchable)
                # no point in testing multiple times below...

                for thing in comparable:
                    # print "MY MATCHER:"
                    # pp.pprint(eval(matchable))
                    # print "COMPARABLE THING (looking for %s)" % keyname
                    # pp.pprint(thing)
                    possible_dict = unpack_unicode_dict_at(keyname, thing['attributes'] if 'attributes' in thing else thing)
                    # print "POSSIBLE DICT?"
                    # pp.pprint(possible_dict)
                    if dict_containing(possible_dict, 'id'):
                        # if a relationship arises we need to check ALL of it to match
                        if isinstance(matcher, list):
                            for submatch in matcher:
                                assert_that(str(submatch.id), equal_to(possible_dict['id']))
                        else:
                            assert_that(str(matcher.id), equal_to(possible_dict['id']))
                    else:
                        if isinstance(possible_dict, (unicode, str)):
                            matchable_value = str(matcher.id)
                        else:
                            matchable_value = matcher.id
...
def unpack_unicode_dict_at(location, d):
    # from app import pp
    # print "SEARCHING FOR ", location, "IN:\n"
    # pp.pprint(d)
    try:
