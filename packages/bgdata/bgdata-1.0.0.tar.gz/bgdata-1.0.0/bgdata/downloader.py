...
class RemoteRepository(object):
    ...
    def get_latest(self, project, dataset, version, timeout=1.0):
        ...
        try:
            request.urlcleanup()
            with request.urlopen(latest_url, timeout=timeout) as fd:
                lines = fd.readlines()
    ...
    def download(self, dest, project, dataset, version, build, retries=3):
        ...
        try:
            download(package_url + compression_format, path=temp_file)
        except Exception as e:
...
class Downloader(object):
    ...
    def __init__(self, forceonline=False):
        ...
        self.remote = RemoteRepository(os.environ.get("BGDATA_REMOTE", self.config['remote_repository']))
