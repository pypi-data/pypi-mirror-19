...
class Blogbook(object):
    def __init__(self, home, blogbook_dir):

        self.home = os.path.abspath(home)
        if blogbook_dir:
            self.blogbook_dir = os.path.abspath(blogbook_dir)
        else:
            self.blogbook_dir = os.path.join(self.home, blogbook.constant.LOGBOOK_DIRECTORY)

        self.verbose = False
        self.setting = {
            'path': {
                'content_exclude_patterns': ['_*', '.*'],
                'layouts': os.path.join(self.home, blogbook.constant.LAYOUTS_DIRECTORY),
                'output': os.path.join(self.home, blogbook.constant.DEFAULT_OUTPUT)
            },
            'blog': {
                'default_layout': 'post',
                'language': 'en',
                'name': os.path.basename(self.home).capitalize(),
            },
            'excerpt': {
                'separator': '\n',
                'max_word': 20
        ...
        self.setting_path = os.path.join(self.blogbook_dir, 'config')
...
@click.pass_context
def main(context, home, blogbook_dir, verbose):
    """Repo is a command line tool that showcases how to build complex
    command line interfaces with Click.
    This tool is supposed to look like a distributed version control
    system to show how something like this can be structured.
    """
    context.obj = Blogbook(home, blogbook_dir)
...
def new(blog, force, layout):
    ...
    with open(os.path.join(blog.blogbook_dir, 'version'), 'w') as stream_logbook:
        stream_logbook.write('0')

    git_dir = os.path.join(blog.blogbook_dir, '.git')
    if os.path.isdir(git_dir):
        shutil.rmtree(git_dir)
    CustomRepo.init(blog.home, os.path.join(blogbook.constant.LOGBOOK_DIRECTORY, '.git'))
...
def push(blog):
    ...
    remote_root = '/var/www/' + os.path.basename(blog.home) + blogbook.constant.LOGBOOK_DIRECTORY
    ...
    if 'output' in blog.setting['remote']:
        client.exec_command(
            'blogbook --blogbook_dir={} config --set path.output {} -p {}'.format(
                remote_root, blog.setting['remote']['output']
            )
        )

    # sftp = client.open_sftp()
    # with sftp.open(remote_root + '/config', mode='w') as s:
    #     if 'output' in blog.setting['remote']:
    #         configuration = configparser.ConfigParser()
    ...
    # with CustomRepo(blog.home, os.path.join(blogbook.constant.LOGBOOK_DIRECTORY, '.git')) as r:
    #
    #     porcelain.add(r)
    #
