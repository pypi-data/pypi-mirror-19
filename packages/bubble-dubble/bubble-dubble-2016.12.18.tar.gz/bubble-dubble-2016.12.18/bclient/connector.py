...
import cryio
from ..bcommon import compressor, default, corrections
    ...
    def getMask(self, maskFile, itype):
        ...
        if os.path.exists(maskFile):
            try:
                ...
                mask = corrections.beamline.correctMask(self.beamline, itype, mask).array
        return mask

    def packMask(self, params, itype):
        mask = self.getMask(params.get('mask', ''), itype)
        if isinstance(mask, np.ndarray):
            params['mask'] = compressor.compressNumpyArray(mask)
