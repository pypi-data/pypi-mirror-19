...
class IBeacon:
    def __init__(self, uuid: bytes = None, major: int = None, minor: int = None) -> None:
        if not uuid:
            uuid = os.urandom(16)
        self.uuid = uuid
...
LISTENERS = [os.urandom(6) for i in range(args.num_listeners)] # type: List[bytes]
...
def main():
    ...
    loop.create_task(loop.create_datagram_endpoint(
        ListenerUdpProtocol, remote_addr=(args.server, args.port)))
