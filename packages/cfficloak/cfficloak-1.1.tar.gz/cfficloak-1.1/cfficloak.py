...
# This can be used to pass in extra details for outargs in  function_skeleton/cmethod
class outarg(object):
...
class CStruct(object):
    ...
    def _ntoh(self, key, val):
        if self._endian_translate:
            fieldtype = self.__fldnames[key].cname
            val = _ntoh[fieldtype](val) if fieldtype in _ntoh else val
        return val
...
class nparray(object):
    ...
    """
    def __init__(self, _cdata, size=-1, dtype=None, ffi=_global_ffi):
        if not numpy:
