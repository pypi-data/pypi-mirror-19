...
def learn(Input,Output):
        cursor.execute('INSERT INTO Male_Conversation_Engine VALUES (?, ?)', (Input, Output))
...
def check_synonym(text):
    ...
    main=cursor.execute('SELECT request FROM Male_Conversation_Engine WHERE request REGEXP?',[tokens[0]]).fetchone()
    if main: 
...
def run_conversation_male():  
       while True:  
           ...
           if len(H) == 0 :
              B= cursor.execute('SELECT Respond FROM Male_Waiting_Engine ORDER BY RANDOM()').fetchone()
              print(("B:%s" % B))
              continue
           ...
           if check_data(H) == True:
              ...
              print(("B:%s" % B))
              continue    
           reply= cursor.execute('SELECT respoce FROM Male_Conversation_Engine WHERE request REGEXP?',[New_H]).fetchone()
           if reply:
              B=reply[0]
              print(("B:%s" % B))
              new_data(H,B)
              ...
              if check == False :
                  B= male_misunderstanding()
                  print(("B:%s" % B))
              else:
                reply_new= cursor.execute('SELECT respoce FROM Male_Conversation_Engine WHERE request REGEXP?',[check]).fetchone()
                if reply_new:
                    B=reply_new[0]
                    print(("B:%s" % B))
                    new_data(check,B)
                    cursor.execute('INSERT INTO Male_Conversation_Engine VALUES (?, ?)', (H, B))
                    conn.commit()
                    continue
                else:    
                     B= male_misunderstanding()
