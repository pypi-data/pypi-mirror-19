...
class ArabicLightStemmer:
	...
	def __init__(self):
		...
		#token pattern
        # letters and harakat
		self.token_pat=re.compile("[^\w\u064b-\u0652']+",re.UNICODE);
	######################################################################
	#{ Attribut Functions
	######################################################################
	...
	def get_starstem(self,prefix_index=-1,suffix_index=-1):
		...
		if prefix_index<0 and suffix_index<0:
			...
			if self.infix_letters!="": 
				newstarstem=re.sub("[^%s]"%self.infix_letters,self.joker,self.starword[left:right]);
			else:
	...
	def get_affix(self,prefix_index=-1, suffix_index=-1):
		...
		@rtype: unicode.
		"""
		return "-".join([self.get_prefix(prefix_index),self.get_suffix(suffix_index)])
	...
	def transformToStars(self,word):
		...
		word=re.sub("[%s]"%(ALEF_MADDA),HAMZA+ALEF,word)
	...
	def get_affix_list(self, ):
		...
		Example:
			...
			{'prefix': u'', 'root': u'فضربن', 'suffix': u'', 'stem': u'فتضرب\\u064aن'}]
