...
def name():
    return "ChipWhisperer-Rev2: SimpleSerial Target w/ Changing Frequency"
...
class userScript(QObject):
    ...
    def run(self):
        ...
        lstexample = [['CW Extra', 'CW Extra Settings', 'Trigger Pins', 'Front Panel A', False],
                      ...
                      ['CW Extra', 'CW Extra Settings', 'Clock Source', 'PLL Input'],
                      ['OpenADC', 'Clock Setup', 'ADC Clock', 'Source', 'EXTCLK Direct'],
                      ...
                      ['OpenADC', 'Clock Setup', 'Reset ADC DCM', None],
                      ['Generic Settings', 'Auxiliary Module', 'Frequency Counter'],
                      ['Frequency Measurement', 'Device', 'PicoScope 5000A'],
                      ]
        ...
        if pll.isPresent():
            ...
            for i in range(1, 6):
                pll.setupOutput(i, enabled=False)

            pll.setupOutput(0, divsource=0)
            pll.setupClockSource(diff=True)  # useIN1=True) #diff=True)
            pll.setupPLL(N=7, M=1)
            pll.setupDivider(7, 1, 0)
        else:
            print "PLL Not Found"
