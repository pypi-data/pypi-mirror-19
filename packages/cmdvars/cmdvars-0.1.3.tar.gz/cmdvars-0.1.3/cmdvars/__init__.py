...
if os.name == 'nt':
    def _get_script_env(script_path):
        try:
            out = subprocess.check_output(
                'cmd /u /c "{}" && set'.format(script_path),
                stderr=subprocess.STDOUT,
            ).decode('utf-16le', errors='replace')
        except subprocess.CalledProcessError as exc:
            raise ShellScriptException("Error executing {}".format(exc.cmd))
    ...
    def list_vars(script_path, ignore=None):
        """
        ...
        :rtype: list
        """
        return get_vars(script_path).keys()


    def get_vars(script_path, ignore=None):
        """
        ...
        if path.isfile(script_path):
            return _get_script_env(script_path)
        else:
    ...
    def get_var(script_path, var):
        ...
        :return: str
        """
        return get_vars(script_path)[var]
else:
