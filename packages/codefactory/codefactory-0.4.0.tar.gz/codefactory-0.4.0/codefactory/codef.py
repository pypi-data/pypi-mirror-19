...
@cli.command('test', short_help='run unit tests in the repo')
def test():
    """Test the repository."""
    print(os.listdir('build'))
    for target in os.listdir('build'):
        if (os.path.isdir('./build/'+target)):
            print('Testing in', target, 'dir')
            try:
                utils.run('cmake', '--build', 'build/'+target, '--target', 'runtest')
            except:
                pass
