	    ...
	    try:
	      ...
	      if rt.status_code == 200:
                if 'Etag' in rt.headers:
                    if rt.headers['Etag'] != '"%s"' % sha1_etag:
                        logger.warn("upload file {file} response with error etag : {etag1}, {etag}".format(file=self._filename, etag=rt.headers['Etag'], etag1='%s' % sha1_etag))
                        continue
                    else:
