...
def header(panel, identifier):
    table = TextTable()
    for descriptor, attr in (("Sample: ", "name"), ("Run: ", "run"), ("Panel: ", "panel")):
        try:
            table.rows.append([descriptor, getattr(identifier, attr)])
        except AttributeError:
            pass 

    for descriptor, key in (("Manifest: ", "Manifest"), ("Design Studio Bedfile:", "DesignStudio"),
                             ("Known variants file: ", "Variants"), ("Panel type:", "ReportType"), ("Minimum Depth: ", "Depth")):
        for catagory in ("file_names", "options"):
            if key in getattr(panel, catagory):
                table.rows.append([descriptor, str(getattr(panel, catagory)[key])])
                break
