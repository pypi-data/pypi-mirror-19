...
Compression
- Update/Fix broken JSON P-Mode generation

Keys/X.509 and Signing:
...
- External payloads or TransportChannel or Transport subdirectory or filename ?

- TransportChannel,  example with simple FTP, SFTP or REST service
...
-  Certificate chain validation only matches the root CA.  Perhaps it should
check all non-leaf certificates?  Or have some way to configure this.

New features:
-  DelegationChannel for three-corner or four-corner models
...
class CPABuilder():
    ...
    def __init__(self, nsmap = {}, agreementidfun = None, default_handler = None):
        ...
        self.mimepart_handlers = {
            ...
            cppa('CompressedSimpleMIMEPart'):
                self.unify_compressed_mime_part,
            cppa('MIMEMultipartRelated'):
                self.unify_mime_multipart_related,
            cppa('CompressedMIMEMultipartRelated'):
                self.unify_compressed_mime_multipart_related,
    ...
    def init_activation_date(self, acpp, bcpp, agreementinfo,
        ...
        for cpp in [acpp, bcpp]:
            activationl = cpp.xpath(
                'child::cppa:ProfileInfo/cppa:ActivationDate/text()',
                namespaces=self.NSMAP)
        ...
        for cpp in [acpp, bcpp]:
            expirationL = cpp.xpath(
                'child::cppa:ProfileInfo/cppa:ExpirationDate/text()',
                namespaces=self.NSMAP)
        ...
        if not expirationdefault is None:
            expirationdate = lxml.etree.SubElement(agreementinfo,
                                                   cppa('ExpirationDate'))
                        ...
                        if appid is not None and bppid is not None:
                            ...
                            logging.info(
                                'Setting attribute to {} for {} {} {} {}'.format(
                                    ...
                                    bcppid,
                                    bppid))
                            actionbinding.set('payloadProfileId',
                                              self.cppaid(acppid, appid, bcppid, bppid))
    ...
    def unify_actionbinding(self, acppid, acpp, bcppid, bcpp, service, action,
        ...
        logging.info(
            "Merging action bindings {} and {} for {} -- {}".format(aid, bid, service, action)
        )
    ...
    def unify_channels_memo(self, acpp, bcpp, abchannelid,
                            a_channelid, b_channelid, direction):
        (acppid, acid, bcppid, bcid) = abchannelid
    ...
    def unify_channel_max_size(self, a_channel, b_channel, ab_channel):
        ...
        if a_max_size == b_max_size == 0:
            ...
            logging.info(
                "Unifying Maxsize, {} {} {}".format(a_max_size, b_max_size, ab_max_size))
    ...
    def unify_ws_addressing(self, acpp, bcpp, abxid,
        ...
        self.unify_wsa_from(acpp, bcpp, abxid, a_addressing, b_addressing, ab_addressing, direction)
        self.unify_complex_subelement(acpp, bcpp, abxid, a_addressing, b_addressing, ab_addressing,
                                      'cppa', 'ebMS3InferredRoutingInput',
                                      self.unify_routinginput, direction)

    def unify_routinginput(self, acpp, bcpp, abxid,
                           a_addressing, b_addressing, ab_addressing, direction):
        self.unify_simple_subelement(a_addressing, b_addressing, ab_addressing,
    ...
    def unify_wsa_from(self, acpp, bcpp, abxid,
                       a_addressing, b_addressing, ab_addressing, direction):
        if direction == 'send':
    ...
    def unify_saml_key_confirmed_subject(self, acpp, bcpp, abxid,
                                               asec, bsec, security, direction):
        (acppid, axid, bcppid, bxid) = abxid
        security.set('id', '{}_{}'.format(asec.get('id'), bsec.get('id')))
        self.unify_simple_subelement(asec, bsec,security, 'cppa', 'SAMLVersion',
                                     required=True)
        self.unify_idp_registration_set_ref(acpp, bcpp, abxid,
                                            asec, bsec, security, direction)
        self.include_sender_signing_certificate_ref(acpp, bcpp, abxid,
                                            asec, bsec, security, direction)
        self.unify_saml_attributes(acpp, bcpp, abxid,
                                   asec, bsec, security, direction)
        self.unify_simple_subelement(asec, bsec,security, 'cppa', 'KeyType',
                                     required=True)

    def unify_saml_attributes(self, acpp, bcpp, abxid,
    ...
    def include_sender_signing_certificate_ref(self, acpp, bcpp, abxid,
                            acppel, bcppel, abcpael, direction):
        if direction == 'send':
    ...
    def unify_idp_registration_set_ref(self, acpp, bcpp, abxid,
                                       acppel, bcppel, abcpael, direction):
    ...
    def unify_signature(self, acpp, bcpp, abxid,
        ...
        self.unify_saml_token_ref(acpp, bcpp, abxid, asec, bsec, security, direction)
        self.unify_complex_subelement(acpp, bcpp, abxid, asec, bsec, security,
                                      'cppa', 'SignElements',
    ...
    def unify_saml_token_ref(self, acpp, bcpp, abxid,
                            acppel, bcppel, abcpael, direction):
    ...
    self.unify_bundling
    """
    def unify_bundling(self, acpp, bcpp, abxid,
                       ael, bel, abel, direction):
        (acppid, axid, bcppid, bxid) = abxid
        self.unify_complex_subelement(acpp, bcpp, abxid, ael, bel, abel,
                                      'cppa', 'Ordering',
    ...
    def unify_bundling_ordering(self, acpp, bcpp, abxid,
                       ael, bel, abel, direction):
    ...
    """
    def unify_splitting(self, acpp, bcpp, abxid,
                        ael, bel, abel, direction):
        (acppid, axid, bcppid, bxid) = abxid
        pass
    ...
    def unify_encryption_cert_and_anchor_send(self, acppid, acpp, bcppid, bcpp,
        ...
        if len(a_encryption_anchorL) == 1 and len(b_encryption_certL) == 1:
            ...
            aanchor = acpp.xpath(
                'cppa:PartyInfo/cppa:TrustAnchor[@id="{}"]'.format(aanchorid),
                namespaces=self.NSMAP)[0]
            bcert = bcpp.xpath(
            ...
            if len(bx509certl) > 0:
                ...
                for a_anchor_ref in aanchor.xpath('cppa:AnchorCertificateRef',
                    ...
                    if check_x509_data_content(encryptioncertid,
                                               bx509rootcert,
                                               a_anchor_certid,
                                               acpp):
                        rootfound = True
                if not rootfound:
                    for embedded_cert in aanchor.xpath(
                            'cppa:Certificate',
                            namespaces=self.NSMAP
                    ):
                        certid = embedded_cert.get('id')
                ...
                if not rootfound:
                    raise UnificationException(
                        'Cert {} does not match a root cert in {}'.format(encryptioncertid,
                                                                          aanchorid))
                ...
                if atransport.tag != btransport.tag:
                    raise UnificationException(
                        'Mismatch in transport type: {} vs {}'.format(atransport.tag,
                                                                      btransport.tag))
                transportxml = lxml.etree.Element(atransport.tag,
        ...
        if len(a_server_anchorL) == 1 and len(b_server_certL) == 1:
            ...
            if len(bx509certL) > 0:
                ...
                for arootcert in aanchor.xpath('cppa:Certificate',
                    ...
                    if arootcert.xpath(
                            'descendant::ds:X509Certificate/text()="{}"'.format(bx509rootcert),
                        namespaces=self.NSMAP):
                        logging.info(
                            'Referenced X509Certificate found in anchor {} cert {}'.format(
                                aanchorid,
                                arootcertid)
                        )
                        rootfound = True
                if not rootfound:
                    raise UnificationException(
                        'Cert {} does not match a root cert in {}'.format(server_certid,
                                                                          aanchorid))
        else:
            logging.info('No encryption anchor/cert specified')
    ...
    def unify_tls_client_cert_and_anchor_send(self, acppid, acpp, bcppid, bcpp,
        ...
        if len(b_client_anchorL) == 1 and len(a_client_certL) == 1:
            ...
            acert = acpp.xpath('cppa:PartyInfo/cppa:Certificate[@id="{}"]'.format(client_certid),
                               namespaces=self.NSMAP)[0]
            banchor = bcpp.xpath(
                'cppa:PartyInfo/cppa:TrustAnchor[@id="{}"]'.format(banchorid),
                namespaces=self.NSMAP)[0]
            ...
            if len(ax509certL) > 0:
                ...
                for arootcert in banchor.xpath('cppa:Certificate',
                    ...
                    arootcertid = arootcert.get('id')
                    if arootcert.xpath(
                            'descendant::ds:X509Certificate/text()="{}"'.format(ax509rootcert),
                        namespaces=self.NSMAP):
                        logging.info(
                            'Referenced X509Certificate found in anchor {} cert {}'.format(
                                banchorid,
            ...
            if not(rootfound):
                raise UnificationException(
                    'Cert {} does not match a root cert in {}'.format(client_certid,
                                                                      banchorid))
        else:
            logging.info('No encryption anchor/cert specified')


    def unify_transport_restart(self, acpp, bcpp, abxid,
                                a, b, ab, direction):
    ...
    def unify_compressed_mime_part(self, apart, bpart, abxid,
        ...
        self.unify_simple_subelement(apart, bpart, mimepart,
                                     'cppa', 'CompressionType',
                                     required=True, strictelements=False)
    ...
    def unify_compressed_mime_multipart_related(self, apart, bpart, abxid,
                                     acpp, bcpp, direction):
        mimepart = lxml.etree.Element(apart.tag, nsmap=self.NSMAP)
        unify_atts(apart, bpart, mimepart, strictatts=True)
        self.unify_mime_part_lists(mimepart, apart, bpart, abxid,
    ...
    def unify_complex_subelement(self,
        ...
        try:
            ...
            if aelementcount != belementcount:
                raise UnificationException(
                    'Mismatch in count for child(ren) {}'.format(childtag)
                )
