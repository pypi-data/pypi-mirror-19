...
class LinkFeedResourceToDistribution(hook.Hook):
    ...
    def __call__(self):
        ...
        logmsg = 'added file #%d (produced by process #%d) as a new version of distribution %#d'
        # No previous file distribution, setting one.
        if not rset:
            self._cw.execute(
                'SET F file_distribution D WHERE D eid %(d)s, F eid %(f)s',
                {'d': distr_eid, 'f': newfile_eid})
            logmsg = 'added file #%d (produced by process #%d) to distribution %#d'
        self.debug(logmsg, newfile_eid, self.eidto, distr_eid)
...
class AddCWSourceForResourceFeedHook(hook.Hook):
    ...
    def __call__(self):
        ...
        rset = self._cw.find('CWSource', url=url)
        if rset:
            source = rset.one()
        else:
            name = url.split('://')[-1][:128]
            config = u'\n'.join([
                u'use-cwuri-as-url=no',
                u'synchronization-interval=24h',
            ])
