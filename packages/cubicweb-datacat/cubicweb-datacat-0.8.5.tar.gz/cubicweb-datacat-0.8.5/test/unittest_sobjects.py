...
class BaseResourceFeedTC(CubicWebTC):
...
class ResourceFeedParserTC(BaseResourceFeedTC):
    ...
    def test_base(self):
        ...
        self.assertIn('attaching file #{0} to resourcefeed #{1}'.format(
            feid, resourcefeed.eid), logs)
        self.assertIn(xml_escape('attaching file #{0} to resourcefeed\'s distribution').format(
            feid), logs)
        with self.admin_access.repo_cnx() as cnx:
