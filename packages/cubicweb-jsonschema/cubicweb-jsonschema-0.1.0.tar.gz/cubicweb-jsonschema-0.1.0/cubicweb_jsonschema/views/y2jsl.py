...
class JslDocumentBuilder(Component):
    ...
    `__regid__` is "jsonschema.map.relation" in the "components" register.
    ...
    def fields_from_yams(self, etype, filter_in=None, filter_out=None,
        ...
        return OrderedDict((rtype, self.generate_field(etype, rtype, role, schema_role))
    ...
    def generate_field(self, etype, rtype, role, schema_role,
        ...
        if target_types is None:
            ...
            target_types = {target.type for target in rschema.targets(etype, role)}
        assert isinstance(target_types, set), target_types
        field_factory = self._cw.vreg['components'].select_or_none(
...
def y2j_etype(vreg, cnx, schema_role=CREATION_ROLE):
    ...
    This jsl document is built using registered 'jsonschema.map.builder' component.
    """
    def decorator(cls):
