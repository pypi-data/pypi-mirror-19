...
class MercurialServerRepositoryCreation(Hook):
    ...
    def __call__(self):
        ...
        try:
            ...
            self.info('init-ed repository %s', uri)
...
class MSCDeletion(Hook):
    ...
    def __call__(self):
        ...
        hgadmin = msc.hgadmin_repo

        if not osp.exists(hgadmin.localcachepath):
            clone_to_local_cache(hgadmin)

        tmpdir = tempfile.mkdtemp()
        try:
            hgrepo = hglib.clone(str(hgadmin.localcachepath), tmpdir)
            with hgrepo:
                hgrepo.pull(source=hgadmin.source_url.encode('ascii'))
                hgrepo.update()

                with open(osp.join(hgrepo.root(), msc.conf_filename), 'wb') as conf_file:
                    conf_file.write('\n')
                hgrepo.remove(osp.join(hgrepo.root(), 'keys', msc.keys_subdir))
                cset = hgrepo.commit(message=message)[1]
                hgrepo.push(dest=hgadmin.source_url.encode('ascii'))
        finally:
            shutil.rmtree(tmpdir)

        rbdata = (msc.eid, msc.hgadmin_repo.eid, msctitle, cset)
...
class MSCRollbackOnDelete(DataOperationMixIn, Operation):
    ...
    def rollback_event(self):
        for msceid, repoeid, msctitle, cset in self.get_data():
            hgadmin = self.cnx.entity_from_eid(repoeid)
            message = 'backout `%s` (eid %s) deletion' % (msctitle, msceid)
            # the backout in the managed context may **or may not**
            # commit (depending on whether the the backed out
            # changeset has children; see `hg help backout`) AND
            # pull_push will **not** push is the commit is empty, so
            # need an explicit push here...
            with hgadmin.pull_push(message) as hgrepo:
                hgrepo.backout(rev=cset, message=message)
                hgrepo.push(dest=hgadmin.source_url.encode('ascii'))
    ...
    def postcommit_event(self):
        with self.cnx.repo.internal_cnx() as cnx:
            for msceid, in cnx.execute('MercurialServerConfig M').rows:
                cnx.call_service('regenerate_hgadmin_repo', eid=msceid,
                                 userdesc=self.cnx.user.format_name_for_mercurial())
            cnx.commit()
...
def remove_key(cnx, repo, keypath):
    ...
    cnx.info('remove public key %s', filename)
...
def add_key(cnx, repo, keypath, pubkey):
    ...
    with open(filename, 'wb') as keyfile:
        ...
        cnx.info('add public key %s', filename)
...
class RegenerateMSCRepoService(Service):
    ...
    def call(self, eid, userdesc=None):
