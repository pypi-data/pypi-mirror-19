...
class VCWiki(AnyEntity):
    ...
    def repository(self):
        ...
        return self.content_repo and self.content_repo[0] or None
    ...
    def manifest(self, revision=None):
        if revision is None:
            revision = self.current_changeset()
        return self.repository.cw_adapt_to('VCSRepo').manifest(rev=revision)
        
    def page_exists(self, path, revision=None):
        return path in self.manifest(revision)
    ...
    def breadcrumbs(self, view=None, recurs=None):
        ...
        if wikipath and self.vcpath in vcwiki: # skip root path and 404
            ...
            while dirs:
                dirpath = u'/'.join(dirs)
                dirname = dirs.pop()
                if vcwiki.page_exists(vcwiki.vcpage_path(dirpath)):
