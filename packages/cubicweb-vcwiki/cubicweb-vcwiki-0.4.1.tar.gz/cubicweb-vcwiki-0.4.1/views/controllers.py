...
class VCWikiViewController(ViewController):
    ...
    def publish(self, rset=None):
        if self.vcwiki is None or self.vcwiki.repository is None:
...
class VCWikiEditController(PreviewControllerMixin, EditController):
    ...
    def create_new_revision(self, vpath, content, msg):
        ...
        new_rev = self.vcwiki.repository.make_revision(
            msg=msg.encode('utf-8'), author=author.encode('utf-8'),
            added=added, deleted=deleted, parentrev='tip').changeset
        repo = self.vcwiki.repository
        if repo.source_url:
            with repo.cw_adapt_to('VCSRepo').hgrepo() as client:
                client.push(dest=repo.source_url, rev=new_rev, force=True)
 ...
 window.parent.handleFormValidationResponse('form', null, null, %s, null);
    ...
    def _action_preview(self):
        ...
        data = TransformData(data, format, encoding, appobject=self.vcwiki)
