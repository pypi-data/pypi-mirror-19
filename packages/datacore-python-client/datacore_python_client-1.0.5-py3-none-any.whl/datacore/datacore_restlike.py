...
import datetime
import logging
from typing import AnyStr, Any, Dict, Iterable, Generator, Sequence, Optional, Union

# Non-standard library imports
import requests

# Package imports
from datacore.types import AggregationQuery, EndpointDefinition, Endpoints, Header, Query, Payload
from datacore.exceptions import DatacoreException
...
class Access(object):
    ...
    def endpoints_from_definition(self, endpoint_definition: EndpointDefinition) \
            -> Endpoints:
        """
        ...
        :rtype: Endpoints
        """
        # First assert that the input is formatted correctly
        # Expectd keys
        expected = {"base_read", "base_write", "api_name", "api_version"}
        # Are they all there?
        try:
            for key in expected:
                assert key in endpoint_definition
        # No, so raise the datacore exception
        except AssertionError:
            raise DatacoreException(
                "BadEndpointDefinition",
                "Missing necessary values in endpoint definition; expected: {e}, got: {g}".format(
                    e=expected,
                    g=endpoint_definition.keys()
                )
            )
        endpoints = {
            "read": endpoint_definition["base_read"] + "/r/{t}/{v}/{u}".format(
                t=endpoint_definition["api_name"],
                v=endpoint_definition["api_version"],
                u=self.user
            ),
            "write": endpoint_definition["base_write"] + "/w/{t}/{v}/{u}".format(
                t=endpoint_definition["api_name"],
                v=endpoint_definition["api_version"],
                u=self.user
            ),
            "bulk": endpoint_definition["base_write"] + "/w/bulk/{t}/{v}/{u}".format(
                t=endpoint_definition["api_name"],
                v=endpoint_definition["api_version"],
                u=self.user
            ),
            "agg": endpoint_definition["base_read"] + "/r/agg/{t}/{v}/{u}".format(
                t=endpoint_definition["api_name"],
                v=endpoint_definition["api_version"],
                u=self.user
            ),
    ...
    def number_of_entries(self, url: AnyStr, query: Query) \
            -> int:
        """
        ...
        if response:
            # If the response is non empty, we should be able to return the number of rows.
            return response[0]["count"]
        else:
            # If the response was empty, that means no data is available. That is to say, zero such rows.
            return 0

    def check_date(self, url: AnyStr, match: Query, group: Query, date_field: AnyStr) \
    ...
    def get(self, url: AnyStr, query: Union[AggregationQuery, Query]=None, fields: Optional[Iterable[str]]=None, sort: AnyStr='', sync: bool=False, agg: bool=False, limit: int=1000) \
            -> Payload:
        ...
        while page < max_page:
            ...
            if sort:
                # In aggregations, this can be placed in the query
                qurl += "&sort=" + sort
    ...
    def yield_get(self, url: AnyStr, query: Optional[Query]=None, fields: Optional[Iterable[str]]=None, limit: int=250, sort: AnyStr='') \
            -> Generator[Payload, None, None]:
        """
        Yields dataCore results for a given page by page.

        There is no support for aggregation queries. For aggregation queries, please use get() and segment
        the query as finely as needed.
        ...
        :param sort: (optional)string(sort_condition)
        :return data: list(data) where entries are dicts. May be empty.
        :param limit: (optional) limit number of rows.
        :raises DatacoreException: when the issue is in the interaction with datacore
        """
    ...
    def _sub_post(self, url: AnyStr, data_set: Payload, sync: bool=False)\
            -> None:
