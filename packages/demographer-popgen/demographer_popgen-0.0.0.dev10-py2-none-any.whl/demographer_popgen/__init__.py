...
class DemoGrapher:
    ...
    def change_units(self):
        if self.new_units == self.old_units:
            ...
            if self.new_units == 'Coalescent units':
                ...
                #self.maxne_label = tk.Label(self.root, text = "Max population sizes (Ne)")
                #self.nus_label = tk.Label(self.root, text="Sizes (Ne)")
                #self.ts_label = tk.Label(self.root, text="Epoch bds (gens)")
                #self.maxt_label = tk.Label(self.root, text="Max time (gens)")
                ...
                #self.maxne_label = tk.Label(self.root, text = "Max population sizes (Ne)")
                #self.nus_label = tk.Label(self.root, text="Sizes (Ne)")
                #self.ts_label = tk.Label(self.root, text="Epoch bds (years)")
                #self.maxt_label = tk.Label(self.root, text="Max time (years)")
                if not (self.maxnes is None):
                    self.maxnes_display = deepcopy(self.maxnes)
                    for i in range(0,len(self.maxnes_display)):
                        self.maxnes_display[i] = self.maxnes[i] * self.Nref
            ...
            #self.app_hist_window.maxt_ent.delete(0, tk.END)
            #if type(self.maxt_display).__module__ == 'numpy':
            #    self.app_hist_window.maxt_ent.insert(0, json.dumps(self.maxt_display.tolist()))
            #else:
            #    self.app_hist_window.maxt_ent.insert(0, json.dumps(self.maxt_display))
            ...
            self.old_units = self.new_units
            self.new_units = None
            #self.maxt_label.grid_forget()
            #self.maxne_label.grid_forget()
            #self.nus_label.grid_forget()
            #self.ts_label.grid_forget()
            #self.mig_label.grid_forget()            
            #self.maxt_label.grid(row = 3, column = 1, sticky = tk.E)
            #self.maxne_label.grid(row = 1, column = 3, sticky = tk.E)
            #self.nus_label.grid(row = 2, column = 3, sticky = tk.E)
            #self.ts_label.grid(row = 3, column = 3, sticky = tk.E)
            #self.mig_label.grid(row = 5, column = 3, sticky = tk.E)
            
            #if (self.new_units == 'Ne/years') or (self.new_units == 'Ne/generations'):
            #    self.Nref_label.grid(row = 5, column = 1, sticky = tk.E)
            #    self.Nref_ent.delete(0, tk.END)
            #    self.Nref_ent.insert(0, json.dumps(np.array(self.Nref).tolist()))
            #    self.Nref_ent.grid(row = 5, column = 2, sticky = tk.W)                
            #else:
            #    self.Nref_label.grid_forget()
            #    self.Nref_ent.grid_forget()
            #
            #if self.new_units == 'Ne/years':
            #    self.years_per_gen_label.grid(row = 6, column = 1, sticky = tk.E)
            #    self.years_per_gen_ent.grid(row = 6, column = 2, sticky = tk.W)                
            #else:
            #    self.years_per_gen_label.grid_forget()
            #    self.years_per_gen_ent.grid_forget()
    ...
    def load_session(self):
        if self.load_session_path is not None:
            ...
            self.ax1.cla()
            self.hist = DraggablePopHistory(self,self.fig1, self.ax1, deepcopy(self.ts), deepcopy(self.nus), deepcopy(self.expmat), deepcopy(self.migs), deepcopy(self.maxnes), deepcopy(self.maxt), deepcopy(self.plotdt)/10, self.new_units, self.Nref, self.years_per_gen)
            ...
            if self.new_units == 'Ne/generations':
    ...
    def setup_session(self,npops,maxne):
        ...
        if self.new_units == 'Units':
            ...
            return
        if self.new_units == 'Coalescent units':
            self.max_popsize = maxne
        if self.new_units == 'Ne/generations':
            self.max_popsize = maxne/self.Nref
        elif self.new_units == 'Ne/years':
            self.max_popsize = maxne/self.Nref
    ...
    def on_create_hist_button_call(self):                
        ...
        #self.fig1 = self.app_hist_plot_window.fig1
        #self.ax1 = self.app_hist_plot_window.ax1
        #self.ax1.cla()  
        self.fig1 = plt.figure(facecolor = 'white')
        self.ax1 = self.fig1.add_subplot(111)  
        self.ax1.cla()
        #self.app_hist_plot_window.ax1.cla()
        
        #for i in range(0,len(self.migs)):
        #    self.migs[i][1] = self.migs[i][1].astype(int)
        #    self.migs[i][2] = self.migs[i][2].astype(int)
    ...
    def on_load_session_button_call(self):
        ...
        if self.demog_main.load_session_path is not None:
            ...
            if self.max_popsize_ent.get():
                ...
                #self.maxnes = np.array([self.max_popsize] * self.npops)
                #for i in range(0,self.npops):
                #    self.maxnes[i] = copy.deepcopy(self.max_popsize)
            else:
    ...
    def get_values(self):
        ...
        if self.nus_ent.get():
            ...
            if self.demog_main.old_units == 'Coalescent units':
                ...
                self.demog_main.nus = nus_unsc / self.demog_main.Nref
            elif self.demog_main.old_units == 'Ne/years':
                self.demog_main.nus = nus_unsc / self.demog_main.Nref            
        else:
        ...
        if self.ts_ent.get():
            ...
            if self.demog_main.old_units == 'Coalescent units':
                ...
                self.demog_main.ts = ts_unsc / (2 * self.demog_main.Nref)
            elif self.demog_main.old_units == 'Ne/years':
                self.demog_main.ts = ts_unsc / (2 * self.demog_main.Nref * self.demog_main.years_per_gen)
        else:
...
class update_units_window:
    def __init__(self, root, demog_main):
        ...
        self.units_list = ['Coalescent units','Eff. Size and Generations','Eff. Size and Years']
        self.units = tk.StringVar()
        self.units.set('Coalescent units')
        self.units_menu = tk.OptionMenu(root,self.units,*self.units_list, command = self.update_units_textboxes)
        self.units_menu.grid(row = 1, column = 2, sticky = tk.EW)        
...
class ms_sim_command_window:
    def __init__(self, root, demog_main,hwxy):
        ...
        'Optional (e.g., 2): Gene conversion parameter f = g/r, where g is the probability that gene conversion initiates, per site, in each generation and r is the probability of a gene conversion recombination, per-site, per-generation.')
    ...
    def on_get_command_button_call(self):
        ...
        if self.num_seqs_ent.get():       
            ...
            if self.num_seqs is not None:
                if self.num_seqs.dtype.char == 'S':
                    ...
                    self.num_seqs = str(mtlb.repmat(self.num_seqs,1,self.demog_main.npops)[0])
        else:
            self.num_seqs = None
        ...
        if self.geneconv_g is not None:
            if self.geneconv_lambda is not None:
    ...
    def on_plot_sfs_button_call(self):
        if self.nentries_ent.get():
            ...
            if np.array(self.nentries).dtype.char == 'S':
                self.demog_main.output_box.insert(INSERT,'ERROR: Specify numeric value for numbers of entries.\n')
                self.demog_main.output_box.see(tk.END)
                self.nentries = None
                return
        else:
        ...
        if self.sfs_plotstyle == 'Plot style':
            self.demog_main.output_box.insert(INSERT,'ERROR: Specify plot style (Line or Heatmap).\n')
            self.demog_main.output_box.see(tk.END)
            return
        ...
        if self.nlft_entries_ent.get():
            ...
            if np.array(self.nlft_entries).dtype.char == 'S':
                self.demog_main.output_box.insert(INSERT,'ERROR: Specify numeric value for numbers of samples.\n')
                self.demog_main.output_box.see(tk.END)
...
class DraggablePopHistory:
    def __init__(self, demog_main, fig, ax, ts, nus, expmat, migs, maxnes, maxt, plotdt, units, Nref, years_per_gen):
        ...
        self.popup_textbox_change = None
    ...
    def on_press(self, event):        
            ...
            for j in range(0,len(self.ts[i])):
                ...
                if contains:
                    ...
                    if event.key == 'v': #Make popup window for user to enter precise values
                        ...
                        popup_window = popup_epoch_textfield(self.demog_main,popind,epind,x+20,y+20)                        
                        #popup_window.toplevel.deiconify()
                        #popup_window.toplevel.wait_window()
        ...
        if self.del_epoch: #Remove the epoch and all the graphics associated with it
            ...
            #print "New epoch complete >>>>>>>>>>>> LINE 1773"
    ...
    def on_motion(self, event):
        ...
        if self.popup_textbox_change is not None: return
    ...
    def on_release(self, event):
        ...
        if self.popup_textbox_change is not None:
            self.selected_ind = None
            self.press = None
            self.popup_textbox_change = None
        if self.lock is not self: return
