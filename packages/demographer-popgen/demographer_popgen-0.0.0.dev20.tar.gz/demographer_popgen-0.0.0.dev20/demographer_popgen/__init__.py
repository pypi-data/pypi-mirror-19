...
class setup_window:
    ...
    def on_load_session_button_call(self):
        ...
        self.demog_main.app_output_box_window.output_box.insert(INSERT,'   edit its numerical value. WARNING: there is a \n')
        self.demog_main.app_output_box_window.output_box.insert(INSERT,'   known bug with "v" (being resolved). If your \n')
        self.demog_main.app_output_box_window.output_box.insert(INSERT,'   window freezes, close it and click "Draw \n')
        self.demog_main.app_output_box_window.output_box.insert(INSERT,'   History" again.\n')
...
class able_command_window:
    def __init__(self, root, demog_main,hwxy):
        ...
        self.root.geometry('%dx%d+%d+%d' % hwxy)
        self.root.wm_title("Command (ABLE)")
        ...
        self.get_command_button = tk.Button(master = self.root, text="Get command", command = self.on_get_command_button_call)
        self.get_command_button.grid(row = 7, column = 2, sticky = tk.W)          
    ...
    def on_get_command_button_call(self):
        ...
        self.command = 'ABLE ' + str(nhap) + ' ' + str(nrep) + ' '
        ...
        for i in range(0,self.demog_main.hist.npops):
            ind = np.array(i * 1).astype(int)
            sz = self.demog_main.hist.nus[i][0] * 1
            if self.demog_main.hist_infer.nus[i][0] is not None:
                self.command = self.command + '-n ' + str(ind+1) + ' ' + 'tbi' + ' '
            else:
                self.command = self.command + '-n ' + str(ind+1) + ' ' + str(sz) + ' '
            if self.demog_main.hist.gammas[i][0] != 0:
                if self.demog_main.hist_infer.gammas[i][0] is not None:
                    self.command = self.command + '-g ' + str(ind+1) + ' ' + 'tbi' + ' '
                else:
                    g = -2*self.demog_main.hist.gammas[i][0] * 1 #Demographer measures time in units of 2Nref generations. ms measures it in units of 4Nref generations. So we scale g by 2 when passing it to ms. Also, we scale by -1 because ms defines the rate to have the opposite sign.
                    self.command = self.command + '-g ' + str(ind+1) + ' ' + str(g) + ' '
        ...
        for i in range(0,len(event_type_vec)):
            ...
            if event_type_vec[i] == 1: #It's a size change
                if time > 0:
                    ...
                    tstr = str(time)
                    szstr = str(sz)
                    if self.demog_main.hist_infer.ts[popind][epind] is not None:
                        tstr = 'tbi'
                    if self.demog_main.hist_infer.nus[popind][epind] is not None:
                        szstr = 'tbi'
                    self.command = self.command + '-en ' + tstr + ' ' + str(popind+1) + ' ' + szstr + ' '
                    #if self.demog_main.hist.expmat[popind][epind] is None:
                    gam = -2*self.demog_main.hist.gammas[popind][epind] #Demographer measures time in units of 2Nref generations. ms measures it in units of 4Nref generations. So we scale g by 2 when passing it to ms. Also, we scale by -1 because ms defines the rate to have the opposite sign.
                    gamstr = str(gam)
                    if self.demog_main.hist_infer.gammas[popind][epind] is not None:
                        gamstr = 'tbi'
                    self.command = self.command + '-eg ' + ' ' + tstr + ' ' + str(popind+1) + ' ' + gamstr + ' '
            elif event_type_vec[i] == 2: #It's a migration start
                migind = event_index_vec[i]
                if self.demog_main.hist.migs[migind][3] >= 0:
                    startpop = np.array(self.demog_main.hist.migs[migind][1]).astype(int)
                    ...
                    frac = -self.demog_main.hist.migs[migind][3]
                dur = self.demog_main.hist.migs[migind][4]
                if self.demog_main.hist_infer.migs[migind][4] is None:
                    if dur == 0:
                        tstr = str(time)
                        fracstr = str(1-frac)
                        if self.demog_main.hist_infer.migs[migind][0] is not None:
                            tstr = 'tbi'
                        if self.demog_main.hist_infer.migs[migind][3] is not None:
                            fracstr = 'tbi'
                        self.command = self.command + '-es ' + tstr + ' ' + str(startpop) + ' ' + fracstr + ' '
                        pop_ct = pop_ct + 1
                        self.command = self.command + '-ej ' + tstr + ' ' + str(pop_ct) + ' ' + str(stoppop) + ' '
                    else:
                        tstr = str(time)
                        mstr = str(4*frac*self.Nref)
                        if self.demog_main.hist_infer.migs[migind][0] is not None:
                            tstr = 'tbi'
                        if self.demog_main.hist_infer.migs[migind][3] is not None:
                            mstr = 'tbi'
                        self.command = self.command + '-em ' + tstr + ' ' + str(startpop) + ' ' + str(stoppop) + ' ' + mstr + ' '
                else:
                        tstr = str(time)
                        mstr = str(4*frac*self.Nref)
                        if self.demog_main.hist_infer.migs[migind][0] is not None:
                ...
                if self.demog_main.hist.migs[migind][3] >= 0:
                    ...
                    frac = -self.demog_main.hist.migs[migind][3]
                tstr = str(time)
                if (self.demog_main.hist_infer.migs[migind][0] is not None) or (self.demog_main.hist_infer.migs[migind][4] is not None):
                    tstr = 'tbi'
                self.command = self.command + '-em ' + tstr + ' ' + str(startpop) + ' ' + str(stoppop) + ' ' + str(0) + ' '
