...
Most of the parameters can be checked in the docker official
`documentation <https://docs.docker.com/reference/run/>`_
or in the `docker-py api documentation <http://docker-py.readthedocs.org/en/latest/api/>`_
Still not all functionalities are implemented but minimal to start a docker container with basic
configuration::
...
* *apt_install* (not implemented yet): install additional packages using apt-get install (this will
  be changed to use a generic way of installing packages in other distros)
...
* *mem_limit*: Amount of ram allowed to use by the container, this is the max value
* *ports*: a dict with ports mapping, key is the port number inside the container, value the host
port.
  If None is provides as value the port number will be assigned randomly by docker service.
...
class DockerV(object):
    """ Helper class for making it easier docker container deployment by code
        ...
        There are two required parameters *image_name* and *command*, others parameters are
        optionals, if you put none in the port it will be autoasigned by docker
        ...
        Only is needed one of them or you can specify both but id will be checked first then the
        container_name
    """
    __config = {}
    __docker_id = None
    ...
    def docker_env(self):
        ...
        :return: dict with var name as key and value
        """
        try:
    ...
    def deploy_container(self):
        ...
        container, for more detail see :func:`~deployv.base.dockerv.DockerV.basic_info` for more
        information about the format and content

        :return: Some basic information about the container
        """
    ...
    def remove_container(self):
        """ Removes the specified container, first stop it. Docker id or name must be in lowercase
        if not it will forced to.
    ...
    @property
    def basic_info(self):
        """ Get some basic information form the container:
        ...
        :return: Return a dict with the information
        """
        inspected = self.inspect()
        res = {}
    ...
    def exec_cmd(self, cmd):
        """ Wraps and log the
        `exec_create <https://docker-py.readthedocs.org/en/latest/api/#exec_create>`_
        and `exec_start <https://docker-py.readthedocs.org/en/latest/api/#exec_start>`_ methods
    ...
    def inspect(self, container_id=False):
        """ Wrapper to docker-py.inspect _container

        :param container_id: Optional parameter with the container name or id

        :return: Information about the container. See
            `inspect_container in the official doc
    ...
    def install_packages(self, apt_packages=None, pip_packages=None):
        """ Install apt and/or pip packages inside the container and return the full operation log
        besides do it quietly, so the log is cleaner. The result have the following
        format if no error:
            ...
            .. code-block:: python

                {
                    'apt_install': [
                        { 'package_name': {
                            'installed': False,
                            'message': 'Error message generated by the installer'
                            }
                        }
                    ]
                    'pip_install': [
        ...
        The results can be mixed in case of just one fail durint the process

        :param apt_packages: List with the packages to be installed via apt
        :param pip_packages: List with the packages to be installed via pip
        :return: Full log operations
        """
        res = {}
    ...
    def _compose_message(self, package_installer, package, result):
        """ Composes the response with the following format:
            ...
            .. code-block:: python

                {
                    'package_name': {
                        'installed': Boolean,
                        'message': ('if any error, this will have the error message,'
                                    ' else won't be present)
                    }
                }
        ...
        :return: Formatted output in a json format
        """
        res = {package: {}}
    ...
    def pull(self, image_name):
        """ This is a simple wrapper for docker pull to use the deployv exceptions instead of
        the docker ones
    ...
    def inspect_image(self, image_name):
        ...
        :return: The result of docker inspect_image, raises an exception
