...
class NginxV(object):
    ...
    :param process_name: Process name inside supervisor config file, by default nginx
    :param supervisor_url: Url used to stablish communication with supervisor,
        by default 'unix:///var/run/supervisor.sock'
    """
    def __init__(self,
                 nginx_working_dir,
                 process_name='nginx',
        ...
        self._supervisor_server = xmlrpclib.ServerProxy(
            'http://127.0.0.1',
            transport=supervisor.xmlrpc.SupervisorTransport(
                None, None, serverurl=supervisor_url))
    ...
    def restart(self):
        """ Restart the nginx process, generally is only needed when the templates are updated
    ...
    def update_sites(self, sites):
        ...
        :param sites: List of dicts with domains and ports in the following format:
