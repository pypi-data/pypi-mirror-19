...
EVENTLISTENER_META = extend_me.ExtensibleByHashType._('EventListener', hashattr='name')


def load_extensions():
    """ This module just loads all the extensions from the corresponding path

    :return: Dict with the loaded modules
    """
    modules = {}
...
@six.add_metaclass(EVENTLISTENER_META)
class EventListenerBase(object):
    """ This is the base class for all the events listener that will be developed in the base
    commands and new ones, all inherited classes must have a class Meta with the required
    information. The is a small example in :class:`~deployv.base.extensions_core.TestListener`
    with the basic information configured
    """
    ...
    def execute(self, *args, **kwargs):
        """ The code that will be executed when the event is *fired* by the event manager
        :class:`~deployv.base.extensions_core.EventManager` according to the event that the
        class registered using the *event* property of the Meta class. Check the example
        class :class:`~deployv.base.extensions_core.TestListener` for more info

        :return: The execution result in a dict
        """
        return


class TestListener(EventListenerBase):
    """ Just an example plugin to test if plugin logic works. Must inherit from
        :class:`~deployv.base.extensions_core.EventListenerBase`
    """

    class Meta:  # pylint: disable=C1001
        """ This is a test event and this is te information associated with it. The event name and
        the event listener name that you want to associate with the object (very useful for logging
        and debugging).
...
def events(function):
    ...
    def process_events(*args, **kwargs):
        args_map = {}
        if args or kwargs:
            args_map = inspect.getcallargs(function, *args, **kwargs)
        if 'self' in args_map:
            obj = args_map['self']
        logger.debug('Executing before.events for %s ', function.__name__)
        pre_event = event_manager.fire_event(
            'before.{}.event'.format(function.__name__), obj)
        res = function(*args, **kwargs)
        logger.debug('Executing after.events for %s ', function.__name__)
        post_event = event_manager.fire_event(
            'after.{}.event'.format(function.__name__), obj, res)
        if pre_event:
