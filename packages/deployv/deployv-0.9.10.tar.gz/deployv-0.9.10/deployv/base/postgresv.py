...
class PostgresShell(object):
    """ This class is used to perform some Postgres operations thar generally are done in the
    console, such as:
    ...
    * dropdb
    ...
    All keys passed in the dictionary to the constructor must be in
    `libpq keys style
    ...
    def dump(self, database, destiny_file):
        ...
        :return: The full dump path and name with .sql extension if success, None otherwise
        """
        ...
        try:
            ...
            if 'does not exist' in error.stderr_output:
                message = 'Database does not exists, check name and try again'
            else:
                message = 'Could not dump database, error message: {0}'\
                    .format(error.stderr_output)
            logger.error(message)
            raise errors.DumpError(message)
        return destiny_file

    def drop(self, database_name):
        """ Drop a database using dropdb command

        :param database_name: Database name to be deleted
        :return: True if success or None otherwise
        """
        cnf = self.__config
        ...
        try:
            ...
            if 'does not exist' in error.stderr_output:
                return True
            logger.error(
                'Could not drop database, error message: %s', error.stderr_output)
            raise OperationalError(error.stderr_output)
        else:
            return True

    def create(self, database_name):
        """ Execute a *createdb* command

        :param database_name: Database name
        :param options: Parameters to create the database
        :return:
        """
        cnf = self.__config
        cnf.update({'dbname': database_name})
        ...
        if self.__config.get('password', False):
            os.environ['PGPASSWORD'] = self.__config.get('password')
        createdb_cmd = 'createdb {dbname} -T template1 -E utf8'
        ...
        try:
            ...
            logger.error(
                'Could not create database, error message: %s', error.stderr_output)
            raise OperationalError(error.stderr_output)
        return database_name

    def restore(self, database_name, dump_name):
        ...
        :return: None if could not restore database, True otherwise
        """
        cnf = self.__config
        cnf.update({'dbname': database_name})
        ...
        try:
            ...
            logger.error(
                'Could not restore database, error message: %s', error.stderr_output)
