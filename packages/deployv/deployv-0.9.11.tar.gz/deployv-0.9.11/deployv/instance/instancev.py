...
from deployv.helpers import utils, container, build_helper, database_helper
...
class InstanceV(DockerV):
    ...
    def __init__(self, config, timeout=300, docker_url="unix://var/run/docker.sock"):
        ...
        if not container_config.get('container_name', False):
            container_config.update({
                'container_name': '{prefix}_odoo'.format(
                    prefix=config.get('instance').get('prefix'))})
            env_vars = config.get('instance').get('config').copy()
            ...
            env_vars.update({'instance_type':
                             config.get('instance').get('instance_type', 'production'),
                             'customer': config.get('instance').get('customer_id')})
    ...
    def start_odoo_container(self):
        ...
        # ports
        if self.config.get('postgres_container', False):
    ...
    def add_authorized_keys(self):
        ...
        with open('authorized_keys', 'w') as authorized_keys:
            for user in self.__full_config.get('instance').get('authorized_users', []):
                try:
        ...
        with tarfile.open('keys.tar', 'w') as tar_keys:
            tar_keys.add('authorized_keys')
        with open('keys.tar', 'r') as tar_data:
        ...
        utils.clean_files(['keys.tar', 'authorized_keys'])
    ...
    def check_postgres(self):
        ...
        while retry <= 3:
            ...
            try:
                PostgresShell(utils.odoo2postgres(self.db_config))
    ...
    def is_running(self):
        ...
        while retry <= 3:
            ...
            if 'STARTING' in res or 'STOPPING' in res:
                retry += 1
                logger.debug('The Odoo process in in a intermediate state, retrying %s', retry)
                time.sleep(4)
            elif 'RUNNING' in res:
                return True
            elif 'STOPPED' in res:
                return False
            elif res == '' or 'no such file' in res and retry < 3:
                retry += 1
                logger.debug('Supervisor returned empty or not running yet, retrying %s', retry)
                time.sleep(4)
            else:
                raise TypeError('Unknown state: %s' % res)
    ...
    def instance_type(self):
        ...
        """
        return self.__full_config.get('instance').get('instance_type') or\
    ...
    def deploy_key(self):
        ...
        """
        ssk_key = base64.b64decode(self.__full_config.get('instance').get('ssh_key'))
        known_hosts = self.__full_config.get('instance').get('known_hosts', [])
        logger.info('Deploying keys')
        ssh_key_filename = path.join(self.temp_folder, 'id_rsa')
        with open(ssh_key_filename, 'w') as key_file:
            key_file.write(ssk_key)
        ...
        known_hosts = known_hosts + ['github.com']
    ...
    def update_db(self, modules, db_name, without_demo=False):
        ...
        update_command = ('su {user} -c "{home}/instance/odoo/{odoo}'
                          ' -d {database} -u {modules} {wodemo} --stop-after-init"') \
            .format(home=odoo_home, user=odoo_user, database=db_name, modules=modules,
                    odoo=command_odoo, wodemo=wodemo)
        return self.run_and_log(update_command)
    ...
    def build_instance(self):
        ...
        with open(branches_filename, 'w') as fout:
            json.dump(self.__full_config.get('instance').get('repositories'),
                      fout, indent=4, ensure_ascii=False,
                      separators=(',', ':'))
        bash_lines = [
            'pip install -r /tmp/helpers-requirements.txt -q ',
    ...
    def get_oca_dependencies(self):
        ...
        clone_cmd = 'su {user} -c "python /tmp/clone_oca_dependencies.py {path} {path} {version}"'\
            .format(path=extra_addons_path, version=odoo_version, user=odoo_user)
        self.exec_cmd(clone_cmd)

    def add_repo_web(self):
        add_repo = True
        branch = (utils.version_cid(self.__full_config.get("instance").get('customer_id')) or
                  self.docker_env.get('version', '8.0'))
        branch_web = {
            "branch": branch,
            "commit": "",
            "depth": 1,
            "is_dirty": False,
            "name": "web",
            "path": "extra_addons/web",
            "repo_url": {
                    "origin": "https://github.com/Vauxoo/web.git"
            },
            "type": "git"}
        if not self.__full_config.get("instance").get("repositories"):
            add_repo = False
            self.__full_config.get("instance").update({'repositories': [branch_web]})
        for repo in self.__full_config.get("instance").get("repositories"):
            if repo.get('name') == 'web':
                add_repo = False
                break
        if add_repo:
            self.__full_config.get("instance").get("repositories").append(branch_web)
    ...
    def db_config(self):
        ...
        if env_vars is None:
            ...
            instance_cfg = self.__full_config.get("instance").get("config")
            logger.debug("Instance config dict from json: %s",
                         str(instance_cfg))
            res = {
                'db_host': instance_cfg.get('db_host', None),
                'db_port': instance_cfg.get('db_port', 5432),
                'db_user': instance_cfg.get('db_user', 'odoo'),
                'db_password': instance_cfg.get('db_password'),
            }
        else:
            if env_vars.get('db_host') == 'localhost' \
                    or env_vars.get('db_host') == '127.0.0.1':
                db_host = '127.0.0.1'
    ...
    def restore_database(self, db_name=False, backup_dir=False):
        res = {}
        customer_id = self.__full_config.get('instance').get('customer_id') or\
            self.docker_env.get("customer")
        use_template = self.__full_config.get('container_config').get('use_template')
        db_owner = self.__full_config.get('container_config').get('db_owner')
        db_owner_password = self.__full_config.get('container_config').get('db_owner_passwd')
        ...
        if use_template:
            ...
            filestore = path.join(path.expanduser(self.config.get('database_folder')),
                                  filestore_dir, 'filestore')
        else:
            if not backup_dir:
                backup_dir = path.expanduser(self.config.get('database_folder'))
        ...
        if not db_name:
            if self.__full_config.get('instance').get('config').get('db_name'):
                db_name = self.__full_config.get('instance').get('config').get('db_name')
            else:
                db_name = utils.generate_dbname(self.__full_config,
                                                path.basename(search_res[1]),
                                                self.__full_config.get('instance').get('prefix'))
        self.stop_instance()
        create_res = db_helper.create_database(candidate, db_name.lower(),
    ...
    def files2backup(self, database_name, tmp_dir):
        ...
        dump_name = path.join(tmp_dir, 'database_dump.sql')
        dump_name = postgres.dump(database_name, dump_name)
        if not dump_name:
    ...
    def generate_backup(self, database_name, dest_folder,
    ...
    def _get_fstore_from_config(self):
        ...
        try:
            ...
            logger.error(
                "Odoo config file is not in the path '%s'", odoo_config_file)
            return None
        for line in res[0].data.split('\n'):
    ...
    def clone_db(self, database):
        ...
        postgres_cfg.update({'isolation_level': True})
        pg_shell = PostgresShell(postgres_cfg)
        pg_shell.drop(new_db)
    ...
    def get_translations(self, database):
        ...
        with PostgresConnector(utils.odoo2postgres(psql_dict)) as db:
            translations = db.execute("""SELECT value,id,name,module FROM ir_translation""")
        return translations
    ...
    def get_odoo_users(self, database):
        ...
        psql_dict.update({'db_name': database})
        with PostgresConnector(utils.odoo2postgres(psql_dict)) as db:
            users = db.execute("SELECT id,login FROM res_users")
        return users
    ...
    def change_password(user_id, new_pass, db_name, db_config):
        ...
        with PostgresConnector(utils.odoo2postgres(db_config)) as db:
            res_user = db.execute(("select password, password_crypt"
    ...
    def check_keys(self):
        ...
        if 'successfully authenticated, but GitHub does not provide shell access.' in res:
    ...
    def build_image(self):
        ...
        if build_res[0] and build_res[1].get('result'):
            ...
            if self.__full_config.get('container_config').get('build_image'):
                self.__full_config.get('instance').update({'repositories': []})
        return build_res
    ...
    def _update_env_vars(self, env):
        ...
        """
        if 'list_db' not in env.keys():
            env.update({'list_db': False})
        if 'dbfilter' not in env.keys():
            db_name = self.__full_config.get('instance').get('config').get('db_name')
            if not db_name:
                db_name = utils.generate_dbname(self.__full_config)
            env.update({'dbfilter': db_name})
        self.__full_config.get('container_config').get('env_vars').update(env)
...
def deactivate_database(config, db_name):
    ...
    sqls = {
        ...
        'rabbit_active': ("update rabbit_config set active = False,"
                          " \"user\" = 'user', password = 'pass', host = '0.0.0.0',"
                          " virtual_host = 'vhost'"),
        'docker_token': ("update docker_images_config set quayio_access_token = 'token_quayio',"
                         " dockerhub_access_token = 'token_hub';"),
        'repos_token': ("update repos_list_config set github_access_token = 'github_token',"
                        " gitlab_access_token = 'gitlab_token', gitlab_security_token = 'token',"
                        " github_security_token = 'token';"),
        'instance_ssh': "update orchest_instance set ssh_file = null;"
    }
    ...
    try:
        with postgresv.PostgresConnector(psql_dict) as db:
            for name, sql in sqls.items():
                logger.debug('Running %s', name)
                try:
                    db.execute(sql)
                except psycopg2.ProgrammingError as error:
                    if 'relation' in error.message:
                        logger.warn(
                            'This database has not %s, nothing to execute', name)
                    else:
                        raise
    except psycopg2.OperationalError as error:
