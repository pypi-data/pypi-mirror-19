...
import deployv
from deployv.base import commandv, dockerv
...
def create(config_file, log_level, backup_dir, external, key_file, without_db, database):
    ...
    if not res_create.get('result'):
        logger.error('Could not create the instance with the given repo: %s',
...
def restore(config_file, log_level, backup_dir, backup_file, database,
    ...
    if config_file and container:
        ...
        sys.exit(1)
    elif container and not prefix:
        logger.error(
            'You must use one of the -n and -x parameters.')
        sys.exit(1)
    if config_file:
...
def deactivate(container, database, log_level, admin_password):
    ...
    """
    logger.setLevel(log_level)
    container_manager = dockerv.DockerV({'container_name': container})
    config = {
        'container_name': container,
        'container_config': container_manager.config,
        'instance': {
            'config': {
...
def build(log_level, config_file, repo, branch, image_name, working_folder, version, force):
    if config_file:
        config = utils.load_json(config_file)
    else:
        config = {}
