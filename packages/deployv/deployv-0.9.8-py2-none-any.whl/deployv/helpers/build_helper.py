...
class BuildImage(object):
    ...
    def _generate_image_name(self):
        ...
        if self.full_stack_image:
            name = '{tid}_full_odoo_stack'.format(tid=self._config.get('instance')
                                                          .get('task_id'))
        elif len(self._config.get('instance').get('repositories')) == 1:
            repo = self._config.get('instance').get('repositories')[0]
    ...
    def _search_odoo_repo(self):
        ...
        :return: dict with the info of the odoo repo if the odo repo was specified by the user
                 in the correct path, empty dict otherwise
        """
    ...
    def _clone_dependencies(self, instance_path, extra_path):
        ...
        if odoo_repo:
            ...
            utils.clone_repo('https://github.com/Vauxoo/odoo.git', self.version,
                             path.join(instance_path, 'odoo'))
        for repo in self._config.get('instance').get('repositories'):
...
def add_repo(config, repo_url, branch):
    ...
    repo_dict = {
        'branch': branch,
        'name': repo_name,
        'path': path.join('extra_addons', repo_name),
        'repo_url': {
            'origin': repo_url
        }
    }
...
def build_image(config, version=False, working_folder='/tmp', force=False):
    ...
    """
    if not version:
        match = re.match(r"([a-z]+)([0-9]+)$", config.get('instance').get('customer_id'), re.I)
        if match:
            res = float(match.group(2)) / 10.0
            version = str(res)
        else:
