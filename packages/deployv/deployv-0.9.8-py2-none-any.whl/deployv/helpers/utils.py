...
def setup_deployv_logger(name, logfile=None, level=logging.DEBUG):
    """
    This function helps to configure logger, if a filename is present logs to a file,
    else logs to std.
...
def merge_config(config_file, keys_file=False, branches=False):
    ...
    if branches:
        ...
        changed = True
    json_branches = new_config.get("instance").get("repositories")
    if new_config.get("instance").get("instance_type", False) == "test" or \
            new_config.get("instance").get("instance_type", False) == "develop":
        if new_config.get("instance").get("repositories"):
            for branch in json_branches:
                if branch.get("name") == "web":
                    break
            else:
                new_config.get("instance").get("repositories").append(
                    {
                        "branch": "8.0",
                        "commit": "",
                        "depth": 1,
    ...
    with fobject(full_tmp_name, mode=modestr) as tar_file:
        for fname in files:
            if hasattr(fname, '__iter__'):
                ...
                tar_file.add(fname, os.path.join(
