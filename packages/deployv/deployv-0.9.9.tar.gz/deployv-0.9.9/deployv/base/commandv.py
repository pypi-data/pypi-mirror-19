...
class CommandV(Extensible):
    ...
    def create(self):
        ...
        without_db = self.__config.get('instance').get("without_db", False)
        without_demo = self.__config.get('instance').get("without_demo", False)
        self.create_db_demo(without_db, without_demo)
    ...
    def deactivate_backup(self, db_config, backup, prefix, store_path, cformat, admin_password):
        if db_config.get('db_host').startswith('172.17.'):
            db_config.update({'db_host': '127.0.0.1'})
        ...
        db_name = utils.generate_dbname({}, backup, prefix)
    ...
    @events
    def updatedb(self, database_name=None, without_demo=False):
        ...
        for module in update_modules:
            while retry <= 1:
                res_update = self.__instance_manager.update_db(module, db_name, without_demo)
    ...
    def create_db_demo(self, without_db=False, without_demo=False):
        """
        This method creates a default database with a demonstration data,
        but specifies the parameter
        :param without_db: Database not created
        :param without_demo: Database not load data demo
        :return: The result from :meth:`deployv.base.commanv.updatedb`
        """
        self._stop_instance()
        self._drop_dbs()
        if without_db:
        ...
        db_config = self.__instance_manager.db_config
        postgres = postgresv.PostgresShell(utils.odoo2postgres(db_config))
        postgres.create(db_name)
        InstallTestRepo.event = 'after.updatedb.event'
        updatedb_demo = self.updatedb(db_name, without_demo)
        return updatedb_demo
