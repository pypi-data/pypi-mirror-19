    ...
    def handle(self, *args, **options):
        ...
        if with_related:
            ...
            for obj in with_related:
                ...
                for group in collector.nested()[1:]:
                    for related_obj in group:
                        print('    ' + self.get_stale_obj_repr(related_obj))

    def get_stale_obj_repr(self, obj):
        url = urlresolvers.reverse("admin:%s_change" % (obj._meta.label.replace('.', '_').lower()), args=(obj.id,))

        return "{0}:{1} {2} http://{3}{4}".format(
            obj._meta.label,
            str(obj.id).ljust(4, ' '),
