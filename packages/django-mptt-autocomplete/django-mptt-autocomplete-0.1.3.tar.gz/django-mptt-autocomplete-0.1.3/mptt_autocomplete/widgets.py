...
def recursive_node_to_dict(node, values, queryset):
    queryset__pks = queryset.values_list('pk', flat=True)
    result = get_doc(node, values)
    children = [
        recursive_node_to_dict(c, values, queryset) for c in
        node.get_children() if c.pk in queryset__pks]
    if children:
...
def get_tree(nodes, values):
    ...
    return [recursive_node_to_dict(n, values, nodes) for n in root_nodes]
