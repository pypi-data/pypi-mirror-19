...
class OfferChooserBlock(blocks.ChooserBlock):

    @cached_property
    def target_model(self):
        return get_model('offer', 'ConditionalOffer')

    @cached_property
    def widget(self):
        from oscar_wagtail.widgets import AdminOfferChooser
        return AdminOfferChooser

    def get_prep_value(self, value):
        if isinstance(value, int):
            return value
        return super(OfferChooserBlock, self).get_prep_value(value)
