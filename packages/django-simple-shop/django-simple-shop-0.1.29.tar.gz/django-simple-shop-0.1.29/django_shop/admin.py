...
from .models import ProductCategory, Product, ProductImage, ProductEntry, \
    ...
    ProductCategoryCustomField, ProductCustomFieldValue, PackagingBox
...
class ProductOrderInline(admin.TabularInline):
    fields = ('__str__', 'quantity', 'stock_price', 'price')
    readonly_fields = ('__str__', 'quantity', 'stock_price', 'price')
    model = ProductOrder
    extra = 0
...
class OrderAdmin(admin.ModelAdmin):
    ...
    def customer_name(self, obj):
        return mark_safe(
            '{}<br>{}'.format(obj.delivery_address.name, obj.delivery_address.email))
    products_string_html.short_description = 'Заказ'
