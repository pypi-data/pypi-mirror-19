...
def checkIfTimeToTzNew(part_where, session_tz, system_tz, format='%Y%m%d%H%M%S'):
    tiempo = {'Time': False, 'Date': False}
    for i in part_where:
        if i[0] in tiempo:
            tiempo[i[0]] = i
            dim_time = i
    dates = changeTimeByTzNew(part_where[dim_time], session_tz, system_tz, format)
    part_where_aux_fechas = {}
    for i in part_where:
        if i == dim_time:
            part_where_aux_fechas[i] = dates
...
def requestCubeNew(col_cube, med_cube, request_data, ifDate, dimension_filters, aux_attrs_to_filter, only_date=False, total_data=False, where_tupla=('Date', 'Id', 'Id'), request_date_previous=False, complete_name=False, format_cube=False, n_c=[], time_zone="", redis="", col_cube_map={}):
    col_cube_map = dict((value, key) for key, value in col_cube_map.iteritems())
    database_cube_dict = {}
    if 'cube_config' in request_data:
        ...
        limit_sim_exe = cube_config['limit_sim_exe']
        limit_time = cube_config['limit_time']

    cube = CUBE(limit_sim_exe=limit_sim_exe, limit_time=limit_time)
    mdx_class = MDX(dim_date=where_tupla, format_date_in=ifDate, format_date_out=ifDate, only_date=only_date)
    ...
    part_order = {}
    mdx = ''
    where = filterWhereNew(dimension_filters, request_data, database_cube_dict, aux_attrs_to_filter, col_cube_map)

    if ifDate:
        if not request_date_previous:
            where[where_tupla] = (formatDateCubeNew(request_data['from_Date'], ifDate), formatDateCubeNew(request_data['to_Date'], ifDate))
        else:
            where[where_tupla] = (formatDateCubeNew(request_data['from_Date_previous'], ifDate), formatDateCubeNew(request_data['to_Date_previous'], ifDate))
    ...
    if ifDate:
        where = checkIfTimeToTzNew(where, request_data['django_timezone'], request_data['system_timezone'], ifDate)
    col_cube = reorderColCube(col_cube, data_order)
    key = name_bbdd+'MDXS_'+str(col_cube)+str(exclude_rows)+str(med_cube)+str(name_cube)+str(where)+str(part_order)+str(name_bbdd)+str(ip_connect)+str(format_cube)
    minute = datetime.now().minute
    second = datetime.now().second
    seconds = segundosHastaY5oEnPunto(minute, second)
    if (is_MDXS_Added_redis(name_bbdd, redis)):
        MDXS = get_MDXS_redis(name_bbdd, redis)
    else:
        MDXS = {}
    if not (key in MDXS.keys()):
        ...
        grid = Format()
        if format_cube:
            json_data = grid.grid(cube, res, database_cube_dict, False, request_data['row_num'], request_data['page'], total_data, complete_name, format_cube, col_cube, col_cube_map=col_cube_map)
            mdx_total = 'select '+mdx_class.without_rows(mdx)
            res = cube.launch_query(mdx_total)
            json_data_footer = grid.grid(cube, res, database_cube_dict, False, request_data['row_num'], request_data['page'], total_data, complete_name, format_cube, col_cube_map=col_cube_map)
        ...
        add_MDX_redis(name_bbdd, MDXS, redis, seconds)
    else:
        json_data = MDXS[key]
        json_data = ujson.dumps(json_data)
    return json_data
...
def requestCube(ip_connect, name_bbdd, name_cube, col_cube, med_cube, request_data, ifDate, dimension_filters, database_cube_dict, total_data=False, where_tupla=('Date', 'Id', 'Id'), request_date_previous=False, complete_name=False, format_cube=False, n_c=[], time_zone="", redis=""):
    ...
    cube = CUBE(limit_sim_exe=limit_sim_exe, limit_time=limit_time, version=request_data['var_settings']['VERSION'])
    mdx_class = MDX()
    cube.connect(ip_connect, name_bbdd, n_c, redis)
    exclude_rows = {}
    where = {}
    ...
    if not (key in MDXS.keys()):
        ...
        grid = Format()
        if total_data:
            json_data = grid.grid(cube, res, database_cube_dict, False, request_data['row_num'], request_data['page'], total_data, complete_name, format_cube, col_cube)
            mdx_total = 'select ([Client].[Client]) on rows,'+mdx_class.without_rows(mdx)
            res = cube.launch_query(mdx_total)
            json_data_footer = grid.grid(cube, res, database_cube_dict, False, request_data['row_num'], request_data['page'], total_data, complete_name, format_cube)
            json_data = json.loads(json_data)
            if json.loads(json_data_footer)['rows']:
                json_data['footerData'] = json.loads(json_data_footer)['rows'][0]
                try:
...
def requestCubeHighcharts(ip_connect, name_bbdd, name_cube, col_cube, med_cube, request_data, ifDate, dimension_filters, database_cube_dict, total_data=True, where_tupla=('Date', 'Id', 'Id'), request_date_previous = False,  format_cube=False, n_c=[], time_zone="", redis=""):
    ...
    where = checkIfTimeToTz(where, request_data['django_timezone'], request_data['system_timezone'])
    key = name_bbdd+'MDXS_'+str(col_cube)+str(exclude_rows)+str(med_cube)+str(name_cube)+str(where)+str(name_bbdd)+str(ip_connect)+str(request_data['button'])
    minute = datetime.now().minute
    second = datetime.now().second
    seconds = segundosHastaY5oEnPunto(minute, second)
    ...
    if not (key in MDXS.keys()):
        ...
        if request_data['button'] == 'ALL':
            json_data = high.highchart(cube, res, types, dimension_v_name='Client', format_cube=format_cube)
        else:
            json_data = high.highchart(cube, res, types, dimension_v_name=request_data['button'], exclude=exclude_rows, format_cube=format_cube)
...
def requestCubeFiltersNew(col_cube, med_cube, request_data, ifDate, dimension_filters, aux_attrs_to_filter, only_date=False, where_tupla=('Date', 'Id', 'Id'), n_c=[], redis="", col_cube_map={}):
    # name_carrier=''
    col_cube_map_aux = dict((value, key) for key, value in col_cube_map.iteritems())
    database_cube_dict = {}
    if 'cube_config' in request_data:
        cube_config = request_data['cube_config']
        ip_connect = cube_config['cube_ip']
    ...
    mdx_class = MDX(dim_date=where_tupla, format_date_in=ifDate, format_date_out=ifDate, only_date=only_date)
    cube.connect(ip_connect, name_bbdd, n_c, redis)
    where = filterWhereNew(dimension_filters, request_data, database_cube_dict, aux_attrs_to_filter, col_cube_map_aux)
    exclude_rows = {}
    if ifDate:
        where[where_tupla] = (formatDateCubeNew(request_data['from_Date'], ifDate), formatDateCubeNew(request_data['to_Date'], ifDate))
        where = checkIfTimeToTzNew(where, request_data['django_timezone'], request_data['system_timezone'], ifDate)

    key = name_bbdd + 'MDXS_' + str(col_cube) + str(exclude_rows) + str(med_cube) + str(name_cube) + str(where) + str(name_bbdd) + str(ip_connect) + str(dimension_filters)
    seconds = segundosHastaY5oEnPunto(datetime.now().minute, datetime.now().second)
    MDXS = {}
    if not (key in MDXS.keys()):
        ...
        grid = Format()
        dimension_filter = [col_cube_map[request_data['types']]]
        filters_cube = grid.filters(mdx, cube, database_cube_dict, dimension_filter, exclude_rows, aux_attrs_to_filter[request_data['types']])
        MDXS[key] = filters_cube
        add_MDX_redis(name_bbdd, MDXS, redis, seconds)
    else:
        filters_cube = MDXS[key]
    json_data = json_encode(filters_cube)
    return json_data
...
def filterColCubeNew(col_cube,col_group_by,col_and_by='',col_final_by=''):
    col_cube_new = []
    if (col_group_by not in ('', 'NULL')):
        col_cube_new.append(col_cube[col_group_by])
    if (col_and_by not in ('', 'NULL')):
        col_cube_new.append(col_cube[col_and_by])
    if (col_final_by not in ('', 'NULL')):
        col_cube_new.append(col_cube[col_final_by])
    return col_cube_new
...
def filterWhereNew(filters, request_data, database_cube_dict, aux_attrs_to_filter={}, col_cube_map={}):
    where = {}
    for f in filters:
        name_filter = col_cube_map[f]
        if name_filter in request_data:
            if request_data[name_filter] != 'NULL' and request_data[name_filter] != (name_filter, 'NULL'):
                if col_cube_map[f] in aux_attrs_to_filter:
                    where[aux_attrs_to_filter[col_cube_map[f]]] = request_data[name_filter]
    return where
...
def accuratelyAdjustment(json_data,med_cube_prec,dict_type_data = False,request_data=False,format_excel= False):
    ...
    json_data_dict = json.loads(json_data,object_pairs_hook=SortedDict)
