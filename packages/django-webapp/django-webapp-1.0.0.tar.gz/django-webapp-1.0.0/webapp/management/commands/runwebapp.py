...
from webapp import get_webapp_taskrunner
from webapp.views import webapp_serve
...
def red(msg, bold=False):
	if bold:
		return '\033[1;%dm%s%s' % (__red, msg, __reset)
	else:
		return '\033[%dm%s%s' % (__red, msg, __reset)
...
class Command(RunserverCommand):
	...
	def get_handler(self, *args, **options):
		...
		if webapp_url:
			webapp_url = '^%s/(?P<path>.*)$' % webapp_url
		else:
			webapp_url = '^(?P<path>.*)$'
		module = import_module(settings.ROOT_URLCONF)
		module.urlpatterns.append(url(webapp_url, webapp_serve, {'document_root': settings.WEBAPP_ROOT}))
