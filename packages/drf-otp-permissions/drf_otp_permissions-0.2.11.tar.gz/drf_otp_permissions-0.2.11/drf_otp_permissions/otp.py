...
def validate_otp(otp, data, key_location):
    ...
    if decrypted_data_str != data_str:
        logger.info("decrypted_data_str does not equal to data_str")
        logger.info("decrypted_data_str = %s" % decrypted_data_str)
        logger.info("data_str = %s" % data_str)
        return False
    ...
    if expired_by:
        datetime_object = datetime.strptime(expired_by, "%Y-%m-%dT%H:%M:%SZ")
        return _validate_due(datetime_object)
    return False