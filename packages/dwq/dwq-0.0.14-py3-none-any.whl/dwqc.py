...
def main():
    ...
    try:
        ...
        if args.batch:
            vprint("client: sending jobs")
            for _tuple in batch:
                queue_job(jobs, *_tuple)
        vprint("client: all jobs sent.")
        ...
        passed = 0
        while jobs:
            for job in Job.wait(status_queue, count=1):
                try:
                    jobs.remove(job["job_id"])
                    done += 1
                    if args.progress:
                        vprint("\033[F\033[K", end="")
                    vprint("client: job %s done. result=%s" % (job["job_id"], job["result"]["status"]))
                    if not args.quiet:
