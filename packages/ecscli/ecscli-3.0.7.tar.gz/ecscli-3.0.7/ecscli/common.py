...
PROD_NAME = 'storageos'
TENANT_PROVIDER = 'urn:ecs:TenantOrg:provider:'
...
#OBJCTRL_PORT = '4443'
OBJCTRL_PORT = '10103'
#OBJCTRL_PORT = '11103' #default port for standalone test system
...
def service_json_request(ip_addr, port, http_method, uri, body, token=None,
                         ...
                         filename=None, customheaders=None, use_version=True):
    print("JMC URI: " + uri)
    print("JMC method: " + http_method)
    if (body is not None):
        print("JMC body: " + body)
    print("-----------------------------------------------------------------------------")
    '''
    ...
    try:
        ...
        if (cookiefile is None):
            install_dir = getenv('ECS_CLI_INSTALL_DIR')
            if (install_dir is None):
                raise SOSError(SOSError.NOT_FOUND_ERR,
                               "ECS_CLI_INSTALL_DIR is not set." +
                               " Please execute ecscli.profile\n")
        ...
        uriversion = ""
        if(use_version):
            uriversion = getenv("ECS_CONTROL_API_VERSION")
            if(uriversion is not None):
                uriversion = "/" + uriversion
            else:
                uriversion = ""
        
        #getenv might have it using ECS_PORT or set to 4443 is ECS_PORT doesn't exist
        ...
        url = protocol + ip_addr + ":" + str(port) + uriversion + uri
        ...
        if (http_method == 'GET'):
            ...
            if (body is not None):
                response = requests.delete(url, data=body, headers=headers, verify=False,
                                           cookies=cookiejar)
            else:
        ...
        if((response.status_code == requests.codes['ok']) or
           (response.status_code == 202)):
            ...
            if(response.status_code == 500):
                ...
                error_msg = "ECS internal server error. Error details: " + \
                    errorDetails
    ...
    try:
        ...
        if (sosenv is None):
            ...
            if sys.platform.startswith('win'):
                filename = 'ecscli.profile.bat'
            else:
                filename = 'ecscli.profile'
            # read the value from ecscli.profile
            soscli_abs_path = os.path.join(soscli_dir_path, filename)
            # In new installation, ecscli.profile file is in one level above.
        ...
        if (sosenv is None) or (len(sosenv) <= 0):
            if (envdefaultvalue):
                ...
                sosenv = None
        if (envvarname == 'ECS_HOSTNAME'):
            if (sosenv) and (sosenv != 'localhost'):
                return sosenv
            sosenv = socket.getfqdn()
            if (sosenv is None):
                sosenv = 'localhost'
        elif (envvarname == 'ECS_PORT'):
            if (sosenv is None):
                sosenv = '4443'
        elif (envvarname == 'ECS_UI_PORT'):
            if (sosenv is None):
                sosenv = '443'
        elif (envvarname == 'ECS_CLI_INSTALL_DIR'):
            if (sosenv is None):
