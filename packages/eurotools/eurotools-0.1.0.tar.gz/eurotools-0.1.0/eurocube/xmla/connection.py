...
@author: norman
'''
from stoneCube.scripts.xmla.interfaces import XMLAException
from suds.client import Client
from suds import WebFault
from . import httptransport
from .utils import schemaNameToMethodName, aslist, dictify
import logging
...
class XMLAConnection(object):
    ...
    def __init__(self, url, location, username, password, spn, sslverify, **kwargs):
        ...
        if password is None:
            transport = httptransport.HttpKerberosAuthenticated(as_user=username, spn=spn, sslverify=sslverify, **kwargs)
        else:
            transport = httptransport.HttpAuthenticated(username=username, password=password, sslverify=sslverify, **kwargs)
    ...
    def Execute(self, command, dimformat="Multidimensional", axisFormat="ClusterFormat", **kwargs):
        ...
        try:
            ...
            logger.error('Failed method from xmla library')
