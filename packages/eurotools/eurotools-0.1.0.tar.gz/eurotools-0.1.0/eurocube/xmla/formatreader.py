from .utils import aslist, stringtypes
from stoneCube.scripts.olap.interfaces import IMDXResult
    ...
    def getSlice(self, properties=None, **kw):
        ...
        for ax in axlist:
            ...
            if ax._name in kw:
                ...
                if toolarge:
                    raise ValueError(
                        "The tuple requested do not exist on axis '%s': %s" % (ax._name, indexrange))
        ...
        while lastdimchange < len(axisranges):
            ...
            if properties is None:
                ...
                if isinstance(properties, stringtypes):
                    d = getattr(cell, properties, None)
                else:
                    d = {}
                    for prop in aslist(properties):
                        d[prop] = getattr(cell, prop, None)
