...
var eurofiltersIU = function (config_data_out, callbackFunction, stoneselectDB){
	...
	*/
	...
	var createSelectTag = function (name){
		...
		selectbox.className = self.config_data['prefix']+name;
		selectbox.name = name;
		selectbox.multiple = 'multiple';
		
		
		var div_general = document.createElement('div');
		div_general.className = self.config_data['prefix']+name+'-container';
		width = ''
		if(self.config_data['vertical']){
			width = 'width:95%;'
		}
		div_general.style.cssText = "display:inline-block;margin:1px;"+width;

		placeHolder = self.options[name]['placeholder']
		if((placeHolder) && typeof(placeHolder) == 'string' ){
			var span_label = document.createElement('span');
			
			span_label.textContent = placeHolder
			span_label.style.cssText = "float:left; text-align:center;margin:5px";
			div_general.appendChild(span_label);
		}
				...
				if(!self.configData(name_filter, 'staticdata')){
					if(!self.configData(name_filter, 'clientdata')){
						if(!self.configData(name_filter, 'local')|| self.stoneselectDB.filters.indexOf(self.options[name_filter]['db']) == -1){
							...
							self.makeRequest(name_filter, params)
	...
	var loadLocalDataSingle = function(db, name){
		self.stoneselectDB.getData(db, self.config_data['prefix'], name);
	}
	...
	self.makeRequest = function(name, params){
		url = self.config_data['url']+params+self.serializeStr()+self.getFiltersNames()+self.getTypes()+self.getFilterSelected();
		$.ajax({
			type: 'GET',
			url: url,
			dataType: 'json',
	...
	var addHtmlToContainer = function(name){
		if( $(self.config_data['container']+' '+'#'+self.config_data['prefix']+name).length == 0 ){
			var selectbox = createSelectTag(name, self.options[name])
			self.html = selectbox;
			$(self.config_data['container']).append(selectbox);	
		}
	}

	var aplyConfigSelect2 = function(name, selected){
		opts = jQuery.extend({}, self.options[name])
		self.filtersObjects[name] = $('.' + self.config_data['prefix'] + name).multiselect(opts).multiselectfilter();
		...
		$.each(data, function(index, value){
			if('children' in value){
				...
				$.each(value['children'], function(index2, value2){
					element = addOneData(name, value2, selected_local)
					if (element){
						html += element
					}
				})
				...
				element = addOneData(name, value, selected_local)
				if (element){
					html += element
				}
			}
	...
	var addOneData = function(name, option, selected){
		...
		if(option['id'] != null){
			...
			option = "<option value='"+ id_option +"' "+ disable +"  "+selected_attr+" "+class_value+">" + text_option + "</option>"
			return option
		}
		else{
			return false
	...
	var addFilterOne = function(name, id, opts){
		...
		$('.'+self.config_data['prefix']+name).multiselect(opts).multiselectfilter();
	...
	self.removeNoActives = function(){
		$.each(self.filters_names, function(index, val){
			$('.'+self.config_data['prefix']+val).next().css({'border-color': '#444'})
			styleFilters(val)
		})
		self.waiting = false;
	}
	...
	var styleOpen = function(name){
		...
		if(self.options[name]['max']!=0){
			$('.ui-multiselect-all').css({'visibility': 'hidden'})
		}
		else{
			$('.ui-multiselect-all').css({'visibility': 'visible'})
		}
	}

	var getDefaultParam = function(){
		opts_default = {header: true ,selectedList : 3, position: {my: 'left top',at: 'left bottom'} , max: 0, maxShow: 3, maxLetters: 3, numToShow: 10, multiple:true,checkAllText:'', uncheckAllText:'', noneSelectedText:self.config_data['noneSelectedText'], /*show: ["bounce", 200],hide: ["bounce", 1000],*/
			click: function(event, ui){
				name = event.target.getAttribute('class').replace(self.config_data['prefix'],'')
			...
			beforeclose: function(event){
				...
				if(!self.configData(self.lastClicked, 'automatic') && self.neededLaunchEvent){
					$('.'+self.config_data['prefix']+self.lastClicked).next().css({'border-color': '#0b93d5'})
				}
			},
			...
			beforeoptgrouptoggle: function(event, elements){
				if (elements['label']=='Selected')
				{
					...
					if ((elements['inputs'].length + $(this).multiselect("widget").find("input:checked").length) > self.options[self.lastClicked]['max'])
					{
						if (self.options[self.lastClicked]['max'] == 1){
			...
			optgrouptoggle: function(event, ui){
			  var values = $.map(ui.inputs, function(checkbox){
				...
				$('.' + self.config_data['prefix'] + self.lastClicked).next().children()[1].removeAttribute('title')
				if(checkedItems.length == 1){
					if(checkedItems[0].title.length>self.options[self.lastClicked]['numToShow']){
						result = checkedItems[0].title.substring(0,self.options[self.lastClicked]['numToShow']-3)+'...'

						$('.' + self.config_data['prefix'] + self.lastClicked).next().children()[1].title = checkedItems[0].title
						class_name = $('select[name*="'+self.lastClicked+'"] option[value*="'+checkedItems[0].value+'"]').attr('class')
						$('.' + self.config_data['prefix'] + self.lastClicked).next().children()[1].className = class_name
					}
					else{
						result = checkedItems[0].title	
					}
				}
					...
					result = numChecked + ' of ' + numTotal + ' checked';				
				}
				return result;
		   }
		}
	...
	var mergeOptions = function(options, index){
		opts_pos = options[index]
		opts_default = getDefaultParam()
	...
	*/
	self.updateConfig = function(data){
		changed_fields = ['automatic','local', 'clientdata', 'staticdata']
		$.each(data, function(key, value){
	...
	var setCategories = function(name, data, selected){
		...
		if(self.options[name]['multiple']){
			if(self.config_data['setCategories'] && (selected.length > 0) ){
				...
				$.each(data, function(index, val){
					if('children' in val){
						other_category = {'id':1, 'text': val['text'], 'children': []}
						$.each(val['children'], function(index2, val2){
							setOptionCategory(selected, selected_options, other_category, val2)
						});
					}
					else{
	...
	*/
	self.updateFilterData = function(name, data, selected_ge){
		...
		selected_ge = typeof selected_ge !== 'undefined' ? selected_ge : self.serializeListIds()[name];
	...
	*/
	self.serializeListIds = function (){
		serialize_list_ids = {};
		$.each(self.filters_names, function(index, name){
			try {
			    ids = self.filtersObjects[name].multiselect('getChecked')
	...
	*/
	...
	self.hideFilter= function(name){
		$('.'+self.config_data['prefix']+name+'-container').hide()
	}

	self.showFilter= function(name){
		$('.'+self.config_data['prefix']+name+'-container').show()
	}
