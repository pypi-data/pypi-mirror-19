...
def jieDiQi_crack_ext_direct_webshell_url(url,serverType, pass_dict_file, ext):
    # 爆破php|asp|aspx|jsp的一句话类型的webshell
    # 表单形式爆破的webshell爆破起来方法一样,不用分类
    # 一句话形式的webshell爆破需要根据后缀对应的脚本的语法的不同来爆破
    def ext_direct_webshell_crack_thread(xxx_todo_changeme):
        ...
        data = urllib.parse.urlencode(values)
        if ext!="jsp":
            checkData=urllib.parse.urlencode(check)
        ...
        if re.search(pattern, html):
            ...
            if exp!="jsp":
                ...
                finalPassword=re.search(r'''"(.*)"''',html).group(1)
            else:
                for each in password:
                    postValues = {'%s' % each: 'A'}
                    postData = urllib.parse.urlencode(postValues)
                    html=post_request(url,postData)
                    if re.search(pattern,html):
                        finalPassword=each
                        break
...
def crack_webshell(url, anyway=0):
    ...
    if anyway == 1 or tmp['y2'] == "direct_bao":
        ...
        if serverType in ["apache","iis","nginx","lighttpd"]:
            return_value = jieDiQi_crack_ext_direct_webshell_url(url, ModulePath + "dicts/webshell_passwords.txt", ext)
        else:
