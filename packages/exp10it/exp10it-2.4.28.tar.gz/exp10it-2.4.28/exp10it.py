...
actual_ip_from_cdn_scan varchar(50) not null default 0,\
cdn_scaned varchar(50) not null,\
...
http_domain varchar(70) not null)" % each_pang_urls_table_name
        ...
        result=execute_sql_in_db("select actual_ip_from_cdn_scan from `%s` where http_domain='%s'" %
                (get_main_target_table_name(target),target))
        if len(result)>0:
            ip=result[0][0]
...
def single_cdn_scan(target):
    ...
    # target可以是主要目标或是主要目标的旁站或子站,但是这时并没有运行到找旁站和子站的模块,这时target只可能是主
    # 要目标
...
def single_port_scan(target):
    ...
    if 1 == get_scan_finished(
        ...
        if target_is_pang_domain==True:
            ...
            strings_to_write= portScan(target)

        for each_table in table_name_list:
            execute_sql_in_db(
                "update `%s` set ='%s' where http_domain='%s'" %
                (each_table, port_scan_info, target), eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
...
def single_whois_scan(target):
    ...
    if 1 == get_scan_finished(
        ...
        if target_is_sub_domain==False:
            ...
            strings_to_write="same to main target"

        whois_info=strings_to_write
        for each_table in table_name_list:
            execute_sql_in_db(
                "update `%s` set ='%s' where http_domain='%s'" %
                (each_table, whois_info, target), eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
