...
def post_requests(url,data,headers):
    import requests
    returnValue=requests.post(url,data,headers)
    return returnValue
...
def check_banned(url):
    ...
    if result['code']!=0:
        ...
        result=get_request(url,proxyUrl=get_one_useful_proxy())
        if result['code']!=0:
            return True
        else:
            result=get_request(url,proxyUrl=get_one_useful_proxy())
...
def blog():
    ...
    cmd="cp /root/myblog/_posts/*隐藏webshell.md %s" % file_abs_path
    os.system(cmd)
    os.system(
...
def get_user_and_pass_form_from_html(html):
    ...
    if index>=1 and user_form_line!=None and pass_form_line!=None:
    ...
    elif has_pass_form==True and index==0:
    ...
    if find_form_action_url and find_form_action_url.group(2) is not None:
        ...
        if form_action_url[:4]=="http":
            http_domain_len=len(get_http_domain_from_url(form_action_url))
            form_action_url=form_action_url[http_domain_len:]
...
def crack_ext_direct_webshell_url(url, pass_dict_file, ext):
    ...
    def ext_direct_webshell_crack_thread(xxx_todo_changeme):
        ...
        sys.stdout.write('-' *(try_time[0]*100 //(sum[0])) +'>' +str(try_time[0]*100 //(sum[0])) +'%' +'%s/%s %s\r' %(try_time[0], sum[0], PASSWORD))
        sys.stdout.flush()
...
def jieDiQi_crack_ext_direct_webshell_url(url, pass_dict_file, ext):
    ...
    def ext_direct_webshell_crack_thread(param1,param2,param3):
        ...
        sys.stdout.write('-' *(try_time[0]*100 //(sum[0])) +'>' +str(try_time[0]*100 //(sum[0])) +'%' +' %s/%s\r' % (try_time[0], sum[0]))
        sys.stdout.flush()
        if re.search(pattern, html):
            get_flag[0] = 1
            # print "\b"*30
...
def crack_webshell(url, anyway=0):
    ...
    if anyway == 1 or tmp['y2'] == "direct_bao":
        ...
        if serverType in ["apache","iis","nginx","lighttpd"]:
            return_value = jieDiQi_crack_ext_direct_webshell_url(url, ModulePath + "dicts/jieDiQi_webshell_passwords.txt", ext)
        else:
        ...
        return_value = crack_allext_biaodan_webshell_url(url, ModulePath + "dicts/user.txt", ModulePath + "dicts/webshell_passwords.txt")
...
you_can_take_notes_in_this_column_for_your_own_pentest text not null comment '人工渗透时笔记列',\
...
def get_source_main_target_domain_of_pang_url(url):
    ...
    print("get_source_main_target_domain_of_pang_url func error,check it")
    sys.exit(0)
...
def get_url_belong_main_target_domain(url):
    ...
    print("current param in error func is %s" % url)
    print("get_url_belong_main_target_domain(%s) error,I will try to get it again and exist." % url)
    get_url_belong_main_target_domain(url)
    print("get it again finished,now exist")
    sys.exit(0)
...
def single_cdn_scan(target):
    ...
    if target_is_pang_domain==True and target_is_sub_domain==True:
        target_is_pang_and_sub=True

    if target_is_pang_domain==False and target_is_sub_domain==False:
        #这时target为主目标 
        xcdnObj=Xcdn(domain)
        ip=xcdnObj.return_value
        if ip==0:
            ...
            strings_to_write=ip
    if target_is_pang_domain==True and target_is_pang_and_sub==False:
        #这时target为旁站
        strings_to_write="ip is same to main target"
    if target_is_sub_domain==True and target_is_pang_and_sub==False:
        #这时target为子站
        xcdnObj=Xcdn(domain)
        ip=xcdnObj.return_value
        if ip==0:
...
def getServerType(url):
    ...
    import requests
    url=get_http_domain_from_url(url)
    r=requests.get(url)
