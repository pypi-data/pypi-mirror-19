...
drawPic('data/demo.jpg', w=400, h=300, x=0.5, y=0.5, anchor_x='left', anchor_y='center') # Draw a zoomed picture on the canvas, and move it
show(3000) # Display current canvas
```
### Show shape
```python
...
drawLines([(x-100,y-100), (x,y), (x+100,y-100)]) # Draw lines on the canvas
show(3000) # Display current canvas
```
