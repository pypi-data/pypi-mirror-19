...
def recordSound(noise_level=500, recording_min=0, recording_max=0, sounding_max=0, trim_side='left', feedback=False, chunk=512, path=''):
    ...
    while True:
        ...
        # whether the onset be detected
        if onset_detected: # count the silence length
            num_sound += len(snd_data)
            ...
            if isSilent(snd_data, noise_level):
                ...
                num_silent = 0
        else: # attempt to find the onset
            if max(snd_data) > speech_threshold:
                for idx in range(len(rec_data), 0, -chunk):
    ...
    if path!='':
        shared.librosa.output.write_wav(path, data, shared.setting['sample_rate'], norm=False)
...
def recordSoundTofile(filename, path='data', noise_level=500, recording_min=0, recording_max=0, sounding_max=0, trim_side='left', feedback=False, chunk=512):
    ...
    try:
        ...
        wf.setnchannels(1)
        wf.setsampwidth(2) # 2 bytes per frame
