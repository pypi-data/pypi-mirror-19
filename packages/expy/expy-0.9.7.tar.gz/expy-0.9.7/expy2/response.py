...
def waitForResponse(allowed_keys=[], out_time=0, has_RT=True, other_events = [], suspending=False):
    '''
    ...
    (Only if has_RT is True) past_time: int
        The millisecond count since the function starts.
    '''
    # def wait(keys, mapping):
    #     shared.watchdog_keys, shared.watchdog_mapping = keys, mapping
    #     while True:
    #         if shared.watchdog_has_event:
    #             if shared.watchdog_event == 'out':
    #                 return 'None', 'None'
    #             else:
    #                 return shared.watchdog_event, shared.watchdog_time

    #         time.sleep(0.0001)
    
    # if out_time > 0:
    #     start_tp = time.time()
    #     shared.watchdog_out_time = start_tp + out_time/1000

    # shared.pg.event.clear()

    # keys, mapping = setKeyMapping(allowed_keys)  # Mapping allowable key(s)

    # KEY, past_time = wait(keys, mapping)
    # shared.watchdog_has_event = False
    # shared.watchdog_out_time = 0
    # shared.watchdog_event = None
    ...
    def wait(start_tp, out_time, other_events):
        while True:
            ...
            for e in shared.events:
                if e['type']=='key_press':
                    return e['key'], int(past_time*1000)
                elif e['type'] in other_events:
                    return e['type'], int(past_time*1000)
                # mouse_press
    ...
    ev, past_time = wait(shared.start_tp, out_time/1000, other_events)
