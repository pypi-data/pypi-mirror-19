...
"""Returns true if authenticaction is accepted"""
def execute_bearer_authentication(self, kind, token, settings, client_model, headers):
    ...
    if not secret:
        logging.warning("Couldn't get client secret from payload")
        return False
    ...
    print client_fields
    print jwt_auth.verify_expiration, jwt_auth.verify_signature
    ...
    if ('Origin' not in headers):
        ...
        return False
    if ('localhost' not in headers.get('Origin') and 'localhost' not in headers.get('host')):
        logging.warning("Unauthorize")
        return False
    urls_white_list = getattr(client_obj, bearer_settings.get('ClientApp').get('Fields').get('UrlsWhiteList'))
