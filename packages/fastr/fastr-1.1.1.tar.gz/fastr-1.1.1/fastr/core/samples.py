...
class SampleItemBase(tuple):
    ...
    def __new__(cls, index, id_, data, jobs=None):
        """
        ...
        :type id_: :py:class:`SampleId <fastr.core.sampleidlist.SampleId>`
        ...
        if not isinstance(id_, SampleId):
            id_ = SampleId(id_)
        ...
        if not isinstance(jobs, (set, tuple, list)):
            raise exceptions.FastrTypeError("Jobs should be a set, tuple or list of jobs or jobids, found {}".format(type(jobs).__name__))

        jobs = {job if isinstance(job, str) else job.jobid for job in jobs}

        return super(SampleItemBase, cls).__new__(cls, [index, id_, data, jobs])
    ...
    def __add__(self, other):
        ...
        return type(self)(self.index, self.id, self.data + other.data, self.jobs | other.jobs)
...
class SampleItem(SampleItemBase):
    def __new__(cls, index, id_, data, jobs=None):
        ...
        return super(SampleItem, cls).__new__(cls, index, id_, data, jobs)
...
class SamplePayload(SampleItemBase):
    def __new__(cls, index, id_, data, jobs=None):
        ...
        return super(SamplePayload, cls).__new__(cls, index, id_, data, jobs)
    ...
    def __add__(self, other):
        ...
        if len(common_keys) > 0:
            raise exceptions.FastrValueError('Cannot combine SamplePayloads, they both contain definitions for {}'.format(common_keys))

        new_data = OrderedDict(self.data)
        new_data.update(other.data)
        return SamplePayload(self.index, self.id, new_data, self.jobs | other.jobs)


class HasSamples(object):
    """
    ...
    def iteritems(self):
        if len(self.size) > 0:
            ...
            for index in itertools.product(*[range(x) for x in self.size]):
                try:
    ...
    def ids(self):
        return [self[x].id for x in self.iteritems()]
...
class SampleBaseId(tuple):
    ...
    def __new__(cls, *args):
        ...
        try:
            ...
            raise exceptions.FastrTypeError('The arguments for {} should be a iterable of {} elements, found non-iterable {}'.format(cls.__name__, etype, args))
        ...
        if not all(isinstance(x, cls._element_type) for x in args):
            raise exceptions.FastrTypeError('The arguments for {} should be a iterable of {} elements, found iterable {}'.format(cls.__name__, cls._element_type, args))
...
class SampleIndex(SampleBaseId):
    ...
    def expand(self, size):
        ...
        index_range = [xrange(*x) if len(x) > 0 else [] for x in index_range]
...
class SampleValue(MutableMapping):
    ...
    def __init__(self, *args, **kwargs):
        try:
            ...
            raise exceptions.FastrTypeError('Cannot create dict based on: args "{}" kwargs and "{}" (reason: {})'.format(args, kwargs, err))
    ...
    def __getstate__(self):
        ...
        for key, value in self._data.items():
            state[key] = tuple(x.__getstate__() for x in value)

        return ('SampleValue', state)

    def __setstate__(self, state):
        if state[0] != 'SampleValue':
            raise exceptions.FastrValueError('Cannot unpickle incorrectly formatted SampleValue data!')
        self._data = {}
        for key, value in state[1].items():
            new_value = []
            for item in value:
                if item[0] not in fastr.typelist:
                    fastr.toollist.data  # Trigger toollist population
                new_item = fastr.typelist[item[0]]()
                new_item.__setstate__(item)
...
class SampleCollection(MutableMapping):
    """
    ...
    def __init__(self, dimnames, parent):
        ...
        """
        self._dimnames = dimnames
        self._ndims = len(dimnames)
        self._size = (0,) * self.ndims
        self._parent = parent
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, SampleId):
            try:
                ...
                raise exceptions.FastrKeyError('Cannot find key {!r} in {}'.format(item, sorted(self._id_lookup.keys())))
        elif isinstance(item, SampleIndex):
            if not item.isslice:
                try:
                    ...
                    if all(0 <= x < s for x, s in zip(item, self.size)):
                        raise exceptions.FastrIndexNonexistent('Could not find index {} in sparse array {}'.format(item, self))
                    else:
                        raise exceptions.FastrKeyError('Cannot find index {!r} in {}'.format(item, sorted(self._index_lookup.keys())))
            else:
                # Unpack slice
                try:
    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(value, SampleItem):
            ...
            if len(value) == 2 and isinstance(value[0], SampleValue) and isinstance(value[1], set):
                ...
                value = SampleItem(sample_index, sample_id, value[0], value[1])
            else:
                raise exceptions.FastrValueError('The tuple value is not a (values, depending)'
            ...
            value = SampleItem(sample_index, sample_id, value, set())
        ...
        if sample_index in self._index_lookup or sample_id in self._id_lookup:
            if self._index_lookup.get(sample_index) is not self._id_lookup.get(sample_id):
                raise exceptions.FastrValueError('The index and id of the item to set point to different items!')

        # Adapt the size
        self._size = tuple(max(x, y + 1) for x, y in zip(self._size, sample_index))
    ...
    def size(self):
        """
        The size of the SampleCollection. The size is the largest index in
        every dimension. For a 2D SampleCollection with 2 entries (10, 2) and
        (6, 6) the size would be (10, 6). As that is the rectangular grid that
