...
"""
This module contains the virtual file system code. This is both an internally
used object as well as an IOPlugin.
"""
...
class VirtualFileSystem(IOPlugin):
    """
    ...
    def fetch_url(self, inurl, outpath):
        ...
        if not os.path.exists(inpath):
            raise exceptions.FastrValueError('Path of the url to fetch ({}) does not point to a file or directory! ({} does not exist)'.format(inurl, inpath))
    ...
    def url_to_path(self, url, scheme=None):
        ...
        if parsed_url.scheme == scheme:
            try:
                ...
                raise exceptions.FastrMountUnknownError("The mount '{}' does not exists in url: {}. Available mounts: {}".format(parsed_url.netloc, url, ", ".join(fastr.config.mounts.keys())))
        else:
            raise exceptions.FastrUnknownURLSchemeError('URL using an unknown scheme ({}) found in {}!'.format(parsed_url.scheme, url))
    ...
    def path_to_url(self, path, mountpoint=None, scheme=None):
        ...
        if mountpoint is not None:
            return "{scheme}://{mnt}/{path}".format(scheme=scheme, mnt=mountpoint, path=path.strip(os.path.sep))
        else:
            abspath = os.path.abspath(os.path.expanduser(path))
            for name, mountpath in fastr.config.mounts.items():
                if abspath.startswith(mountpath):
                    mnt = name
                    break
            else:
                raise exceptions.FastrMountUnknownError("Mountpath for {} cannot be found in config.mounts".format(abspath))
            path = abspath.replace(fastr.config.mounts[mnt], "", 1)
            return "{scheme}://{mnt}/{path}".format(scheme=scheme, mnt=mnt, path=path.strip(os.path.sep))
