...
class FastrNodeNotValidError(FastrStateError):
    """
    A Node is not in a valid state where it should be, typically an invalid
    Node is passed to the executor causing trouble.
    """
    pass
...
class FastrNodeNotPreparedError(FastrStateError):
    """
    When trying to access executation data of a Node, the Node must be prepare.
    The Node has not been prepared by the execution, so the data is not
    available!
    """
    pass
...
class FastrNodeAreadyPreparedError(FastrStateError):
    """
    A attempt is made at preparing a Node for the second time. This is not
