...
class EnvironmentModules(object):
    ...
    except exceptions.FastrImportError:
    ...
    def __init__(self, protected=None):
        ...
        if not self._module_settings_loaded:
            raise exceptions.FastrValueError('Could not load the environmentmodules settings during import, this module is non-functional!')
    ...
    def sync(self):
        """
        Sync the object with the underlying enviroment. Re-checks the
    ...
    def _run_commands_string(self, value):
        ...
        for command in value.splitlines():
            match1 = re.match(r'os.environ\[\'(\w+)\'\] = \'(.*)\'\W*', command)
            match2 = re.match(r'del os.environ\[\'(\w+)\'\]', command)
            if match1 is None and match2 is None:
                fastr.log.warning('WARNING: Evaluating unchecked code: "{}"'.format(command))
    ...
    def avail(self, namestart=None):
        ...
        for module in self._avail_modules:
            ...
            if namestart is None or module.startswith(namestart):
                if default:
                    fastr.log.info('{} (default)'.format(module))
                else:
                    fastr.log.info(module)
    ...
    def load(self, module):
        ...
        if self.isloaded(module):
            fastr.log.warning('Module {} is already loaded, unloading first!'.format(module))
            self.unload(module)

        stdout, _ = self._module('load', module)
    ...
    def unload(self, module):
        ...
        if module in self._protected:
            fastr.log.debug('Module {} is protected, skipping unload!'.format(module))
            return

        if not self.isloaded(module):
            fastr.log.warning('Module {} is not loaded, cannot unload!'.format(module))

        stdout, _ = self._module('unload', module)
        self._run_commands_string(stdout)
    ...
    def reload(self, module):
        ...
        module = self.tostring_modvalue(module)
        if not self.isloaded(module):
            fastr.log.warning(': Module {} is not loaded, cannot unload first!'.format(module))
        else:
            stdout, _ = self._module('unload', module)
            self._run_commands_string(stdout)
        stdout, _ = self._module('load', module)
