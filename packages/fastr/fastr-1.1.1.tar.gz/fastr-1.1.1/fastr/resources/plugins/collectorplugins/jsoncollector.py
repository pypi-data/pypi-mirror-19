...
class JsonCollector(FastrInterface.collector_plugin_type):
    ...
    6. The data is parsed by :py:meth:`set_result <fastr.core.node.Node.set_result>`

    The structure of the JSON has to follow the a predefined format. For normal
    :py:class:`Nodes <fastr.core.node.Node>` the format is in the form::

      [value1, value2, value3]

    where the multiple values represent the cardinality.

    For a :py:class:`FlowNodes <fastr.core.node.FlowNode>` the format is the form::
    ...
    def _collect_results(self, interface, output, result):
        ...
        if location is not None:
            ...
            specials, inputs, outputs, input_parts, outputs_parts = interface.get_specials(result.payload, output, cardinality)
            loc = location.format(input=inputs, output=outputs, special=specials, input_parts=input_parts, output_parts=outputs_parts)
