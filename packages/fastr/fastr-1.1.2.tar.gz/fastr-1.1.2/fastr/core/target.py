...
class ProcessUsageCollection(Sequence):
    ...
    def __getitem__(self, item):
        ...
        if item < len(self.minutes_info):
            return self.minutes_info[item]
        else:
            return self.seconds_info[item - len(self.minutes_info)]
    ...
    def monitor_process(self, process, resources):
        ...
        while process.poll() is None:
            try:
                ...
                usage = resources.usage_type(timestamp=datetime.datetime.utcnow(),
...
class DockerTarget(Target):
    ...
    docker-py.
    """
    def __init__(self, binary, docker_image, **kwargs):
        """
        ...
        self._docker_client = docker.Client(self.docker_api)
    ...
    def run_command(self, command):
        ...
        binds = {x: {'bind': x, 'ro': True} for x in mounts if os.path.exists(x)}
        binds[fastr.config.mounts['tmp']]['ro'] = False
        fastr.log.info('DOCKER BINDS: {}'.format(binds))

        host_config = self._docker_client.create_host_config(binds=binds)
        fastr.log.info('DOCKER HOSTCONFIG: {}'.format(host_config))

        fastr.log.info('DOCKER CREATE CONTAINER')
        container = self._docker_client.create_container(image=self._docker_image,
                                                         command=command,
                                                         volumes=mounts,
        ...
        # Start monitoring
        sysuse = []
        monitor_thread = threading.Thread(target=self.monitor_docker, name='DockerMonitor', args=(container.get('Id'), sysuse))
        monitor_thread.daemon = True  # Make sure this Thread does not block exiting the script
        monitor_thread.start()
        start_time = time.time()
        ...
        fastr.log.info('DOCKER START')
        response = self._docker_client.start(container=container.get('Id'))
        fastr.log.info('DOCKER START RESPONE: {}'.format(response))

        fastr.log.info('DOCKER WAIT')
        return_code = self._docker_client.wait(container.get('Id'))
        fastr.log.info('DOCKER WAIT DONE')
        end_time = time.time()
        stdout = self._docker_client.logs(container.get('Id'), stdout=True, stderr=False, stream=False, timestamps=True)
        stderr = self._docker_client.logs(container.get('Id'), stdout=False, stderr=True, stream=False, timestamps=True)
    ...
    def monitor_docker(self, container, resources):
        ...
        """
        try:
            for stat in self._docker_client.stats(container, decode=True):
