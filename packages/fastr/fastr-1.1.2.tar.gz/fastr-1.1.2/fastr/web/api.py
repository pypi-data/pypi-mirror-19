...
def update_status(job, job_status):
    job_status[job.jobid] = str(job.status), str(job.nodeid)
...
class ToolApi(Resource):
    ...
    def get(self, id_, version=None):
        ...
        """
        try:
            return fastr.toollist[id_, version].dumps(method='json')
        except KeyError:
            abort(404)
...
class ToolListApi(Resource):
    ...
    def get(self):
        ...
        """
        data = {'tools': [url_for('api_version_tool', id=x.ns_id, version=x.command_version) for x in fastr.toollist.values()]}
