...
class VolumeCloneTest( BaseCliTest):
    ...
    @patch( "services.safeguard.safeguard_task_client.SafeGuardTaskClient.list_tasks_by_volume",
    ...
    def test_clone_remote(self, mockCloneRemote, mockId, mockTimelinePresets, mockListTasks):
        ...
        from_last_clone_remote = mockCloneRemote.call_args[0][2]
    ...
    def test_service_clone_remote(self, mock_get_volume, mock_post, mock_url, mock_write, mock_pretty):
        ...
        service.clone_remote(remote_volume, snapshot_id, from_last_clone_remote=False)

        # The service clone_remote is a url producer
        url = mock_post.call_args[0][1]
        assert mock_post.call_count == 1
        assert url == "http://localhost:7777/fds/config/v09/volumes/-1/exports?snapshot_id=365"
