...
class SnapshotTest(BaseCliTest):
    ...
    @patch( "services.abstract_service.AbstractService.get_url_preamble", return_value="http://www.fds.com")
    @patch( "services.rest_helper.RESTHelper.post", return_value=None)
    def test_create_snap_may_return_none(self, postcall, urlBuild ):
        '''
        Create snapshot may return null because the snap isnt ready.
        We need to handle this
        '''
        print( "Testing handling of the NoneType")

        snaps = Snapshot( volume_id=3, name="SomeSnap", retention=7200 )

        volService = VolumeService( FdsAuth() )
        volService.create_snapshot( snaps )
