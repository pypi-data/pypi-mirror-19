...
from model.event.event_source_info import EventSourceInfo
...
class TestConverters(BaseCliTest):
    ...
    def test_event_converter_and_source_info(self):
        ...
        which includes testing of ``model.event.event_source_info.EventSourceInfo``
        """

        s = EventSourceInfo("101", 1, "192.168.1.1", 555, "StorMgr", 103, "SM", "v1", 1, "local", None)
        e = Event(123, "test.Key", 0, "Event message", "NORMAL", ["CAT1", "CAT2"], ["101"], s)
        ...
        assert e.source_info.source_id == e2.source_info.source_id
        assert e.source_info.node_id == e2.source_info.node_id
        assert e.source_info.node_address == e2.source_info.node_address
        assert e.source_info.pid == e2.source_info.pid
        assert e.source_info.process_name == e2.source_info.process_name
        assert e.source_info.service_id == e2.source_info.service_id
        assert e.source_info.service_type == e2.source_info.service_type
        assert e.source_info.service_version == e2.source_info.service_version
        assert e.source_info.domain_id == e2.source_info.domain_id
        assert e.source_info.domain_name == e2.source_info.domain_name
        assert e.source_info.attributes == e2.source_info.attributes
