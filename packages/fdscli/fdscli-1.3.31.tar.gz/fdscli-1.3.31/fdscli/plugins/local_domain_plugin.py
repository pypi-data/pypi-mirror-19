...
class LocalDomainPlugin( AbstractPlugin):
    ...
    def build_parser(self, parentParser, session): 
        ...
        self.create_check_parser( self.__subparser )
        self.create_fix_parser( self.__subparser )
    ...
    def create_check_parser(self, subparser):
        '''
        Create the parser for domain tokens check command
        '''
        __check_parser = subparser.add_parser( "check", help="Check the tokens data integrity in a given domain" )
        self.add_format_arg( __check_parser )
        __check_parser.add_argument( "-" + AbstractPlugin.domain_id_str, help="The ID of the domain you wish to start.", required=True )
        __check_parser.set_defaults( func=self.check_tokens, format="tabular" )

    def create_fix_parser(self, subparser):
        '''
        Create the parser for domain tokens fix command
        '''
        __fix_parser = subparser.add_parser( "fix", help="Check and repair out of sync tokens in a given domain" )
        self.add_format_arg( __fix_parser )
        __fix_parser.add_argument( "-" + AbstractPlugin.domain_id_str, help="The ID of the domain you wish to start.", required=True )
        __fix_parser.set_defaults( func=self.fix_tokens, format="tabular" )
    ...
    def check_tokens(self, args):
        '''
        check tokens in a local domain
        '''
        domain = self.get_local_domain_service().find_domain_by_id(args[AbstractPlugin.domain_id_str])
        ...
        print("Issuing domain level tokens check. This may take a while...")
        response = self.get_local_domain_service().check_tokens(domain)
        
        if not isinstance( response, FdsError ):
            print("Domain tokens check completed successfully. No errors were found")
        else:
            #TODO - be more specific w.r.t. fdserror code
            print("Domain tokens check completed successfully. Some errors were found")
            return 1

    def fix_tokens(self, args):
        '''
        check tokens in a local domain
        '''
        domain = self.get_local_domain_service().find_domain_by_id(args[AbstractPlugin.domain_id_str])
        
        print("Issuing domain level tokens repair. This may take a while...")
        if domain is None:
            print("The Domain ID entered does not exist.  Please use an ID found from the command: local_domain list")
            return
        
        response = self.get_local_domain_service().fix_tokens(domain)
        
        if not isinstance( response, FdsError ):
            print("Domain tokens repair completed successfully. No errors were found")
        else:
            #TODO - be more specific w.r.t. fdserror code
            print("Domain tokens repair completed successfully. Some errors were found")
            return 1
