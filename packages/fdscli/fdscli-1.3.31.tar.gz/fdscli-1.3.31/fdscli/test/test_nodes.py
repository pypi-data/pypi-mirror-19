...
class TestNodes( BaseCliTest ):
    ...
    def test_object_creation(self):
        '''
        Simple test to check the property setting of the node values
        '''
        ...
        node1.address.ipv4address = "127.0.0.1"
        node2.address.ipv4address = "127.0.0.2"
        
        assert node1.id == "one"
        assert node2.id == "two"
        assert node1.state == "UP"
        assert node2.state == "DOWN"
        assert node1.address.ipv4address == "127.0.0.1"
        assert node2.address.ipv4address == "127.0.0.2"
    ...
    @patch( "services.node_service.NodeService.list_nodes", side_effect=mock_functions.listDiscoveredNodes)        
    def test_add_nodes_with_conversion(self, mockList, mockPost, mockAdd):
        '''
