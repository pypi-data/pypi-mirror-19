...
from model.volume.recurrence_rule import Frequency, RecurrenceRule
...
class SnapshotPolicyPlugin( AbstractPlugin):
    ...
    def create_edit_parser(self, subparser):
        ...
        __edit_parser.add_argument( "-" + AbstractPlugin.retention_str, help="The time (in seconds) that you want to keep snapshots that are created with this policy. 0 = forever", type=int)
        ...
        __edit_parser.add_argument( "-" + AbstractPlugin.frequency_str, help="The frequency for which you would like snapshots to be taken on volumes this policy is applied to.", choices=["DAILY", "HOURLY", "WEEKLY", "MONTHLY", "YEARLY"])
        __edit_parser.add_argument( "-" + AbstractPlugin.day_of_week_str, help="The day(s) of the week you would like the policy to run (when applicable).", choices=["SU","MO","TU","WE","TH","FR","SA"], nargs="+")
        ...
        __edit_parser.add_argument( "-" + AbstractPlugin.hour_str, help="The hour(s) of the day you wish this policy to run.", nargs="+", type=SnapshotPolicyValidator.hour_value)
        __edit_parser.add_argument( "-" + AbstractPlugin.minute_str, help="The minute(s) of the day you wish this policy to run.", nargs="+", type=SnapshotPolicyValidator.minute_value)
    ...
    def create_snapshot_policy(self, args):
        ...
        if ( args[AbstractPlugin.data_str] is not None ):
            ...
            policy.retention_time_in_seconds = args[AbstractPlugin.retention_str]
            ...
            if ( args[AbstractPlugin.recurrence_rule_str] is not None ):
                ...
                # by hour doesn't make sense if it's hourly
                if ( policy.recurrence_rule.frequency != Frequency.HOURLY ):
                    policy.recurrence_rule.byhour = args[AbstractPlugin.hour_str]
                
                # day of week is not applicable if the frequency is daily or hourly
                if ( policy.recurrence_rule.frequency != Frequency.DAILY and policy.recurrence_rule.frequency != Frequency.HOURLY ):
                    policy.recurrence_rule.byday = args[AbstractPlugin.day_of_week_str]
                
                # only applicable to yearly jobs
                if ( policy.recurrence_rule.frequency == Frequency.YEARLY ):
                    policy.recurrence_rule.bymonth = args[AbstractPlugin.month_str]
                    
                #only applicable to monthly or yearly jobs
                if ( policy.recurrence_rule.frequency == Frequency.YEARLY or policy.recurrence_rule.frequency == Frequency.MONTHLY ):    
                ...
                # the only thing that applies to all policy frequencies
    ...
    def edit_snapshot_policy(self, args):
        ...
        '''
        policy = SnapshotPolicy()
        v_id = args[AbstractPlugin.volume_id_str]
        
        if ( args[AbstractPlugin.data_str] is not None ):
            policy = SnapshotPolicyConverter.build_snapshot_policy_from_json( args[AbstractPlugin.data_str])
        else:
            
            p_id = args[AbstractPlugin.policy_id_str]
            
            policy = self.get_snapshot_policy_service().get_snapshot_policy(v_id, p_id)
            
            if args[AbstractPlugin.name_str] is not None:
                policy.name = args[AbstractPlugin.name_str]
                
            if args[AbstractPlugin.retention_str] is not None:
                policy.retention_time_in_seconds = args[AbstractPlugin.retention_str]
                
            if args[AbstractPlugin.frequency_str] is not None:
                
                if policy.recurrence_rule is None:
                    policy.recurrence_rule = RecurrenceRule()
            ...
            if ( args[AbstractPlugin.recurrence_rule_str] is not None ):
                ...
                if ( policy.recurrence_rule.frequency != Frequency.DAILY):
                    policy.recurrence_rule.byday = args[AbstractPlugin.day_of_week_str]
                
                if ( policy.recurrence_rule.frequency == Frequency.HOURLY ):
                    policy.recurrence_rule.byday = None
                    policy.recurrence_rule.byhour = None
                    policy.recurrence_rule.bymonth = None
                    policy.recurrence_rule.bymonthday = None
                    policy.recurrence_rule.byyearday = None
                ...
                if ( policy.recurrence_rule.frequency == Frequency.WEEKLY ):
                    policy.recurrence_rule.bymonth = None
                    policy.recurrence_rule.bymonthday = None
                    policy.recurrence_rule.byyearday = None
                    
                if ( policy.recurrence_rule.frequency == Frequency.MONTHLY ):
                    policy.recurrence_rule.bymonth = None    
                    policy.recurrence_rule.byyearday = None                                    
                ...
                if args[AbstractPlugin.hour_str] is not None:    
                    policy.recurrence_rule.byhour = args[AbstractPlugin.hour_str]
                    
                if args[AbstractPlugin.minute_str] is not None:
                    policy.recurrence_rule.byminute = args[AbstractPlugin.minute_str]
            
        new_policy = self.get_snapshot_policy_service().edit_snapshot_policy( v_id, policy )
