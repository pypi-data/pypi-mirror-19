...
from fs1.local_functools import wraps
...
def silence_fserrors(f, *args, **kwargs):
    """Perform a function call and return ``None`` if an :class:`~fs1.errors.FSError` is thrown
...
class NullFile(object):
    ...
    Null files are returned by the :meth:`~fs1.base.FS.safeopen` method in FS objects when the
...
class FS(object):
    ...
    def getmeta(self, meta_name, default=NoDefaultMeta):
        ...
        :raises `fs1.errors.NoMetaError`: If specified meta value is not present, and there is no default
    ...
    def validatepath(self, path):
        """Validate an fs1 path, throws an :class:`~fs1.errors.InvalidPathError` exception if validation fails.
        ...
        :param path: an fs1 path to validatepath
        :raises `fs1.errors.InvalidPathError`: if `path` does not map on to a valid path on this filesystem
    ...
    def getsyspath(self, path, allow_none=False):
        ...
        :raises `fs1.errors.NoSysPathError`: if the path does not map on to a system path, and allow_none is set to False (default)
        :rtype: unicode
    ...
    def getpathurl(self, path, allow_none=False):
        ...
        then a :class:`~fs1.errors.NoPathURLError` exception is thrown. Otherwise the URL will be
        ...
        :type allow_none: bool
        :raises `fs1.errors.NoPathURLError`: If no URL form exists, and allow_none is False (the default)
        :rtype: unicode
    ...
    def open(self, path, mode='r', buffering=-1, encoding=None, errors=None, newline=None, line_buffering=False, **kwargs):
        ...
        :raises `fs1.errors.ParentDirectoryMissingError`: if an intermediate directory is missing
        :raises `fs1.errors.ResourceInvalidError`: if an intermediate directory is an file
        :raises `fs1.errors.ResourceNotFoundError`: if the path is not found
    ...
    def safeopen(self, path, mode='r', buffering=-1, encoding=None, errors=None, newline=None, line_buffering=False, **kwargs):
        """Like :py:meth:`~fs1.base.FS.open`, but returns a
        :py:class:`~fs1.base.NullFile` if the file could not be opened.
    ...
    def __iter__(self):
        """ Iterates over paths returned by :py:meth:`~fs1.base.listdir` method with default params. """
    ...
    def listdir(self,
        ...
        :raises `fs1.errors.ParentDirectoryMissingError`: if an intermediate directory is missing
        :raises `fs1.errors.ResourceInvalidError`: if the path exists, but is not a directory
        :raises `fs1.errors.ResourceNotFoundError`: if the path is not found
    ...
    def listdirinfo(self,
        ...
        :py:meth:`~fs1.base.FS.getinfo` on each individual item returned by :py:meth:`~fs1.base.FS.listdir`, particularly
    ...
    def ilistdir(self,
        ...
        This method behaves identically to :py:meth:`~fs1.base.FS.listdir` but returns an generator
        instead of a list.  Depending on the filesystem this may be more
        efficient than calling :py:meth:`~fs1.base.FS.listdir` and iterating over the resulting list.
    ...
    def ilistdirinfo(self,
        ...
        This method behaves identically to :py:meth:`~fs1.base.listdirinfo` but returns an generator
        instead of a list.  Depending on the filesystem this may be more
        efficient than calling :py:meth:`~fs1.base.listdirinfo` and iterating over the resulting
        list.
    ...
    def makedir(self, path, recursive=False, allow_recreate=False):
        ...
        :raises `fs1.errors.DestinationExistsError`: if the path is already a directory, and allow_recreate is False
        :raises `fs1.errors.ParentDirectoryMissingError`: if a containing directory is missing and recursive is False
        :raises `fs1.errors.ResourceInvalidError`: if a path is an existing file
        :raises `fs1.errors.ResourceNotFoundError`: if the path is not found
    ...
    def remove(self, path):
        ...
        :raises `fs1.errors.ParentDirectoryMissingError`: if an intermediate directory is missing
        :raises `fs1.errors.ResourceInvalidError`: if the path is a directory
        :raises `fs1.errors.ResourceNotFoundError`: if the path does not exist
    ...
    def removedir(self, path, recursive=False, force=False):
        ...
        :raises `fs1.errors.DirectoryNotEmptyError`: if the directory is not empty and force is False
        :raises `fs1.errors.ParentDirectoryMissingError`: if an intermediate directory is missing
        :raises `fs1.errors.ResourceInvalidError`: if the path is not a directory
        :raises `fs1.errors.ResourceNotFoundError`: if the path does not exist
    ...
    def opendir(self, path):
        ...
        from fs1.wrapfs.subfs import SubFS
        if not self.exists(path):
    ...
    def move(self, src, dst, overwrite=False, chunk_size=16384):
        ...
        :raise `fs1.errors.DestinationExistsError`: if destination exists and `overwrite` is False
    ...
    def printtree(self, max_levels=5):
        ...
        """
        from fs1.utils import print_fs
    ...
    def browse(self, hide_dotfiles=False):
        ...
        """
        from fs1.browsewin import browse
    ...
    def getmmap(self, path, read_only=False, copy=False):
        ...
        :raises `fs1.errors.NoMMapError`: Only paths that have a syspath can be opened as a mmap
