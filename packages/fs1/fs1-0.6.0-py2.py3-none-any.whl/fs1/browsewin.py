...
from fs1.path import isdotfile, pathsplit
...
class BrowseFrame(wx.Frame):
    ...
    def __init__(self, fs1, hide_dotfiles=False):

        wx.Frame.__init__(self, None, size=(1000, 600))

        self.fs1 = fs1
        self.hide_dotfiles = hide_dotfiles
        self.SetTitle("FS Browser - " + unicode(fs1))
    ...
    def expand(self, item_id):
        ...
        try:
            paths = ( [(True, p) for p in self.fs1.listdir(path, absolute=True, dirs_only=True)] +
                      [(False, p) for p in self.fs1.listdir(path, absolute=True, files_only=True)] )
        except FSError, e:
        ...
        #paths = [(self.fs1.isdir(p), p) for p in self.fs1.listdir(path, absolute=True)]
        ...
        for is_dir, new_path in paths:
            ...
            info = self.fs1.getinfo(new_path)
            ...
            self.tree.SetItemText(new_item, self.fs1.desc(new_path), 1)
    ...
    def OnItemActivated(self, e):
        ...
        info_frame = InfoFrame(self, path, self.fs1.desc(path), info)
        info_frame.Show()
        info_frame.CenterOnParent()


def browse(fs1, hide_dotfiles=False):
    ...
    app = wx.PySimpleApp()
    frame = BrowseFrame(fs1, hide_dotfiles=hide_dotfiles)
    frame.Show()
    app.MainLoop()
