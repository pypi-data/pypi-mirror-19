#!/usr/bin/env python
from fs1.utils import copyfile, copyfile_non_atomic
from fs1.path import pathjoin, iswildcard
...
class FileOpThread(threading.Thread):
    ...
    def run(self):
        ...
        while not self.finish_event.isSet():
            try:
                path_type, fs1, path, dest_path = self.queue.get(timeout=0.1)
            except queue.Empty:
                continue
            try:
                if path_type == FScp.DIR:
                    ...
                    self.action(fs1, path, self.dest_fs, dest_path, overwrite=True)
            except Exception, e:
                self.on_error(e)
                ...
                self.queue.task_done()
                self.on_done(path_type, fs1, path, self.dest_fs, dest_path)
...
Copy SOURCE to DESTINATION"""
    ...
    def do_run(self, options, args):
        ...
        if self.options.threads > 1:
            ...
            for path_type, fs1, path, dest_path in copy_fs_dirs:
