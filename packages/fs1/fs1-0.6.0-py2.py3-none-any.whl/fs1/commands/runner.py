...
from fs1.opener import opener, OpenerError, Opener
from fs1.errors import FSError
from fs1.path import splitext, pathsplit, isdotfile, iswildcard
...
#            fname = u'%s\x1b[36m%s\x1b[0m' % (name, ext)
    ...
    def open_fs(self, fs_url, writeable=False, create_dir=False):
        fs1, path = opener.parse(fs_url, writeable=writeable, create_dir=create_dir)
        fs1.cache_hint(True)
        return fs1, path

    def expand_wildcard(self, fs1, path):
        if path is None:
            return [], []
        pathname, resourcename = pathsplit(path)
        if iswildcard(resourcename):
            dir_paths = fs1.listdir(pathname,
                                   wildcard=resourcename,
                                   absolute=True,
                                   dirs_only=True)

            file_paths = fs1.listdir(pathname,
                                    wildcard=resourcename,
                                    absolute=True,
                                    files_only=True)
    ...
    def get_resources(self, fs_urls, dirs_only=False, files_only=False, single=False):
        ...
        for fs1, path in fs_paths:
            if path and iswildcard(path):
                if not files_only:
                    dir_paths = fs1.listdir(wildcard=path, dirs_only=True)
                    for path in dir_paths:
                        resources.append([fs1, path, True])
                if not dirs_only:
                    file_paths = fs1.listdir(wildcard=path, files_only=True)
                    for path in file_paths:
                        resources.append([fs1, path, False])
            else:
                path = path or '/'
                is_dir = fs1.isdir(path)
                resource = [fs1, path, is_dir]
    ...
    def get_optparse(self):
        ...
        optparse.add_option('--fs1', dest='fs1', action='append', type="string",
                            help="import an FS opener e.g --fs1 foo.bar.MyOpener", metavar="OPENER")
        return optparse
    ...
    def run(self):
        ...
        ilocals = {}
        if options.fs1:
            for import_opener in options.fs1:
                ...
                try:
                    if not issubclass(new_opener, Opener):
                        self.error('%s is not an fs1.opener.Opener\n' % import_opener)
                        return 0
                except TypeError:
