...
class ArchiveMountFS(mountfs.MountFS):
    ...
    @synchronize
    def close(self):
        # Close and delete references to any other fs1 instances.
    ...
    def unmount(self, path):
        ...
        # one open.
        if callable(getattr(fs1, 'close', None)):
            fs1.close()
    ...
    def getsyspath(self, path, allow_none=False):
        """A getsyspath() override that returns paths relative to the root fs1."""
    ...
    def open(self, path, *args, **kwargs):
        ...
        if libarchive.is_archive_name(path) and self.ismount(path):
            self.unmount(path)
        fs1, _mount_path, delegate_path = self._delegate(path, auto_mount=False)
        return fs1.open(delegate_path, *args, **kwargs)
    ...
    def getinfo(self, path):
        ...
        if isinstance(fs1, ArchiveFS) and path == _mount_path:
    ...
    def getsize(self, path):
        ...
        if isinstance(fs1, ArchiveFS) and path == _mount_path:
            ...
            return fs1.getsize(delegate_path)
    ...
    def makedir(self, path, *args, **kwargs):
        ...
        return fs1.makedir(delegate_path, *args, **kwargs)
    ...
    def copy(self, src, dst, overwrite=False, chunk_size=1024*64):
        ...
        if srcfs is dstfs and srcfs is not self:
            # Both src and dst are on the same fs1, let it do the copy.
