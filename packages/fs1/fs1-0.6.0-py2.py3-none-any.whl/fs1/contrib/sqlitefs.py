...
from fs1.path import iteratepath, normpath,dirname,forcedir
from fs1.path import frombase, basename,pathjoin
...
class SqliteFsFileBase(object):
    ...
    '''
    def __init__(self, fs1, path, id, real_file=None):
        assert(fs1 != None)
        assert(path != None)
        assert(id != None)
        self.fs1 = fs1
    ...
    def close(self):
        if not self.closed and self.real_stream is not None:
            self._do_close()
            self.fs1._on_close(self)
            self.real_stream.close()
            self.closed = True

    def __str__(self):
        return "<SqliteFS File in %s %s>" % (self.fs1, self.path)

    __repr__ = __str__

    def __unicode__(self):
        return u"<SqliteFS File in %s %s>" % (self.fs1, self.path)
...
class SqliteWritableFile(SqliteFsFileBase):
    ...
    '''
    def __init__(self,fs1, path, id):
        super(SqliteWritableFile, self).__init__(fs1, path, id)
    ...
    def _do_close(self):
        ...
        self.fs1._writeblob(self.id, self.real_stream)
...
class SqliteReadableFile(SqliteFsFileBase):
    def __init__(self,fs1, path, id, real_file):
        super(SqliteReadableFile, self).__init__(fs1, path, id, real_file)
...
#    fs1 = SqliteFS(sqlfilename)
#    fs1.makedir('/test')
#    f = fs1.open('/test/test.txt', "w")
#    f.write("This is a test")
#    f.close()
#    f = fs1.open('/test/test.txt', "r")
#    contents = f.read()
#    print contents
#    f.close()
#    print "testing file rename"
#    fs1.rename('/test/test.txt', '/test/test1.txt')
#    f = fs1.open('/test/test1.txt', "r")
#    print contents
#    f.close()
#    print "done testing file rename"
#    print "testing directory rename"
#    fs1.rename('/test', '/test1')
#    f = fs1.open('/test1/test1.txt', "r")
#    contents = f.read()
#    print contents
#    f.close()
#    print "done testing directory rename"
#    flist = fs1.listdir('/', full=True,absolute=True,files_only=True)
#    print flist
#    fs1.close()
#
