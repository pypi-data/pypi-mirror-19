...
Example (it will use publicly available (but slow) Tahoe-LAFS cloud)::
    ...
    print "Keep it safe!"
    fs1 = TahoeLAFS(dircap, autorun=False, webapi='http://insecure.tahoe-lafs.org')
    f = fs1.open("foo.txt", "a")
    f.write('bar!')
    f.close()
    print "Now visit %s and enjoy :-)" % fs1.getpathurl('foo.txt')
...
from fs1.path import abspath, relpath, normpath, dirname, pathjoin
from fs1.base import FS, NullFile
from fs1 import _thread_synchronize_default, SEEK_END
from fs1.remote import CacheFSMixin, RemoteFileBuffer
from fs1.base import fnmatch, NoDefaultMeta
...
logger = fs1.getLogger('fs1.tahoelafs')
