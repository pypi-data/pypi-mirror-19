...
from fs1.errors import convert_fs_errors
...
class FTPFS(ftpserver.AbstractedFS):
    ...
    AbstractedFS. This class will cause the FTP server to serve the given fs1 instance.
    """
    encoding = 'utf8'
    "Sets the encoding to use for paths."

    def __init__(self, fs1, root, cmd_channel, encoding=None):
        self.fs1 = fs1
    ...
    def open(self, path, mode, **kwargs):
        return self.fs1.open(path, mode, **kwargs)
    ...
    def chdir(self, path):
        ...
        if not isinstance(path, unicode):
            ...
            # pyftpdlib 1.x
            unipath = path
        # TODO: can the following conditional checks be farmed out to the fs1?
        ...
        if not self.fs1.exists(unipath):
    ...
    def listdir(self, path):
        return map(lambda x: x.encode(self.encoding), self.fs1.listdir(path))
    ...
    def stat(self, path):
        ...
        # Give the fs1 a chance to provide the uid/gid. Otherwise echo the current
        # uid/gid.
        ...
        if 'st_mode' in info:
            ...
            if self.fs1.isfile(path):
                mode |= stat.S_IFREG
            elif self.fs1.isdir(path):
                mode |= stat.S_IFDIR
...
class FTPFSFactory(object):
    ...
    while sharing the same fs1.
    """
    def __init__(self, fs1, encoding=None):
        """
        Initializes the factory with an fs1 instance.
        """
        self.fs1 = fs1
        self.encoding = encoding
    ...
    def __call__(self, root, cmd_channel):
        ...
        as well as the previously provided fs1 instance and encoding.
        """
        return FTPFS(self.fs1, root, cmd_channel, encoding=self.encoding)
...
def serve_fs(fs1, addr, port):
    ...
    ftp_handler.abstracted_fs = FTPFSFactory(fs1)
