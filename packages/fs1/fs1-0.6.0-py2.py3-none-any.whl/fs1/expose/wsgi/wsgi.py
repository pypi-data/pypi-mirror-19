...
from fs1.path import basename, pathsplit
...
class WSGIServer(object):
    ...
    def serve_dir(self, request):
        ...
        isdir = fs1.isdir
        path = request.path
        dirinfo = fs1.listdirinfo(path, full=True, absolute=True)
        entries = []
        ...
        html = self.dir_template.render(**dict(fs1=self.serve_fs,
                                               path=path,
...
def serve_fs(fs1, indexes=True):
    """Serves an FS object via WSGI"""
    application = WSGIServer(fs1, indexes)
    return application
