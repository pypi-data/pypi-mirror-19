...
class RPCFSInterface(object):
    ...
    def getmeta_default(self, meta_name, default):
        meta = self.fs1.getmeta(meta_name, default)
        if isinstance(meta, basestring):
            meta = self.decode_path(meta)
        return meta

    def hasmeta(self, meta_name):
        return self.fs1.hasmeta(meta_name)

    def get_contents(self, path, mode="rb"):
        path = self.decode_path(path)
        data = self.fs1.getcontents(path, mode)
        return xmlrpclib.Binary(data)

    def set_contents(self, path, data):
        path = self.decode_path(path)
        self.fs1.setcontents(path, data.data)

    def exists(self, path):
        path = self.decode_path(path)
        return self.fs1.exists(path)

    def isdir(self, path):
        path = self.decode_path(path)
        return self.fs1.isdir(path)

    def isfile(self, path):
        path = self.decode_path(path)
        return self.fs1.isfile(path)

    def listdir(self, path="./", wildcard=None, full=False, absolute=False, dirs_only=False, files_only=False):
        path = self.decode_path(path)
        entries = self.fs1.listdir(path, wildcard, full, absolute, dirs_only, files_only)
        return [self.encode_path(e) for e in entries]

    def makedir(self, path, recursive=False, allow_recreate=False):
        path = self.decode_path(path)
        return self.fs1.makedir(path, recursive, allow_recreate)

    def remove(self, path):
        path = self.decode_path(path)
        return self.fs1.remove(path)

    def removedir(self, path, recursive=False, force=False):
        path = self.decode_path(path)
        return self.fs1.removedir(path, recursive, force)

    def rename(self, src, dst):
        src = self.decode_path(src)
        dst = self.decode_path(dst)
        return self.fs1.rename(src, dst)
    ...
    def settimes(self, path, accessed_time, modified_time):
        ...
        return self.fs1.settimes(path, accessed_time, modified_time)
    ...
    def desc(self, path):
        path = self.decode_path(path)
        return self.fs1.desc(path)

    def getxattr(self, path, attr, default=None):
        path = self.decode_path(path)
        attr = self.decode_path(attr)
        return self.fs1.getxattr(path, attr, default)

    def setxattr(self, path, attr, value):
        path = self.decode_path(path)
        attr = self.decode_path(attr)
        return self.fs1.setxattr(path, attr, value)

    def delxattr(self, path, attr):
        path = self.decode_path(path)
        attr = self.decode_path(attr)
        return self.fs1.delxattr(path, attr)

    def listxattrs(self, path):
        path = self.decode_path(path)
        return [self.encode_path(a) for a in self.fs1.listxattrs(path)]

    def copy(self, src, dst, overwrite=False, chunk_size=16384):
        src = self.decode_path(src)
        dst = self.decode_path(dst)
        return self.fs1.copy(src, dst, overwrite, chunk_size)

    def move(self, src, dst, overwrite=False, chunk_size=16384):
        src = self.decode_path(src)
        dst = self.decode_path(dst)
        return self.fs1.move(src, dst, overwrite, chunk_size)

    def movedir(self, src, dst, overwrite=False, ignore_errors=False, chunk_size=16384):
        src = self.decode_path(src)
        dst = self.decode_path(dst)
        return self.fs1.movedir(src, dst, overwrite, ignore_errors, chunk_size)

    def copydir(self, src, dst, overwrite=False, ignore_errors=False, chunk_size=16384):
        src = self.decode_path(src)
        dst = self.decode_path(dst)
        return self.fs1.copydir(src, dst, overwrite, ignore_errors, chunk_size)
...
class RPCFSServer(SimpleXMLRPCServer):
    ...
    Example::
        ...
        fs1 = OSFS('/var/srv/myfiles')
        s = RPCFSServer(fs1,("",8080))
        s.serve_forever()
    ...
    def __init__(self, fs1, addr, requestHandler=None, logRequests=None):
        ...
        self.register_instance(RPCFSInterface(fs1))
