...
from fs1.path import pathsplit, abspath, dirname, recursepath, normpath, pathjoin, isbase
...
class _FTPFile(object):
    ...
    @fileftperrors
    def seek(self, pos, where=fs1.SEEK_SET):
        ...
        try:
            ...
            current = self.tell()
            new_pos = None
            if where == fs1.SEEK_SET:
                new_pos = pos
            elif where == fs1.SEEK_CUR:
                new_pos = current + pos
            elif where == fs1.SEEK_END:
...
class FTPFS(FS):
    ...
    def __init__(self, host='', user='', passwd='', acct='', timeout=_GLOBAL_DEFAULT_TIMEOUT, port=21, dircache=True, follow_symlinks=False):
        ...
        :param dircache: If True then directory information will be cached,
            ...
            :meth:`~fs1.ftpfs.FTPFS.clear_dircache` is called
