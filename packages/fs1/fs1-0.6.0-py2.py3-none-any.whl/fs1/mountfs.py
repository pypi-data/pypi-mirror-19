...
from fs1 import _thread_synchronize_default
from fs1 import iotools


class DirMount(object):
    def __init__(self, path, fs1):
        self.path = path
        self.fs1 = fs1

    def __str__(self):
        return "<DirMount %s, %s>" % (self.path, self.fs1)

    def __repr__(self):
        return "<DirMount %s, %s>" % (self.path, self.fs1)

    def __unicode__(self):
        return u"<DirMount %s, %s>" % (self.path, self.fs1)
...
class MountFS(FS):
    ...
    def _delegate(self, path):
        ...
        if type(object) is MountFS.DirMount:
            return object.fs1, head_path, tail_path
    ...
    def getsyspath(self, path, allow_none=False):
        fs1, _mount_path, delegate_path = self._delegate(path)
        if fs1 is self or fs1 is None:
            if allow_none:
                return None
            else:
                raise NoSysPathError(path=path)
        return fs1.getsyspath(delegate_path, allow_none=allow_none)
    ...
    def getpathurl(self, path, allow_none=False):
        ...
        return fs1.getpathurl(delegate_path, allow_none=allow_none)
    ...
    def desc(self, path):
        ...
        if fs1 is self:
            if fs1.isdir(path):
                ...
                return "Mounted file"
        return "Mounted dir, maps to path %s on %s" % (abspath(delegate_path) or '/', str(fs1))
    ...
    def isdir(self, path):
        ...
        return fs1.isdir(delegate_path)
    ...
    def isfile(self, path):
        ...
        return fs1.isfile(delegate_path)
    ...
    def exists(self, path):
        ...
        if fs1 is self:
            return True
        return fs1.exists(delegate_path)
    ...
    def listdir(self, path="/", wildcard=None, full=False, absolute=False, dirs_only=False, files_only=False):
        ...
        if fs1 is None:
            ...
            paths = fs1.listdir(delegate_path,
                               wildcard=wildcard,
                               full=False,
                               absolute=False,
                               dirs_only=dirs_only,
    ...
    def ilistdir(self, path="/", wildcard=None, full=False, absolute=False, dirs_only=False, files_only=False):
        ...
        if fs1 is self:
            ...
            paths = fs1.ilistdir(delegate_path,
                                wildcard=wildcard,
                                full=False,
                                absolute=False,
                                dirs_only=dirs_only)
    ...
    def makedir(self, path, recursive=False, allow_recreate=False):
        ...
        return fs1.makedir(delegate_path, recursive=recursive, allow_recreate=allow_recreate)
    ...
    def open(self, path, mode='r', buffering=-1, encoding=None, errors=None, newline=None, line_buffering=False, **kwargs):
        ...
        return fs1.open(delegate_path, mode, **kwargs)
    ...
    def setcontents(self, path, data=b'', encoding=None, errors=None, chunk_size=64*1024):
        ...
        return fs1.setcontents(delegate_path, data, encoding=encoding, errors=errors, chunk_size=chunk_size)
    ...
    def createfile(self, path, wipe=False):
        ...
        if fs1 is self or fs1 is None:
            raise ParentDirectoryMissingError(path)
        return fs1.createfile(delegate_path, wipe=wipe)
    ...
    def remove(self, path):
        ...
        return fs1.remove(delegate_path)
    ...
    def removedir(self, path, recursive=False, force=False):
        ...
        return fs1.removedir(delegate_path, recursive, force)
    ...
    @synchronize
    def mountdir(self, path, fs1):
        """Mounts a host FS object on a given path.

        :param path: A path within the MountFS
        :param fs1: A filesystem object to mount

        """
        path = abspath(normpath(path))
        self.mount_tree[path] = MountFS.DirMount(path, fs1)
    mount = mountdir
    ...
    def mountfile(self, path, open_callable=None, info_callable=None):
        ...
        :param open_callable: A callable that returns a file-like object,
            `open_callable` should have the same signature as :py:meth:`~fs1.base.FS.open`
        :param info_callable: A callable that returns a dictionary with information regarding the file-like object,
            `info_callable` should have the same signagture as :py:meth:`~fs1.base.FS.getinfo`
    ...
    def settimes(self, path, accessed_time=None, modified_time=None):
        ...
        fs1.settimes(delegate_path, accessed_time, modified_time)
    ...
    def getinfo(self, path):
        ...
        if fs1 is self:
            ...
            return {}
        return fs1.getinfo(delegate_path)
    ...
    def getsize(self, path):
        ...
        return fs1.getinfo(delegate_path).get("size", None)
    ...
    def getxattr(self,path,name,default=None):
        ...
        if fs1 is self:
            return default
        return fs1.getxattr(delegate_path,name,default)
    ...
    def setxattr(self,path,name,value):
        ...
        return fs1.setxattr(delegate_path,name,value)
    ...
    def delxattr(self,path,name):
        ...
        if fs1 is self:
            return True
        return fs1.delxattr(delegate_path, name)
    ...
    def listxattrs(self,path):
        ...
        if fs1 is self:
            return []
        return fs1.listxattrs(delegate_path)
