...
writeable fs1 with the `setwritefs` method -- which does not have to be
...
from fs1.base import FS, synchronize
...
class MultiFS(FS):
    ...
    @synchronize
    def __str__(self):
        return "<MultiFS: %s>" % ", ".join(str(fs1) for fs1 in self.fs_sequence)

    __repr__ = __str__

    @synchronize
    def __unicode__(self):
        return u"<MultiFS: %s>" % ", ".join(unicode(fs1) for fs1 in self.fs_sequence)
    ...
    @synchronize
    def addfs(self, name, fs1, write=False, priority=0):
        ...
        :param fs1: The filesystem to add
        :param write: If this value is True, then the `fs1` will be used as the writeable FS
        ...
        self.fs_sequence.append(fs1)
        self.fs_lookup[name] = fs1
    ...
    def setwritefs(self, fs1):
        ...
        :param fs1: An FS object that will be used to open writeable files
    ...
    def removefs(self, name):
        ...
        fs1 = self.fs_lookup[name]
        self.fs_sequence.remove(fs1)
    ...
    def getsyspath(self, path, allow_none=False):
        fs1 = self._delegate_search(path)
        if fs1 is not None:
            return fs1.getsyspath(path, allow_none=allow_none)
        if allow_none:
            return None
    ...
    def desc(self, path):
        ...
        name, fs1 = self.which(path)
        if name is None:
            return ""
        return "%s (in %s)" % (fs1.desc(path), name)
    ...
    def open(self, path, mode='r', buffering=-1, encoding=None, errors=None, newline=None, line_buffering=False, **kwargs):
        ...
        for fs1 in self:
            if fs1.exists(path):
                fs_file = fs1.open(path, mode=mode, buffering=buffering, encoding=encoding, errors=errors, newline=newline, line_buffering=line_buffering, **kwargs)
                return fs_file
        raise ResourceNotFoundError(path)
    ...
    def listdir(self, path="./", *args, **kwargs):
        ...
        for fs1 in self:
            try:
                paths += fs1.listdir(path, *args, **kwargs)
            except FSError:
                pass
