...
import fs1.utils
from fs1.base import threading, FS
...
from fs1.filelike import StringIO, SpooledTemporaryFile, FileWrapper
from fs1 import SEEK_SET, SEEK_CUR, SEEK_END
...
class RemoteFileBuffer(FileWrapper):
    ...
    def __init__(self, fs1, path, mode, rfile=None, write_on_flush=True):
        ...
        if getattr(fs1, "_lock", None) is not None:
            self._lock = fs1._lock.__class__()
        else:
    ...
    def _setcontents(self):
        ...
        if "w" in self.mode or "a" in self.mode or "+" in self.mode:
            ...
            self.fs1.setcontents(self.path, self.wrapped_file)
...
class ConnectionManagerFS(LazyFS):
    ...
    (class,args) or (class,args,kwds) tuple. For example::
        ...
        >>> fs1 = ConnectionManagerFS(MyRemoteFS("http://www.example.com/"))
...
class CacheFSMixin(FS):
    ...
    def open(self, path, mode='r', buffering=-1, encoding=None, errors=None, newline=None, line_buffering=False, **kwargs):
        ...
        try:
            ...
            try:
                ...
                if not fs1.utils.isdir(super(CacheFSMixin, self), ppath, pci.info):
                    raise ResourceInvalidError(path)
                if pci.has_full_children:
                    raise ResourceNotFoundError(path)
        else:
            if not fs1.utils.isfile(super(CacheFSMixin, self), path, ci.info):
                raise ResourceInvalidError(path)
    ...
    def isdir(self, path):
        ...
        try:
            ...
            return fs1.utils.isdir(super(CacheFSMixin, self), path, info)
    ...
    def isfile(self, path):
        ...
        try:
            ...
            return fs1.utils.isfile(super(CacheFSMixin, self), path, info)
