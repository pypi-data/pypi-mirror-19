...
class from the :mod:`~fs1.expose.xmlrpc` module.
...
class RPCFS(FS):
    ...
    object, and is dual to the RPCFSServer class defined in fs1.expose.xmlrpc.

    Example::

        fs1 = RPCFS("http://my.server.com/filesystem/location/")
    ...
    def listxattrs(self, path):
        path = self.encode_path(path)
        return [self.decode_path(a) for a in self.fs1.listxattrs(path)]
