...
"""
  ...
  fs1.tests.test_remote:  testcases for FS remote support utilities
...
from fs1.remote import *

from fs1 import SEEK_END
from fs1.wrapfs import WrapFS, wrap_fs_methods
...
class TestRemoteFileBuffer(unittest.TestCase, FSTestCases, ThreadingTestCases):
    ...
    def setUp(self):
        self.fs1 = RemoteTempFS()
        self.original_setcontents = self.fs1.setcontents
    ...
    def fakeOn(self):
        ...
        '''
        self.fs1.setcontents = self.fake_setcontents

    def fakeOff(self):
        ''' Switch off fake_setcontents(). '''
        self.fs1.setcontents = self.original_setcontents
    ...
    def test_writeonflush(self):
        ...
        '''
        self.fakeOn()
        f = self.fs1.open('test.txt', 'wb', write_on_flush=True)
        ...
        f = self.fs1.open('test.txt', 'wb', write_on_flush=False)
        f.write(b('Sample text'))
...
class TestCacheFS(unittest.TestCase,FSTestCases,ThreadingTestCases):
    ...
    def setUp(self):
        ...
        self.fs1 = CacheFS(self.wrapped_fs,cache_timeout=0.01)
    ...
    def test_values_are_used_from_cache(self):
        old_timeout = self.fs1.cache_timeout
        self.fs1.cache_timeout = None
        try:
            self.assertFalse(self.fs1.isfile("hello"))
            self.wrapped_fs.setcontents("hello",b("world"))
            self.assertTrue(self.fs1.isfile("hello"))
            self.wrapped_fs.remove("hello")
            self.assertTrue(self.fs1.isfile("hello"))
            self.fs1.clear_cache()
            self.assertFalse(self.fs1.isfile("hello"))
        finally:
            self.fs1.cache_timeout = old_timeout
...
class TestConnectionManagerFS(unittest.TestCase,FSTestCases):#,ThreadingTestCases):
    ...
    def setUp(self):
        self._check_interval = sys.getcheckinterval()
        sys.setcheckinterval(10)
        self.fs1 = ConnectionManagerFS(TempFS())
...
class DisconnectingFS(WrapFS):
    ...
    def __init__(self,fs1=None):
        if fs1 is None:
            fs1 = TempFS()
        self._connected = True
        self._continue = True
        self._bounce_thread = None
        super(DisconnectingFS,self).__init__(fs1)
...
class TestConnectionManagerFS_disconnect(TestConnectionManagerFS):
    ...
    def setUp(self):
        ...
        self.fs1 = DisconnectRecoveryFS(c_fs)
