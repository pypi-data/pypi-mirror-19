"""
  ...
  fs1.tests.test_s3fs:  testcases for the S3FS module
...
class TestS3FS(unittest.TestCase,FSTestCases,ThreadingTestCases):
    ...
    def setUp(self):
        self.fs1 = s3fs.S3FS(self.bucket)
        for k in self.fs1._s3bukt.list():
            self.fs1._s3bukt.delete_key(k)
...
class TestS3FS_prefix(TestS3FS):
    ...
    def setUp(self):
        self.fs1 = s3fs.S3FS(self.bucket,"/unittest/files")
        for k in self.fs1._s3bukt.list():
            self.fs1._s3bukt.delete_key(k)
