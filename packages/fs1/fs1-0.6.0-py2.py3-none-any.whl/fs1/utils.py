...
from fs1.path import pathjoin
from fs1.errors import DestinationExistsError, RemoveRootError
...
def copyfile(src_fs, src_path, dst_fs, dst_path, overwrite=True, chunk_size=64*1024):
    ...
    # If the src and dst fs1 objects are the same, then use a direct copy
    if src_fs is dst_fs:
...
def copydir_progress(progress_callback, fs1, fs2, create_destination=True, ignore_errors=False, chunk_size=64*1024):
    """
    Copies the contents of a directory from one fs1 to another, with a callback function to display progress.
...
def countbytes(fs1):
    ...
    :param fs1: A filesystem object

    """
    total = sum(fs1.getsize(f) for f in fs1.walkfiles())
    return total


def isdir(fs1,path,info=None):
...
def contains_files(fs1, path='/'):
    """Check if there are any files in the filesystem"""
    try:
        iter(fs1.walkfiles(path)).next()
    except StopIteration:
        return False
    return True

def find_duplicates(fs1,
                    compare_paths=None,
                    quick=False,
    ...
    For example, the following will list all the duplicate .jpg files in "~/Pictures"::
        ...
        >>> from fs1.utils import find_duplicates
        >>> from fs1.osfs import OSFS
        >>> fs1 = OSFS('~/Pictures')
        >>> for dups in find_duplicates(fs1, fs1.walkfiles('*.jpg')):
        ...     print list(dups)
    ...
    for path in compare_paths:
        file_sizes[fs1.getsize(path)].append(path)
...
def print_fs(fs1,
    ...
    For example, the following will print a tree of the files under the current working directory::
        ...
        >>> from fs1.osfs import *
        >>> from fs1.utils import *
        >>> fs1 = OSFS('.')
        >>> print_fs(fs1)
    ...
    def print_dir(fs1, path, levels=[]):
        ...
        try:
            dirs = fs1.listdir(path, dirs_only=True)
            if dirs_only:
                files = []
            else:
                files = fs1.listdir(path, files_only=True, wildcard=files_wildcard)
        ...
        for i, (is_dir, item) in enumerate(dir_listing):
            ...
            if is_dir:
                ...
                if max_levels is not None and len(levels) + 1 >= max_levels:
                    ...
                    print_dir(fs1, pathjoin(path, item), levels[:] + [is_last_item])
            else:
...
class AtomicWriter(object):
    ...
    def __init__(self, fs1, path, mode='w'):
        self.fs1 = fs1
        self.path = path
        self.mode = mode
        self.tmp_path = path + '~'
    ...
    def __enter__(self):
        self._f = self.fs1.open(self.tmp_path, self.mode)
        return self._f
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        if exc_type is None:
            if self._f is not None:
                ...
                self._f.close()
                self._f = None
                self.fs1.rename(self.tmp_path, self.path)
        else:
            if self._f is not None:
                self._f.close()


def open_atomic_write(fs1, path, mode='w'):
    ...
    """
    return AtomicWriter(fs1, path, mode=mode)
