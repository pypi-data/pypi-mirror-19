...
class EVENT(object):
    ...
    def __unicode__(self):
        return u"<fs1.watch.%s object (path='%s') at %s>" % (self.__class__.__name__,self.path,hex(id(self)))

    def clone(self,fs1=None,path=None):
        if fs1 is None:
            fs1 = self.fs1
        if path is None:
            path = self.path
        return self.__class__(fs1,path)
...
class MODIFIED(EVENT):
    ...
    def __init__(self,fs1,path,data_changed=False, closed=False):
        super(MODIFIED,self).__init__(fs1,path)
        self.data_changed = data_changed
        self.closed = closed

    def clone(self,fs1=None,path=None,data_changed=None):
        evt = super(MODIFIED,self).clone(fs1,path)
...
class MOVED_DST(EVENT):
    ...
    def __init__(self,fs1,path,source=None):
        super(MOVED_DST,self).__init__(fs1,path)
        if source is not None:
            source = abspath(normpath(source))
        self.source = source

    def __unicode__(self):
        return u"<fs1.watch.%s object (path=%r,src=%r) at %s>" % (self.__class__.__name__,self.path,self.source,hex(id(self)))

    def clone(self,fs1=None,path=None,source=None):
        evt = super(MOVED_DST,self).clone(fs1,path)
        if source is None:
            source = self.source
        evt.source = source
        return evt
...
class MOVED_SRC(EVENT):
    ...
    def __init__(self,fs1,path,destination=None):
        super(MOVED_SRC,self).__init__(fs1,path)
        if destination is not None:
            destination = abspath(normpath(destination))
        self.destination = destination

    def __unicode__(self):
        return u"<fs1.watch.%s object (path=%r,dst=%r) at %s>" % (self.__class__.__name__,self.path,self.destination,hex(id(self)))

    def clone(self,fs1=None,path=None,destination=None):
        evt = super(MOVED_SRC,self).clone(fs1,path)
...
class Watcher(object):
    ...
    def __init__(self,fs1,callback,path="/",events=None,recursive=True):
        if events is None:
            events = (EVENT,)
        else:
            events = tuple(events)
...
class WatchedFile(FileWrapper):
    ...
    def __init__(self,file,fs1,path,mode=None):
    ...
    def flush(self):
        ...
        if Watcher is not None:
            if self.was_modified:
                self.fs1.notify_watchers(MODIFIED,self.path,True)
...
def ensure_watchable(fs1,wrapper_class=PollingWatchableFS,*args,**kwds):
    """Ensure that the given fs1 supports watching, simulating it if necessary.
    ...
    """
    if isinstance(fs1,wrapper_class):
        return fs1
    try:
        w = fs1.add_watcher(lambda e: None,"/",recursive=False)
    except (AttributeError,FSError):
        return wrapper_class(fs1,*args,**kwds)
    else:
        fs1.del_watcher(w)
    return fs1
...
class iter_changes(object):
    ...
    def __init__(self,fs1=None,path="/",events=None,**kwds):
        self.closed = False
        self._queue = Queue.Queue()
        self._watching = set()
        if fs1 is not None:
            self.add_watcher(fs1,path,events,**kwds)
    ...
    def next(self,timeout=None):
        ...
        if isinstance(event,CLOSED):
            event.fs1.del_watcher(self._enqueue)
            self._watching.remove(event.fs1)
        return event

    def close(self):
        if not self.closed:
            self.closed = True
            for fs1 in self._watching:
                fs1.del_watcher(self._enqueue)
            self._queue.put(None)

    def add_watcher(self,fs1,path="/",events=None,**kwds):
        w = fs1.add_watcher(self._enqueue,path,events,**kwds)
        self._watching.add(fs1)
        return w
