'''
    ...
    Basic usage:
        fs1 = DebugFS(OSFS('~'), identifier='OSFS@home', \
                skip=('_lock', 'listdir', 'listdirinfo'))
        print fs1.listdir('.')
        print fs1.unsupportedfunction()
    ...
    Error levels:
        ...
        CRITICAL: Print only exceptions not derived from fs1.errors.FSError

    How to change error level:
        import logging
        logger = logging.getLogger('fs1.debugfs')
...
logger = fs1.getLogger('fs1.debugfs')
logger.setLevel(logging.DEBUG)
logger.addHandler(logging.StreamHandler())

class DebugFS(object):
    def __init__(self, fs1, identifier=None, skip=(), verbose=True):
        '''
            fs1 - Reference to object to debug
