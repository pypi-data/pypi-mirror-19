...
class _TempWriteFile(object):
    ...
    def __init__(self, fs1, filename, close_callback):
        self.fs1 = fs1
        self.filename = filename
        self._file = self.fs1.open(filename, 'wb+')
...
class ZipFS(FS):
    ...
    def __init__(self, zip_file, mode="r", compression="deflated", allow_zip_64=False, encoding="CP437", thread_synchronize=True):
        ...
        :raises `fs1.errors.ZipOpenError`: thrown if the zip file could not be opened
        :raises `fs1.errors.ZipNotFoundError`: thrown if the zip file does not exist (derived from ZipOpenError)
