...
# Store some identifiers in the fs1 namespace
import os
...
class _NullHandler(_logging.Handler):
    def emit(self,record):
        pass
_logging.getLogger("fs1").addHandler(_NullHandler())
def getLogger(name):
    """Get a logger object for use within the pyfilesystem library."""
    assert name.startswith("fs1.")
