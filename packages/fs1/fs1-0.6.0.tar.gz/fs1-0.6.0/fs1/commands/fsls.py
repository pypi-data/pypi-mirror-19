...
from fs1.path import pathsplit, abspath, isdotfile, iswildcard
...
List contents of [PATH]"""
    ...
    def do_run(self, options, args):
        ...
        for fs_url in args:
            ...
            if path != '.' and fs1.isfile(path):
                if not options.dirsonly:
                    file_paths.append(path)
            else:
                if not options.filesonly:
                    dir_paths += fs1.listdir(path,
                                            wildcard=wildcard,
        ...
        if options.syspath:
            ...
            dir_paths = filter(None, [fs1.getsyspath(path, allow_none=True) for path in dir_paths])
            file_paths = filter(None, [fs1.getsyspath(path, allow_none=True) for path in file_paths])

        if options.url:
            # Path without a syspath, just won't be displayed
            dir_paths = filter(None, [fs1.getpathurl(path, allow_none=True) for path in dir_paths])
            file_paths = filter(None, [fs1.getpathurl(path, allow_none=True) for path in file_paths])
