...
Mounts a file system on a system path"""
    ...
    def do_run(self, options, args):
        ...
        fs1, path = self.open_fs(fs_url, create_dir=True)
        if path:
            if not fs1.isdir(path):
                self.error('%s is not a directory on %s' % (fs_url, fs1))
                return 1
