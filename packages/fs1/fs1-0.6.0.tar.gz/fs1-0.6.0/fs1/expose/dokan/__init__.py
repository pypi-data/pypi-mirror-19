...
and exposes the given FS as that drive::
    ...
    >>> from fs1.expose import dokan
    >>> fs1 = MemoryFS()
    >>> mp = dokan.mount(fs1,"Q")
    >>> mp.drive
    'Q'
    >>> mp.path
    'Q:\\'
...
spawning a new process, set the 'foreground' option::
    ...
    >>> dokan.mount(fs1,"Q",foreground=True)
...
to subprocess.Popen::
    ...
    >>> mp = dokan.MountProcess(fs1,"Q",stderr=PIPE)
...
from fs1.base import threading
...
try:
    ...
    is_available = False
    sys.modules.pop("fs1.expose.dokan.libdokan", None)
    libdokan = None
else:
    is_available = True
...
import logging
logger = logging.getLogger("fs1.expose.dokan")
...
class FSOperations(object):
    ...
    def __init__(self, fs1, fsname="Dokan FS", volname="Dokan Volume"):
        if libdokan is None:
    ...
    def ReadFile(self, path, buffer, nBytesToRead, nBytesRead, offset, info):
        ...
        try:
            ...
            if file.closed:
                file = self.fs1.open(path, file.mode)
    ...
    def FindFiles(self, path, fillFindData, info):
        path = normpath(path)
        for (nm, finfo) in self.fs1.listdirinfo(path):
    ...
    def SetFileTime(self, path, ctime, atime, mtime, info):
        ...
        try:
            self.fs1.settimes(path, atime, mtime)
    ...
    def DeleteDirectory(self, path, info):
        path = normpath(path)
        for nm in self.fs1.listdir(path):
    ...
    def MoveFile(self, src, dst, overwrite, info):
        ...
        dst = normpath(dst)
        if info.contents.IsDirectory:
            self.fs1.movedir(src, dst, overwrite=overwrite)
        else:
            self.fs1.move(src, dst, overwrite=overwrite)
    ...
    def GetDiskFreeSpaceEx(self, nBytesAvail, nBytesTotal, nBytesFree, info):
        ...
        nBytesFree[0] = self.fs1.getmeta("free_space", large_amount)
        nBytesTotal[0] = self.fs1.getmeta("total_space", 2 * large_amount)
...
def mount(fs1, drive, foreground=False, ready_callback=None, unmount_callback=None, **kwds):
    ...
    if foreground:
        ...
        ops = FSOperationsClass(fs1, **kwds)
        if ready_callback:
    ...
    #  to be ready before returning.
        ...
        mp = MountProcess(fs1, drive, kwds)
        check_ready(mp)
        if unmount_callback:
...
class MountProcess(subprocess.Popen):
    ...
    def __init__(self, fs1, drive, dokan_opts={}, nowait=False, **kwds):
        if libdokan is None:
            raise OSError("the dokan library is not available")
        ...
        cmd = cmd + "from fs1.expose.dokan import MountProcess; "
        cmd = cmd + "MountProcess._do_mount(data)"
        cmd = cmd % (repr(cPickle.dumps((fs1,drive,dokan_opts,nowait),-1)),)
...
if __name__ == "__main__":
    ...
    try:
        fs1 = OSFS(path)
        #fs1 = MemoryFS()
        fs1.setcontents("test1.txt",b("test one"))
        flags = DOKAN_OPTION_DEBUG|DOKAN_OPTION_STDERR|DOKAN_OPTION_REMOVABLE
        mount(fs1, "Q", foreground=True, numthreads=1, flags=flags)
        fs1.close()
    finally:
        rmtree(path)
