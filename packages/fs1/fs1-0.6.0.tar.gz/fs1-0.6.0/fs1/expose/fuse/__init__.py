...
and exposes the given FS at that path::
    ...
    >>> from fs1.expose import fuse
    >>> fs1 = MemoryFS()
    >>> mp = fuse.mount(fs1,"/mnt/my-memory-fs1")
    >>> mp.path
    '/mnt/my-memory-fs1'
    >>> mp.unmount()
...
spawning a new process, set the 'foreground' option::
    ...
    >>> fuse.mount(fs1,"/mnt/my-memory-fs1",foreground=True)
...
to subprocess.Popen::
    ...
    >>> mp = fuse.MountProcess(fs1,"/mnt/my-memory-fs1",stderr=PIPE)
...
import logging
logger = logging.getLogger("fs1.expose.fuse")

from fs1.base import flags_to_mode, threading
...
try:
    if PY3:
        from fs1.expose.fuse import fuse_ctypes as fuse
    else:
        from fs1.expose.fuse import fuse3 as fuse
...
class FSOperations(Operations):
    ...
    def __init__(self, fs1, on_init=None, on_destroy=None):
        self.fs1 = fs1
    ...
    def create(self, path, mode, fi):
        ...
        fh = self._reg_file(self.fs1.open(path, mode), path)
        fi.fh = fh
        fi.keep_cache = 0
    ...
    def getxattr(self, path, name, position=0):
        ...
        try:
            value = self.fs1.getxattr(path, name)
        except AttributeError:
    ...
    def listxattr(self, path):
        path = path.decode(NATIVE_ENCODING)
        try:
            return self.fs1.listxattrs(path)
        except AttributeError:
    ...
    def mkdir(self, path, mode):
        path = path.decode(NATIVE_ENCODING)
        try:
            self.fs1.makedir(path, recursive=True)
        except TypeError:
            self.fs1.makedir(path)
    ...
    def open(self, path, fi):
        path = path.decode(NATIVE_ENCODING)
        mode = flags_to_mode(fi.flags)
        fi.fh = self._reg_file(self.fs1.open(path, mode), path)
        fi.keep_cache = 0
        return 0
    ...
    def readdir(self, path, fh=None):
        path = path.decode(NATIVE_ENCODING)
        entries = ['.', '..']
        for (nm, info) in self.fs1.listdirinfo(path):
    ...
    def removexattr(self, path, name):
        ...
        name = name.decode(NATIVE_ENCODING)
        try:
            return self.fs1.delxattr(path, name)
        except AttributeError:
    ...
    def rename(self, old, new):
        ...
        try:
            self.fs1.rename(old, new)
        except FSError:
            if self.fs1.isdir(old):
                self.fs1.movedir(old, new)
            else:
                self.fs1.move(old, new)

    @handle_fs_errors
    def rmdir(self, path):
        path = path.decode(NATIVE_ENCODING)
        self.fs1.removedir(path)
    ...
    def setxattr(self, path, name, value, options, position=0):
        ...
        name = name.decode(NATIVE_ENCODING)
        try:
            return self.fs1.setxattr(path, name, value)
        except AttributeError:
    ...
    def truncate(self, path, length, fh=None):
        ...
        if fh is None and length == 0:
            self.fs1.open(path, "wb").close()
        else:
            if fh is None:
                f = self.fs1.open(path, "rb+")
    ...
    def utimens(self, path, times=None):
        ...
        self.fs1.settimes(path, accessed_time, modified_time)
...
def mount(fs1, path, foreground=False, ready_callback=None, unmount_callback=None, **kwds):
    ...
    if foreground:
        op = FSOperations(fs1, on_init=ready_callback, on_destroy=unmount_callback)
        return FUSE(op, path, raw_fi=True, foreground=foreground, **kwds)
    else:
        mp = MountProcess(fs1, path, kwds)
        if ready_callback:
            ready_callback()
        if unmount_callback:
...
class MountProcess(subprocess.Popen):
    ...
    #  pickled (fs1,path,opts) tuple on the command-line.  Something like this:
    #
    ...
    def __init__(self, fs1, path, fuse_opts={}, nowait=False, **kwds):
        self.path = path
        if nowait or kwds.get("close_fds", False):
            ...
            cmd = cmd + 'from fs1.expose.fuse import MountProcess; '
            cmd = cmd + 'MountProcess._do_mount_nowait(data)'
            cmd = cmd % (repr(cPickle.dumps((fs1, path, fuse_opts), -1)),)
            ...
            cmd = cmd % (repr(cPickle.dumps((fs1, path, fuse_opts, r, w), -1)),)
            cmd = [sys.executable, "-c", cmd]
            super(MountProcess, self).__init__(cmd, **kwds)
            os.close(w)
...
if __name__ == "__main__":
    ...
    mount_point = os.path.join(os.environ["HOME"], "fs1.expose.fuse")
