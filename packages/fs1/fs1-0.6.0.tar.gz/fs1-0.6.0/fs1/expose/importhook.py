...
sys.meta_path, you will be able to import modules from the exposed filesystem::
    ...
    >>>
    >>> import sys
    >>> from fs1.expose.importhook import FSImportHook
...
from fs1.opener import fsopendir, OpenerError
...
class FSImportHook(object):
    ...
    URL, which is automatically opened using fs1.opener.
    """
    ...
    def get_code(self,fullname,info=None):
        ...
        if info is None:
            info = self._get_module_info(fullname)
        (path,type,ispkg) = info
        code = self.fs1.getcontents(path, 'rb')
    ...
    def get_source(self,fullname,info=None):
        ...
        if type != imp.PY_SOURCE:
            return None
        return self.fs1.getcontents(path, 'rb').replace(b("\r\n"),b("\n"))

    def get_data(self,path):
        """Read the specified data file."""
        try:
            return self.fs1.getcontents(path, 'rb')
        except FSError, e:
            raise IOError(str(e))
