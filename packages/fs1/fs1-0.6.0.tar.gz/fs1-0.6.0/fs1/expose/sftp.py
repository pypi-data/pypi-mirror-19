...
class based on the standard SocketServer module.  Use it like so::
    ...
    server = BaseSFTPServer((hostname,port),fs1)
...
class SFTPServerInterface(paramiko.SFTPServerInterface):
    ...
    the fs1 object to be exposed.  Use it to set up a transport subsystem
    handler like so::

      t.set_subsystem_handler("sftp",SFTPServer,SFTPServerInterface,fs1)
    ...
    def __init__(self, server, fs1, encoding=None, *args, **kwds):
        self.fs1 = fs1
        if encoding is None:
            encoding = "utf8"
    ...
    def list_folder(self, path):
        ...
        stats = []
        for entry in self.fs1.listdir(path, absolute=True):
    ...
    def rename(self, oldpath, newpath):
        ...
        if self.fs1.isfile(oldpath):
            self.fs1.move(oldpath, newpath)
        else:
            self.fs1.movedir(oldpath, newpath)
        return paramiko.SFTP_OK
    ...
    def chattr(self, path, attr):
        ...
        if attr._flags:
            ...
            with self.fs1.open(path,"r+") as f:
...
class SFTPHandle(paramiko.SFTPHandle):
    ...
    def __init__(self, owner, path, flags):
        ...
        self.path = path
        self._file = owner.fs1.open(path, mode)
...
class SFTPRequestHandler(SocketServer.BaseRequestHandler):
    ...
    def setup(self):
        ...
        self.transport.set_subsystem_handler("sftp", SFTPServer, SFTPServerInterface, self.server.fs1, encoding=self.server.encoding)
...
class BaseSFTPServer(ThreadedTCPServer):
    ...
    def __init__(self, address, fs1=None, encoding=None, host_key=None, RequestHandlerClass=None):
        self.fs1 = fs1
        self.encoding = encoding
        if host_key is None:
