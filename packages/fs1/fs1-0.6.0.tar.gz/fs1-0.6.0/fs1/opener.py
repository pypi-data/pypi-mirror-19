...
object a path. Here's an example of how to use the default opener registry::
    ...
    (<fs1.ftpfs.FTPFS object at 0x96e66ec>, u'pub')
...
the `open` builtin::
    ...
    <fs1.ftpfs._FTPFile object at 0x973764c>
...
them::
    ...
    from fs1.opener import fsopendir, fsopen
...
from fs1.path import pathsplit, join, iswildcard, normpath
from fs1.osfs import OSFS
from fs1.filelike import FileWrapper
...
def _FSClosingFile(fs1, file_object, mode):
...
''', re.VERBOSE)
    ...
    def add(self, opener):
        ...
        :param opener: a class derived from fs1.opener.Opener
    ...
    def parse(self, fs_url, default_fs_name=None, writeable=False, create_dir=False, cache_hint=True):
        """Parses a FS url and returns an fs1 object a path within that FS object
        ...
        fs1, fs_path = opener.get_fs(self, fs_name, fs_name_params, fs_url, writeable, create_dir)
        fs1.cache_hint(cache_hint)

        if fs_path and iswildcard(fs_path):
            pathname, resourcename = pathsplit(fs_path or '')
            if pathname:
                fs1 = fs1.opendir(pathname)
            return fs1, resourcename

        fs_path = join(fs_path, path)

        if create_dir and fs_path:
            if not fs1.getmeta('read_only', False):
                fs1.makedir(fs_path, allow_recreate=True)
    ...
    def open(self, fs_url, mode='r', **kwargs):
        ...
        fs1, path = self.parse(fs_url, writeable=writeable)
        file_object = fs1.open(path, mode)

        file_object = _FSClosingFile(fs1, file_object, mode)
        #file_object.fs1 = fs1
        return file_object
    ...
    def getcontents(self, fs_url, mode='rb', encoding=None, errors=None, newline=None):
        ...
        """
        fs1, path = self.parse(fs_url)
        return fs1.getcontents(path, mode, encoding=encoding, errors=errors, newline=newline)
    ...
    def opendir(self, fs_url, writeable=True, create_dir=False):
        ...
        """
        fs1, path = self.parse(fs_url, writeable=writeable, create_dir=create_dir)
        if path and '://' not in fs_url:
            # A shortcut to return an OSFS rather than a SubFS for os paths
            return OSFS(fs_url)
        if path:
            fs1 = fs1.opendir(path)
        return fs1
    ...
    * foo/bar.baz (file:// is the default opener)"""
    ...
    def get_fs(cls, registry, fs_name, fs_name_params, fs_path, writeable, create_dir):
        ...
        if create_dir and not os.path.exists(path):
            from fs1.osfs import _os_makedirs
            _os_makedirs(path)
...
example:
    * debug:ftp://ftp.mozilla.org (displays details of calls made to a ftp filesystem)"""
    ...
    @classmethod
    def get_fs(cls, registry, fs_name, fs_name_params, fs_path,  writeable, create_dir):
        from fs1.wrapfs.debugfs import DebugFS
        if fs_path:
            fs1, _path = registry.parse(fs_path, writeable=writeable, create_dir=create_dir)
            return DebugFS(fs1, verbose=False), None
...
* temp://"""
    ...
    def get_fs(cls, registry, fs_name, fs_name_params, fs_path,  writeable, create_dir):
        ...
        fs1 = LazyFS((TempFS,(),{"identifier":fs_name_params}))
        return fs1, fs_path
...
class TahoeOpener(Opener):
    ...
    * tahoe://http://pubgrid.tahoe-lafs.org/uri/URI:DIR2:h5bkxelehowscijdb [...]"""
    ...
    @classmethod
    def get_fs(cls, registry, fs_name, fs_name_params, fs_path, writeable, create_dir):
        from fs1.contrib.tahoelafs import TahoeLAFS
        ...
        fs1 = TahoeLAFS(dircap, webapi=url)

        if '/' in path:
            dirname, _resourcename = pathsplit(path)
            if create_dir:
                fs1 = fs1.makeopendir(dirname)
            else:
                fs1 = fs1.opendir(dirname)
            path = ''
...
* dav://example.org/dav"""
    ...
    @classmethod
    def get_fs(cls, registry, fs_name, fs_name_params, fs_path, writeable, create_dir):
        from fs1.contrib.davfs import DAVFS
        ...
        fs1 = DAVFS(url, credentials=credentials)
...
* http://www.example.org/index.html"""
    ...
    @classmethod
    def get_fs(cls, registry, fs_name, fs_name_params, fs_path, writeable, create_dir):
        from fs1.httpfs import HTTPFS
        if '/' in fs_path:
            dirname, resourcename = fs_path.rsplit('/', 1)
        else:
            dirname = fs_path
            resourcename = ''
        fs1 = HTTPFS('http://' + dirname)
        return fs1, resourcename
...
* appuserdata://examplesoft:anotherapp.1.3"""
    ...
    def get_fs(cls, registry, fs_name, fs_name_params, fs_path, writeable, create_dir):
        import fs1.appdirfs
        fs_class = getattr(fs1.appdirfs, cls.FSClass)
        if ':' in fs_path:
        ...
        fs1 = fs_class(appname, appauthor=appauthor, version=appversion, create=create_dir)

        if '/' in path:
            subdir, path = path.rsplit('/', 1)
            if create_dir:
                fs1 = fs1.makeopendir(subdir, recursive=True)
            else:
                fs1 = fs1.opendir(subdir)
...
example:
* mount://fs1.ini
* mount://fs1.ini!resources
* mount://fs1.ini:fs2"""

    @classmethod
    def get_fs(cls, registry, fs_name, fs_name_params, fs_path, writeable, create_dir):

        from fs1.mountfs import MountFS
