...
def normpath(path):
    ...
    try:
        ...
        from fs1.errors import BackReferenceError
...
def join(*paths):
    ...
    used as ``fs1.path.join`` in direct correspondence with ``os.path.join``.
...
def split(path):
    ...
    used as ``fs1.path.split`` in direct correspondence with ``os.path.split``.
