...
class TestFSImportHook(unittest.TestCase):
    ...
    def _init_modules(self,fs1):
        fs1.setcontents("fsih_hello.py",b(dedent("""
            message = 'hello world!'
        """)))
        fs1.makedir("fsih_pkg")
        fs1.setcontents("fsih_pkg/__init__.py",b(dedent("""
            a = 42
        """)))
        fs1.setcontents("fsih_pkg/sub1.py",b(dedent("""
            import fsih_pkg
            from fsih_hello import message
            a = fsih_pkg.a
        """)))
        fs1.setcontents("fsih_pkg/sub2.pyc",self._getpyc(b(dedent("""
            import fsih_pkg
            from fsih_hello import message
            a = fsih_pkg.a * 2
        """))))
