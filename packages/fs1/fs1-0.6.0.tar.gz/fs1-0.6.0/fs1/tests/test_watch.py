"""
  ...
  fs1.tests.test_watch:  testcases for change watcher support
...
try:
    from fs1.osfs import watch_inotify
except ImportError:
    watch_inotify = None

if sys.platform == "win32":
    try:
        from fs1.osfs import watch_win32
    except ImportError:
        watch_win32 = None
else:
    watch_win32 = None
...
class WatcherTestCases:
    ...
    def test_watch_readfile(self):
        self.setupWatchers()
        self.fs1.setcontents("hello", b("hello world"))
        self.assertEventOccurred(CREATED,"/hello")
        self.clearCapturedEvents()
        old_atime = self.fs1.getinfo("hello").get("accessed_time")
        self.assertEquals(self.fs1.getcontents("hello"), b("hello world"))
        if not isinstance(self.watchfs,PollingWatchableFS):
            #  Help it along by updting the atime.
            #  TODO: why is this necessary?
            if self.fs1.hassyspath("hello"):
                syspath = self.fs1.getsyspath("hello")
                mtime = os.stat(syspath).st_mtime
                atime = int(time.time())
                os.utime(self.fs1.getsyspath("hello"),(atime,mtime))
            ...
            for i in xrange(10):
                if self.fs1.getinfo("hello").get("accessed_time") != old_atime:
    ...
    def test_watch_writefile(self):
        ...
        self.assertEventOccurred(CREATED,"/hello")
        self.clearCapturedEvents()
        self.fs1.setcontents("hello", b("hello again world"))
    ...
    def test_watch_single_file_remove(self):
        self.fs1.makedir("testing")
        self.fs1.setcontents("testing/hello", b("hello world"))
        events = []
        self.watchfs.add_watcher(events.append,"/testing/hello",(REMOVED,))
        self.fs1.setcontents("testing/hello", b("hello again world"))
        self.waitForEvents()
        self.fs1.remove("testing/hello")
        self.waitForEvents()
    ...
    def test_watch_iter_changes(self):
        ...
        self.fs1.makedir("test1")
        self.fs1.setcontents("test1/hello", b("hello world"))
        self.waitForEvents()
        self.fs1.removedir("test1",force=True)
        self.waitForEvents()
        self.watchfs.close()
...
class TestWatchers_MemoryFS(unittest.TestCase,FSTestCases,WatcherTestCases):
    ...
    def setUp(self):
        self.fs1 = self.watchfs = WatchableFS(memoryfs.MemoryFS())
...
class TestWatchers_MemoryFS_polling(TestWatchers_MemoryFS):
    ...
    def setUp(self):
        ...
        self.watchfs = ensure_watchable(self.fs1,poll_interval=0.1)
