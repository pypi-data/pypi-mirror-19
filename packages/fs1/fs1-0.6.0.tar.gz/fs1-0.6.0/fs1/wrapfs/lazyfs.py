...
try:
    ...
    from fs1.base import DummyLock as Lock
...
class LazyFS(WrapFS):
    ...
    def __init__(self, fs1):
        super(LazyFS, self).__init__(fs1)
        ...
        try:
            ...
            try:
                try:
                    ...
                    fs1 = self._fsclass(*self._fsargs,**self._fskwds)
                    self.__dict__["wrapped_fs"] = fs1
            finally:
                self._lazy_creation_lock.release()
        return fs1

    def _set_wrapped_fs(self, fs1):
        if isinstance(fs1,FS):
            self.__dict__["wrapped_fs"] = fs1
        elif isinstance(fs1,type):
            self._fsclass = fs1
            self._fsargs = []
            self._fskwds = {}
        elif fs1 is None:
