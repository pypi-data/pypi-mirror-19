...
from fs1.base import FS, threading, synchronize
...
class LimitSizeFS(WrapFS):
    ...
    def __init__(self, fs1, max_size):
        super(LimitSizeFS,self).__init__(fs1)
        if max_size < 0:
            try:
                max_size = fs1.getmeta("total_space") + max_size
            except NoMetaError:
...
class LimitSizeFile(FileWrapper):
    ...
    def __init__(self, file, mode, size, fs1, path):
        super(LimitSizeFile,self).__init__(file,mode)
        self.size = size
        self.fs1 = fs1
        self.path = path
        self._lock = fs1._lock

    @synchronize
    def _write(self, data, flushing=False):
        pos = self.wrapped_file.tell()
        new_size = self.fs1._ensure_file_size(self.path, pos+len(data))
    ...
    def _truncate(self, size):
        new_size = self.fs1._ensure_file_size(self.path,size,shrink=True)
    ...
    def close(self):
        ...
        self.fs1._file_closed(self.path)
