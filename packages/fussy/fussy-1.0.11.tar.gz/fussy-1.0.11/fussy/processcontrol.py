...
def alive( pid ):
    ...
    try:
        ...
        if err.args[0] == errno.ESRCH: # no such process
            ...
            log.warn( "Attempted to send signal to process we cannot signal" )
            return False 
...
def proc_map():
    ...
    procs = [
        x for x in os.listdir('/proc') if x.isdigit()
    ]
    children = {}
    parent = {}
    for proc in procs:
...
def describe( pid ):
    ...
    try:
        return nbio.Process( ['ps','f','--pid', str(pid)])().decode('utf-8').strip().splitlines()[1]
            ...
            for sig in [signal.SIGINT,signal.SIGTERM,signal.SIGKILL]:
                try:
                    ...
                    if err.args[0] == errno.ESRCH:
                        ...
                        log.info(
                            "Kill returned unexpected failure: %s", err 
                        )
                wait( 0.1* (2**attempt) )
                if not alive( pid ):
                    return True 
