...
def string_to_hex(string):
    return safe_hexlify(from_string_to_bytes(string))


def num_to_hex(num, min_bytes=0):
    """provides little endian hex representation of a number"""
    return safe_hexlify(encode(num, 256, min_bytes)[::-1])


def encode_license(license):
    """ Encode license to hex format.

    This encoder only uses fields: `name`, `description`, `issuer`,
    `fee_collector`, `metadata_link`, 'member_control`, `upper_limit`. All the other
    ...
    def string_field_to_hex(license_field):
        result = num_to_hex(len(from_string_to_bytes(license_field)))
    ...
    def is_license_valid(license):
        ...
        MAX_UPPER_LIMIT = 10**10
        MIN_UPPER_LIMIT = 0
        
        if len(from_string_to_bytes(license['name'])) > MAX_NAME_LEN:
            return False
        elif len(from_string_to_bytes(license['description'])) > MAX_DESCRIPTION_LEN:
            return False
        elif (license['upper_limit'] > MAX_UPPER_LIMIT
              or license['upper_limit'] < MIN_UPPER_LIMIT):
            return False
    ...
    result = '72'  # hardcode initial value
    result += '110110'  # hardcode version
    result += string_field_to_hex(license['name'])
    result += string_field_to_hex(license['description'])
    result += string_field_to_hex(license['issuer'])
    result += '01'  # hardcode divisibilty to 'true'
    result += '00000000'  # hardcode fee type to 'fixed'
    result += '0000000000000000'  # hardcode fee rate
    result += string_field_to_hex(license['fee_collector'])
    result += num_to_hex(license['upper_limit'], 8)
    result += '00000000'  # hardcode mint schedule to "free"
    result += '01' if license['member_control'] else '00'  # hardcode member control to 'true'
    result += string_field_to_hex(license['metadata_link'])
