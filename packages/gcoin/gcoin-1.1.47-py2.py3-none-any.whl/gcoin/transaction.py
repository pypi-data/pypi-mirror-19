...
def serialize(txobj):
    ...
    for inp in txobj["ins"]:
        ...
        o.append(num_to_var_int(len(inp["script"])) +
...
def mk_op_return_script(message):
    message_len = len(from_string_to_bytes(message))
    if message_len > MAX_SCRIPT_ELEMENT_SIZE:
        raise Exception("op_return message too long.")

    script = num_to_hex(OP_RETURN)
    script += script_push_data(message)
    return script


def mk_pubkeyhash_script(addr):
    # Keep the auxiliary functions around for altcoins' sake
    return '76a914' + b58check_to_hex(addr) + '88ac'


def mk_pubkey_script(pubkey):
    return '21' + encode_pubkey(pubkey, 'hex_compressed') + 'ac'
...
def address_to_script(addr):
    if addr[0] == '3' or addr[0] == '2':
        ...
        return mk_pubkeyhash_script(addr)
...
def make_raw_tx(ins, outs, tx_type=TX_TYPE['NORMAL']):
    ...
    output: {
        'script':'', (optional)
        'address': '', (required if 'script' is not given, else optional)
        'value': 0,
        'color': 0
    }
    ...
    """
    tx_obj = {'locktime': 0, 'version': 1, 'ins': [], 'outs': [], 'type': tx_type}
    for i in ins:
        tx_obj['ins'].append({
            'outpoint': {
                'hash': i['tx_id'],
    ...
    for o in outs:
        out_obj = {'script': o.get('script') or address_to_script(o['address']),
                   'value': o['value'],
                   'color': o['color']}
...
def make_mint_raw_tx(pubkey, color, value):
    """Make raw transaction for minting asset.

    :param pubkey: The pubkey to mint.
    ...
    ins = [{
        'tx_id': '0' * 64,
        'index': 0xffffffff,
        'script': mk_pubkey_script(pubkey),
    }]
    ...
    return make_raw_tx(ins, outs, TX_TYPE['MINT'])
