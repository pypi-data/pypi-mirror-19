...
def can_save_txt(app):
	...
	ret, val = QInputDialog.getItem(None, "Export Data", "Which data group would you like to export?", [str(i) for i in data.keys()], editable=False)
	if not val:
		return False
	to_save = to_df(data[ret])
	return True

def save_txt(application, path):
...
def getDistances(points, other=[], mode='all_pairs'):
	
	if len(other) > len(points):
		points, other = other, points
	...
	for i, p1 in enumerate(points):
		if len(others) == 0:
			if mode == 'all_pairs':
				...
				others = points
		p_dists = [np.linalg.norm(np.subtract(p1, p2)) for p2 in others]

		if mode == 'nearest':
			dists.append(min(p_dists))
		else:
			dists.extend(p_dists)
	return dists
...
class SynapseWidget(QtGui.QWidget):
	...
	def pipeline(self, preview=False):
		...
		if not preview:
			...
			modes = ['no_duplicates', 'nearest']
			mode = modes[self.allMode.currentIndex()]
			...
			if len(centroids) == 1:
				self.distances_viewer = self.plotDistances(centroids[0], zc, mode, simulate=sim)
			elif len(centroids) == 2:
				self.distances_viewer = self.plotDistances(centroids[0], zc, mode, data2=centroids[1], simulate=sim)
	...
	def plotDistances(self, data, zc, mode, data2=[], simulate=False):
		cols = ['Xc', 'Yc', 'Zc'] if zc else ['Xc', 'Yc']
		pts1 = data[cols].values
		if len(data2) > 0:
			pts2 = data2[cols].values
			distances = getDistances(pts1, other=pts2, mode=mode)
		else:
			distances = getDistances(pts1, mode=mode)
...
def show_gui(session, data):
	#viewer = session.application.new_data_viewer(VispyScatterViewer, data[0])
	global cluster_ui
