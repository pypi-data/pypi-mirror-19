...
# ConfigOptionName_ChConversionFactor = "ConversionFactor"
# ConfigOptionName_ChOffset = "ChannelOffset"
...
def CheckForUpdates():
    ...
    try:
        with urllib.request.urlopen(UpdateFileLink) as f:
...
class AcqusitionWindow(QWidget):
    ...
    def __init__(self, parent = None):
        ...
        #The next field is: How many batches to process before updating the plot
        #a very small number like "1" would make the refresh rate of the plot too high and the program will become slow
        # and unable to catch up with the data pulled from the device
        self.skipPlotEvery = 5
        #initial counter value
        self.skipPlotCounter = 0 #do not change this
    ...
    def processDataFromThread(self,data,dataCollected):
        ...
        if not self.plotLocked:
            ...
            # Skip plotting every self.skipPlotEvery
            if self.skipPlotCounter >= self.skipPlotEvery:
                self.dataAnalysisWidget.doPlotData()
                self.skipPlotCounter = 0
