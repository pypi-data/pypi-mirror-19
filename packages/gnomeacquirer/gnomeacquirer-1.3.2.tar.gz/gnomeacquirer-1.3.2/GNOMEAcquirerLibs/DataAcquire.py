...
def fillQueue_process(devSpecs, acquisitionSpecs, shared_stopped, shared_return_list, shared_lock):
    ...
    try:
        ...
        print("Error while trying to open instrument: " + devSpecs["DevName"])
        ...
        while not shared_stopped[0]:
            ...
            while len(acqBuffer) < acquisitionSpecs["AcqMaxBufferLines"] and (not shared_stopped[0]):
                try:
                    ...
                    print("Error was thrown while pulling data. The error says: " + str(e))
    ...
    try:
        ...
        print("Unable to close device " + devSpecs["DevName"] + ". Exception says: " + str(e))
...
class DataAcquirer(QObject):
    ...
    def readLineFake(self):
        ...
        if self.fakeReaderCounter == -1:
            ...
            self.fakeRetVal = "Date: " + self.fakeCurrentTime.strftime(DataBatch.f_dateFormat)
        elif self.fakeReaderCounter == 1:
            self.fakeRetVal = "Time: " + self.fakeCurrentTime.strftime(DataBatch.f_timeFormat)
        elif self.fakeReaderCounter == 2:
            self.fakeRetVal = "Week number: 1024"
        elif self.fakeReaderCounter == 3:
            self.fakeRetVal = "Time of week: 12345"
            ...
            self.fakeRetVal = "Receiver mode: 7"
            ...
            self.fakeRetVal = "Decoding status: 0x01 No GPS time"
        elif self.fakeReaderCounter == 10:
            self.fakeRetVal = "Temperature internal [C]: 12.34"
        elif self.fakeReaderCounter == 11:
            self.fakeRetVal = "Temperature external [C]: --"
        elif self.fakeReaderCounter == 12:
            self.fakeRetVal = "Latitude [deg]: 49.9905505"
        elif self.fakeReaderCounter == 13:
            self.fakeRetVal = "Longitude [deg]: 8.2345987"
        elif self.fakeReaderCounter == 14:
            self.fakeRetVal = "Altitude [m]: 198.62"
    ...
    def run(self):
        ...
        self.fakeChannelStates = "Ch1 +/-1.25 [V], Ch2  +/-2.5 [V], Ch3 +/-5 [V], Ch4 +/-10 [V]"
