...
class GotchaTwitter:
    ...
    def parse(self, item, max_page):
        parsers = \
            {'timeline':
                 parse_user_timeline(item, self._connector, self._output_header, max_page, **self._parser_conf)}
        return parsers.get(self._job)

    def crawl(self, max_page=float('inf')):
        with tqdm(self._input) as _inputs, \
            ...
            for _input in _inputs:
                ...
                try:
                    parsed_items = self.parse(_input, max_page)
