...
# TODO JP - need to put dict decodes inside try-except and return an appropriate error message if elasticsearch responce is not as expected. Currently error message is unhelpful.


class Esearch(object):
    def __init__(self, host, default, control, token="",
...
# query functions
    ...
    def getDocuments(self, type, **kwargs):
    ...
    def getAssociateList(self, index, aType):
    ...
    def getArticles(self, **kwargs):
        r = self.getDocuments("article", **kwargs)
        return r["hits"]

    def getItems(self, **kwargs):
        r = self.getDocuments("item", **kwargs)
        return r["hits"]

    def getSnippets(self, **kwargs):
        r = self.getDocuments("snippet", **kwargs)
        return r["hits"]

    def getDocument(self, index, type, id):
    ...
    def getSimilar(self, index, keywords):
        articles = []
    ...
    def es_getCluster(self, index, id, size=100):
    ...
    def getClusterList(self, index, **kwargs):
        ...
        siz = res["aggregations"]["clusters"]["cluster_count"]["value"]
    ...
    # editing stuff
    #
    def getWhere(self, index, item):
        dsl = {"fields": ["id"], "query": {"term": {"items.item": item}}, "size": 1000}
        url = self.makeUrl(index, 'articles', '_search')
        deps = []
        for h in self.request("get", url, data=dsl)["hits"]["hits"]:
    ...
    def postLog(self, index, seq, log):
        try:
    ...
    def getStatus(self, index, seq=None):
        if seq:
    ...
    def postEnd(self, index, seq, newcommit, status, log=None):
    ...
    def resetUp(self, index, seq):
        url = self.makeUrl(index, "commits", "{0}-{1}".format(index, seq))
        self.request("delete", url)

    def postUp(self, index, last, newcommit):
        up = {}
        # start new commit
        seq = int(last["seq"]) + 1
        up["seq"] = seq
    ...
    def postKM(self, index, oType, kmj, repl=False):
        try:
            id_value = kmj["id"]
        except:
    ...
    def delKM(self, index, oType, id=""):
        try:
            ...
            if (id == '*'):
                data = {"query": {
                    "match_all": {}}}  # note this is deprecated in later versions of ES and cannot be done in v 2.0 onwards
