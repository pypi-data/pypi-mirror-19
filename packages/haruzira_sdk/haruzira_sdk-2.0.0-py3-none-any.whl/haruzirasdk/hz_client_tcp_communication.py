...
class ClientTcpCommunication(object):
    ...
    #メッセージ受信時イベント定義
    evNotifyCompleteSpeech = None                   #読み上げ完了通知受信時イベント
    evNotifyMessageEvent = None                     #Exception発生時イベント（切断以外）
    evNotifyReceivedDisconnectEvent = None          #切断発生時イベント（サーバー側からの切断など）
    evNotifySendSpeechRecognitionCommand = None     #音声認識コマンド送信受信時イベント
    #endregion
    ...
    def __init__(self):
        ...
        self.__Version = "2.0.0.0"     #SDKバージョン
    ...
    def __reqStartCommProc(self, name = "", passwd = "", strEncKey = ""):
        ...
        try:
            ...
            self.__HzTrace.trace_comm_message("[CL]Send, Communication Start Request(0x{0:02x})：Cer[{1:d}] Enc[{2:d}], Port[{3:d}], Name[{4:s}], Passwd[{5:s}], Name Len[{6:d}], Passwd Len[{7:d}]".format(
                reqStartComm.id, reqStartComm.cer_flg, reqStartComm.enc_flg, reqStartComm.port, name if reqStartComm.name is not None else "", 
                passwd if reqStartComm.passwd is not None else "", reqStartComm.name_len, reqStartComm.passwd_len))
    ...
    def __sendSpeechData(self):
        ...
        try:
            ...
            self.__HzTrace.trace_comm_message("[CL]Send, Send Speech Data(0x{0:02x})：Enc[{1:d}], Size[{2:d}]".format(sendSpeechData.id, sendSpeechData.enc_flg, sendSpeechData.size))
