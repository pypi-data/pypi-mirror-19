...
class RpcClient(object):
    ...
    def call(self, body, method_name):
        ...
        message_properties = pika.BasicProperties(reply_to=self.callback_queue, correlation_id=self.corr_id, )
...
class RpcServer(threading.Thread):
    def __init__(self, name, username, password, exchange='', host='localhost'):
        ...
        self.connection = pika.BlockingConnection(pika.ConnectionParameters(virtual_host='/', port=5432, host=host, credentials=credentials))
        self.channel = self.connection.channel()
    ...
    def return_acknowledgement(self, ch, method, props, response):
