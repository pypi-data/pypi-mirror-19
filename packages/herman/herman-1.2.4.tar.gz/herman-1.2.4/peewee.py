...
__peewee_version__ = '2.8.5'
__herman_version__ = '1.2.4'
...
# superset of reserved words between: sqlite3, postgres, mysql, mssql     
SQL_RESERVED_WORDS = set('A ABORT ABS ABSOLUTE ACCESS ACCESSIBLE ACTION ADA ADD ADMIN AFTER AGAINST AGGREGATE ALGORITHM ALIAS ALL ALLOCATE ALSO ALTER ALWAYS ANALYSE ANALYZE AND ANY ARE ARRAY AS ASC ASCII ASENSITIVE ASSERTION ASSIGNMENT ASYMMETRIC AT ATOMIC ATTACH ATTRIBUTE ATTRIBUTES AUTHORIZATION AUTHORS AUTOEXTEND_SIZE AUTOINCREMENT AUTO_INCREMENT AVG AVG_ROW_LENGTH BACKUP BACKWARD BEFORE BEGIN BERNOULLI BETWEEN BIGINT BINARY BINLOG BIT BITVAR BIT_LENGTH BLOB BLOCK BOOL BOOLEAN BOTH BREADTH BREAK BROWSE BTREE BULK BY BYTE C CACHE CALL CALLED CARDINALITY CASCADE CASCADED CASE CAST CATALOG CATALOG_NAME CEIL CEILING CHAIN CHANGE CHANGED CHAR CHARACTER CHARACTERISTICS CHARACTERS CHARACTER_LENGTH CHARACTER_SET_CATALOG CHARACTER_SET_NAME CHARACTER_SET_SCHEMA CHARSET CHAR_LENGTH CHECK CHECKED CHECKPOINT CHECKSUM CIPHER CLASS CLASS_ORIGIN CLIENT CLOB CLOSE CLUSTER CLUSTERED COALESCE COBOL CODE COLLATE COLLATION COLLATION_CATALOG COLLATION_NAME COLLATION_SCHEMA COLLECT COLUMN COLUMNS COLUMN_NAME COMMAND_FUNCTION COMMAND_FUNCTION_CODE COMMENT COMMIT COMMITTED COMPACT COMPLETION COMPRESSED COMPUTE CONCURRENT CONDITION CONDITION_NUMBER CONFLICT CONNECT CONNECTION CONNECTION_NAME CONSISTENT CONSTRAINT CONSTRAINTS CONSTRAINT_CATALOG CONSTRAINT_NAME CONSTRAINT_SCHEMA CONSTRUCTOR CONTAINS CONTAINSTABLE CONTEXT CONTINUE CONTRIBUTORS CONVERSION CONVERT COPY CORR CORRESPONDING COUNT COVAR_POP COVAR_SAMP CPU CREATE CREATEDB CREATEROLE CREATEUSER CROSS CSV CUBE CUME_DIST CURRENT CURRENT_DATE CURRENT_DEFAULT_TRANSFORM_GROUP CURRENT_PATH CURRENT_ROLE CURRENT_TIME CURRENT_TIMESTAMP CURRENT_TRANSFORM_GROUP_FOR_TYPE CURRENT_USER CURSOR CURSOR_NAME CYCLE DATA DATABASE DATABASES DATAFILE DATE DATETIME DATETIME_INTERVAL_CODE DATETIME_INTERVAL_PRECISION DAY DAY_HOUR DAY_MICROSECOND DAY_MINUTE DAY_SECOND DBCC DEALLOCATE DEC DECIMAL DECLARE DEFAULT DEFAULTS DEFERRABLE DEFERRED DEFINED DEFINER DEGREE DELAYED DELAY_KEY_WRITE DELETE DELIMITER DELIMITERS DENSE_RANK DENY DEPTH DEREF DERIVED DESC DESCRIBE DESCRIPTOR DESTROY DESTRUCTOR DES_KEY_FILE DETACH DETERMINISTIC DIAGNOSTICS DICTIONARY DIRECTORY DISABLE DISCARD DISCONNECT DISK DISPATCH DISTINCT DISTINCTROW DISTRIBUTED DIV DO DOMAIN DOUBLE DROP DUAL DUMP DUMPFILE DUPLICATE DYNAMIC DYNAMIC_FUNCTION DYNAMIC_FUNCTION_CODE EACH ELEMENT ELSE ELSEIF ENABLE ENCLOSED ENCODING ENCRYPTED END END-EXEC ENDS ENGINE ENGINES ENUM EQUALS ERRLVL ERROR ERRORS ESCAPE ESCAPED EVENT EVENTS EVERY EXCEPT EXCEPTION EXCLUDE EXCLUDING EXCLUSIVE EXEC EXECUTE EXISTING EXISTS EXIT EXP EXPANSION EXPLAIN EXTENDED EXTENT_SIZE EXTERNAL EXTRACT FAIL FALSE FAST FAULTS FETCH FIELDS FILE FILLFACTOR FILTER FINAL FIRST FIXED FLOAT FLOAT4 FLOAT8 FLOOR FLUSH FOLLOWING FOR FORCE FOREIGN FORTRAN FORWARD FOUND FRAC_SECOND FREE FREETEXT FREETEXTTABLE FREEZE FROM FULL FULLTEXT FUNCTION FUSION G GENERAL GENERATED GEOMETRY GEOMETRYCOLLECTION GET GET_FORMAT GLOB GLOBAL GO GOTO GRANT GRANTED GRANTS GREATEST GROUP GROUPING HANDLER HASH HAVING HEADER HELP HIERARCHY HIGH_PRIORITY HOLD HOLDLOCK HOST HOSTS HOUR HOUR_MICROSECOND HOUR_MINUTE HOUR_SECOND IDENTIFIED IDENTITY IDENTITYCOL IDENTITY_INSERT IF IGNORE IGNORE_SERVER_IDS ILIKE IMMEDIATE IMMUTABLE IMPLEMENTATION IMPLICIT IMPORT IN INCLUDING INCREMENT INDEX INDEXED INDEXES INDICATOR INFILE INFIX INHERIT INHERITS INITIALIZE INITIALLY INITIAL_SIZE INNER INNOBASE INNODB INOUT INPUT INSENSITIVE INSERT INSERT_METHOD INSTALL INSTANCE INSTANTIABLE INSTEAD INT INT1 INT2 INT3 INT4 INT8 INTEGER INTERSECT INTERSECTION INTERVAL INTO INVOKER IO IO_THREAD IPC IS ISNULL ISOLATION ISSUER ITERATE JOIN K KEY KEYS KEY_BLOCK_SIZE KEY_MEMBER KEY_TYPE KILL LANCOMPILER LANGUAGE LARGE LAST LATERAL LEADING LEAST LEAVE LEAVES LEFT LENGTH LESS LEVEL LIKE LIMIT LINEAR LINENO LINES LINESTRING LIST LISTEN LN LOAD LOCAL LOCALTIME LOCALTIMESTAMP LOCATION LOCATOR LOCK LOCKS LOGFILE LOGIN LOGS LONG LONGBLOB LONGTEXT LOOP LOWER LOW_PRIORITY M MAP MASTER MASTER_CONNECT_RETRY MASTER_HEARTBEAT_PERIOD MASTER_HOST MASTER_LOG_FILE MASTER_LOG_POS MASTER_PASSWORD MASTER_PORT MASTER_SERVER_ID MASTER_SSL MASTER_SSL_CA MASTER_SSL_CAPATH MASTER_SSL_CERT MASTER_SSL_CIPHER MASTER_SSL_KEY MASTER_SSL_VERIFY_SERVER_CERT MASTER_USER MATCH MATCHED MAX MAXVALUE MAX_CONNECTIONS_PER_HOUR MAX_QUERIES_PER_HOUR MAX_ROWS MAX_SIZE MAX_UPDATES_PER_HOUR MAX_USER_CONNECTIONS MEDIUM MEDIUMBLOB MEDIUMINT MEDIUMTEXT MEMBER MEMORY MERGE MESSAGE_LENGTH MESSAGE_OCTET_LENGTH MESSAGE_TEXT METHOD MICROSECOND MIDDLEINT MIGRATE MIN MINUTE MINUTE_MICROSECOND MINUTE_SECOND MINVALUE MIN_ROWS MOD MODE MODIFIES MODIFY MODULE MONTH MORE MOVE MULTILINESTRING MULTIPOINT MULTIPOLYGON MULTISET MUMPS MUTEX MYSQL_ERRNO NAME NAMES NATIONAL NATURAL NCHAR NCLOB NDB NDBCLUSTER NESTING NEW NEXT NO NOCHECK NOCREATEDB NOCREATEROLE NOCREATEUSER NODEGROUP NOINHERIT NOLOGIN NONCLUSTERED NONE NORMALIZE NORMALIZED NOSUPERUSER NOT NOTHING NOTIFY NOTNULL NOWAIT NO_WAIT NO_WRITE_TO_BINLOG NULL NULLABLE NULLIF NULLS NUMBER NUMERIC NVARCHAR OBJECT OCTETS OCTET_LENGTH OF OFF OFFSET OFFSETS OIDS OLD OLD_PASSWORD ON ONE ONE_SHOT ONLY OPEN OPENDATASOURCE OPENQUERY OPENROWSET OPENXML OPERATION OPERATOR OPTIMIZE OPTION OPTIONALLY OPTIONS OR ORDER ORDERING ORDINALITY OTHERS OUT OUTER OUTFILE OUTPUT OVER OVERLAPS OVERLAY OVERRIDING OWNER PACK_KEYS PAD PAGE PARAMETER PARAMETERS PARAMETER_MODE PARAMETER_NAME PARAMETER_ORDINAL_POSITION PARAMETER_SPECIFIC_CATALOG PARAMETER_SPECIFIC_NAME PARAMETER_SPECIFIC_SCHEMA PARSER PARTIAL PARTITION PARTITIONING PARTITIONS PASCAL PASSWORD PATH PERCENT PERCENTILE_CONT PERCENTILE_DISC PERCENT_RANK PHASE PIVOT PLACING PLAN PLI PLUGIN PLUGINS POINT POLYGON PORT POSITION POSTFIX POWER PRAGMA PRECEDING PRECISION PREFIX PREORDER PREPARE PREPARED PRESERVE PREV PRIMARY PRINT PRIOR PRIVILEGES PROC PROCEDURAL PROCEDURE PROCESSLIST PROFILE PROFILES PROXY PUBLIC PURGE QUARTER QUERY QUICK QUOTE RAISE RAISERROR RANGE RANK READ READS READTEXT READ_ONLY READ_WRITE REAL REBUILD RECHECK RECONFIGURE RECOVER RECURSIVE REDOFILE REDO_BUFFER_SIZE REDUNDANT REF REFERENCES REFERENCING REGEXP REGR_AVGX REGR_AVGY REGR_COUNT REGR_INTERCEPT REGR_R2 REGR_SLOPE REGR_SXX REGR_SXY REGR_SYY REINDEX RELATIVE RELAY RELAYLOG RELAY_LOG_FILE RELAY_LOG_POS RELAY_THREAD RELEASE RELOAD REMOVE RENAME REORGANIZE REPAIR REPEAT REPEATABLE REPLACE REPLICATION REQUIRE RESET RESIGNAL RESTART RESTORE RESTRICT RESULT RESUME RETURN RETURNED_CARDINALITY RETURNED_LENGTH RETURNED_OCTET_LENGTH RETURNED_SQLSTATE RETURNS REVERT REVOKE RIGHT RLIKE ROLE ROLLBACK ROLLUP ROUTINE ROUTINE_CATALOG ROUTINE_NAME ROUTINE_SCHEMA ROW ROWCOUNT ROWGUIDCOL ROWS ROW_COUNT ROW_FORMAT ROW_NUMBER RTREE RULE SAVE SAVEPOINT SCALE SCHEDULE SCHEMA SCHEMAS SCHEMA_NAME SCOPE SCOPE_CATALOG SCOPE_NAME SCOPE_SCHEMA SCROLL SEARCH SECOND SECOND_MICROSECOND SECTION SECURITY SECURITYAUDIT SELECT SELF SEMANTICKEYPHRASETABLE SEMANTICSIMILARITYDETAILSTABLE SEMANTICSIMILARITYTABLE SENSITIVE SEPARATOR SEQUENCE SERIAL SERIALIZABLE SERVER SERVER_NAME SESSION SESSION_USER SET SETOF SETS SETUSER SHARE SHOW SHUTDOWN SIGNAL SIGNED SIMILAR SIMPLE SIZE SLAVE SLOW SMALLINT SNAPSHOT SOCKET SOME SONAME SOUNDS SOURCE SPACE SPATIAL SPECIFIC SPECIFICTYPE SPECIFIC_NAME SQL SQLCODE SQLERROR SQLEXCEPTION SQLSTATE SQLWARNING SQL_BIG_RESULT SQL_BUFFER_RESULT SQL_CACHE SQL_CALC_FOUND_ROWS SQL_NO_CACHE SQL_SMALL_RESULT SQL_THREAD SQL_TSI_DAY SQL_TSI_FRAC_SECOND SQL_TSI_HOUR SQL_TSI_MINUTE SQL_TSI_MONTH SQL_TSI_QUARTER SQL_TSI_SECOND SQL_TSI_WEEK SQL_TSI_YEAR SQRT SSL STABLE START STARTING STARTS STATE STATEMENT STATIC STATISTICS STATUS STDDEV_POP STDDEV_SAMP STDIN STDOUT STOP STORAGE STRAIGHT_JOIN STRICT STRING STRUCTURE STYLE SUBCLASS_ORIGIN SUBJECT SUBLIST SUBMULTISET SUBPARTITION SUBPARTITIONS SUBSTRING SUM SUPER SUPERUSER SUSPEND SWAPS SWITCHES SYMMETRIC SYSID SYSTEM SYSTEM_USER TABLE TABLES TABLESAMPLE TABLESPACE TABLE_CHECKSUM TABLE_NAME TEMP TEMPLATE TEMPORARY TEMPTABLE TERMINATE TERMINATED TEXT TEXTSIZE THAN THEN TIES TIME TIMESTAMP TIMESTAMPADD TIMESTAMPDIFF TIMEZONE_HOUR TIMEZONE_MINUTE TINYBLOB TINYINT TINYTEXT TO TOAST TOP TOP_LEVEL_COUNT TRAILING TRAN TRANSACTION TRANSACTIONS_COMMITTED TRANSACTIONS_ROLLED_BACK TRANSACTION_ACTIVE TRANSFORM TRANSFORMS TRANSLATE TRANSLATION TREAT TRIGGER TRIGGERS TRIGGER_CATALOG TRIGGER_NAME TRIGGER_SCHEMA TRIM TRUE TRUNCATE TRUSTED TRY_CONVERT TSEQUAL TYPE TYPES UESCAPE UNBOUNDED UNCOMMITTED UNDEFINED UNDER UNDO UNDOFILE UNDO_BUFFER_SIZE UNENCRYPTED UNICODE UNINSTALL UNION UNIQUE UNKNOWN UNLISTEN UNLOCK UNNAMED UNNEST UNPIVOT UNSIGNED UNTIL UPDATE UPDATETEXT UPGRADE UPPER USAGE USE USER USER_DEFINED_TYPE_CATALOG USER_DEFINED_TYPE_CODE USER_DEFINED_TYPE_NAME USER_DEFINED_TYPE_SCHEMA USER_RESOURCES USE_FRM USING UTC_DATE UTC_TIME UTC_TIMESTAMP VACUUM VALID VALIDATOR VALUE VALUES VARBINARY VARCHAR VARCHARACTER VARIABLE VARIABLES VARYING VAR_POP VAR_SAMP VERBOSE VIEW VIRTUAL VOLATILE WAIT WAITFOR WARNINGS WEEK WHEN WHENEVER WHERE WHILE WIDTH_BUCKET WINDOW WITH WITHIN WITHOUT WORK WRAPPER WRITE WRITETEXT X509 XA XML XOR YEAR YEAR_MONTH ZEROFILL ZONE'.split())
