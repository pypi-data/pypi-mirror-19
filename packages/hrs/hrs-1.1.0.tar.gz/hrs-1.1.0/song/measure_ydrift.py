...
def scan_fits_header(fps):
    ...
    for i in trange(len(fps), ncols=100, ascii=False, desc='**'+cfn+'**', unit=" file"):
        fp = fps[i]
        #        print i, len(fps)
        # read fits
        hl = fits.open(fp)
...
def measure_xshift_2dcorr(imfps, bias, flat, xmax=12):
    drifts = np.zeros(len(imfps))
    cfn = sys._getframe().f_code.co_name
    for i in trange(len(imfps), ncols=100, ascii=False, desc='**'+cfn+'**', unit=" file"):
        data = ccdproc.CCDData.read(imfps[i], unit='adu')
        data = ccdproc.subtract_bias(data, bias_temp)
        shift, corr2d = thar_corr2d(data.data, flat_temp.data, xtrim=(512, 1536), ytrim=(512, 1536),
                                    x_shiftmax=12, y_shiftmax=0, verbose=False)
