...
class Controller(object):
    ...
    def __instantiate_container(self):
        ...
        for name, definition in params['items'].items():
            ...
            using_parameters[param_name] = definition

        for expected_param in expected_params:
            param_name = expected_param.name
            param_kind = expected_param.kind

            if param_kind == expected_param.VAR_POSITIONAL:
                # FIXME Currently not supported. To be fixed in future releases.

                continue

            if param_kind == expected_param.VAR_KEYWORD:
                # Currently implement as part of `using_parameters`.
            ...
            if expected_param.default == inspect._empty:
                ...
                raise UndefinedDefaultValueException(
                    'Constructor: {make_method_name}{signature} expects the parameter "{parameter_name}" ({make_method})'.format(
