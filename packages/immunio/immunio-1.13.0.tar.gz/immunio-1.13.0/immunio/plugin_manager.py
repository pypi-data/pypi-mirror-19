...
from immunio.timer import Timer
...
PROTECTION_PLUGINS = [
    ...
    "sqli_mysqldb",
    "xss_jinja2",
    # "xss_django", - disabled by default for first release.
]
...
class PluginManager(object):
    ...
    def __init__(self, config, get_agent_func=None):
        ...
        self.debug_mode = config.get("debug_mode", False, datatype=bool)
    ...
    def set_timer_callback(self, timer_callback):
        """
        Set the `Timer` instance to be used to measure plugin durations. If
        no timer is set, the plugin durations will not be timed.
        """
        self._timer.set_report_callback(timer_callback)
    ...
    def set_plugin_status(self, name, status, meta=None):
        if self._plugin_status_callback:
    ...
    def register(self, plugin_module):
        ...
        if plugin_module.NAME not in self.active_plugins:
            self.set_plugin_status(plugin_module.NAME, DISABLED)
            return
        ...
        try:
            ...
            if getattr(plugin_module, "LATE_HOOK", False):
                meta = plugin_module.add_hooks(run_hook, self.set_plugin_status,
                    get_agent_func=self.get_agent_func, timer=self._timer)
                self.set_plugin_status(plugin_module.NAME, PENDING, meta)
            else:
                meta = plugin_module.add_hooks(run_hook,
                    get_agent_func=self.get_agent_func, timer=self._timer)
                self.set_plugin_status(plugin_module.NAME, LOADED, meta)
        except:
            if self.debug_mode:
                raise
            self.set_plugin_status(plugin_module.NAME, FAILED)
