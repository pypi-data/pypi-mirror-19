...
from intogen.ma.model import MaResult
...
class VepCache(object):
    ...
    def open(self):
        ...
        if self.part_size is not None and part_size != self.part_size:
            raise Exception(
                "The database part_size ({0}) doesn't match with the asked one ({1})".format(part_size, self.part_size))
    ...
    def add(self, chr, start, ref, alt, ref_genome_variant, gene, uniprot, info, uniprot_variant, func_impact,
            fi_score):
