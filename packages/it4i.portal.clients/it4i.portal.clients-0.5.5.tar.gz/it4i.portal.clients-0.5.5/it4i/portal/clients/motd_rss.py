...
class Feed(object):
    ...
    def render_text(self):
        ...
        for item in self.filtered_feed:
            updated = True
            print >> output
            for title_line in wrapper.wrap(item['title']):
                m = re.search('^(.*)\s(\([\d-]*\sto\s[\d-]*\)$)', item['title'])
                if m:
                    print >> output, m.group(1).center(self.width).encode('utf-8')
                    print >> output, m.group(2).center(self.width).encode('utf-8')
                    updated = False
                else:
                    print >> output, title_line.center(self.width).encode('utf-8')

            if updated: print >> output, ('(%s)' % item['updated']).center(self.width)
            print >> output
    ...
    def render_html(self):
        ...
        print >> output, template(items=self.filtered_feed).encode('utf-8')
...
def main(argv=sys.argv[1:]):
    ...
    # parse opts and args
    try:
        opts, args = getopt.getopt(argv, "hil:m:nrs:tvw:", ["help",
                                                            "stdin",
                                                            "limit=",
                                                            "html=",
                                                            "cron",
