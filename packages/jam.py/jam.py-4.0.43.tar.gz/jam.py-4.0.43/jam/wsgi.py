...
class App():
    ...
    def process_request(self, env, method, session_id=None, task_id=None, item_id=None, params=None, ext=None):
        ...
        try:
            ...
            if not data:
                data = self.get_response(is_admin, env, method, session, task_id, obj, params, ext)
        finally:
            self._busy -= 1
        return {'status': common.RESPONSE, 'data': data, 'version': task.version, 'server_date': self.started}

    def get_response(self, is_admin, env, method, session, task_id, item, params, ext):
        if ext:
            ...
            if session.has_privilege(item, 'can_view'):
                return item.select_records(params)
            else:
                return [], item.task.lang['cant_view'] % item.item_caption
        elif method == 'get_record_count':
            return item.get_record_count(params)
        elif method == 'apply_changes':
            return item.apply_changes(params)
            ...
            return self.init_client(session, is_admin)
    ...
    def init_client(self, session, is_admin):
        if is_admin:
            task = self.admin
        else:
            task = self.task
        ...
        if request.method == 'POST':
            try:
                ...
                if self.admin.safe_mode:
                    session = self.sessions.check_session(session_id)
                    if not session:
                        return Response()
                for s in data[pos:]:
                    header_str += s
