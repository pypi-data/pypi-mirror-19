...
def format_list(lines):
    ...
    for line in lines:
        ...
        for k,v in line.items():
            row = u"{k:<{max_len}} : {v}".format(k=k, v=v, max_len=max_len)
            click.echo(row)
...
@click.option('--input', '-i', type=click.Choice(['json', 'jsonl']), default='jsonl')
@click.option('--output', '-o', type=click.Choice(['json', 'jsonl', 'table', 'list', 'tsv']), default="jsonl")
@click.option('--format')
@click.option('--filter') 
@click.option('--keys', '-k')
def cli(input_file, input, output, format, filter, keys):
