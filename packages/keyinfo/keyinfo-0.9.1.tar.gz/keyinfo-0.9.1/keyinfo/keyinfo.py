...
def to_keyinfo_sig1(cert_or_list, prefix='',
                    dsa_add_pq=False,
                    dsa_add_g=False):
    return to_keyinfo(cert_or_list,
                      prefix,
                      version=XMLSIG1,
                      dsa_add_pq=dsa_add_pq,
                      dsa_add_g=dsa_add_g)
...
def to_keyinfo(cert_or_list,
    ...
    _keyvalue(first_cert, keyinfo, dsa_add_pq, dsa_add_g)
    _x509data(first_cert, other_certs, keyinfo, prefix, version)
    return keyinfo

def from_keyinfo(keyinfo, chain=False):
    return _cert_or_chain_from_keyinfo(keyinfo, chain=chain)
...
def _keyvalue(cert, keyinfo, dsa_add_pq=False, dsa_add_g=False):
    ...
    if isinstance(public_key, rsa.RSAPublicKey):
        ...
        eckeyvalue = etree.SubElement(keyvalue, _dsig11('ECKeyValue'))
        namedcurve = etree.SubElement(eckeyvaue, _dsig11('NamedCurve'))
        namedcurve.set('URI', public_key.curve.name)
        dspublickey = etree.SubElement(keyvalue, _dsig11('PublicKey'))
        dspublickey.text = base64.b64encode(
            public_key.public_numbers().encode_point()
        )
    elif isinstance(public_key, dsa.DSAPublicKey):
        dsakeyvalue = etree.SubElement(keyvalue, _ds('DSAKeyValue'))
        public_numbers = public_key.public_numbers()
        if dsa_add_pq or dsa_add_g:
            parameter_numbers = public_numbers.parameter_numbers
            if dsa_add_pq:
                p_el = etree.SubElement(dsakeyvalue, _ds('P'))
                p_el.text = str(parameter_numbers.p)
                q_el = etree.SubElement(dsakeyvalue, _ds('Q'))
                q_el.text = str(parameter_numbers.q)
            if dsa_add_g:
                g_el = etree.SubElement(dsakeyvalue, _ds('G'))
                g_el.text = str(parameter_numbers.g)
        y_el = etree.SubElement(dsakeyvalue, _ds('Y'))
        y_el.text = str(public_numbers.y)

    else:
        raise NotImplementedError('Only RSA, DSA and EC Keys supported in this version.')
