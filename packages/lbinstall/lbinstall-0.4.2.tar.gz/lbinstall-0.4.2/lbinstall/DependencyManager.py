...
class LbYumClient(object):
    ...
    def __init__(self, localConfigRoot, repourls, checkForUpdates=True):
        ...
        # files
        self._initializeRepositories(repourls, checkForUpdates,
    ...
    def listPackages(self, nameRegexp=None, versionRegexp=None,
        ...
        for rep in self.repositories.values():
            for pa in rep.getAllPackages():
                ...
                versionmatch = True
                releasematch = True
                if nameRegexp is not None and re.match(nameRegexp,
                                                       pa.name) is None:
                    namematch = False
                if versionRegexp is not None and re.match(versionRegexp,
                                                          pa.name) is None:
                    versionmatch = False
                if releaseRegexp is not None and re.match(releaseRegexp,
                                                          pa.name) is None:
                    releasematch = False
