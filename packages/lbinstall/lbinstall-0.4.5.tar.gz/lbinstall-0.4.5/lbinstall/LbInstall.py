...
class LbInstallClient(object):
    ...
    def run(self, opts, args):
        ...
        if mode == LbInstallClient.MODE_QUERY:
            ...
            self.listInstalledPackages(* args[1:])
              ...
              mode == LbInstallClient.MODE_DOWNLOAD):
            i = 1
            first_pass = True
            rpms_list = []
            while i < len(args):
            ...
            if mode == LbInstallClient.MODE_CHECK:
                ...
                for rpmname, version, release in rpms_list:
                    self.log.info("Downloading RPMs for : %s %s %s"
                                  % (rpmname, version, release))
                self.install(rpms_list, download_only=True)
    ...
    def checkPacakges(self, rpms_to_verify):
        ...
        packagelist = self.installer.getPackageListFromTuples(rpms_to_verify,
                                                              local=True)
        self.installer.checkPackages(packagelist)
