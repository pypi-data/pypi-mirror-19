...
class Installer(object):
    ...
    * Passing a full InstallAreaConfig as the named parameter config,
    ...
    def getPackageListToInstall(self, packagelist,
        ...
        @param: dry_run_update Special flag to be used in update. When
        ...
        if not nodeps and not self._nodeps:
            for p in packagelist:
                ...
                rpmtoinstall_tmp.sort(key=lambda tup: tup[1], reverse=True)
                for rpm, level in rpmtoinstall_tmp:
    ...
    def install(self, rpms_to_install, dry_run_update=False,
                download_only=False,
                justdb=False, overwrite=False, dry_run=False):
        """
    ...
    def _install(self, packagelist,
        ...
        if overwrite:
            rpmtoinstall.extend(packagelist)
    ...
    def _getPackagesToInstall(self, p,
        ...
        @param: extrapackages the list of packages that will
                              be installed
        @param: extrainfo dict of extra information used in the installation
        @param: dry_run_update Special flag to be used in update. When
                               updating, and the dry_run flag is true, we are
                               not actually removing the files before
                               installing new ones, so the install procedure
                               ...
                               to avoid an exception such as the files are
                               already installed.
        @param: level the level of dependency.( A-depends on->B-depends on->C,
                      A is at level 0,
                      B is at level 1,
                      C is at level 2)
        '''
        ...
        for req in p.requires:
            ...
            if tmp is not None and not dry_run_update:
                ...
                match = self._lbYumClient.findLatestMatchingRequire(req)
                if match and match not in extrapackages:
                    toinstall += self._getPackagesToInstall(
                        match, extrapackages, extrainfo,
                        dry_run_update=dry_run_update,
                        level=(level+1))
        return toinstall
    ...
    def _downloadfiles(self, installlist, location=None, metadata=False,
        ...
        @param: the files that need download for installation
        @param: the location where to download data
                ...
                while counter_download < 2:
                    ...
                    try:
                        ...
                        if os.path.exists(tmp_name):
                            self.log.warning("Removing temporary file: %s" %
                                             tmp_name)
                            os.unlink(tmp_name)
                    counter_download += 1
    ...
    def update(self, rpms_list):
        ...
        try:
            ...
            self.log.error("Error removing file %s when updating:\n%s" % (
                rpm_name, e))
        self.install(rpms_list, dry_run_update=True, overwrite=True)
    ...
    def checkPacakges(self, rpms_to_verify):
