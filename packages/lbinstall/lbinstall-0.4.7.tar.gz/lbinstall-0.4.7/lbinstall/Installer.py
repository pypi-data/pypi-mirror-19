...
class Installer(object):
    ...
    * Passing a full InstallAreaConfig as the named parameter config,
    ...
    # Package update
    # ############################################################################
    def update(self, rpms_list, dry_run=False, justdb=False):
        """
        ...
        try:
            ...
            self.log.error("Error removing file %s when updating:\n%s" % (
                rpm_name, e))
        self.install(rpms_list, dry_run=dry_run, dry_run_update=True,
                     nodeps=True, overwrite=True, justdb=justdb)
