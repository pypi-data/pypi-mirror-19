                                                ...
                                                const double &weight)
  {
    ...
    for (typename GI::Corner_t corner = 0; corner < n_corners/2; ++corner) {
      ...
      if (!almost_zero(temp_vol_frac)) {
        ...
        weights[compute_global_bin<GI>(shifted_bins, sizes, D)] += weight*temp_vol_frac;
      }
      if (corner != opposite_corner && !almost_zero(temp_opposite_vol_frac)) {
        ...
        weights[compute_global_bin<GI>(shifted_bins, sizes, D)] += weight*temp_opposite_vol_frac;
      }
    }
                          ...
                          double *result_weights)  // grid points
{
  ...
  ComputeBinsAndVolumeFractionsDyn<GI> cbavf(D, extents, bin_sizes, sizes, result_weights);
  ...
  typename GI::Corner_t n_grid_points = 1;
  for (unsigned long i = 0; i < D; ++i)
    n_grid_points *= sizes[i];

  vector<unsigned long> _bins(D);
  for (typename GI::GIndex_t i = 0; i < n_grid_points; ++i) {
    const typename GI::GIndex_t index = i*D;
    compute_bin_numbers<GI>(D, i, sizes, _bins);
    for (unsigned long i = 0; i < D; ++i)
...
void linear_binning_impl (const unsigned long D, // space dimension
                          const double *X, // n_samples x D
                          const double *weights, // n_samples
                          const unsigned long n_samples,
