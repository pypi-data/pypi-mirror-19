...
class _Guay(ComplexPlugin):
    ...
    def trigger_build(self):
        ...
        """
        external_archive_url = "http://{}/{}".format(CI["external_fqdn"], self.share_memory["archive_url"])
        try:
            self.share_memory["build_id"] = self.guay.BUILD.StartBuild(build_request={
                "commit_id": self.package_info["commit_id"],
                "build_id": "",
                "image": "{}/{}".format(self.package_info["build_info"]["registry_prefix"],
                                        self.package_info["artifact_name"]),
                "version": self.share_memory["latest_version"],
