    ...
    def __ensure_runtime_resources(self):
        ...
        if self._run_student:
            ...
            if os.path.exists(self._database):
                self.__copy_resource(self._database, "./student/{}".format(self._database))
...
class Rel(DBMSContext):
    ...
    def __init__(self, database, run_student=True):
        ...
        :param run_student: a flag indicating whether or not you want the subprocess
                            ...
                            (in a student context in inginious parlance).
        '''
        home_base = "." if run_student else os.path.dirname(os.path.abspath(__file__))
    ...
    def runtime_requires(self):
        ...
        '''
        return {'Rel-standalone': os.path.join(os.path.dirname(os.path.abspath(__file__)), 'resources/Rel-standalone')}

    def command(self):
        cmd_pattern = "java -classpath {home} -jar RelDBMS.jar -f{db}"
        return cmd_pattern.format(home=self._proc_home, db=self._database)
...
class SQL(DBMSContext):
    ...
    def command(self):
        return "sqlite3 {}".format(self._database)
