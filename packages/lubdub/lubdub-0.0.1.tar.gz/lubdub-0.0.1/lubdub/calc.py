...
def rmssd(values):
    ...
    :param values: (list)
    :param normalization: (float) percentage above or below previous value to disqualify a successive value.
    :return rmmsd: (float)
    """
    return math.sqrt( (1/len(values)) * _sum_of_squares_of_successive_differences(values) ) 

def normalize_rr(values, normalization=20.0, max_gap=2100, min_gap=200):
    ...
    1a) checking for gaps: any value falling above max_gap.  When encountered, 
       ...
       into the length of the gap and inserting into the resultant array a 
    ...
    :param max_gap: (float) millisecond threshold above which we consider this a "gap" in heartbeat detection
    :param min_gap: (float) discard values under this millisecond threshold
    """
...
def nn_to_rolling_hrv(nn_values, window=12):
    ...
    length can be controlled via the `window` parameter.
    ...
    # keep track of our current average RR -- part of how we deduce how to "fill" apparent
    # gaps in rr coverage.
    rrs = deque(nn_values[:window-1])
    hrvs = []
    for idx in range(window,len(nn_values)-1):
        if nn_values[idx] == -1:
            try:
                rrs.popleft()
            except IndexError:
