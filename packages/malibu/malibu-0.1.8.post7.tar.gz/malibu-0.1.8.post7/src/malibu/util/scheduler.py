...
def parse_cronish_time_designation(cron_str):
    """ Parses a cron-like time designation for repetitive jobs.

        `cron_str` is expected to be either a cron-like time designation
        OR one of the following available directives.

        the cron-ish time designation is whitespace delimited, either spaces
        or tabs. each field can be a comma-delimited list of times,
        "time divisions" (*/2 -- every two seconds)

        cron-ish time designation:
        ...
        :param str cron_str: cron-like string to parse and turn in to times.
        :returns: repetitive time generator
    """
