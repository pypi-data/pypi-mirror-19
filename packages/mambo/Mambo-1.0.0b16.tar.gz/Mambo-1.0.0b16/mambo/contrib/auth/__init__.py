...
from mambo import (register_package,
                   get_config,
                   abort,
                   send_mail,
                   url_for,
...
def change_password(user, password, password_confirm):
    """
    ...
    def cb():
        user_login = user.get_login()
        return user_login.change_password(password, password_confirm)
...
def send_password_reset_email(user=None, email=None, base_url=None, template=None, method="TOKEN"):
    """
    ...
    method = options.get("reset_password_method") or method
    ...
    if method.upper() == "TOKEN":
        ...
        url = _url_for_email(views.AuthLogin.index, base_url=base_url)
...
def send_email_verification_email(user, base_url=None, template=None):
...
def send_registration_welcome_email(user, base_url=None, template=None):
