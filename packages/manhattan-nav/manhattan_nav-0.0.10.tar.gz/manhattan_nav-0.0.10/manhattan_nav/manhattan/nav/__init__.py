...
class Nav:
    ...
    def query(cls, endpoint, **view_args):
        ...
        - allowed if the caller is allowed to call the endpoint with view
          arguments,
        - active if the endpoint with view arguments is the target of the
          current request,
        ...
        # Normalize the URLs
        #
        # To determine if a URL is active we need to check it against the full
        # path, which by default will end in a question mark (?) if no query
        # exists. For this reason we strip the final question mark from the full
        # path when testing.
        #
        # We can avoid this issue by normalizing the URLs using `urlsplit` and
        # `geturl` on the result.
        full_path = urlsplit(flask.request.full_path).geturl()
        ...
        return QueryResult(
            ...
            active=url == full_path,
            active_sub=path.startswith(url + '/' if url else '-'),
            url=url
            )
