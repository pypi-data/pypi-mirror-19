...
### title: preprocessor ###
    ...
    LF = Char('\r', expression="'\\x0d'", name='LF')
    CR = Char('\n', expression="'\\x0a'", name='CR')
    CRLF = Word('\n\r', expression='"\\x0a\\x0d"', name='CRLF')
    EOL = Choice([CRLF, LF, CR], expression='CRLF / LF / CR', name='EOL')
    TAB = Word('\t', expression='"\t"', name='TAB')
    QUOTE = Word('"', expression='"\\""', name='QUOTE')
    L_BRACKET = Word('[', expression='"["', name='L_BRACKET')
    R_BRACKET = Word(']', expression='"\\]"', name='R_BRACKET')
    L_BRACKET_DROP = Word('[', expression='"["', name='L_BRACKET_DROP')(toolset['drop'])
    R_BRACKET_DROP = Word(']', expression='"\\]"', name='R_BRACKET_DROP')(toolset['drop'])
    ...
    SOURCE_TAG_START = Word('<source', expression='"<source"', name='SOURCE_TAG_START')
    SOURCE_TAG_GT = Clone(GT, expression='GT', name='SOURCE_TAG_GT')
    SOURCE_TAG_ATTRIBUTE = Sequence([Repetition(SPACETABEOL, numMin=False, numMax=False, expression='SPACETABEOL*'), Word('lang=', expression='"lang="'), QUOTE, Repetition(Klass(u'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 ', expression='[a..zA..Z0..9 ]'), numMin=1, numMax=False, expression='[a..zA..Z0..9 ]+'), QUOTE], expression='SPACETABEOL* "lang=" QUOTE [a..zA..Z0..9 ]+ QUOTE', name='SOURCE_TAG_ATTRIBUTE')
    SOURCE_BEGIN = Sequence([SOURCE_TAG_START, Option(SOURCE_TAG_ATTRIBUTE, expression='SOURCE_TAG_ATTRIBUTE?'), Repetition(SPACETABEOL, numMin=False, numMax=False, expression='SPACETABEOL*'), SOURCE_TAG_GT], expression='SOURCE_TAG_START SOURCE_TAG_ATTRIBUTE? SPACETABEOL* SOURCE_TAG_GT', name='SOURCE_BEGIN')
    SOURCE_END = Word('</source>', expression='"</source>"', name='SOURCE_END')
    ...
    esc_char_drop = Choice([L_BRACKET_DROP, R_BRACKET_DROP, PIPE, L_BRACE, R_BRACE, LT, GT, AMP, SEMICOLON], expression='L_BRACKET_DROP/R_BRACKET_DROP/PIPE/L_BRACE/R_BRACE/LT/GT/AMP/SEMICOLON', name='esc_char_drop')
    raw_char = Sequence([NextNot(esc_char_drop, expression='!esc_char_drop'), any_char], expression='!esc_char_drop any_char', name='raw_char')
    ...
    value_content = Repetition(Choice([inline, Sequence([NextNot(Sequence([Repetition(SPACETABEOL, numMin=False, numMax=False, expression='SPACETABEOL*'), Choice([TEMPLATE_END, PIPE], expression='TEMPLATE_END / PIPE')], expression='SPACETABEOL* (TEMPLATE_END / PIPE)'), expression='!(SPACETABEOL* (TEMPLATE_END / PIPE))'), Choice([any_char, EOL_KEEP], expression='any_char / EOL_KEEP')], expression='!(SPACETABEOL* (TEMPLATE_END / PIPE)) (any_char / EOL_KEEP)')], expression='inline / (!(SPACETABEOL* (TEMPLATE_END / PIPE)) (any_char / EOL_KEEP))'), numMin=False, numMax=False, expression='(inline / (!(SPACETABEOL* (TEMPLATE_END / PIPE)) (any_char / EOL_KEEP)))*', name='value_content')(toolset['keep'])
    ...
    parameter_name = Repetition(Sequence([NextNot(Choice([esc_char_drop, parameter_equal], expression='esc_char_drop/parameter_equal'), expression='!(esc_char_drop/parameter_equal)'), raw_char], expression='!(esc_char_drop/parameter_equal) raw_char'), numMin=1, numMax=False, expression='(!(esc_char_drop/parameter_equal) raw_char)+', name='parameter_name')(toolset['join'])
    ...
    template = Sequence([TEMPLATE_BEGIN, Repetition(SPACETABEOL, numMin=False, numMax=False, expression='SPACETABEOL*'), page_name, parameters, Repetition(SPACETABEOL, numMin=False, numMax=False, expression='SPACETABEOL*'), TEMPLATE_END], expression='TEMPLATE_BEGIN SPACETABEOL* page_name parameters SPACETABEOL* TEMPLATE_END', name='template')(toolset['substitute_template'])
    ...
    source_text = Repetition(Sequence([NextNot(SOURCE_END, expression='!SOURCE_END'), Choice([TAB_KEEP, any_char, EOL_KEEP], expression='TAB_KEEP/any_char/EOL_KEEP')], expression='!SOURCE_END (TAB_KEEP/any_char/EOL_KEEP)'), numMin=False, numMax=False, expression='(!SOURCE_END (TAB_KEEP/any_char/EOL_KEEP))*', name='source_text')(toolset['join'])
    source = Sequence([SOURCE_BEGIN, source_text, SOURCE_END], expression='SOURCE_BEGIN source_text SOURCE_END', name='source')(toolset['restore'], toolset['liftValue'])
    
    # Text types
    
    styled_text = Choice([template, template_parameter, entity], expression='template / template_parameter / entity', name='styled_text')
    not_styled_text = Choice([html_comment, preformatted, nowiki, source], expression='html_comment / preformatted / nowiki / source', name='not_styled_text')
    allowed_char = Repetition(esc_char_drop, numMin=1, numMax=1, expression='esc_char_drop{1}', name='allowed_char')(toolset['restore'], toolset['liftValue'])
