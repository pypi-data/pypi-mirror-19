...
### title: wikitext ###
    ...
    inline_cell = Recursive(name='inline_cell')
    inline = Recursive(name='inline')
    clean_inline = Recursive(name='clean_inline')
    # Codes
    
    LF = Char('\n', expression="'\\x0a'", name='LF')(toolset['drop'])
    CR = Char('\r', expression="'\\x0d'", name='CR')(toolset['drop'])
    CRLF = Word('\r\n', expression='"\\x0d\\x0a"', name='CRLF')(toolset['drop'])
    EOL = Choice([CRLF, LF, CR], expression='CRLF / LF / CR', name='EOL')(toolset['drop'])
    ...
    HTTPS_DROP = Word('https://', expression='"https://"', name='HTTPS_DROP')(toolset['drop'])
    HTTP_DROP = Word('http://', expression='"http://"', name='HTTP_DROP')(toolset['drop'])
    FTP_DROP = Word('ftp://', expression='"ftp://"', name='FTP_DROP')(toolset['drop'])
    protocol_DROP = Choice([HTTPS, HTTP, FTP], expression='HTTPS / HTTP / FTP', name='protocol_DROP')(toolset['drop'])
    ...
    QUOTE_KEEP = Clone(QUOTE, expression='QUOTE', name='QUOTE_KEEP')(toolset['restore'])
    SOURCE_TAG_START = Word('<source', expression='"<source"', name='SOURCE_TAG_START')(toolset['drop'])
    SOURCE_TAG_GT = Clone(GT, expression='GT', name='SOURCE_TAG_GT')(toolset['drop'])
    SOURCE_LANG_NAME = Repetition(Klass(u'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 ', expression='[a..zA..Z0..9 ]'), numMin=1, numMax=False, expression='[a..zA..Z0..9 ]+', name='SOURCE_LANG_NAME')
    SOURCE_TAG_LANG = Sequence([Word('lang=', expression='"lang="'), Option(QUOTE_KEEP, expression='QUOTE_KEEP?'), SOURCE_LANG_NAME, Option(QUOTE_KEEP, expression='QUOTE_KEEP?')], expression='"lang=" QUOTE_KEEP? SOURCE_LANG_NAME QUOTE_KEEP?', name='SOURCE_TAG_LANG')
    SOURCE_TAG_ATTRIBUTE = Sequence([Repetition(SPACETABEOL, numMin=False, numMax=False, expression='SPACETABEOL*'), SOURCE_TAG_LANG], expression='SPACETABEOL* SOURCE_TAG_LANG', name='SOURCE_TAG_ATTRIBUTE')(toolset['liftNode'])
    SOURCE_BEGIN = Sequence([SOURCE_TAG_START, Option(SOURCE_TAG_ATTRIBUTE, expression='SOURCE_TAG_ATTRIBUTE?'), Repetition(SPACETABEOL, numMin=False, numMax=False, expression='SPACETABEOL*'), SOURCE_TAG_GT, Option(Sequence([Repetition(SPACETAB, numMin=False, numMax=False, expression='SPACETAB*'), EOL], expression='SPACETAB* EOL'), expression='(SPACETAB* EOL)?')], expression='SOURCE_TAG_START SOURCE_TAG_ATTRIBUTE? SPACETABEOL* SOURCE_TAG_GT (SPACETAB* EOL)?', name='SOURCE_BEGIN')(toolset['render_source_open'])
    SOURCE_END = Word('</source>', expression='"</source>"', name='SOURCE_END')(toolset['drop'])
    HR_OPTIONS = Choice([Word('<hr />', expression='"<hr />"'), Word('<hr>', expression='"<hr>"'), Word('<hr/>', expression='"<hr/>"'), Word('</hr>', expression='"</hr>"')], expression='"<hr />" / "<hr>" / "<hr/>" / "</hr>"', name='HR_OPTIONS')(toolset['drop'])
    HR = Sequence([Choice([Word('<hr />', expression='"<hr />"'), Word('<hr>', expression='"<hr>"'), Word('<hr/>', expression='"<hr/>"'), Word('</hr>', expression='"</hr>"')], expression='"<hr />" / "<hr>" / "<hr/>" / "</hr>"'), Repetition(SPACE, numMin=False, numMax=False, expression='SPACE*')], expression='("<hr />" / "<hr>" / "<hr/>" / "</hr>") SPACE*', name='HR')(toolset['render_hr'])
    _SPECIAL_TAG = Choice([NOWIKI_BEGIN, NOWIKI_END, PRE_BEGIN, PRE_END], expression='NOWIKI_BEGIN/NOWIKI_END/PRE_BEGIN/PRE_END', name='_SPECIAL_TAG')
    SPECIAL_TAG = Choice([_SPECIAL_TAG, SOURCE_BEGIN, SOURCE_END, HR], expression='_SPECIAL_TAG/SOURCE_BEGIN/SOURCE_END/HR', name='SPECIAL_TAG')
    
    # Characters
    
    
    _ESC_CHAR = Choice([L_BRACKET, R_BRACKET, PIPE, L_BRACE, R_BRACE, SLASH, AMP, SEMICOLON, TAB, protocol_DROP], expression='L_BRACKET/R_BRACKET/PIPE/L_BRACE/R_BRACE/SLASH/AMP/SEMICOLON/TAB/protocol_DROP', name='_ESC_CHAR')
    ESC_CHAR = Choice([_ESC_CHAR, LT, GT], expression='_ESC_CHAR/LT/GT', name='ESC_CHAR')
    ...
    tag = Sequence([NextNot(SPECIAL_TAG, expression='!SPECIAL_TAG'), Choice([tag_autoclose, tag_open, tag_close], expression='tag_autoclose / tag_open / tag_close')], expression='!SPECIAL_TAG (tag_autoclose / tag_open / tag_close)', name='tag')(toolset['liftNode'])
    ...
    TAB_KEEP = Clone(TAB, expression='TAB', name='TAB_KEEP')(toolset['restore'])
        # Preformatted acts like nowiki (disables wikitext parsing) except it cannot be inline
    tab_to_2_spaces = Clone(TAB, expression='TAB', name='tab_to_2_spaces')(toolset['replace_by_2_spaces'])
    pre_text = Repetition(Choice([tab_to_2_spaces, Sequence([NextNot(PRE_END, expression='!PRE_END'), Choice([any_char, EOL_KEEP], expression='any_char/EOL_KEEP')], expression='!PRE_END (any_char/EOL_KEEP)')], expression='tab_to_2_spaces / (!PRE_END (any_char/EOL_KEEP))'), numMin=False, numMax=False, expression='(tab_to_2_spaces / (!PRE_END (any_char/EOL_KEEP)))*', name='pre_text')(toolset['join'])
    preformatted = Sequence([PRE_BEGIN, pre_text, PRE_END, Repetition(SPACE, numMin=False, numMax=False, expression='SPACE*')], expression='PRE_BEGIN pre_text PRE_END SPACE*', name='preformatted')(toolset['liftValue'], toolset['render_preformatted'])
        # We allow any char without parsing them as long as the tag is not closed
    ...
    sourcetext = Repetition(Sequence([NextNot(SOURCE_END, expression='!SOURCE_END'), Choice([Klass(u' !"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~', expression='[\\x20..\\xff]'), TAB_KEEP, EOL_KEEP], expression='[\\x20..\\xff] / TAB_KEEP /EOL_KEEP')], expression='!SOURCE_END ([\\x20..\\xff] / TAB_KEEP /EOL_KEEP)'), numMin=False, numMax=False, expression='(!SOURCE_END ([\\x20..\\xff] / TAB_KEEP /EOL_KEEP))*', name='sourcetext')(toolset['join'], toolset['render_source_text'])
    source = Sequence([SOURCE_BEGIN, sourcetext, SOURCE_END, Repetition(SPACE, numMin=False, numMax=False, expression='SPACE*')], expression='SOURCE_BEGIN sourcetext SOURCE_END SPACE*', name='source')(toolset['liftValue'], toolset['render_source'])
    
    # Text types
    
    inline_end = Choice([source, preformatted, HR], expression='source / preformatted / HR', name='inline_end')
    ...
    allowed_text = Sequence([NextNot(inline_end, expression='!inline_end'), Choice([raw_text, LT, GT, tab_to_space, allowed_char], expression='raw_text / LT / GT / tab_to_space / allowed_char')], expression='!inline_end (raw_text / LT / GT / tab_to_space / allowed_char)', name='allowed_text')(toolset['liftNode'])
    allowed_text_in_cell = Sequence([NextNot(Choice([PIPE, BANG], expression='PIPE/BANG'), expression='!(PIPE/BANG)'), allowed_text], expression='!(PIPE/BANG) allowed_text', name='allowed_text_in_cell')(toolset['liftNode'])
    clean_inline **= Repetition(Choice([nowiki, source, styled_text, raw_text], expression='nowiki / source / styled_text / raw_text'), numMin=1, numMax=False, expression='(nowiki / source / styled_text / raw_text)+', name='clean_inline')
    inline **= Repetition(Choice([nowiki, source, styled_text, allowed_text], expression='nowiki / source / styled_text / allowed_text'), numMin=1, numMax=False, expression='(nowiki / source / styled_text / allowed_text)+', name='inline')
    inline_cell **= Repetition(Choice([nowiki, source, styled_text, allowed_text_in_cell], expression='nowiki / source / styled_text / allowed_text_in_cell'), numMin=1, numMax=False, expression='(nowiki / source / styled_text / allowed_text_in_cell)+', name='inline_cell')
    ...
    paragraph_inline = Sequence([NextNot(inline_end, expression='!inline_end'), inline], expression='!inline_end inline', name='paragraph_inline')(toolset['liftNode'])
    paragraph_line = Sequence([NextNot(Choice([special_line_begin, inline_end], expression='special_line_begin / inline_end'), expression='!(special_line_begin / inline_end)'), paragraph_inline, Option(EOL, expression='EOL?')], expression='!(special_line_begin / inline_end) paragraph_inline EOL?', name='paragraph_line')(toolset['liftValue'])
    ...
    paragraphs = Repetition(Choice([blank_paragraph, paragraph], expression='blank_paragraph/paragraph'), numMin=1, numMax=False, expression='(blank_paragraph/paragraph)+', name='paragraphs')
    ...
    preformatted_inline = Repetition(Choice([tab_to_8_spaces, source, nowiki, styled_text, any_char_but_tab], expression='tab_to_8_spaces / source / nowiki / styled_text / any_char_but_tab'), numMin=1, numMax=False, expression='(tab_to_8_spaces / source / nowiki / styled_text / any_char_but_tab)+', name='preformatted_inline')
    ...
    preformatted_lines = Repetition(preformatted_line, numMin=1, numMax=False, expression='preformatted_line+', name='preformatted_lines')(toolset['render_preformatted'])
    ...
    horizontal_rule = Sequence([Repetition(DASH, numMin=4, numMax=4, expression='DASH{4}'), Repetition(DASH, numMin=False, numMax=False, expression='DASH*'), Repetition(inline, numMin=False, numMax=False, expression='inline*'), EOL], expression='DASH{4} DASH* inline* EOL', name='horizontal_rule')(toolset['liftValue'], toolset['keep'], toolset['render_hr'])
        ...
        # This should never happen
    invalid_line = Sequence([any_text, EOL], expression='any_text EOL', name='invalid_line')(toolset['liftValue'], toolset['render_invalid'])
    ...
    table_wikitext = Choice([list, horizontal_rule, preformatted_lines, title, table_structure], expression='list/horizontal_rule/preformatted_lines/title/table_structure', name='table_wikitext')
    table_inline = Sequence([NextNot(Choice([PIPE, BANG], expression='PIPE/BANG'), expression='!(PIPE/BANG)'), inline_cell, Option(EOL, expression='EOL?')], expression='!(PIPE/BANG) inline_cell EOL?', name='table_inline')(toolset['liftValue'])
    ...
    table_multiline_content = Repetition(Choice([table_paragraph, source, preformatted, table_wikitext, EOL], expression='table_paragraph / source / preformatted / table_wikitext / EOL'), numMin=False, numMax=False, expression='(table_paragraph / source / preformatted / table_wikitext / EOL)*', name='table_multiline_content')
    ...
    valid_syntax = Choice([list, horizontal_rule, preformatted_lines, title, table, source, preformatted, HR, EOL, paragraphs], expression='list/horizontal_rule/preformatted_lines/title/table/source/preformatted/HR/EOL/paragraphs', name='valid_syntax')
