"""
The setup_py tool provides the ``sdist`` and ``lambda-package`` commands.
"""
...
@mtool.add_arg_parser.register(SetupPy)
def add_arg_parser(tool, parser):
    parser.help = "setup.py tool"
    parser.description = "Invoke helpers that use setup.py"
