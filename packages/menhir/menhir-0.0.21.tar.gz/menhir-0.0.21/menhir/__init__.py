...
def apply_tool(dependencies, dirs, roots, tool, args, **kwargs):
    ...
    def post_f(state, path, child_states):
        ...
        if not state.get('failed') and path not in state['seen']:
            res = apply_tool_to_path(path, info, tool, args, **kwargs)
            if res['status'] == 'fail':
...
def apply_tool_to_path(path, info, tool, args, **kwargs):
    import menhir.tool as mtool
    if tool not in set(info['tools']):
        log.debug('Tool %s not used in %s', tool, path)
        return NOTHING_TO_DO
    res = mtool.execute_build_phase(
        tool, path, info, args, **kwargs
    )
    log.debug('Execute tool %s in %s returns %s', mtool.name(tool), path, res)
    if res['status'] == 'fail':
        log.debug('Tool %s in %s failed %s', mtool.name(tool), path, res)
