...
@mtool.name.register(DynamoDB)
def name(arg):
    return "dynamodb"


@mtool.is_using_tool.register(DynamoDB)
def is_using_tool(tool, path):
    from os.path import exists, join
    path = join(path, 'menhir-config.yaml')  # TODO: fix this hack
    return exists(path)


@mtool.dependencies.register(DynamoDB)
def dependencies(tool, path):
    return []


@mtool.add_arg_parser.register(DynamoDB)
def add_arg_parser(tool, parser):
    parser.description = "Commands to upload JSON to dynamodb table."
...
@mtool.execute_build_phase.register(DynamoDB)
...
def task_put(path, info, args):
    if not (info.get('changed') or info.get('changed_dependents')):
        return OK
...
def put(path, info, target, args, spec):
    ...
    table_name = spec.get('table', 'cpnfig.%(project)s.json')
