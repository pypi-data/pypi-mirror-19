...
@mtool.name.register(Git)
def name(arg):
    return "git"


@mtool.is_using_tool.register(Git)
...
@mtool.dependencies.register(Git)
def dependencies(tool, path):
    return []


@mtool.add_arg_parser.register(Git)
def add_arg_parser(tool, parser):
    parser.description = "git utilities"
...
@mtool.execute_build_phase.register(Git)
def execute_build_phase(tool, path, info, args, **kwargs):
    """Execute git task."""
    return task(path, info, args, **kwargs)


@multi
def task(path, info, args, **kwargs):
    """Execute a task."""
    return args.phase


@method(task, 'install-hooks')
def install_hooks(path, info, args, **kwargs):
    ...
    hook_dir_name = info['config'].get('git', {}).get('hook-dir', "git-hooks")
...
def with_stashed_changes(path, info, args, **kwargs):
    from menhir import gitutils
    from menhir.tool_utils import call, working_dir
