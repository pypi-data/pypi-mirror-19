...
def action(action, path, info, config, args):
    ...
    values_and_args = {
        'target': directory,
        'project': project_name_for_dir(path),
    }
...
def remote_state(remote_config, project, values):
    ...
    return remote_state_backend(backend, backend_config, project, values)


@multi
def remote_state_backend(backend, backend_config, project, values):
    """Configure a remote backend."""
    return backend


@method(remote_state_backend, 's3')
def remote_state_s3(backend, backend_config, project, values):
    from os import getenv
...
def task_install(path, info, config, args):
    ...
    url = "https://releases.hashicorp.com/terraform/%s/%s" % (
        version,
        file_name
    )
