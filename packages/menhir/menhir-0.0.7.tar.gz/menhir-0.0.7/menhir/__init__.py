...
def apply_phase(dependencies, dirs, roots, phase_name, args, **kwargs):
    ...
    def post_f(state, path, child_states):
        ...
        if not state.get('failed') and path not in state['seen']:
            res = apply_phase_to_path(path, info, phase_name, args, **kwargs)
            if res['status'] == 'fail':
...
def apply_phase_to_path(path, info, phase_name, args, **kwargs):
    import menhir.tool as mtool
    for tool in info['tools']:
        res = mtool.execute_build_phase(
            tool, path, info, phase_name, args, **kwargs
        )
