...
- ``.expand_keywords`` enables CVS keyword expansion (replacement of ``$Name: 0.7.0 $``
  and similar constructs). Note that it works differently than keyword extension, replacements are commited (so will always be present in the checked out code).
...
Tags not matching the pattern and local tags (rarely used Mercurial
feature) are also ignored.
...
will be scanned for CVS keywords like ``$Name: 0.7.0 $`` (or ``$Name: value
$``), ``$Revision: 9754f628932a $``, etc, and those will be appropriately
updated. This change will be commited, making those changes permanent
(until they are replaced by newer tag).

This is an alternative to standard keyword extension, which updates
those keywords on update (and makes various problems with merges,
...
to get proper ``$Name: 0.7.0 $``, everything else (``$Revision: 9754f628932a $``, ``$Header: README.txt,v 9754f628932a 2017/01/09 00:12:41 Marcin Exp $``
