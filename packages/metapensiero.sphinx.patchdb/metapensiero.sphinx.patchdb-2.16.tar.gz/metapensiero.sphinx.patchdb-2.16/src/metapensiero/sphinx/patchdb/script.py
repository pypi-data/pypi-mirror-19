...
def add_scripts_crossrefs(app, doctree, fromdocname):
    ...
    for node in doctree.traverse(Script):
        ...
        try:
            add_detail(dlist, _('Depends on'), patch.depends)
            add_detail(dlist, _('Preceeds'), patch.preceeds)
            add_detail(dlist, _('Brings'), patch.brings)
...
class PatchDBDomain(Domain):
    ...
    def resolve_xref(self, env, fromdocname, builder, type, target, node, contnode):
        ...
        if script:
            ...
            env.warn_node(_('Reference to an unknown script: %(scriptid)r')
                          % dict(scriptid=ref), node)
