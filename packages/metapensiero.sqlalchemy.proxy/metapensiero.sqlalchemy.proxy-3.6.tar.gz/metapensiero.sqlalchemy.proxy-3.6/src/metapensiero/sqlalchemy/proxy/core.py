...
from .utils import apply_filters, get_adaptor_for_type
...
class ProxiedQuery(ProxiedBase):
    ...
    def filterQueryWithArgs(self, session, conditions, args):
        ...
        The query gets then massaged by :py:func:`.utils.apply_filters()`,
        further filtered and modified as specified by the `args` dictionary.
    ...
    def getMetadata(self, query, countslot, resultslot, successslot):
        ...
        result = super(ProxiedQuery, self).getMetadata(query,
        ...
        if pk and pkl == len(pk):
            ...
            log.warning("Could not determine primary key fields"
                        " for query %s", query)
