...
from .helpers import (convert_date_string, convert_time_string, get_activity_name,
                      get_config, get_logger)
from .proxy import RedmineProxy, ValidationError
...
def mine_time(all, date, batch, timelog, silent, report, debug, input):
    ...
    if report:
        return print_project_report(proxy, report)
    if batch:
...
def print_project_report(proxy, pid):
    click.clear()
    try:
    ...
    period_times = []
    username = lambda item: item['user']['name']
    for iv in p.versions:
        period_time_entries = [t for t in p.time_entries if day_between(datetime.date(iv.created_on), iv.due_date, t.spent_on)]
        ...
        si = [s for s in p.issues if (s.tracker.id == 5 and hasattr(s, 'fixed_version') and (s.fixed_version.id == iv.id) )]
        period_times.append((iv,
                             total_period,
                             period_time_entries,
    ...
    p2 = 'Total Worked Hours: {}'.format('TODO')
    pline = space_line(p1, p2) + '\n'

    click.echo(sep)
    click.echo(pline)
    click.echo(sep)
    sort_on_due_date = lambda item: item[0]['due_date']
    period_times.sort(key=sort_on_due_date)
    for vperiod, period_total, period_entries, periodi, scenarios in period_times:

        v1 = u'V: {} ({})'.format(vperiod.name, vperiod.status)
        v2 = 'Period From {} To {}'.format(vperiod.created_on.date(), vperiod.due_date)
        ...
        for s in scenarios:
            ...
            click.secho(echostr, fg='white', bg=scolor[s.status.id])
    ...
    for k, v in list(D.items()):
        if len(v) == 1:
            ...
            for timelog in v:
                ...
                mcomments += (timelog.comments.strip() + '|')
...
def get_batch_timelogs(logger, batch, input, activity_id, date, all):
    ...
    for line in input:
        ...
        # reset day
            ...
            try:
                raw_issue_id, description = record_issue.split(' ', 1)
                t = Timelog(issue_id=int(raw_issue_id), hours=hours, spent_on=record_date,
                            activity_id=activity_id, comments=description)
                timelogs.append(t)
            except ValueError:
                # add to drop list
