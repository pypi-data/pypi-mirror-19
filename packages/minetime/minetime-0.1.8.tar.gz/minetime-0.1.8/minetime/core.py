...
def print_project_report(p, data):
    ...
    for vperiod, period_total, period_entries, periodi, \
        ...
        if unestimated_issues:
            click.echo('Issues with no estimated_hours:')
            click.echo(tabulate(unestimated_issues))
        if new_issues_worked:
            click.echo('New Issues with time entries:')
            click.echo(tabulate(new_issues_worked))
        click.echo(sep)
