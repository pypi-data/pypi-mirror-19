...
def prompt_valid_project(pid, proxy):
...
def project_report(config, proxy, logger):
    ...
    p = prompt_valid_project(pid, proxy)
    if not p:
        return False
...
def print_project_report(p, data):
    ...
    scenario = lambda item: item['issue']['id'] if hasattr(item, 'issue') and hasattr(item['issue'], 'id') else '-'
    click.clear()
    sep = '-' * 80
    ...
    for vperiod, period_time_entries, scenarios, unestimated_issues, \
        ...
        time_by_user = groupby(period_time_entries, username)
        for key, group in time_by_user:
            onesum = sum([item["hours"] for item in group])
            click.echo(u'{} : {}'.format(key, onesum))

        click.echo('')
        period_time_entries.sort(key=scenario)
        time_by_scenario = groupby(period_time_entries, scenario)
        for key, group in time_by_scenario:
            onesum = sum([item["hours"] for item in group])
            click.echo(u'[{} : {}] '.format(key, onesum))
        click.echo('')
        click.echo('')
