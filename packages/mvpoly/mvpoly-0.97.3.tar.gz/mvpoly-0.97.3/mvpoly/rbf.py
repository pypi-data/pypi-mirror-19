...
class RBFBase(object):
    ...
    *args : arrays
    ...
    @cached_property
    def kdtree(self):
        return scipy.spatial.KDTree(self.xi.T)
    ...
    def _poly_matrix(self):
        ...
        if not self.poly_basis:
            self.poly_basis = self.poly_class.monomials(self.dim,
                                                        self.poly_order,
                                                        dtype=np.float64)
        K = np.vstack(p(*[c for c in self.xi]) for p in self.poly_basis)
        return K
...
class RBFDense(RBFBase):
    ...
    def rbf(self, *args, **kwargs):
        ...
        if large:
            L = [np.dot(self.radial(RBFDense.norm(m[:, np.newaxis], self.xi)),
                        self.rbf_coefs)
                 for m in x.T]
...
class RBFSparse(RBFBase):
    ...
    def _sparse_radial_matrix(self, xtree):
        # What we would like to do here is call sparse_distance_matrix
        # and apply the radial function to that, it is created as 'coo'
        # but converted to 'dok' on return, so removes all the zeros which
        # occur for centre-interpolation coincidence. Grrr, isuse raised
        #
        #   https://github.com/scipy/scipy/issues/5563
        #
        # but until that is addressed we use this dirty hack

        # get distance matrix with coincident points missing
        D = self.kdtree.sparse_distance_matrix(xtree, self.radius)

        # apply the radial function to it
        K = zip(D.keys(), self.radial(np.array(list(D.values()))))
        D.update(K)

        # get the coincident points, then add the radial values
        # for those
        Z = self.kdtree.query_ball_tree(xtree, sys.float_info.min)
        Z0 = self.radial(0)
        K = [((i, j), Z0) for i in range(self.N) for j in Z[i]]
        D.update(K)

        # transpose of 'dok' is really expensive, 'coo' much cheaper
        return D.tocoo().transpose()
    ...
    def _rbf_matrix(self):
        ...
        if self.smooth != 0.0:
            if self.sign > 0 :
                ...
                which = 'SA'
            eig = scipy.sparse.linalg.eigsh(A,
                                            k=1,
                                            which=which,
                                            maxiter=1000,
                                            return_eigenvectors=False)[0]
            A = A + scipy.sparse.identity(self.N,
                                          format='dok') * self.smooth * eig
        return A
    ...
    def _subclass_coefficients(self):
        ...
        if K is None:
            ...
            A = scipy.sparse.vstack((scipy.sparse.hstack((A, K.T)),
                                     scipy.sparse.hstack((K, Z))))
    ...
    def rbf(self, *args, **kwargs):
        ...
        if not all(arg.shape == shp for arg in args):
            raise ValueError('Array lengths must be equal')
        x = np.asarray([a.flatten() for a in args], dtype=np.float64)
        xtree = scipy.spatial.KDTree(x.T)
