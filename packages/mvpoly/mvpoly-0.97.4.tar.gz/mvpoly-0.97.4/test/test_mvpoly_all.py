...
class TestMVPolyInit(unittest.TestCase):
    ...
    def test_default_dtype(self):
        ...
        for C in classes:
            x, y = C.variables(2)
            self.assertTrue(x.dtype == def_dtype, "variable")
            p = C.zero()
        ...
        for C in classes:
            ...
            for r in [pi+pf, pf+pi, pf+pf, 1+pf, pf+1,
                ...
                self.assertTrue(r.dtype == float,
                                "{0!s} {1!s} add".format(C.__name__, repr(r.dtype)))
    ...
    def test_dtype_inherit_minus(self):
        for C in classes:
            ...
            for r in [pi-pf, pf-pi, pf-pf, 1-pf, pf-1,
                      1.0-pf, pf-0.1, 1.0-pi, pi-1.0]:
                self.assertTrue(r.dtype == float,
                                "{0!s} {1!s} minus".format(C.__name__, repr(r.dtype)))
            for r in [pi-pi, pi-1, 1-pi]:
                self.assertTrue(r.dtype == int,
                                "{0!s} {1!s} minus".format(C.__name__, repr(r.dtype)))
    ...
    def test_dtype_inherit_mul(self):
        for C in classes:
            ...
            for r in [pi*pf, pf*pi, pf*pf, 1*pf, pf*1,
                      1.0*pf, pf*0.1, 1.0*pi, pi*1.0]:
                self.assertTrue(r.dtype == float,
                                "{0!s} {1!s} mul".format(C.__name__, repr(r.dtype)))
            for r in [pi*pi, pi*1, 1*pi]:
                self.assertTrue(r.dtype == int,
                                "{0!s} {1!s} muk".format(C.__name__, repr(r.dtype)))

    def test_dtype_inherit_scalar(self):
        for C in classes:
            x, = C.variables(1, dtype=np.int32)
            v = np.int16(99)
            froo = x.__rmul__(v)
...
class TestMVPolyGetitem(unittest.TestCase):
    ...
    def test_getitem_numargs_bivar(self):
        for C in classes:
            x, y = C.variables(2, dtype=np.int32)
            p = 4 + 3*y + 2*x**2
...
class TestMVPolySetitem(unittest.TestCase):
    ...
    def test_setitem_univariate_zero(self):
        for C in classes:
            p = C.zero(dtype=int)
            q = C.one(dtype=int)
            q[0] = 0
            self.assertTrue(p == q,
                            "setitem univariate zero {0!s}".format(C))

    def test_setitem_univariate_one(self):
        for C in classes:
            p = C.zero(dtype=int)
            q = C.one(dtype=int)
            p[0] = 1
            self.assertTrue(p == q,
                            "setitem univariate one {0!s}".format(C))
    ...
    def test_setitem_constant(self):
        for C in classes:
            ...
            p[0, 0, 0] = 3
            q = 3 * C.one(dtype=int)
            self.assertTrue(p == q, "setitem constant")

    def test_setitem_variable(self):
        for C in classes:
            for dtype in [np.int32, np.float]:
                x, y, z = C.variables(3, dtype=dtype)
                q = 2 * x**3 * y**2 * z**4
...
class TestMVPolyEval(unittest.TestCase):
    ...
    def test_eval_zero_scalar(self):
        for C in classes:
            p = C.zero()
            self.assertTrue(p(2) == 0, "zero scalar")

    def test_eval_zero_scalars(self):
        for C in classes:
            p = C.zero()
            self.assertTrue(p(2, 4) == 0, "zero scalars")

    def test_eval_zero_array(self):
        a = np.array([[4, 5]])
        b = np.array([[0, 0]])
        for C in classes:
            p = C.zero()
            self.assertTrue(np.array_equal(p(a), b),
                            "{0!s}: {1!s} != {2!s}".format(C, p(a), b))

    def test_eval_one_scalar(self):
        for C in classes:
            p = 3 * C.one()
            self.assertTrue(p(2) == 3, "one scalar")

    def test_eval_one_scalars(self):
        for C in classes:
            p = 3 * C.one()
            self.assertTrue(p(2, 4, 7) == 3, "one scalars")

    def test_eval_one_array(self):
        a = np.array([[4, 5]])
        b = np.array([[3, 3]])
        for C in classes:
            p = 3 * C.one()
    ...
    def test_eval_complex(self):
        for C in classes:
            x, y = C.variables(2, dtype=complex)
            p = x**2 - y
    ...
    def test_diff_dtype(self):
        for C in classes:
            for dt in (np.int32, np.float64):
                ...
                self.assertTrue(exp == obt,
                                "{0!s} bad datatype {1!s} {2!s}".format(C, repr(obt), repr(exp)))
...
class TestMVPolyInt(unittest.TestCase):
    ...
    def test_diff_0d(self):
        for C in classes:
            x, = C.variables(1, dtype=np.double)
    ...
    def test_int_2d_x(self):
        for C in classes:
            x, y = C.variables(2, dtype=np.double)
            p = 3*x**2 + 4*x*y + 3*y**2
    ...
    def test_int_invariant(self):
        for C in classes:
            ...
            self.assertTrue(p0 == p1,
                            "ingegration side-effect {0!s} {1!s}".format(repr(p0), repr(p1)))
    ...
    def test_int_dtype(self):
        for C in classes:
            for dt in [np.float32, np.float64, np.complex]:
                ...
                obt = q.dtype
                self.assertTrue(exp == obt,
                                "bad datatype {0!s} {1!s}".format(repr(obt), repr(exp)))
    ...
    def test_int_integer_warning(self):
        for C in classes:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                p.int(1, 1)
                self.assertTrue(len(w), "expected 1 warning, got {0:d}".format((len(w))))
                wcat = w[0].category
                self.assertTrue(issubclass(wcat, RuntimeWarning),
                                "expected RuntimeWarning, got {0!s}".format((wcat)))
    ...
    def test_intd_1d(self):
        for C in classes:
            ...
            for interval in [ [0, 1],
                ...
                obt = p.intd(interval)
                self.assertTrue(exp == obt,
                                "bad integral {0!s}".format((repr(obt))))
...
class TestMVPolyMaxmodND(unittest.TestCase):
    ...
    def test_maxmodnd_bivariate(self):
        ...
        for C in classes:
            ...
            obt = p.maxmodnd(epsilon=eps)[0]
            self.assertTrue(np.abs(exp - obt) < eps*exp,
                            "bad 2d maxmodnd {0!s}".format((repr(obt))))
    ...
    def test_maxmodnd_kaijser_varopoulos(self):
        ...
        eps = 1e-5
        for C in classes:
            x, y, z = C.variables(3, dtype=complex)
            p = x**2 + y**2 + z**2 - 2*x*y - 2*x*z - 2*y*z
            exp = 5
            obt = p.maxmodnd(epsilon=eps)[0]
            self.assertTrue(np.abs(exp - obt) < eps*exp,
                            "bad kaijser-varopoulos maxmodnd {0!s}".format((repr(obt))))

    def test_maxmodnd_gupta_ray(self):
        ...
        # z = (z1, z2, ...) and zi = +/-1.
        #
        # We generate a random 3x3 matrix A, form A^T A (which is
        ...
        for C in classes:
            ...
            S = A.T * A
            p = mvpoly.cube.MVPolyCube(S, dtype=complex).asclass(C)
            expected = 0
            for ztup in itertools.product(*((-1, 1) for _ in range(n))):
                ...
                if zSz > expected:
                    expected = zSz
            obt = p.maxmodnd(epsilon=eps)[0]
            self.assertTrue(np.abs(obt - expected) < eps*expected,
                            "Gupta-Ray maxmod: {0!s} {1!s}".format((repr(obt)), repr(expected)))
...
class TestMVPolyNormLp(unittest.TestCase):
    ...
    def test_norm_Lp_univariate(self):
        for C in classes:
            x, = C.variables(1, dtype=np.complex)
            p = x + 1
    ...
    def test_norm_Lp_bivariate(self):
        for C in classes:
            x, y = C.variables(2, dtype=np.complex)
            p = x + y + 1
    ...
    def test_norm_Lp_trivariate(self):
        for C in classes:
            x, y, z = C.variables(3, dtype=np.complex)
            p = x + y + z + 1
...
class TestMVPolyMonomials(unittest.TestCase):

    def test_monomials_variable_1(self):
        for C in classes:
            one = C.one()
            x, = C.variables(1)
            self.assertTrue(C.monomials(1, 0) == [one])
            self.assertTrue(C.monomials(1, 1) == [one, x])
            self.assertTrue(C.monomials(1, 2) == [one, x, x*x])

    def test_monomials_variable_2(self):
        for C in classes:
            one = C.one()
            x, y = C.variables(2)
            self.assertTrue(C.monomials(2, 0) == [one])
            self.assertTrue(C.monomials(2, 1) == [one, x, y])
            self.assertTrue(C.monomials(2, 2) == [one, x, y, x*x, x*y, y*y])

    def test_monomials_dtype_default(self):
        for C in classes:
            for m in C.monomials(3, 2):
                self.assertTrue(m.dtype == np.float, "monomial default dtype")

    def test_monomials_dtype_user(self):
        for C in classes:
            for m in C.monomials(3, 2, dtype=np.complex):
                self.assertTrue(m.dtype == np.complex, "monomial user dtype")

    def test_monomials_bad_arg_order(self):
        for C in classes:
            with self.assertRaises(ValueError):
                C.monomials(0, 3)

    def test_monomials_bad_arg_degree(self):
        for C in classes:
            with self.assertRaises(ValueError):
                C.monomials(2, -1)
...
class TestMVPolyBellPartial(unittest.TestCase):

    def test_bell_partial_size(self):
        for C in classes:
            p = C.bell_partial(6, 2)
            self.assertTrue(p.degrees == (1, 1, 2, 1, 1), "bell_partial degrees")

    def test_bell_partial_coefs(self):
        for C in classes:
            x = C.variables(5)
            p = C.bell_partial(6, 2)
            q = 6*x[4]*x[0] + 15*x[3]*x[1] + 10*x[2]**2
            self.assertTrue(p == q, "bell_partial explicit")

    def test_bell_bad_args(self):
        for C in classes:
            with self.assertRaises(ValueError):
                C.bell_partial(2, 6)
            with self.assertRaises(ValueError):
...
class TestMVPolyBell(unittest.TestCase):

    def test_bell_size(self):
        for C in classes:
            p = C.bell(4)
            self.assertTrue(p.degrees == (4, 2, 1, 1), "bell degrees")

    def test_bell_coefs(self):
        for C in classes:
            x1, x2, x3, x4 = C.variables(4)
            p = C.bell(4)
            q = x1**4 + 6*x2*x1**2 + 4*x1*x3 + 3*x2**2 + x4
            self.assertTrue(p == q, "bell explicit")

    def test_bell_args(self):
        for C in classes:
            with self.assertRaises(ValueError):
                C.bell(-1)
