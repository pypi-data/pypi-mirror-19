...
class TestMVPolyDictMonomials(unittest.TestCase):
...
class TestMVPolyDictDtype(unittest.TestCase):
    ...
    def test_construct_dtype_of_coef_explict(self):
        t = self.f[0, 2].dtype
        self.assertTrue(t == np.float32, "bad dtype: {0!s}".format(repr(t)))
        t = self.i[0, 2].dtype
        self.assertTrue(t == np.int64, "bad dtype: {0!s}".format(repr(t)))
    ...
    def test_eval_point(self):
        for i in range(self.n):
            ...
            self.assertTrue(expected == obtained,
                            "bad eval: {0!s} != {1!s}".format(repr(obtained),
                                                     repr(expected) ))
