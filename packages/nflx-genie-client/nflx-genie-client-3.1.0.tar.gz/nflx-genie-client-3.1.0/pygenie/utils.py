...
def call(url, method='get', headers=None, raise_not_status=None, none_on_404=False,
         auth_handler=None, failure_codes=None, attempts=4, backoff=3, *args, **kwargs):
    """
    ...
    for i in range(attempts):
        ...
        if i < attempts - 1 and (resp is None or resp.status_code not in failure_codes):
            msg = ''
            if resp is not None:
