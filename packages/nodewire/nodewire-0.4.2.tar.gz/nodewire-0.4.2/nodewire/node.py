...
class Node:
    def __init__(self):
        self.nw = NodeWire('MyNode', account='sadiq.a.ahmad@gmail.com', password='secret', process=self.process)
        self.nw.debug = True
        self.ports = []
    ...
    def process(self, msg):
        if msg.Command == 'get' or msg.Command == 'getportvalue':
            if msg.Params[0] == 'properties':
                p = filter(lambda pp: pp.name == msg.Params[1], self.ports)
                if len(p) != 0: self.nw.send(msg.Sender,'properties', p[0].name, p[0].properties())
                ...
                self.nw.send(msg.Sender, 'portvalue', msg.Params[0], v)
    ...
    def get(self, port):
        p = filter(lambda pp: pp.name==port, self.ports)
