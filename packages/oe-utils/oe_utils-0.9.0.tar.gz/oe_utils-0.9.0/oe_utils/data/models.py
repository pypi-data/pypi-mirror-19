...
class Wijziging(Base):
    ...
    def upgrade():
        op.create_table('wijzigingshistoriek',
                        ...
                        sa.Column('resource_object_json', JSON, nullable=False),
                        sa.Column('actie', sa.String(length=50), nullable=False),
                        sa.PrimaryKeyConstraint('versie', name='wijzigingshistoriek_pk')
        )

        op.execute('grant select, insert, update on wijzigingshistoriek to <user>_dml')
    ...
    resource_object_json = Column(MutableDict.as_mutable(JSON()), nullable=False)
