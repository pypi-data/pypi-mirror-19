    ...
    def save(self):
        ...
        try:
            ...
            if e.response.status_code == 400:
                ...
                msg = pretty_error_msg(err_json)
                if not msg:
                    msg = 'Validation error. Requires: {}'.format(', '.join([err.get('validationOf', {}).get('required') for err in err_json]))
...
# Error message pretty print helper
def pretty_error_msg(err_json):
    msg = '; '.join(err.get('message', '') for err in err_json)
    if not msg:
        try:
            required_fields = [err.get('validationOf', {}).get('required', '') for err in err_json]
            msg = 'Validation error. Required properties: {}'.format(', '.join(required_fields))
        except Exception:
            msg = 'Bad request.'
    return msg
