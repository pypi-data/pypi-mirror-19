...
from .packages.oraclebmc import exceptions, config, clients
...
def build_client(service_name, ctx):
    ...
    try:
        client = {
            ...
            "os": clients.ObjectStorageClient,
            "blockstorage": clients.BlockstorageClient,
            "compute": clients.ComputeClient,
            "virtual_network": clients.VirtualNetworkClient
...
def wrap_exceptions(func):
    ...
    def wrapped_call(ctx, *args, **kwargs):
        ...
        except (exceptions.ServiceError, exceptions.NetworkError) as exception:
            if ctx.obj["debug"]:
                raise
            tpl = "{exc}:\n{details}"
