...
from .packages.oraclebmc import exceptions, config
from .packages.oraclebmc.core import BlockstorageClient
from .packages.oraclebmc.core import ComputeClient
from .packages.oraclebmc.core import VirtualNetworkClient
from .packages.oraclebmc.identity import IdentityClient
from .packages.oraclebmc.object_storage import ObjectStorageClient
...
def build_client(service_name, ctx):
    ...
    try:
        ...
        }[service_name](client_config)

        if ctx.obj['endpoint']:
            client.base_client.endpoint = ctx.obj['endpoint']

        if ctx.obj['cert_bundle']:
            # TODO: Update this once alternate certs are exposed in the SDK, DEX-139.
...
def build_config(command_args):
    ...
    if command_args['endpoint']:
        ...
        # TODO: sort this out in the SDK with DEX-578.
...
def parse_json_parameter(parameter_name, parameter_value, default=None):
    ...
    try:
        ...
        sys.exit('Parameter {!r} must be in JSON format.'.format(parameter_name))
