...
###############################################################################
"""Kibana server
$Id: server.py 4581 2017-01-11 01:10:53Z roger.ineichen $
"""
...
class KibanaServer(object):
    ...
    def __init__(self, interface, hosts=['0.0.0.0:9200'], interval=5,
        timeout=4.0, maxQueueSize=MAX_QUEUE_SIZE, maxBatchSize=MAX_BATCH_SIZE,
        maxminddb=None, maxitems=MAX_GEO_DATA_ITEMS, debug=False, loglevel=0):
        ...
        if not hosts:
            self.exit("you must specify (elasticsearch) hosts")
        self._backend = p01.kibana.backend.ElasticSearchBackend(self,
            hosts=hosts, timeout=timeout, maxminddb=maxminddb,
            maxitems=maxitems)
