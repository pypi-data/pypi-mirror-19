...
class TestP4(TestP4Python):
   ...
   Other specs like depot or label require a parameter. Streams are particularily picky.
   ...
   created server, it will succeed if all is compiled correctly.
    ...
    def testUntaggedSpecs( self ):
        ...
        client = self.p4.fetch_client("testclient")
        client._description = "Client for testing specs"
        self.p4.save_client(client)
        client= self.p4.fetch_client("testclient")

        untagged = self.p4.fetch_client("testclient", tagged=False)
        untagged_direct = self.p4.run_client("-o", "testclient", tagged=False)[0]

        self.assertEqual(untagged, untagged_direct, "fetch and direct do not match")

        parsed = self.p4.parse_client(untagged)

        self.assertEqual(parsed, client, "parsed and fetched do not match")
