...
class CustomMessage(ValidationTestBase):
    ...
    def test_custom_negate_message(self):
        """
        Check that negated validations can have custom messages
        :return:
        """
        validator = InverseValidation(InRangeValidation(min=0), message=self.message)
        for error in validator.get_errors(pd.Series(
                [
                    1,
                    2,
