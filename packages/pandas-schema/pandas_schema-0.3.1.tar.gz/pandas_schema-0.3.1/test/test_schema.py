...
class TypeChecking(unittest.TestCase):
    """
    Tests that the Schema constructor fails when arguments of the wrong type are used
    """
    ...
    def test_bad_ordered(self):
        with self.assertRaises(BaseException):
            Schema([Column('')], ordered='a')

class UnorderedSchema(unittest.TestCase):
    """
    Tests that an unordered schema (ordered=False) validates correctly
    """
    schema = Schema([
        Column('a'),
...
class OrderedSchema(unittest.TestCase):
    """
    Tests that an ordered schema (ordered=True) validates correctly
    """
    schema = Schema([
