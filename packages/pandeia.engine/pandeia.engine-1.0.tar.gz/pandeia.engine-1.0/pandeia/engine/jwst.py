...
class MIRI(JWSTInstrument):
    ...
    def get_variance_fudge(self, wave):
        ...
        the quantum yield. The information posted in https://github.com/STScI-SSB/pandeia/issues/2167 suggests that
        they want the noise scaled by an extra factor of the quantum yield so that the SNR scales inversely
        with quantum yield. The MIRI team has been asked to provide the chromatic fudge factor they want as a separate
        reference file. Until that's delivered, we'll use the quantum yield squared to achieve the desired effect.
        """
        det_pars = self.get_detector_pars()
        scalar_fudge = det_pars.get('var_fudge', 1.0)
