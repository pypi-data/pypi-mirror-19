...
class ApiClient(object):
    ...
    def __call_api(self, resource_path, method,
        ...
        if post_params or files:
            params_and_files = self.prepare_post_parameters(post_params, files)
            post_params = params_and_files.get('params')
            post_files = params_and_files.get('files')
    ...
    def request(self, method, url, query_params=None, headers=None,
                post_params=None, body=None, _preload_content=True, _request_timeout=None, files=None):
        """
            ...
            for k, v in iteritems(files):
                ...
                for n in file_names:
                    with open(n, 'rb') as f:
                        ...
                        # only use the array notation for keys if there are multiple values
                        if len(file_names) == 1:
                            k = k.replace('[]', '')
                        files_prepared.append(tuple([k, tuple([filename, filedata, mimetype])]))

        return {'params': params, 'files': files_prepared}
    ...
    def __deserialize_file(self, response):
        ...
        with open(path, "w") as f:
            if isinstance(response.data, unicode):
                f.write(str(bytearray(response.data, 'utf8')))
            else:
