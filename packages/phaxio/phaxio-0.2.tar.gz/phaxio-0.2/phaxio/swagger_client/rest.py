...
class RESTClientObject(object):
    ...
    def request(self, method, url, query_params=None, headers=None,
                body=None, post_params=None, _preload_content=True, _request_timeout=None, files=None):
        """
            ...
            if method in ['POST', 'PUT', 'PATCH', 'OPTIONS', 'DELETE']:
                ...
                if re.search('json', headers['Content-Type'], re.IGNORECASE):
                    ...
                    r = self.pool_manager.request(method, url,
                                                  ...
                                                  timeout=timeout,
                                                  headers=headers)
                elif headers['Content-Type'] == 'multipart/form-data' and method == 'POST':
                    del headers['Content-Type']
                    logger.debug('params={}'.format(post_params))
                    logger.debug('files={}'.format(files))
                    resp = requests.post(url, data=post_params, files=files, headers=headers)
                    r = HTTPResponse(body=resp.text, headers=resp.headers, status=resp.status_code)
                elif headers['Content-Type'] == 'multipart/form-data':
                    # must del headers['Content-Type'], or the correct Content-Type FOOFOOFOOF
    ...
    def POST(self, url, headers=None, query_params=None, post_params=None, body=None, _preload_content=True,
             _request_timeout=None, files=None):
