...
def get_str_param_from_get(request,param_name,full_match=True,default=''):
    '从request.GET获得字符串参数'
    result = ''
    if full_match:
...
def remote_load_pic(request,url):
    ...
    try:
        ...
        f = open(file_info['real_file'],'rwb')
        f.write(data)
        ...
        file_type = filelib.get_file_type(f)
        if not file_type:
            file_type = ext
...
if __name__ == '__main__':
    ...
    print(remote_load_pic(request=None,url='http://s.news.bandao.cn/news_upload/201612/20161226064913_M0GNAIO9.jpg'))
