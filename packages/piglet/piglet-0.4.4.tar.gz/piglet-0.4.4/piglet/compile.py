...
from piglet.astutil import (add_arg_default,
                            add_kwarg,
                            add_locations,
                            astwalk,
                            get_comparison,
    ...
    def annotate_errors(*args, **kwargs):
        try:
            ...
            srcnode = next((item
                            ...
                            if isinstance(item, im.IntermediateNode)), None)
...
def compile_to_module(codeob, filename, c=count(), bootstrap=None):
    ...
    module_name = 'piglet_tmpl_{}_{}'.format(next(c),
...
class Compiler:
    ...
    def unique_id(self, prefix='tmp'):
        ...
        """
        return '__piglet_{}{}'.format(make_identifier(prefix),
                                       next(self.uniquec[prefix]))
    ...
    def _compile(self, srcnode, parent=None):
        ...
        try:
            ...
            reraise(cls,
                    SyntaxError(e.args[0] +
                                ' at template line {}, {}'
                                .format(srcnode.pos[0], self.filename)),
                    tb)
    ...
    def compile_interpolatenode(self, srcnode, parent, mode='yield'):
        ...
        if not srcnode.autoescape:
            parent.body.append(
                self.annotate_runtime_errors(Expr(value=Yield(value)),
                                             srcnode))
            return
    ...
    def compile_call(self, srcnode, parent):
        ...
        parent.body.append(
            self.annotate_runtime_errors(Expr(value=Yield(call)),
                                         srcnode))
    ...
    def _compile_function(self,
        ...
        :param descend: compile srcnode.children into the created function
                        node's body?
        """
        fn = FunctionDef(name=str(funcname),
                         args=arguments(args=[],
                                        defaults=[],
                                        vararg=None,
                                        kwonlyargs=[],
                         ...
                         body=[],
                         decorator_list=[
                             Attribute(value=Name(id='__piglet_rt', ctx=Load()),
                                       attr='flatten',
                                       ctx=Load())],
                         returns=None)
        set_pos(fn, srcnode)
