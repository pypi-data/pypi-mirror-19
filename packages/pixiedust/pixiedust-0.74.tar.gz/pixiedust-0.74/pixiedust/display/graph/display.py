    ...
    def doRender(self,handlerId):
        ...
        if ( handlerId == "nodeLinkGraph"):
            ...
            for r in g.vertices.map(lambda row: """"{0}":{{"id":"{0}","name":"{1}","latitude":{2},"longitude":{3}}}"""
                .format(row.id, row.name,0.0 if row.latitude is None else row.latitude,0.0 if row.longitude is None else row.longitude)).collect():
            ...
            myLogger.debug(self.renderTemplate("graphMap.js", graphNodesJson=graphNodesJson, graphLinksJson=graphLinksJson))
