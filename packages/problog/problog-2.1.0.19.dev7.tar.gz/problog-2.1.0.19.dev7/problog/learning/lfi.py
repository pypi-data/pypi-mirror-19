...
class LFIProblem(SemiringProbability, LogicProgram):
    ...
    def __init__(self, source, examples, max_iter=10000, min_improv=1e-10, verbose=0, knowledge=None,
                 leakprob=None, propagate_evidence=True, normalize=False, nprocesses=1, **extra):
        """
    ...
    def _evaluate_examples(self):
        ...
        examples = list(self._compiled_examples)
        if self._evaluator is None:
            self._evaluator = ExampleEvaluator(self._weights, examples)

        if self.pool is None:
            return map(self._evaluator, range(0, len(examples)))
        else:
            return list(self.pool.map(self._evaluator, range(0, len(examples))))
...
class ExampleSet(object):
    ...
    def add(self, index, atoms, values):
        # self._examples[(index, atoms, values)] = Example(index, atoms, values)
        # return
    ...
    def __getitem__(self, index):
        return self._examples.values()[index]
...
class ExampleEvaluator(SemiringProbability):
    ...
    def __init__(self, weights, examples):
        SemiringProbability.__init__(self)
    ...
    def __call__(self, example_number):
        """Evaluate the model with its current estimates for all examples."""
