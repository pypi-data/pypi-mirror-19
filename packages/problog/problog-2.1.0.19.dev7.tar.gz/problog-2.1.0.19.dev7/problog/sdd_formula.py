...
class SDD(DD):
    ...
    def _create_manager(self):
        return SDDManager(auto_gc=self.auto_gc, variable_order=self.variable_order)
    ...
    def to_dot(self, *args, **kwargs):
        if kwargs.get('as_sdd'):
            dotstr = {}
            for qn, qi in self.queries():
                if self.is_probabilistic(qi):
                    ds = self.get_manager().to_dot(self.get_inode(qi))
                    dotstr[qi] = self._process_sdd_dot(ds, **kwargs)
            return dotstr
        else:
            return self.to_formula().to_dot(*args, **kwargs)

    def _process_sdd_dot(self, dotstr, with_names=False, **kwargs):

        # Assume letters
        if with_names:
            for var, atom in self.var2atom.items():
                varname = chr(var + 64)
                atomname = self.get_node(atom).name
                dotstr = dotstr.replace('<L>%s' % varname, '<L>%s' % atomname)
                dotstr = dotstr.replace('<L>&not;%s' % varname, '<L>&not;%s' % atomname)
                dotstr = dotstr.replace('<R>%s' % varname, '<R>%s' % atomname)
                dotstr = dotstr.replace('<R>&not;%s' % varname, '<R>&not;%s' % atomname)
                dotstr = dotstr.replace('width=.65', 'width=4')
        return dotstr
...
class SDDManager(DDManager):
    ...
    def __init__(self, varcount=0, auto_gc=True, vtree=None, variable_order=None, vtree_style='right'):
        ...
        if vtree is None:
            if variable_order:
                a = sdd.long_array(variable_order)
                vtree = sdd.sdd_vtree_new_with_var_order(len(variable_order), a, vtree_style)
                varcount = len(variable_order)
            else:
