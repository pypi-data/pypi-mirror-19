...
def run_star_fusion(job, fastqs, univ_options, star_fusion_options):
    ...
    if found_fusion:
        fusion_bed_path = os.path.join(work_dir, 'output/FusionInspector/FusionInspector.bed')
        transcript_path = os.path.join(work_dir,
                                       'output/FusionInspector/FusionInspector.gmap_trinity_GG.fusions.fasta')
        transcript_gff_path = os.path.join(work_dir,
...
def get_gene_ids(fusion_bed):
    ...
    :return:
    """
    with open('FusionInspector.bed', 'r') as f:
        gene_to_id = {}
