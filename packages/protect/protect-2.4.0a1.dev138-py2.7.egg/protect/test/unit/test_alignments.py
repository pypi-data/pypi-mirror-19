...
class TestAlignments(ProtectTest):
    ...
    def test_star2(self):
        """
        Test the functionality of align_rna without downloading all the reference files
        """
        univ_options = self._getTestUnivOptions()
        a = Job.wrapJobFn(self._get_test_star_files)
        b = Job.wrapJobFn(self._get_star_reference)
        c = Job.wrapJobFn(align_rna, a.rv(), univ_options, b.rv()).encapsulate()
        a.addChild(b)
        b.addChild(c)
        Job.Runner.startToil(a, self.options)
    ...
    @staticmethod
    def _get_star_reference(job):
        star_options = {}
        call = 's3am download s3://cgl-protect-data/ci_references/GRCh37_chr6_v19_CTAT_lib_ready.tar.gz ./GRCh37_chr6_v19_CTAT_lib_ready.tar.gz'
        subprocess.check_call(call.split(' '))
        star_options['tool_index'] = job.fileStore.writeGlobalFile('GRCh37_chr6_v19_CTAT_lib_ready.tar.gz')
        star_options['type'] = 'star'
        star_options['n'] = 6
        return star_options
...
_get_star_reference = TestAlignments._get_star_reference
