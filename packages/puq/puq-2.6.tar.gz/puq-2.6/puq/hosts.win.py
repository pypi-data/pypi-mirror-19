...
class InteractiveHost(Host):
    ...
    def _run(self):
        ...
        for j in self.jobs:
            if j['status'] == 0 or j['status'] == 'X':
                ...
                p = psutil.Popen(cmd, shell=True, stdout=sout, stderr=serr)
                j['status'] = 'R'
                self._running.append((p, j))
                self._monitor.start_job(j['cmd'], p.pid)
        self.wait(0)

    def handle_error(self, code, j):
        print(40 * '*')
        print("ERROR: %s returned %s" % (j['cmd'], code))
        try:
    ...
    def wait(self, cpus):
        ...
        while len(self._running):
            ...
            print()
            gone, alive = psutil.wait_procs(waitlist, timeout=1)
            for g in gone:
                print('g=', g, g.pid, g.returncode)
                for p, j in self._running:
                    if p == g:
