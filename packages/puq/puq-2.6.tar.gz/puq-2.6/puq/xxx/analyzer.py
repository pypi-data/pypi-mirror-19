...
def upload_file(filename, rename=''):
    ...
    request = urllib.request.Request(addr, datagen, headers)
...
class MyPrefs(Dialog):
    ...
    def body(self, master):
        ...
        self.box = tkinter.ttk.Combobox(uframe, textvariable=self.box_value, state='readonly', width=70)
...
class InitFrame:
    def __init__(self, parent):
        ...
        self.desc = tkinter.scrolledtext.ScrolledText(dframe, height=5)
        ...
        script = tkinter.scrolledtext.ScrolledText(sframe)
        try:
...
class BasicFrame:
    def __init__(self, parent):
        self.txt = tkinter.scrolledtext.ScrolledText(parent)
...
class TextFrame:
    ...
    def state(self, st, val, path):
        ...
        if st == 'SENSITIVITY':
            ...
            max_name_len = max(list(map(len, [p[0] for p in val])))
            tval = "SENSITIVITY:\n"
            tval += "Var%s     u*            dev\n" % (' '*(max_name_len))
            ...
            if path in h5:
                outvars = list(map(str, list(h5[path].keys())))
                numjobs = len(h5['/output/jobs']) - 1
                tval = "%s Jobs\n\nOUTPUT VARIABLES:\n" % numjobs
                for var in outvars:
        ...
        self.stext = tkinter.scrolledtext.ScrolledText(self.parent)
...
class ProgressFrame:
    ...
    def state(self, st, val, path):
        if st == 'PROGRESS':
            ...
            self.txt = tkinter.scrolledtext.ScrolledText(self.parent)
...
class ParFrame:
    ...
    def state(self, st, val, path):
        ...
        t = tkinter.ttk.Treeview(self.parframe, yscrollcommand=scrollbar.set, height=len(val.params))
        t["columns"] = ("desc", "pdf")
        t.column("#0", width=75)
...
class ParFrame2:
    ...
    def state(self, st, val, path):
        ...
        params = list(map(str, list(h5['/input/params'].keys())))

        # Parameters Table with scrollbar
        scrollbar = Scrollbar(self.parframe)
        scrollbar.pack(side=RIGHT, fill=Y)
        t = tkinter.ttk.Treeview(self.parframe, yscrollcommand=scrollbar.set, height=len(params))
        t["columns"] = ("desc", "pdf")
        t.column("#0", width=75)
        t.column("desc", width=100)
        t.column("pdf", width=400)
...
class SurFrame:
    ...
    def state(self, st, val, path):
        ...
        if isinstance(val, SampledFunc):
            ...
            # Surface Pane
            self.eqn = tkinter.scrolledtext.ScrolledText(self.surframe, height=2, )
...
class MyCombobox:
    def __init__(self, parent, txt, values, current, callback=None):
        ...
        self.callback = callback
        self.cb = tkinter.ttk.Combobox(parent, textvariable=self.var, values=values)
...
class MyApp:
    ...
    def __init__(self, parent, h5, errors):
        ...
        scrollbar.pack(side=RIGHT, fill=Y)
        self.tree = tkinter.ttk.Treeview(tframe, yscrollcommand=scrollbar.set)
    ...
    def load_tree_widget(self):
        ...
        keys = sorted(map(int, [x for x in list(h5['/output/jobs'].keys()) if x.isdigit()]))
        self.errlist = []
        for j in keys:
            tout = None
            terr = None
