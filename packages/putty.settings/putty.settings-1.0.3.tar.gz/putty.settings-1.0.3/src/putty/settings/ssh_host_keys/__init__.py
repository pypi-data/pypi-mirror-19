...
class SshHostKeys:
    ...
    def save_to_file(self, filename: pathlib.Path, save_type: SaveType = SaveType.known_hosts):
        if save_type == SaveType.known_hosts:
            host_keys = paramiko.HostKeys()
            self.add_to_paramiko_host_keys(host_keys)
            host_keys.save(str(filename))

    def load_from_file(self, filename: pathlib.Path, load_type: SaveType = SaveType.known_hosts):
        if load_type == SaveType.known_hosts:
