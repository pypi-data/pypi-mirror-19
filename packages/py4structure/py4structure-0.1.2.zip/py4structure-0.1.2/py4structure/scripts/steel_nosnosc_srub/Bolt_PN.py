...
class Bolt_PN :
    ...
    def get_AvailableBoltGrade(self):
        return [i['grade'] for i in Bolt_PN.__BoltGradeList]

    def get_AvailableBoltDim(self):
        return [i['dim'] for i in Bolt_PN.__BoltDimList]

    def set_BoltGrade(self, newgrade = '3.6'):
        for i in Bolt_PN.__BoltGradeList:
            if newgrade==i['grade']:
                self.Grade = i['grade']
                self.Rm = i['Rm']
                self.Re = i['Re']
    ...
    def set_BoltDim(self, newdim = 'M12'):
        for i in Bolt_PN.__BoltDimList:
            if newdim==i['dim']:
                self.Dim = i['dim']
                self.As = i['As']
                self.d = i['d']
