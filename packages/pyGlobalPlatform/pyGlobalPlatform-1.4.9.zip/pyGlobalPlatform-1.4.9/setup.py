...
if 'win32' == get_platform():
    ...
    if ('install' in sys.argv) or ('bdist' in sys.argv) or ('sdist' in sys.argv) or ('bdist_wininst' in sys.argv):
        for arg in sys.argv:
            if arg.startswith('--libgp-root='):
                gp_root = arg[len('--libgp-root=') :]
                gp_include_dir = gp_root
                gp_library_dir = gp_root
                sys.argv.remove(arg)
                break

        if len(gp_include_dir) == 0:
            print('Please specify root of library globalplatform with argment --libgp-root=/path/to/globalplatform/root.')
            exit(-1)
