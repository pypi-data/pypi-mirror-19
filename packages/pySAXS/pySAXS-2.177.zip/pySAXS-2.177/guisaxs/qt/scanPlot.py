...
class scanPlot(QtGui.QDialog):
    def __init__(self, parent=None):
        ...
        #print self.x(), self.y(), self.plotframe.x(), self.plotframe.y()
    ...
    def OnClickFileButton(self):
        ...
        fd = QtGui.QFileDialog(self)#," File dialog ", filter="log Files (*.log)")#, selected_filter)
        #fd.setFileMode(QtGui.QFileDialog.ExistingFile)
        #fd.setNameFilters( [('log Files (*.log)'),('All Files (*)')] )
        #fd.setDefaultSuffix( '.log' )
        
        filename = fd.getOpenFileName(filter="log Files (*.log)")#directory=self.workingdirectory)
    ...
    def FillList(self):
        ...
        for item in sorted(self.scanList,reverse=True):
            ...
            self.ui.tableWidget.setItem(i, 0, QtGui.QTableWidgetItem(str(scan.No)))
            self.ui.tableWidget.setItem(i, 1, QtGui.QTableWidgetItem(scan.description))
            self.ui.tableWidget.setRowHeight(i, 20)          
            i += 1
        self.cellClicked(0,0)
        
    def cellClicked(self,row,col):
        #print 'row selectionned ',row
        #print self.ui.tableWidget.item(row,0)
