...
from pySMS.utils.excepetions import NoRecordsFoundError
...
class macros():
    ...
    def log(self, _type, text):
        try:
            ...
            self.current_func_data['critical'] = self.self.log_list_critical
            self.current_func_data['debug'] = self.log_list_debug

            print(self.current_func_data)
    ...
    def reportAdjustmentErrors(self):
        return self.adjustmentErrCount
    ...
    def login(self):  
        print(Fore.YELLOW + 'Logging in as ' + self.logininfo[0])
        self.log('info', '[INFO] [DEBUG] Logging in as ' + self.logininfo[0])
        try:        
            self.css(loginpage.username).send_keys(self.logininfo[0])
            self.css(loginpage.password).send_keys(self.logininfo[1])
            time.sleep(1)
            try:
                self.css(loginpage.loginbutton).click()
                print(Fore.YELLOW + 'Logged in successfully as ' + self.logininfo[0])
                self.log('info', '[INFO] [DEBUG] Logged in successfully as ' + self.logininfo[0])
            except Exception as _e:
                print(Fore.LIGHTRED_EX + 'Logged in Failed')
                print(Fore.LIGHTMAGENTA_EX + 'Trackback: ' + Fore.LIGHTRED_EX + str(_e))
                self.err.err_custom('Unable to login as ' + self.logininfo[0] + '. Please refresh the page and login manually.', 'Error', 'MB_OK', 'ICON_EXCLAIM')
                self.log('info', '[INFO] [ERROR] Unable to login as ' + self.logininfo[0] + '.')
                self.log('warning', str(_e))
        except Exception as e:
            print(Fore.LIGHTRED_EX + 'Logged in Failed')
            print(Fore.LIGHTMAGENTA_EX + 'Trackback: ' + Fore.LIGHTRED_EX + str(e))
            self.err.err_custom('Unable to login as ' + self.logininfo[0] + '. Please refresh the page and login manually.', 'Error', 'MB_OK', 'ICON_EXCLAIM')
            self.log('info', '[INFO] [ERROR] Unable to login as ' + self.logininfo[0] + '.')
    ...
    def change_site(self, site):
        ...
        for key, value in self.siteData['listBoxSites'].items():
            if site_name == str(key):
                self.driver.get(loginpage.url)
                time.sleep(1)
                self.wait_for(loginpage.username)
                self.login()

                self.wait_for_click(uS.upUser)

                self.wait_for_click(uS.upUserSettingsButton)

                self.wait_for_click(self.siteData['listBoxSites'][str(key)]['path'])

                self.css(uS.upSaveButton).click()
                time.sleep(1)
                self.driver.get(loginpage.url)
                time.sleep(1)
                self.wait_for(loginpage.username)
            ...
            if self.checksite == site_name:
                print(Fore.LIGHTYELLOW_EX + 'Successfully changed site to: ' + Fore.MAGENTA + site_name)
                self.log('info', '[INFO] [DEBUG] Successfully changed site to: ' + site_name)
            else:
                self.err.err_basic('Could not change site to: ' + site_name + '. Please change the site, refresh the page, and login.', 'Site Change Error')
                self.log('info','[INFO] [ERROR] Unable to check site to: ' + site_name)

    def testClick(self, cssselector):
        self.css(cssselector).click()
    ...
    def stProdChooserAddItem(self, item_dict):
        ...
        try:
            ...
            input('Unable to add item: ' + item_dict['prodNumber'] + '. Please add this manually and hit enter to continue...')
    ...
    def stMutation(self, dict, data):
        ...
        try:
            ...
            try:
                self.wait_for_click(stmutate.stMutateCancelButton)
            except:
    ...
    def stAdjustStock(self):
        time.sleep(1)
        self.wait_for_click(adj.adjAdjustButton)

        # Adjust Quantity
        self.actions = None
        self.quantity = self.css(adj.adjMenuQuantity)
        self.quantity.click()
        self.actions = ActionChains(self.driver)
        self.clickinto = self.actions.move_to_element(self.quantity).send_keys('-1').perform()
        self.actions = None
        ...
        # Add Note
        self.actions = None
        self.note = self.css(adj.adjMenuNote)
        self.note.click()
        self.actions = ActionChains(self.driver)
        self.clickinto = self.actions.move_to_element(self.note).send_keys('Late Debit Cleanup').perform()
        self.actions = None

        # Save
        self.wait_for_click(adj.adjMenuSaveButton)
        time.sleep(1)

        #go back a page
        self.driver.execute_script("window.history.go(-1)")

    def stAdjustPayback(self, y):
        self.css(sttable.stTableContainerFront + str(y) + sttable.stTableContainerBackInfo).click()
        time.sleep(10)
        self.driver.execute_script("window.history.go(-1)")
        self.stStockTabButton()
    ...
    def stTableParsing(self, dict, funcType):
        data = [dict['caseNumber'], dict['lotNumber'], dict['serialNumber']]
        ...
        for y in range(dr[0], dr[1] + 1):
            ...
            if funcType == 'adjustment':
                self.stBoolDataAdjustment( {'rowNumber': str(y),
                                            'prodnumber': tableProdNumber,
                                            'description': tableDescription,
                                            'lot_serial': tableLotSerial,
                                            'container': tableContainer,
                                            'location': tableLocation,
                                            'location_type': tableLocationType}, data)
    ...
    def stBoolDataAdjustment(self, dict, data):
        if str(data[0]).rstrip('.0') in dict['container']:
            if str(data[1]).rstrip('.0') in dict['lot_serial']:
                print(Fore.LIGHTGREEN_EX + 'Found Match!' + '\n')
                lv.formatListToTable(dict, lv.formatDictotList(dict), Fore.LIGHTYELLOW_EX)
                try:
                    self.wait_for_click(sttable.stTableProductFront + dict['rowNumber'] + sttable.stTableProductBack)
                    print(Fore.LIGHTYELLOW_EX + 'Adjusting item ' + Fore.LIGHTGREEN_EX + self.css(
                          sttable.stTableProductFront + dict['rowNumber'] + sttable.stTableProductBack).get_attribute(
                          'innerHTML') + '\n')
                    self.stAdjustStock()
                except Exception as e:
                    self.err.err_traceback(e, 'Unable to Adjust Product: ' + dict['prodnumber'] + ' Lot: ' + dict[
                                           'lot_serial'] + ' Case: ' + str(data[1]).rstrip(
                                           '.0') + ', please mutate and hit enter to continue')
                    self.log('warning', str(e))
            else:
                pass
        else:
    ...
    def adjust(self, dict):
        prodNumber = dict['productNumber']
        try:
            self.stProdChooserSelect()
        except Exception as e:
            self.err.err_basic('Unable to open product chooser on stock page, please open and click ok to continue', 'Error')
            self.log('info', '[INFO] [ERROR] Unable to open product chooser on stock page')
            self.adjustmentErrCount += 1
        try:
            self.stProdChooserSearch(prodNumber)
        except Exception as e:
            self.err.err_basic('Unable to search for ' + prodNumber + ', please search and click ok to continue', 'Error')
            self.log('info', '[INFO] [ERROR] Unable to search for ' + prodNumber)
            self.adjustmentErrCount += 1
        try:
            self.stProdChooserAddItem()
        except Exception as e:
            self.err.err_basic('Unable to add item, please add and click ok to continue', 'Error')
            self.log('info', '[INFO] [ERROR] Unable to add item: ' + prodNumber)
            self.adjustmentErrCount += 1
        try:
            self.stProdChooserFinish()
        except Exception as e:
            self.err.err_basic('Unable to click finish, please click finish and click ok to continue', 'Error')
            self.log('info', '[INFO] [ERROR] Unable to click finish')
            self.adjustmentErrCount += 1
        try:
            self.stSearchButton()
        except Exception as e:
            self.err.err_basic('Unable to click search button, please click search button and click ok to continue', 'Error')
            self.log('info', '[INFO] [ERROR] Unable to click search button')
            self.adjustmentErrCount += 1

        self.stTableParsing(dict, 'adjustment')
    ...
    def mutate(self, _dict):
        ...
        _results = self.stTableParsing(_dict, 'mutation')
        if _results != True:
