...
from pySMS.utils.excepetions import NoRecordsFoundError, WaitTimeOutError
...
class macros():
    ...
    def login(self):  
        ...
        try:
            self.css(loginpage.username).send_keys(self.parse_settings_for('USER', 'username'))
            self.css(loginpage.password).send_keys(self.parse_settings_for('USER', 'password'))
            time.sleep(1)
            try:
    ...
    def wait_for_click(self, selector):
        ...
        while i < 10:
            try:
                ...
                if i >= 10:
                    self.log('warning', "Selector timed out : {" + selector + "} ")
                    raise WaitTimeOutError(str(10), selector)
                    break
                else:
                    i += 1
                    time.sleep(0.6)
