...
class Protocols:
    ...
    def test_connection(self, retry=5, address=POSTGRES_ADDRESS):
        for _ in range(retry):
            try:
                self.pg.test_connection(connection="MANUAL", username="doarni", password="ZimmerBiomet", host=address)
                return
            except:
                if _ < retry:
...
class NetworkDrive:
    ...
    def __init__(self):
        ...
        self.version_list = self.get_versions_from_network_drive()
    ...
    def get_versions_from_network_drive(self, print_versions=False):
        _path = self.set_current_dir(r"\SMSA\Python\versions")
        self.version_list = [f for f in os.listdir(_path) if f.endswith('')]

        if print_versions == True:
            print('Versions available in network drive: ')
            for f in self.version_list:
                if f.endswith('.zip'):
                    print(' > ' + f)
    ...
    def move_files_to_network_drive(self, file_list, version, zip_files=True, remove_on_finish=False):
        _path = self.set_current_dir(r"\SMSA\Python\versions")

        def move_f(_list):
            for file in _list:
                os.system('xcopy ' + file + ' ' + _path + '\\' + version)
        ...
        def zip_f(src, dst):
            ...
            for dir_name, sub_dirs, files in os.walk(src):
                for filename in files:
                    ...
                    print('zipping %s as %s' % (os.path.join(dir_name, filename),
                                                arc_name))
                    zf.write(abs_name, arc_name)
            zf.close()

        if os.path.exists(_path):
            if os.path.exists(_path + '\\' + version):
                if zip_files == True:
                    move_f(file_list)
                    zip_f("\\SMSA\\Python\\versions\\" + version,
                          "\\SMSA\\Python\\versions\\")
                else:
                    move_f(file_list)
            else:
                raise NetworkDriveError('This version already exists in the current update directory,'
                                        ' please change the version and re run.', issue='VersionAlreadyExists')
        else:
            raise NetworkDriveError('Please check to see if the drive is mapped correctly and that you'
                                    ' are connected to a ZimmerBiomet VPN / Internet.',issue='DriveNotFound')
