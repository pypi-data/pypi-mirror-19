...
class parse():
    ...
    def readRows(filename, y, sheetIndex):
        ...
        if sheetIndex == 2:
            ...
            if lotNumber == '' and serialNumber == '':
                print(Fore.LIGHTRED_EX + productNumber + " at row " + str(y + 1) + " has no lot or serial! It was not added to the mutation queue")
                _pass = True
            else:
                print(Fore.LIGHTYELLOW_EX  +'Adding to queue: \tProduct: [{}] \t\tlot\serial: [{}]'.format(Fore.LIGHTCYAN_EX + productNumber + Fore.LIGHTYELLOW_EX, Fore.LIGHTCYAN_EX + (lotNumber if lotNumber != '' else serialNumber)) + Fore.LIGHTYELLOW_EX)
