...
class SqsListener(object):
    ...
    def listen(self, daemonize=False):
        if not daemonize:
            print "Listening to queue " + self._queue_name
            self._start_listening()
        else:
            print "Launching listener as daemon on queue " + self._queue_name
            print "Logging messages will be diverted to standard output"

            with daemon.DaemonContext(
                    working_directory='/var/lib/sqs_daemon',
                    umask=0o002,
                    pidfile=pidfile.TimeoutPIDLockFile('/var/run/sqs_daemon.pid'),
            ) as context:
                self._start_listening()
