...
namespace gum {
  ...
  INLINE HashTableList<Key, Val, Alloc>::HashTableList(
      typename HashTableList<Key, Val, Alloc>::BucketAllocator*
          allocator ) noexcept : __alloc_bucket{allocator} {}
  ...
  template <typename Key, typename Val, typename Alloc>
  INLINE HashTableList<Key, Val, Alloc>::~HashTableList() {
    for ( Bucket *next_ptr, *ptr = __deb_list; ptr != nullptr;
          ptr = next_ptr ) {
      next_ptr = ptr->next;
      __alloc_bucket->destroy( ptr );
      __alloc_bucket->deallocate( ptr, 1 );
  ...
  operator=( const HashTable<Key, Val, OtherAlloc>& from ) {
    // avoid self assignment
    if ( this !=
         reinterpret_cast<const HashTable<Key, Val, Alloc>*>( &from ) ) {
      // for debugging purposes
      GUM_OP_CPY( HashTable );
  ...
  HashTable<Key, Val, Alloc>::__insert( HashTableBucket<Key, Val>* bucket ) {
    ...
    if ( __resize_policy &&
         ( __nb_elements >=
           __size * HashTableConst::default_mean_val_by_slot ) ) {
      resize( __size << 1 );
  ...
  HashTable<Key, Val, Alloc>::insert( std::pair<Key, Val>&& elt ) {
    ...
    try {
      __alloc.construct( bucket,
                         std::move( reinterpret_cast<value_type&>( elt ) ) );
    } catch ( ... ) {
      __alloc.deallocate( bucket, 1 );
      throw;
    }
  ...
  template <typename Key, typename Val, typename Alloc>
  INLINE void HashTable<Key, Val, Alloc>::set( const Key& key,
                                               const Val& value ) {
    Bucket* bucket = __nodes[__hash_func( key )].bucket( key );
  ...
  template <typename Key, typename Val, typename Alloc>
  INLINE const Key&
  HashTable<Key, Val, Alloc>::keyByVal( const Val& val ) const {
