...
void Scanner::Init() {
  ...
  eofSym = 0;
	...
	keywords.set(L"net", 7);
	keywords.set(L"node", 12);
	keywords.set(L"data", 14);
	keywords.set(L"experience", 15);
	keywords.set(L"potential", 16);
...
case_0:
      ...
      t->kind = recKind; break;
		...
		case 2:
			case_2:
			recEnd = pos; recKind = 1;
			if (ch == L'.' || (ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || ch == L'_' || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_2;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 3:
			case_3:
		...
		case 13:
			...
			else if ((ch >= L'0' && ch <= L'9')) {AddCh(); goto case_13;}
