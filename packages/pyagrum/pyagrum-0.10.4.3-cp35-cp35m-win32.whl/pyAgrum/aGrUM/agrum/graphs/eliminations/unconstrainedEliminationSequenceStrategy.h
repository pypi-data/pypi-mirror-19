 ...
 *the
 * only information needed to compute an elimination ordering. For instance,
 *this
 * is only what is needed for computing junction trees for Bayesian networks or
 * GAI networks. For such cases, the elimination sequence algorithms derived
 *from
 * UnconstrainedEliminationSequenceStrategy are appropriate. There are however
 * other cases in which additional informations must be passed to the
 *elimination
 ...
 * knowledge of a partial ordering on the nodes to be eliminated. For such
 *cases,
