...
# TODO: Test and debug node and clusterTree classes
class Node(object):
    """
    ...
    def __init__(self, name, value=None, parent=None, dist=0, labels=None, children=[], compute_mean=False):
        ...
        if labels is None:
            ...
            self.labels = np.hstack(tuple(temp_labels)).sort()
...
class ClusterTree(object):
    ...
    def __init__(self, linkage_pairing, labels, distances=None, centroids=None):
        ...
        self.nodes = list(np.arange(1 + np.max(linkage_pairing[:, 0:2]), dtype=np.uint16))
        ...
        for clust_id in xrange(self.num_leaves):
            which_pos = np.where(self.labels == clust_id)
            if centroids is not None:
                self.nodes[clust_id] = Node(clust_id, value=centroids[clust_id], labels=which_pos)
            else:
                self.nodes[clust_id] = Node(clust_id, labels=which_pos)

        for row in xrange(linkage_pairing.shape[0]):
            ...
            for col in xrange(linkage_pairing.shape[1]):
            ...
            # If distances are provided, add the distances attribute to this node. This is the distance between the children
            if distances is not None:
                new_node.dist = distances[row]
                # add this node to the look-up table:
            self.nodes[row + self.num_leaves] = new_node
