...
class wafer:
    ...
    def __init__(self, fac_id=None, lot_cd=None, end_tm=None, size=None, unit_cnt=25, pattern=['G', 'B'], p=[.8, .2]):

        assert len(size) == 2
        assert len(pattern) == len(p)
        assert sum(p) == 1
        assert isinstance(unit_cnt, int) == True
        self.fac_id = fac_id
        self.lot_cd = lot_cd
        self.end_tm = end_tm
        self.size = size
    ...
    def __new__(cls, fac_id=None, lot_cd=None, end_tm=None, size=None, unit_cnt=25, pattern=['G', 'B'], p=[.8, .2]):
        
        coords = Ellipse(size).coordinates()
        #xsize, ysize = coords.xcoord.max(), coords.ycoord.max()
        
        lotData = df(columns=['fac_id', 'lot_cd', 'lot_id', 'unit_id',
        ...
        ## wf_id
        unitIter = [list(it.repeat(num, len(coords))) for num in range(1, unit_cnt+1)]
        ...
        ## res_val
        failList = [ np.random.choice(pattern, size=len(coords), p=p) for num in range(1, unit_cnt+1)]
    ...
    def unitplot(data, columns=['xcoord', 'ycoord'], lot_id='', unit_id=None, val='val', figsize=(12, 9), pattern=['B'], m=['s'], s=[100], c=['red'], a=[.5], shown='y'):
        
        assert len(figsize) == 2
        assert len(columns) == 2
        assert isinstance(lot_id, str) == True
        assert len(pattern) == len(m) == len(s) == len(c) == len(a)
        
        
        # Check if there is a given Wafer ID
        if unit_id != None:
            pltData = data.loc[data.unit_id==unit_id, [columns[0], columns[1], val]]
        else:
...
#            plt.ioff()
        ...
        canvas = {'pattern':'+', 's':1000, 'c':'grey', 'a':.1}

        # Draw plot
        fig = plt.figure(figsize=figsize)
        plt.scatter(pltData.xcoord, pltData.ycoord, marker=canvas['pattern'], s=canvas['s'], c=canvas['c'], alpha=canvas['a'])
...
class displaypanel(wafer):
    ...
    def __new__(cls, fac_id=None, lot_cd=None, end_tm=None, size=None, unit_cnt=25, pattern=['G', 'B'], p=[.8, .2]):
        ...
        ## pn_id
        unitIter = [list(it.repeat(num, xsize * ysize)) for num in range(1, unit_cnt+1)]
        lotData.unit_id = list(it.chain.from_iterable(unitIter))
        lotData.unit_id = lotData.unit_id.astype(str).str.zfill(2)
