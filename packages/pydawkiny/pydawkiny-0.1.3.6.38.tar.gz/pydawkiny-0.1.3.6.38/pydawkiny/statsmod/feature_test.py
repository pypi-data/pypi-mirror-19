...
def feature_selection_vif(data, thresh=.5):
    ...
    while dropCondition:
        ...
        # 1. Calculate a VIF
        yvar = data.columns[0]
        Xvar = data.columns[list(range(1, data.shape[1]))]

        vifList = [vif(data[yvar], data[Xvar]) for col in range(data.shape[1])]
        ...
        # Keep it
        dropped.append({'var': max_var, 'vif': max_val}, ignore_index=True)

        # 2. IF VIF values are over the threshold, THEN drop it
        if max(vifList) >= thresh:

            # Drop it
            data = data.drop(data.columns[max_idx], axis=1)

            # Print it
            print("Dropping '" + data.columns[max_idx] + "'" +
