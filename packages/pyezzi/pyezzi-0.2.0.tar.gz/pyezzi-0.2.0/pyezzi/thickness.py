...
def compute_thickness(labeled_image,
    ...
    Input image must be labeled as specified (background=0, inside=1, wall=2 by default)

    :param labeled_image:np.ndarray of ints
    :param spacing:iterable Defines the spacing between voxels along the 3 axes. 1, 1, 1 by default
    ...
    :param crop:bool Crop image before computations (true by default, improves speed)
    :param debug:bool Print debug messages
    :return:np.ndarray of floats, representing thickness at each wall point
    """
    ...
    if debug:
        print("Thickness computation: {} iterations, max_error = {}".format(_, __))
