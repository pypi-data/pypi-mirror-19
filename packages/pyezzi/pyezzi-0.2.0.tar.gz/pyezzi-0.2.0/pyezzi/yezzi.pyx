...
def iterative_relaxation_3D(np.ndarray wall,
                            np.ndarray[DTYPE_FLOAT, ndim=3] laplace_grid,
                            float tolerance=0.0,
                            int max_iterations=100,
                            np.ndarray[DTYPE_FLOAT, ndim=1] spacing=np.array(
                                [1.0, 1.0, 1.0])):
    """
    :param wall: A boolean 3D ndarray representing the wall that we want to
                 measure. Beware: wall cannot touch image borders!
    ...
    cdef np.ndarray[DTYPE_FLOAT, ndim=3] sum_abs_vectors = \
        abs_vectors[0] / hi + abs_vectors[1] / hj + abs_vectors[2] / hk
    ...
    cdef np.ndarray[DTYPE_FLOAT, ndim=3] L1 = np.zeros_like(wall, float)
    cdef np.ndarray[DTYPE_FLOAT, ndim=3] prev_L0, prev_L1

    cdef DTYPE_FLOAT error = 0
    cdef DTYPE_FLOAT max_error = float("Inf")
    cdef DTYPE_INT iteration = 0

    cdef DTYPE_INT i, j, k
    cdef DTYPE_FLOAT L0_i, L0_j, L0_k, L1_i, L1_j, L1_k
    ...
    while max_error > tolerance and iteration < max_iterations:
        ...
        for n in range(n_points):
            ...
            if vectors[2, i, j, k] > 0:
                ...
                L0_k = abs_vectors[2, i, j, k] * L0[i, j, k + 1]
                L1_k = abs_vectors[2, i, j, k] * L1[i, j, k - 1]
            L0[i, j, k] = (L0_i / hi + L0_j / hj + L0_k / hk + 1) \
                          / sum_abs_vectors[i, j, k]
            L1[i, j, k] = (L1_i / hi + L1_j / hj + L1_k / hk + 1) \
                          / sum_abs_vectors[i, j, k]
        max_error = np.nanmax(
            [np.nanmax(np.abs(L0[wall_idx_i, wall_idx_j, wall_idx_k] -
                              prev_L0[wall_idx_i, wall_idx_j, wall_idx_k])),
             np.nanmax(np.abs(L1[wall_idx_i, wall_idx_j, wall_idx_k] -
                              prev_L1[wall_idx_i, wall_idx_j, wall_idx_k]))]
        )

    return L0, L1, iteration, max_error
