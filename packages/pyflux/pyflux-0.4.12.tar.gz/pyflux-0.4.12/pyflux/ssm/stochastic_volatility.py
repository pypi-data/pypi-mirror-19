...
class NLLEV(tsm.TSM):
    ...
    **** NON-GAUSSIAN LOCAL LEVEL MODEL ****
    ...
    def neg_loglik(self, beta):
        ...
        return self.family.neg_loglikelihood(self.data, self.link(states), scale, shape, skewness)  # negative loglikelihood for model
    ...
    def _preoptimize_model(self):
        ...
        if self.model_name2 == 't GAS':
            ...
            def temp_function(params):
                return -np.sum(dst.skewt.logpdf(x=self.data,df=np.exp(params[0]),
    ...
    def initialize_approx_dist(self, phi, start_diffuse):
    ...
    def _bbvi_fit(self, optimizer='RMSProp', iterations=1000, print_progress=True,
        ...
        BBVIResults object
        """
        if self.name in ["t GAS", "Skewt GAS"]:
        ...
        q_list = self.initialize_approx_dist(phi, start_diffuse)
