...
class LMSEGARCH(tsm.TSM):
    """ Inherits time series methods from TSM class.

    **** Long Memory skew BETA-t-EGARCH MODELS ****
    ...
    def __init__(self,data,p,q,target=None):
        ...
        super(LMSEGARCH,self).__init__('LMSEGARCH')

        # Latent variables
        self.p = p
        self.q = q
        self.z_no = self.p*2 + self.q*2 + 4
        self.max_lag = max(self.p,self.q)
        self.leverage = False
        self.model_name = "LMSEGARCH(" + str(self.p) + "," + str(self.q) + ")"
    ...
    def _create_latent_variables(self):
        ...
        for component in range(2):
            ...
            for p_term in range(self.p):
                self.latent_variables.add_z("Component " + str(component+1) + ' p(' + str(p_term+1) + ')', fam.Normal(0,0.5,transform=None), fam.Normal(0,3))
                if p_term == 0:
                    self.latent_variables.z_list[1+p_term+component*(self.p+self.q)].start = 0.00

            for q_term in range(self.q):
                self.latent_variables.add_z("Component " + str(component+1) + ' q(' + str(q_term+1) + ')', fam.Normal(0,0.5,transform=None), fam.Normal(0,3))
                if p_term == 0 and component == 0:
                    self.latent_variables.z_list[1+self.p+q_term+component*(self.p+self.q)].start = 0.001
                elif p_term == 0 and component == 1:
                    self.latent_variables.z_list[1+self.p+q_term+component*(self.p+self.q)].start = 0.01
        ...
        self.latent_variables.z_list[0].start = self.latent_variables.z_list[0].prior.itransform(np.log(np.mean(np.power(self.data,2))))
    ...
    def _model(self,beta):
        ...
        for t in range(0,Y.shape[0]):
            ...
            scores[t] = gas.SkewBetatScore.mu_adj_score(Y[t],theta[t],lmda[t],parm[-2],parm[-3])
    ...
    def plot_fit(self,**kwargs):
        ...
        if self.latent_variables.estimated is False:
            ...
            plt.plot(date_index,np.exp(sigma2/2.0),label='LMSEGARCH(' + str(self.p) + ',' + str(self.q) + ') Conditional Volatility',c='black')                   
