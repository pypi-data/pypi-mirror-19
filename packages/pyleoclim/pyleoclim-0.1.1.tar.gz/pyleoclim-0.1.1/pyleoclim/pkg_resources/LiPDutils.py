...
def getTSDict(path=""):
    """
    This function load the lipd files and extract the timeseries if not
    previously done by the user
    """
    # Make sure the LiPD files are not already loaded
    if not hasattr(lpd, 'path'):
        if not path:
            path = lpd.loadPath()
        else:
            path = lpd.loadPath(path)
        lpd.loadLipds()
    timeseries_dict = lpd.extractTs()

    return timeseries_dict

def enumerateTSO(timeseries_dict):
    """
    ...
    available_y = []
    dataSetName =[]
    for index,val in enumerate(timeseries_dict):
...
def getTSO(timeseries_dict):
    ...
    """        
    enumerateTSO(timeseries_dict)
    select_TSO = promptforVariable()
    new_TSO = timeseries_dict[select_TSO]
