...
class GraphView(QtWidgets.QGraphicsView):
    ...
    nodeAdded = pyqtSignal(Node)
    nodeRemoved = pyqtSignal(Node)
    nodeNameChanged = pyqtSignal(str, str)
    beginDeleteSelection = pyqtSignal()
    endDeleteSelection = pyqtSignal()

    beginConnectionManipulation = pyqtSignal()
    endConnectionManipulation = pyqtSignal()
    connectionAdded = pyqtSignal(Connection)
    connectionRemoved = pyqtSignal(Connection)

    beginNodeSelection = pyqtSignal()
    endNodeSelection = pyqtSignal()
    selectionChanged = pyqtSignal(list, list)

    # During the movement of the nodes, this signal is emitted with the incremental delta.
    selectionMoved = pyqtSignal(set, QtCore.QPointF)

    # After moving the nodes interactively, this signal is emitted with the final delta.
    endSelectionMoved = pyqtSignal(set, QtCore.QPointF)

    _clipboardData = None

    _backgroundColor = QtGui.QColor("#474747")
    _gridPenS = QtGui.QPen(QtGui.QColor("#2a2a2a"), 0.2)
    _gridPenL = QtGui.QPen(QtGui.QColor("#1a1a1a"), 0.5)
    _gridSizeFine = 30
    _gridSizeCourse = 300
