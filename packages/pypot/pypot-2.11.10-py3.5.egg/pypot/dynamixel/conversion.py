...
def baudrate_to_dxl(value, model):
    for k, v in dynamixelBaudrates.items():
        if (abs(v - value) / float(value)) < 0.05:
            return k
    raise ValueError('incorrect baudrate {} (possible values {})'.format(value, list(dynamixelBaudrates.values())))
...
control_mode_to_dxl = lambda mode, _: (next((v for v, m in list(control_modes.items())
                                             if m == mode), None))
