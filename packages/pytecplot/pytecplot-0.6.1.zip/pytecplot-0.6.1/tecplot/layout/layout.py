...
def load_layout(filename):
    ...
    with ArgList(FNAME=path.abspath(filename)) as arglist:
...
def save_layout(filename, include_data=None, include_preview=None,
    ...
    Parameters:
        filename (`string <str>`): The path to the output filename. Relative
    ...
    .. note::
        If you receive an exception with the error message
        "Journal should be valid in all frames", then you must save a data file
        using `save_tecplot_ascii` or `save_tecplot_binary` before saving the
        layout.
...
def pages(pattern=None):
    ...
    """
    for i in range(_tecutil.PageGetCount()):
