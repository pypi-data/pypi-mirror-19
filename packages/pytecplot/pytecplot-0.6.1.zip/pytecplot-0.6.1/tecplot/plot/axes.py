...
from .. import session
from ..tecutil import (ArgList, inherited_property, flatten_args, lock,
                       log_setattr, sv)
...
class OrientationAxes(session.Style):
    """The orientation axes for 3D Field plots.

    This is the small (x, y, z) reference axes object which can moved, resized
    and modified using this class::

        >>> oaxes = plot.axes.orientation_axes
        >>> oaxes.size *= 2.0
        >>> oaxes.position = (15, 15)
        >>> oaxes.show_variable_name = True
    """
    def __init__(self, axes):
        self.axes = axes
        super().__init__(axes._sv, sv.FRAMEAXIS, **axes._style_attrs)

    @property
    def show(self):
        """Enable drawing of the orientation axes.

        :type: `boolean <bool>`

        Example usage::

            >>> plot.axes.orientation_axes.show = False
        """
    ...
    @property
    def size(self):
        """Size of the orientation axes.

        :type: `float`

        Example usage::

            >>> plot.axes.orientation_axes.size = 4.0
        """
    ...
    def line_thickness(self):
        """Line thickness used when drawing the orientation axes.

        :type: `float`

        Example usage::

            >>> plot.axes.orientation_axes.line_thickness = 0.8
        """
    ...
    def color(self):
        ...
        Example usage::
            ...
            >>> from tecplot.constant import Color
            >>> plot.axes.orientation_axes.color = Color.Cyan
        """
    ...
    def position(self):
        ...
        The position is in percent from the lower-left corner of the viewport::
            ...
            >>> plot.axes.orientation_axes.position = (15, 15)
        """
        x = self._get_style(float, sv.XYPOS, sv.X)
        y = self._get_style(float, sv.XYPOS, sv.Y)
        return OrientationAxes._Position(x, y)

    @position.setter
    def position(self, *pos):
        pos = OrientationAxes._Position(*flatten_args(*pos))
    ...
    def show_variable_name(self):
        ...
        Example usage::
            ...
            >>> plot.axes.orientation_axes.show_variable_name = True
        """
