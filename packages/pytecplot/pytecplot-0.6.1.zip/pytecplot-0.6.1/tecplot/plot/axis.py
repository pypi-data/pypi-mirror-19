...
from ..tecutil import Index, lock, log_setattr, sv
...
class Axis(session.Style):
    ...
    def fit_range(self):
        ...
        with self.axes.plot.frame.activated():
            axis_name = ctypes.c_char(bytes(self.name[0], 'utf-8'))
            axis_num = getattr(self, 'index', 0) + 1
            if not _tecutil.ViewAxisFit(axis_name, axis_num):
    ...
    def fit_range_to_nice(self):
        ...
        with self.axes.plot.frame.activated():
            axis_num = getattr(self, 'index', 0) + 1
            if not _tecutil.ViewAxisNiceFit(ctypes.c_char(bytes(self.name[0], 'utf-8')), axis_num):
                raise TecplotSystemError()
    ...
    def adjust_range_to_nice(self):
        ...
        This adjusts the axis label values such that all currently displayed
        values are set to have the smallest number of significant digits
        possible::
        ...
        with self.axes.plot.frame.activated():
            axis_num = getattr(self, 'index', 0) + 1
            if not _tecutil.ViewAxisMakeCurValsNice(ctypes.c_char(bytes(self.name[0], 'utf-8')), axis_num):
                raise TecplotSystemError()
...
class FieldAxis(DataAxis):
    ...
    @lock()
    def fit_range(self, consider_blanking=False):
        """Set range of axis to variable minimum and maximum.

        Example usage::

            >>> plot.axes.x_axis.fit_range(consider_blanking=True)
        """
        with self.axes.plot.frame.activated():
            if consider_blanking:
                cmd = r'''\
                    $!VIEW AXISFIT
                      ...
                      AXIS = '{}'
                      AXISNUM = 1'''.format(self.name[0])
                macro.execute_command(cmd)
            else:
                axis_name = ctypes.c_char(bytes(self.name[0], 'utf-8'))
                if not _tecutil.ViewAxisFit(axis_name, 1):
                    raise TecplotSystemError()
...
class AxisLine2D(AxisLine):
    ...
    (`Cartesian2DAxisLine`). Polar plots use this class directly. Example
    usage::
