...
class IsosurfaceGroupStyle(Style):
...
class IsosurfaceContour(IsosurfaceGroupStyle):
    ...
    def use_lighting_effect(self):
        ...
        .. note::
            Setting `IsosurfaceContour.use_lighting_effect` will also set
            the same value for `IsosurfaceShade.use_lighting_effect`,
            and vice-versa.
...
class IsosurfaceShade(IsosurfaceGroupStyle):
    ...
    def use_lighting_effect(self):
        ...
        .. note::
            Setting `IsosurfaceShade.use_lighting_effect` will also set
            the same value for `IsosurfaceContour.use_lighting_effect`,
            and vice-versa.
...
class IsosurfaceGroup(Style):
    ...
    @property
    def marching_cubes_algorithm(self):
        """Marching cubes algorithm.

        Select a marching cube algorithm:

            * `MarchingCubeAlgorithm.Classic` - This uses the original marching
               cubes algorithm, which only considered 15 different ways to
               slice a cube.
            * `MarchingCubeAlgorithm.ClassicPlus` - This enhances the classic
               algorithm by filling in portions of a cell when we have saddle
               points. This eliminates holes in the isosurface but does not
               work with szl-backed data.
            * `MarchingCubeAlgorithm.MC33` - This uses an enhanced algorithm
               which considers over 255 separate use cases for how cubes can be
               sliced. This is more accurate, eliminates holes in the isosurfaces
               and works with szl backed data.
        ...
        """
        return self._get_style(MarchingCubeAlgorithm, sv.MARCHINGCUBEALGORITHM)

    @marching_cubes_algorithm.setter
    def marching_cubes_algorithm(self, value):
        self._set_style(MarchingCubeAlgorithm(value), sv.MARCHINGCUBEALGORITHM)
