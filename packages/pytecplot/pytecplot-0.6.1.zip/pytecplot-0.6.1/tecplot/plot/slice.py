...
from ..tecutil import Index, sv, _tecutil, color_spec
from numbers import Integral
from collections import namedtuple
from ..tecutil.util import lock


class SliceGroupStyle(Style):
...
class SliceEdge(SliceGroupStyle):
    ...
    def edge_type(self):
        ...
        There are two types of edges in |Tecplot 360 EX|:
        creases and borders.
...
class SliceGroup(Style):
    ...
    def _set_slice_position(self, value, style):
        ...
        if self._is_ijk():
            self._set_style(int(value[0]), style, sv.I)
            self._set_style(int(value[1]), style, sv.J)
            self._set_style(int(value[2]), style, sv.K)
        else:
    ...
    def _get_slice_position(self, style):
        ...
        style_type = int if is_ijk else float
    ...
    def set_arbitrary_from_points(self, p1, p2, p3):
        ...
        if not _tecutil.SliceSetArbitraryUsingThreePoints(
                self.uniqueid, self.offset1+1, *itertools.chain(p1, p2, p3)):
