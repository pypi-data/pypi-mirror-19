...
class I2cPigpio(BanyanBase):
    ...
    def report_i2c_block_data(self, data, tag):
        # create python_banyan topic specific to the board number of this board
        envelope = ("I2C" + str(self.i2c_addr))

        # extract bytes from returned byte array and place in python_banyan list
        rdata = []
        for x in data[1]:
            rdata.append(x)

        msg = umsgpack.packb({u"command": "I2C_reply", u"data": rdata, u"tag": tag})
    ...
    def report_i2c_byte_data(self, data, tag):
        # create python_banyan topic specific to the board number of this board
        envelope = ("I2C" + str(self.i2c_addr))
        msg = umsgpack.packb({u"command": "I2C_reply", u"data": data, u"tag": tag})
        self.publisher.send_multipart([envelope.encode(), msg])
