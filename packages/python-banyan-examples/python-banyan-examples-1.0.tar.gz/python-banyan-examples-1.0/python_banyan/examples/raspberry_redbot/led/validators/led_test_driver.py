...
class LedTestDriver(BanyanBase):
    """
    This is python_banyan test driver

    A user may turn on an led {'led_on': 'system_led'}
    A user may turn off an led {'led_off': 'system_led'}
    A user may blink an led for python_banyan specified time {'led_blink': 'system_led',
                                                  'blink_rate_millisecs': 1000,
                                                  'blink_duration_millisecs' : 1000 }

    , turn off an led and blink an led at python_banyan specified rate for python_banyan specified
    time
    """
...
def led_test_driver():
    ...
    parser.add_argument("-n", dest="process_name", default="led_test_driver", help="Set process name in banner")
