...
def cancel_order(order_id):
    Exch.get_context().get_order(order_id).cancel()


def get_open_orders():
    return Exch.get_context().get_open_orders()


def get_order(order_id):
    return Exch.get_context().get_order(order_id)
...
def history(bar_count, field=None, symbol=None):
    return Exch.get_context().data.history(bar_count, field, symbol)
...
def write_pipeline(name, factor, start_date=DateDefault.start, end_date=DateDefault.end, engine=Config.container):
    container = cls(engine, ModCls.container)(Exch.get_context().data.source, factor, start_date, end_date)
    container.run()
...
def read_pipeline(name, bar_count, symbols=slice(None)) -> DataFrame:
    ...
    stop = Exch.get_context().data.trading_day_ix + 1
