...
class Portfolio:
    ...
    def update_transaction(self, trade):
        ...
        pos.update_transaction(trade)

        self.cash -= trade.cost
        self.update_freezed(trade.freezed)

    def update(self, trading_day: datetime, quotes):
        if not self.start_date:
            self.start_date = trading_day

        self.market_value = 0
        for symbol, pos in self.positions.items():
            pos.update(trading_day, quotes[symbol])
        ...
        self.returns.update(trading_day, last_vaule, self.value)
