...
class Order:
    ...
    def shares(self, symbol, amount):
        ...
        lot = Exch.get_context().data.last("lot", symbol)
        amount = int(amount) // lot * lot if amount > 0 else int(amount)
        return self._open(amount)

    def lots(self, symbol, amount):
        return self.shares(symbol, amount * Exch.get_context().data.last("lot", symbol))
    ...
    def target_position(self, symbol, amount):
        return self.shares(symbol, amount - Exch.get_context().portfolio.get_position(symbol).quantity)
...
class ExchContext:
    ...
    def get_order(self, order_or_id) -> Order:
        if isinstance(order_or_id, Order):
            return self._total_orders[order_or_id.id]
        elif isinstance(order_or_id, int):
            return self._total_orders[order_or_id]
