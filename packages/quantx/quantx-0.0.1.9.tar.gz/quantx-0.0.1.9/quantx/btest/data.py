...
from pandas import DataFrame, Series, to_datetime, Timestamp, isnull
...
class Data:
    ...
    @property
    def trading_day(self) -> Timestamp:
    ...
    def get_bar(self, symbol):
        return self.last(symbol=symbol)

    def is_stop(self, symbol):
        return not self.get_bar(symbol).active

    def is_delist(self, symbol):
        close = self.last("close", symbol)
        return isnull(close) or not close
