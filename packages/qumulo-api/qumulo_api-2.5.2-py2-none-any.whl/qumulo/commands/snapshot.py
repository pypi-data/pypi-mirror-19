...
SNAP_TTL_HELP = ("Duration after which to expire the snapshot, in format "
    ...
    "seconds], e.g. 5days or 1hours. Empty string indicates the snapshot "
    "should never expire.")
...
class CreateSnapshotCommand(qumulo.lib.opts.Subcommand):
    ...
    def main(conninfo, credentials, args):
        print snapshot.create_snapshot(
            ...
            args.name, args.expiration, args.time_to_live)
...
def parse_period(period_str):
    ...
    if units_str.startswith('m'):
        units = 'FIRE_IN_MINUTES'
    elif units_str.startswith('h'):
        units = 'FIRE_IN_HOURS'
    else:
...
def create_hourly_or_less(conninfo, credentials, args):
    print snapshot.create_policy(
        conninfo, credentials,
        args.name,
        get_schedule_info(get_schedule_hourly_or_less(args),
...
def get_schedule_monthly(args):
    ...
    return {
        ...
        'timezone' : args.timezone,
        'day_of_month' : 255 if args.last_day_of_month else args.day_of_month,
        'hour' : at_time_of_day.hour,
        'minute' : at_time_of_day.minute,
    }
...
def create_monthly(conninfo, credentials, args):
    print snapshot.create_policy(
        conninfo, credentials,
        args.name,
        get_schedule_info(get_schedule_monthly(args),
            args.time_to_live if args.time_to_live else ''))
...
def change_name_and_ttl(conninfo, credentials, args):
...
class ModifyPolicyCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        change_name_and_ttl_parser = subparsers.add_parser(
            'modify_name_and_or_time_to_live',
            parents=[common_parser],
            formatter_class=argparse.ArgumentDefaultsHelpFormatter)
        change_name_and_ttl_parser.set_defaults(func=change_name_and_ttl)
...
class GetPolicyCommand(qumulo.lib.opts.Subcommand):
    ...
    DESCRIPTION = 'List a single policy.'
