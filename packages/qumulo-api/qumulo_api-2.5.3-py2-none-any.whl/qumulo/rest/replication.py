...
@request.request
def replicate(conninfo, credentials, replication_source, address, port):
    body = {'replication_source': replication_source,
            'address': address,
            'port': port}
    method = "POST"
    uri = UriBuilder(path="/v1/replication/replicate")
    return request.rest_request(
        conninfo, credentials, method, unicode(uri), body=body)
