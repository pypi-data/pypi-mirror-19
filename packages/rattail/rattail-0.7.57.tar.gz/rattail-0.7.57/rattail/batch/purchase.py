...
class PurchaseBatchHandler(BatchHandler):
    ...
    def refresh_totals(self, row, cost, initial):
        ...
        if batch.mode == self.enum.PURCHASE_BATCH_MODE_ORDERING:
            row.po_unit_cost = self.get_unit_cost(product, batch.vendor)
            if row.po_unit_cost:
    ...
    def get_units_ordered(self, row):
        return (row.units_ordered or 0) + row.case_quantity * (row.cases_ordered or 0)

    def get_units_shipped(self, row):
        units_received = (row.units_received or 0) + row.case_quantity * (row.cases_received or 0)
        ...
        return units_received + units_damaged + units_expired
