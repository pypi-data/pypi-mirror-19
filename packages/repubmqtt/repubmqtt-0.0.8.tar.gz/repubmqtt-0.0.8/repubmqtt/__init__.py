...
class RuleSet:
	...
	def publish(self, pub, new_record):
		...
		for publish in publish_dict:
			...
			elif publish['protocol'] == 'restful':
				self.publish_restful(publish, output_data, new_record)
			elif publish['protocol'] == 'mqtt':
				self.publish_mqtt(publish, output_data, new_record)
	...
	def publish_mqtt(self, publish, output_data, data):
		...
		if publish['_testmode']:
			...
			return
		self.repub.mqttclient.publish(topic, output_data, retain=retain)
	...
	def publish_restful(self, publish,  output_data, data):
		url = publish['url'] % data
		headers = publish['headers'] % data
		if publish['_testmode']:
			log('test', "publish_restful (%s) %s" % (url, output_data))
			return
		...
		dbgprint(1, "restful: url='%s' data='%s'" % (url, output_data))
