		...
		for match_ops in filter_match_list:
			...
			if match_type == 're':
				...
				try:
					...
					r = False
				if DBG >= 3: logger.debug("eval: %s %s %s --> %s", msgv, match_type, matchv, r)
				if r:
					match_result = r
					break   # 
			else:
...
class Republish:
	...
	def process_message(self, jrecord):
		...
		def filter_record(self, jrecord, filter_code):
		...
		def select_record(jrecord, selector_name):
			try:
				select_result = extractfromdict(jrecord, selector_name)
			except KeyError as e:
				logger.error("selector ruleset '%s', '%s' field %s not in record '%s'", rule_name, selector_name, e, str(jrecord))
				return None
			except ValueError as e:
				logger.error("selector ruleset '%s', '%s' field %s  value mismatch  '%s'", rule_name, selector_name, e, str(jrecord))
				return None
			except SyntaxError as e:
		...
		for rule in self.rulesets:
			...
			for filter_name, selector_name, publish_name in ruleset['rules']:
				...
				if not selector_name:
					...
					select_result = select_record(jrecord, selector_name)
