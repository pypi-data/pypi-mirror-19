...
# noinspection PyBroadException
class QuickDecryptItem(BackedUpItem):
    """ Class which handles the quick decryption of only one file. This is a bastardized version of BackedUpItem as we can't call BackedUpItem.decrypt_file()
    without instantiating the class with things like path to SQL database, AWS S3 credentials, etc and we can't supply all of those when we just want to
    decrypt only one file which was manually fetched from S3
    DO NOT USE ANY OTHER METHOD BESIDE self.decrypt_file() as the constructor is missing most of the critical info
    """

    def __init__(self, args, encrypt_password):
        """
        :args                  => argparse object containing parameters specified on the command line
        :encrypt_password      => password to use for encrypting files (when performing file encryption)
        """
...
def get_args():
    ...
    parser.add_argument('-c', '--config', required=False, action="store", default=None,
                        help='Path to configuration file. Configuration file is mandatory for all commands except "quick_decrypt". If the command '
                             '"quick_decrypt" is specified then if you also specified a config file then that is ignored')
    parser.add_argument('command', action="store", choices=['backup', 'initdb', 'list_remote', 'restore', 'stats', 'syncdb_remote', 'update_s3_metadata',
                                                            'quick_decrypt'], help='Command to run')

    arguments = parser.parse_args()

    # if command is quick_decrypt then don't bother checking if we also had a config file specified (as we'll ignore it)
    if arguments.command == 'quick_decrypt':
        return arguments
    elif arguments.config is None:
        logging.error('any command except "quick_decrypt" requires that you also specify a config file')
        sys.exit(1)
...
def quick_decrypt(args):
    """ Decrypt file after prompting the user for the path to the input file, path for the output and the password to use for decypting
    :param args: object returned from get_args()
    :return: True on Success or False otherwise
    """
    try:
        while True:
            file_to_decrypt = raw_input("Please specify file to decrypt: ")
            if not os.path.isfile(file_to_decrypt) or not os.access(file_to_decrypt, os.R_OK):
                logging.error('The specified input file "{}" does not exist or you don\'t have permissions to read it.'.format(file_to_decrypt))
            else:
                break
        while True:
            output_folder = raw_input("Please specify folder where to save decrypted file. The output file will have the exact same name as the input file. "
                                      "Output folder: ")
            if not os.path.isdir(output_folder) or not os.access(output_folder, os.W_OK):
                logging.error('The specified output folder "{}" does not exist or you don\'t have permissions to write in that folder.'.format(output_folder))
            else:
                break
        password = getpass.getpass('Decrypt password: ')
    except KeyboardInterrupt:
        logging.error('Cancelled by user via CTRL + C')
        sys.exit(1)
    target_file = output_folder + os.sep + file_to_decrypt.rsplit(os.sep, 1)[1]
    logging.info('Attempting to decrypt file "{}" and write result to "{}"'.format(file_to_decrypt, target_file))
    item_to_process = QuickDecryptItem(args=args,encrypt_password=password)
    item_to_process.decrypt_file(in_filename=file_to_decrypt, out_filename=target_file,check_size=False)
    print ""
...
def main():
    ...
    if args.command == 'quick_decrypt':
        # switch log level to INFO if running a quick_decrypt if we did not have DEBUG logging selected
        if not args.debug:
            logging.getLogger().setLevel(logging.INFO)
        logging.info('Performing a quick_decrypt which means one file will be decrypted and a config file is not necesarry. You must fetch yourself the file '
                     'and place it somewhere on the local machine so the tool can read it.\nThis option is useful if you downloaded from AWS S3 a file you had'
                     ' encrypted and don\'t want to setup a full blown config file just so you can decrypt the downloaded file')
        quick_decrypt(args=args)
