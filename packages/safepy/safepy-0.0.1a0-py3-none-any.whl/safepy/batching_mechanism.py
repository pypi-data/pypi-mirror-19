...
def unpack_result(result, index):
    return result[index]


def return_result(result, index):
    return result


def return_result_copy(result, index):
    return copy(result)
...
def batched_args(max_batch_size, timeout):
    def decorator(function):
        batch_factory = partial(LimitedBatch, max_batch_size=max_batch_size)
        batching_mechanism = BatchingMechanism(function, timeout, batch_factory)
...
def batched(timeout: float, max_batch_size: int):
    batch_factory = partial(LimitedBatch, max_batch_size=max_batch_size)
    result_distribution_strategy = return_result
