...
def cli(ctx,
    ...
    cmd = ['1sidehphobe_markms', working_dir/"rec_nmin.pdb", working_dir/"rec_nmin.ms"]
    run(cmd, stderr=DEVNULL, check=True)

    if rec_psf is None:
        ctx.fail("receptor PSF required")
    ...
    if not movable_sidechains:
        logger.error("No movable sidechains")
        ctx.fail()
