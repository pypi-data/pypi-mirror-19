...
def _payload(object_, holder, right, namespace):
    return {
        'object'    : object_,
        'holder'    : holder,
        'right'     : right,
...
def search(objects=None, holders=None, rights=None, namespace=None, privileged=False):
    http_method = 'GET'
    filters = []
    if objects:
...
def create(object_, holder, right, namespace, privileged=False):
    http_method = 'POST'
    payload = _payload(object_, holder, right, namespace)
    response = _send_request(http_method, payload=payload, privileged=privileged)
    return response.headers['Location']

def update(uri, object_, holder, right, namespace, privileged=False):
    http_method = 'PUT'
    payload = _payload(object_, holder, right, namespace)
