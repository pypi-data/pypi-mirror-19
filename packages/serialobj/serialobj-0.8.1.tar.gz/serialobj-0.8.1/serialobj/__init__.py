...
AdvancedField = namedtuple('AdvancedField', ['type', 'default', 'required'])


def required(type_, default=None):
    return AdvancedField(type_, default, True)


def default(type_, default):
    return AdvancedField(type_, default, False)
...
class SerialObjectMeta(type):
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        default = attrs.get('__default__', {})
        required = attrs.get('__required__', set())
        ...
        def __init__(self, *args, **kwargs):
            for key in self.__fields__.keys():
                data = kwargs.get(key)
                if data is None:
                    data = self.__default__.get(key)
                if data is None and key in self.__required__:
                    raise StrictError("Argument '{}' is required.".format(key))
