...
class SearchQuery:
    ...
    def _name(self):
        return (
            self.release.effective_search_name
            .replace('_', ' ')
            .replace('\'', '')
        )
...
class SearchResults:
    ...
    def __init__(self, query, results, min_seeders) -> None:
        self.query = query
...
class TorrentSearch(Process, Logging):
    ...
    def __init__(self, monitor, requester, search_engine, min_seeders) -> None:
        self._result = Queue()
    ...
    def search(self, monitor):
        return (
            ...
            .map2(L(SearchResults)(_, _, self.min_seeders))
        )
