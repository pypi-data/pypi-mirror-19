...
from .commands.envvarsmanager import get_env_vars_cmd, delete_env_var_cmd, put_env_vars_cmd
from .commands.pipes import get_pipe_cmd, get_pipes_cmd, add_pipes_cmd, delete_pipe_cmd, modify_pipe_cmd
    ...
    if hasattr(args, "subparser_func"):
        ...
        if argspec.args[0] == "connection":
            ...
            if jwt_auth_token is not None:
                ...
                remaining_time_until_expiry = int(expire_time - time.time())
                if remaining_time_until_expiry < 0:
                    if got_jwt_from_commandline_argument:
                        main_utils.exit_with_error(
                            "The JWT authorization token in the '%s' file expired %d seconds ago." % (
                               jwt_filename, -remaining_time_until_expiry))
                    else:
                        main_utils.exit_with_error(
                            ("The sesam client is logged in (that is, there is a JWT authorization token in the "
                             "'%s' file), but the JWT authorization token expired %d seconds ago. Tip: do a new "
                             "'sesam login' and try again.") % (get_configfilename(), -remaining_time_until_expiry))
