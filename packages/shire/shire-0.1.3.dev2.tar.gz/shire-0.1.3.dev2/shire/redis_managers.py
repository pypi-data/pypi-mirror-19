...
class PoolStatusManager(BaseRedisManager):
    ...
    def get_all(self, pool=None):
        if pool:
            return self.db.keys(self.POOL_PATH.format(pool=pool) + ':*')
        return self.db.keys(self.BASE_PATH + ':*')

    def get_status(self, pool, _uuid):
        return self.db.hget(self.UUID_PATH.format(pool=pool, uuid=_uuid), 'status')
