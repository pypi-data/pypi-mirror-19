...
class Options(object):
    ...
    def __init__(self, enable_sjts=None, enable_msgpack=None, point_serializer='shooju_point'):
        super(Options, self).__init__()
        self._enable_sjts = self._enable_msgpack = None
        if enable_sjts is not None:
            self.enable_sjts = enable_sjts
        if enable_msgpack is not None:
            self.enable_msgpack = enable_msgpack
        self.default_serializer = self._choose(msgpack, json)  # auto by default
        self.series_serializer = self._choose(sjts, msgpack, json)
        self.point_serializer = point_serializer

    @property
    def enable_sjts(self):
        return self._enable_sjts

    @enable_sjts.setter
    def enable_sjts(self, val):
        if val:
            assert sjts is not None, 'sjts package is not installed'
            self.series_serializer = sjts
        else:  # disable if False
            self.series_serializer = self.default_serializer
        self._enable_sjts = val

    @property
    def enable_msgpack(self):
        return self._enable_msgpack

    @enable_msgpack.setter
    def enable_msgpack(self, val):
        if val:
            assert msgpack is not None, 'msgpack package is not installed'
            self.default_serializer = msgpack
        else:
            self.default_serializer = json
        self._enable_msgpack = val
...
class PandasFormatter(object):
    ...
    def search(self, query='', date_start=None, date_finish=None,
        ...
        response = self.api_client.search_series(query, date_start, date_finish, max_points,
                                                 per_page=per_page, fields=fields, size=size, query_size=query_size)
    ...
    def get_points(self, series_id, date_start=None, date_finish=None, size=10):
        ...
        points = response.get('points', []) if response is not None else []
        index = [milli_to_datetime(p[0]) for p in points]
    ...
    def get_fields(self, series_id, fields='*'):
        ...
        return pandas.Series(response.get('fields') if response is not None else {})
        ...
        if has_points and has_fields:
            ...
            for s in response['series']:
                ...
                if key_field:
                    ...
                    key = s['series_id']
                for p in s.get('points', []):
                    dates.append(milli_to_datetime(p[0]))
                    values.append(p[1])
                data[key] = pandas.Series(index=dates, data=values)
        else:
            data = list()
            for s in response['series']:
...
class ShoojuApi(object):
    ...
    def get_series(self, series_id, date_start=None, date_finish=None,
        ...
        return self.client.get(self.API_SERIES, params=params, deserializer=options.series_serializer)['series'][0]
...
class Client(object):
    ...
    def _call(self, path=None, ignore_exceptions=False, params=None,
        ...
        if binary_response:
            ...
            return _check_errors(deserializer.loads(r.content))
        elif json_response:
            return deserializer.loads(r.content)
        return r.text
...
class GetBulk(object):
    ...
    def _process_response(self, get_type, response, index):
        ...
        if get_type == "get_points":
            return list(map(lambda p: point(p[0], p[1]), response.get('points', [])))
