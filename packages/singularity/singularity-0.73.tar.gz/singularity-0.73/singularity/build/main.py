...
def run_build(build_dir,params,verbose=True):
    ...
    if os.path.exists(params['spec_file']):
        ...
        if params['size'] == None:
            bot.logger.info("Size not detected for build. Will estimate with %sMB padding.",padding)
        ...
        tags = get_tags(package=image_package,diff=diff)
        ...
        os_sims = estimate_os(package=image_package,return_top=False)
