...
class VHost:
	def __init__ (self, wasc, clusters, cachefs, static_max_age, enable_apigateway, apigateway_authenticate, apigateway_realm):
		self.wasc = wasc
		...
		if enable_apigateway:
			self.access_handler = api_access_handler.Handler (self.wasc, apigateway_authenticate, apigateway_realm, self.proxypass_handler)
