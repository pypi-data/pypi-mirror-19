...
WEBSOCKET_MULTICAST: 
  ...
  - Deprecated in version 0.24
  - Replaced with non-threading async version: WS_GROUPCHAT
...
.. code:: python
  ...
  def echo (was, message, client_id):
    if was.wsinit ():
      return was.wsconfig (skitai.WS_SIMPLE, 60)

    return "ECHO:" + message

First 2 args (message, client_id except 'was') are essential. Although you need other args, you must position after theses 2 essential args. client_id is unique channel id for distinquishing websocket channel and it makes managing client state using global object.
...
.. code:: python
  ...
  @app.route ("/websocket/echo")
  def echo (was, message, client_id, clinent_name):
    global num_sent
...
In this case, first 4 args (message, client_id, room_id, event = None) are essential.
