...
class	WAS (Skitai.Loader):
	...
	def configure (self):
		...
		if config.getopt ("server", "ssl") in ("yes", "1") and config.getopt ("server", "certfile"):
			certfile, keyfile = config.getopt ("server", "certfile"), config.getopt ("server", "keyfile")
			...
			self.config_certification (certfile, keyfile, config.getopt ("server", "passphrase"))
		...
		self.config_webserver (
			...
			config.getint ("server", "keep_alive", 5),
		...
		self.install_handler (
			...
			config.getopt ("server", "enable_proxy") == "yes",  
			config.getint ("server", "static_max_age", 300),
			config.getopt ("server", "blacklist_dir", None)
		)
