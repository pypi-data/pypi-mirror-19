...
class	WAS (Skitai.Loader):
	...
	def test_config (cls, conf):
		...
		if config.getopt ("ssl", "ssl") == "yes":
			assert config.getopt ("ssl", "certfile"), "enable ssl but certfile is not given"
			assert config.getopt ("ssl", "pass_phrase"), "enable ssl but pass_phrase is not given"
		assert config.getint ("server", "port", 5000) > 0, "posrt should br int > 0"		
		assert config.getint ("performance", "static_max_age", 300) >= 0, "static_max_age should be int >= 0"
		assert config.getint ("performance", "keep_alive", 10) >= 0, "keep_alive should be int >= 0"
		assert config.getint ("performance", "response_timeout", 10) >= 0, "response_timeout should be int >= 0"
		assert config.getint ("performance", "num_result_cache_max", 1000) >= 0, "num_result_cache_max should be int >= 0"
	...
	def configure (self):
		...
		if config.getopt ("ssl", "ssl") in ("yes", "1") and config.getopt ("server", "certfile"):
			certfile, keyfile = config.getopt ("ssl", "certfile"), config.getopt ("ssl", "keyfile")
		...
		self.config_rcache (config.getint ("performance", "num_result_cache_max", 1000))
		...
		self.config_webserver (
			...
			config.getopt ("ssl", "ssl") in ("yes", "1") or False,
			config.getint ("performance", "keep_alive", 5),
			config.getint ("performance", "response_timeout", 10)
		)
		...
		self.install_handler (
			...
			config.getint ("performance", "static_max_age", 300),
			config.getopt ("etc", "blacklist_dir", None),
			config.getopt ("proxy", "enable_unsecure_https") == "yes"
		)
