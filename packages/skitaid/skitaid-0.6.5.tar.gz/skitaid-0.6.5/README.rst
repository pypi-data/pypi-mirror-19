...
API Gateway Server Authentification
====================================
...
But Skitai only support API token authorization for accessing API Gateway.
...
Object Tokens may be any object has get (token) method. This method return tuple has 3 elements:

.. code:: python

  def get (token):
    # username, roles, timeout
    return ("hansroh", ["user", "admin"], 0)

If timeout is 0, this token is never expired.
