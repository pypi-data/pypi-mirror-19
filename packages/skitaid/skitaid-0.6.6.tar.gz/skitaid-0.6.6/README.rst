...
- 0.6.5 - Support bearer authorization
...
But Skitai only support API token authorization for accessing API Gateway. And ID-Password authentificating, token generating, providing and storing process should be built in seperatly. API Gateway server just need some information about permisssion related things with token.
...
.. code:: python
  ...
  def startup (wasc):
  	storage = Tokens ()
  	wasc.handler.set_token_storage (storage)
  	
  @app.route ("/")
  def index (was):
  	return "<h1>API Gateway</h1>"

Object Tokens may be any object has get (token, request, callback) method. This method should call callback with request and (username, roles, timeout) tuple. If timeout is 0, this token is never expired.
...
.. code:: python
  ...
  requests.get (
    ...
    headers={
    "Authorization": "Bearer 12345678-1234-123456"
  )
