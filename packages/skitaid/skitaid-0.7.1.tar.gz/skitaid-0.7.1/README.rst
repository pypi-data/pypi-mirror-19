...
.. code:: python
  ...
  class Authorizer:
    ...
    def handle_claim (self, request, callback):
      ...
      if expires and expires < time.time ():
        return callback (request)
      callback (request, claim.get ("username"), claim.get ("roles"))
...
Object Tokens may be any object has handle_token or handle_claim with receiving args (request, callback) method. This method should call callback with request, username, roles. 'handle_token' could get user information including username and roles by a token but it needn't for handle_claim. For more information about JWT visit JWT_ homepage.
