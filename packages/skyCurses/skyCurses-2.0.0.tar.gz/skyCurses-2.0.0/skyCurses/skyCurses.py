...
class SkyCurses(SkyChatClient):
    def on_creation(self):
        ...
        self.chat_win         = WDisp(self.main_win,  Coord(4, 0),            Coord(-8, (0.85, True)), borders=[0]*8)
        self.input_win        = WInput(self.main_win, Coord(-3, 0),           Coord(1, (1, True)),     borders=[0]*8)
        self.clients_list_win = WList(self.main_win,  Coord(4, (0.85, True)), Coord(-8, (0.15, True)), borders=[0]*8)
        ...
        # Sounds
        self.sounds = {
                'nomi' : simpleaudio.WaveObject.from_wave_file(full_path("sounds/nomi.wav")),
                'msg'  : simpleaudio.WaveObject.from_wave_file(full_path("sounds/msg.wav")),
                'mp'   : simpleaudio.WaveObject.from_wave_file(full_path("sounds/mp.wav")),
                'end'  : simpleaudio.WaveObject.from_wave_file(full_path("sounds/end.wav")),
                }
    ...
    def on_message(self, msg):
        ...
        if msg['pseudo'].lower() != self.pseudo.lower():
            if msg['message'].lower().find('@' + self.pseudo.lower()) != -1:
                ...
                self.sound_on and self.sounds['msg'].play()
    ...
    def on_private_message(self, msg):
        if msg['pseudo'].lower() == self.pseudo.lower():
            ...
            self.sound_on and self.sounds['mp'].play()
    ...
    def on_yt_sync(self, msg):
        ...
        if self.player_on and 'id' in msg and msg['id'] != self.current_vid:
    ...
    def _execute(self, msg):
        commands = {
                ...
                'mute'    : lambda : self._set_player(False),
                'unmute'  : lambda : self._set_player(True),
                }
    ...
    def _handle_spec_key(self, key):
        keys = {
                'KEY_PPAGE' : lambda : self.chat_win.change_offset(-4),
                'KEY_NPAGE' : lambda : self.chat_win.change_offset(4),
                'KEY_HOME'  : lambda : self.chat_win.change_offset(0, True),
                'KEY_END'   : lambda : self.chat_win.change_offset(-1, True),
                'CTRL_N'    : lambda : change_chat(1),
                'CTRL_P'    : lambda : change_chat(-1),
                'KEY_UP'    : lambda : self._get_history(-1),
                'KEY_DOWN'  : lambda : self._get_history(1),
                }
    ...
    def _set_title(self):
        self.title_win.set_title("SkyCurses, the unofficial curses SkyChat Client (%s) " % (self.chat_list[self.cur_chat]) + ("Q" if not self.sound_on else "") + ("M" if not self.player_on else ""))
