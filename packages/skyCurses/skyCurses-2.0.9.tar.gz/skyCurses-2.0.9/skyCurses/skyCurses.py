...
class SkyCurses(SkyChatClient):
    ...
    def _disp_msg(self, msg, chat='main'):
        ...
        pseudo_color = color_equiv[msg['color']][0] if 'color' in msg else 'b'
    ...
    def _handle_spec_key(self, key):
        ...
        def change_chat(shift):
            ...
            self._change_chat(chat_num)

        if key in keys:
            keys[key]()

    def _change_chat(self, chat_num):
        chat_num = int(chat_num)
        if 0 <= chat_num < len(self.chat_list) and chat_num != self.cur_chat:
            self.cur_chat          = chat_num
            self.chat_win.offset   = 0
            self.chat_win.raw_hist = self.raw_msg_hist[self.chat_list[self.cur_chat]][-CHAT_BUFF_SIZE:]
