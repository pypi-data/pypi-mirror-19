...
class SkyWiseResource(object):
    ...
    def find_by_id_async(cls, resource_id, headers=None):
        ...
        if cls._use_session_for_async:
            ...
            greq = grequests.get(url, auth=HTTPBasicAuth(cls.get_user(), cls.get_password()), headers=headers)
    ...
    def find_async(cls, id_=None, headers=None, **kwargs):
        ...
        if cls._use_session_for_async:
            r = grequests.get(url, session=cls._s(), headers=headers)
        else:
            r = grequests.get(url, auth=HTTPBasicAuth(cls.get_user(), cls.get_password()), headers=headers, params=args)
    ...
    def _get(cls, id_, headers=None, **kwargs):
        ...
        args = cls._path_args(**kwargs)
        res = cls._s().get(url, params=args, headers=headers)
        if res.status_code != 200:
            try:
...
class SkyWiseJSON(object):
    ...
    def destroy(self, **kwargs):
        kwargs.update(self._data)
        url = self._resource_path(self.id, **kwargs)
        res = self._s().delete(url, headers=self.get_headers())
        if res.status_code != 200:
            res.raise_for_status()
        self.id = None
    ...
    def _post(self, **kwargs):
        ...
        res = self._s().post(url, data=json.dumps(data), headers=self.get_headers())
    ...
    def _put(self, **kwargs):
        ...
        if self._serialize:
            self._serialize.extra = REMOVE_EXTRA
            data = self._serialize(data)
        res = self._s().put(url, data=json.dumps(data), headers=self.get_headers())
        if res.status_code != 200:
            res.raise_for_status()
        data = res.json()
        if self._deserialize:
...
class SkyWiseResourceList(Exception):
