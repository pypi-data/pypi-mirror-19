        ...
        *args :
        ...
        while True:
            ...
            all_neighbors = get_one_exchange_neighbourhood(incumbent,
                                                    seed=self.rng.seed())
            neighbors_looked_at += len(all_neighbors)
            
            neighbors_ = map(lambda x: impute_inactive_values(x).get_array(), all_neighbors)
            neighbors_ = np.array(list(neighbors_))
            start_time = time.time()
            acq_vals = self.acquisition_function(neighbors_, *args)
            time_n.append((time.time() - start_time) / len(all_neighbors))
            best_neighbor_indx = np.argmax(acq_vals)
            
            if acq_vals[best_neighbor_indx] > acq_val_incumbent + self.epsilon:
                self.logger.debug("Switch to one of the neighbors")
                incumbent = all_neighbors[best_neighbor_indx]
                acq_val_incumbent = acq_vals[best_neighbor_indx]
                changed_inc = True
