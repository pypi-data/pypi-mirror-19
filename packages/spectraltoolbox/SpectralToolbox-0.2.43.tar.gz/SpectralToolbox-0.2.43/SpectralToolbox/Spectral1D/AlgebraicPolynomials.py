    ...
    def GradEvaluate(self, x, n, m, k=0, norm=True):
        ...
        if k == 0:
            ...
            if n == m == 0:
                ...
                m -= 1  # -1 for ConstantExtended
                n -= 1  # -1 for ConstantExtended
                ntot = m+n
                hpoly = self.base_fun.hpf.H
                c1 = hpoly.MonomialCoeffs(n, norm=norm)
                c2 = hpoly.MonomialCoeffs(m, norm=norm)
                ...
                for nn in range(ch2.shape[0]):
                    if nn % 2 == 0 and ch2[nn] != 0.:
                        pi_erf_c += ch2[nn] * factorial2(nn-1) / 2**(nn/2+1)
                        for k in range(1,nn//2+1):
                            exp_c[nn-2*k+1] += ch2[nn] * \
                                               (factorial2(nn-1) * 2**(nn//2-k) \
                                                / factorial2(nn-2*k+1)) \
                                               / 2.**(nn//2)
...
class SquaredConstantExtendedHermiteProbabilistsFunction(object):
    ...
    def GradEvaluate(self, x, n, m, k=0, norm=True):
        ...
        if k != 0:
            if m == 0 or n == 0: # Remove /sqrt(2.) zero-th function
                out *= np.sqrt(2.)
        if k == 0:
            if m >= 1 and n >= 1:
                out /= np.sqrt(2.)
