...
def var_check(change_from, change_to, seq,var_limit):
	...
	flag=1
	for one in tmp:
		if seq.count(one) < var_limit/(float(len(tmp))+1):
			flag=0
	if flag==1:
		return True
	else:
...
def recover_sam(sam_in_dir,sam_out_dir, var_limit=20,poly_limit=10,rm_multi=0):
	...
	for line in fi:
		...
		if line[0]=='@':
			...
			if len(record)==len(seq[9]) and len(record)-changed_read.count(change_to) > var_limit and var_check(change_from,change_to,seq[9],var_limit) and poly_check(seq[9],poly_limit):
