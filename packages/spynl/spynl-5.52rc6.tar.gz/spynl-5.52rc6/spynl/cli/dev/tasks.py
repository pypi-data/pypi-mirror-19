...
@task(help={'scm_url': 'The URL needed to clone the repo, either for '
...
def install(ctx, scm_url=None, developing=True, revision=None,
    ...
    if scm_url is None:
        raise Exit("[spynl dev.install] Please give the scm_url parameter.")
    ...
    if scm_type == 'git' and repo_name.split('/').endswith('.git'):
        repo_name = repo_name.split('/')[:-4]
