    ...
    for rcomp in route:
        ...
        for rbase in ["/metadata", "/v" + api_version + "/metadata"]:
            for rext in ["", ".json"]:
                ...
                app.add_url_rule(rte, 'return_metadata', return_metadata)


def return_metadata(app):
    """
    ...
    ----------
    app : :class:`flask.Flask` instance
        Flask application for the microservice.

    Returns
    -------
    `str`
        JSON representation of service metadata.
