...
from sqreen.runner import RunnerSettings, process_initial_commands
...
from sqreen.utils import configure_raven_breadcrumbs
from sqreen.exceptions import UnsupportedFrameworkVersion, UnsupportedPythonVersion
...
def before_hook_point():
    ...
    if RUNNER_THREAD_PID != os.getpid() and RUNNER_THREAD.isAlive() is False and not runner_stopped:
...
def get_session(url, token):
    ...
    the session.
    """
    LOGGER.warning("Connection to %s", url)
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                session = get_session(config['URL'], token)
