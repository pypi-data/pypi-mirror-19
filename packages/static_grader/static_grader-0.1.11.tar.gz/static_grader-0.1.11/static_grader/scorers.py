...
def precision_score(truth, test):
  """
  simplified precision_score from sklearn.

  Returns
  -------
  float : number of true positives / number of positive calls
  """
  truth = np.array(truth)
  test = np.array(test)

  return np.count_nonzero(truth) / np.count_nonzero(test)
