...
class StoneRedis(redis.client.Redis):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # retries is the number of times that reconnect will try to execute any command
        ...
        self.register = {}
        self.register['lua_multi_rpush_limit'] = self.register_script(lua_multi_rpush_limit)
        self.register['lua_rpush_limit'] = self.register_script(lua_rpush_limit)
        self.register['lua_setx'] = self.register_script(lua_setx)
        self.register['lua_hsetx'] = self.register_script(lua_hsetx)
        self.register['lua_hmsetx'] = self.register_script(lua_hmsetx)
    ...
    def rpush_limit(self, queue, value, limit=100000):
        ...
        self.register['lua_rpush_limit']([queue, limit], [value])
    ...
    def wait_for_reset_lock(self, lockname, locktime=60, auto_renewal=False):
        ''' Gets a lock or waits until able to get it without the need of lock-signal
This method works with reset_lock, reinstancing the log and not blocking on acquire for each try. Allowing the discovery faster due to abscence of lock-signal '''
        pid = os.getpid()
        caller = inspect.stack()[0][3]
        try:
            # rl = redlock.Redlock([{"host": settings.REDIS_SERVERS['std_redis']['host'], "port": settings.REDIS_SERVERS['std_redis']['port'], "db": settings.REDIS_SERVERS['std_redis']['db']}, ])
    ...
    # TODO: Valorar utilidad del parametro 'force'
    ...
    def reset_lock(self, lock):
        '''Use this with care if you want reset a lock'''
        ''' Resets a lock '''
        pid = os.getpid()
        caller = inspect.stack()[0][3]
        lock.reset()
        if self.logger:
            self.logger.debug('Process {0} ({1}) reseted lock'.format(pid, caller))
    ...
    def setx(self, key, value):
        ...
        return bool(self.register['lua_setx'](['key', 'value'], [key, value]))
    ...
    def hsetx(self, key, field, value):
        ...
        return bool(self.register['lua_hsetx'](['key', 'field', 'value'], [key, field, value]))
    ...
    def hmsetx(self, key, dictionary):
        ...
        return bool(self.register['lua_hmsetx'](keys, values))
...
class StonePipeline(redis.client.Pipeline, StoneRedis):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # max_sleep is the amount of time between reconnection attmpts by safe_reconnect
        self.max_sleep = kwargs.pop('max_sleep', 30)
        # register is a functions registereds in Redis
