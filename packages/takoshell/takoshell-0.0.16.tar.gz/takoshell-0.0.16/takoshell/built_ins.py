...
from takoshell.proc import (ProcProxy, SimpleProcProxy, pause_call_resume,
...
def _un_shebang(x):
    if x == '/usr/bin/env':
        ...
        x = 'python'
    if x == 'tako':
        return ['python', '-m', 'takoshell.main']
    return [x]
...
def run_subproc(cmds, captured=False):
    ...
    if (env.get('TAKO_INTERACTIVE') and
        ...
        pause_call_resume(prev_proc, builtins.__tako_shell__.settitle)
    if background:
        return
    if prev_is_proxy:
        prev_proc.wait()
...
def subproc_captured_inject(*cmds):
    ...
    return [i.strip() for i in run_subproc(cmds, captured='stdout')]
...
def load_builtins(execer=None, config=None, login=False, ctx=None):
    ...
    builtins.__tako_env__ = env = Env(default_env(config=config, login=login))
