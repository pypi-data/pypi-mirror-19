...
from takoshell.built_ins import load_builtins, unload_builtins


class Execer(object):
    """Executes tako code in a context."""

    def __init__(self, filename='<tako-code>', debug_level=0, parser_args=None,
                 unload=True, config=None, login=True, tako_ctx=None):
        """Parameters
        ----------
        ...
        unload : bool, optional
            Whether or not to unload tako builtins upon deletion.
        config : str, optional
            Path to configuration file.
        tako_ctx : dict or None, optional
            Tako xontext to load as builtins.__tako_ctx__
        """
        ...
        load_builtins(execer=self, config=config, login=login, ctx=tako_ctx)
    ...
    def parse(self, input, ctx, mode='exec', transform=True):
        """Parses tako code in a context-aware fashion. For context-free
    ...
    def compile(self, input, mode='exec', glbs=None, locs=None, stacklevel=2,
        ...
        """Compiles tako code into a Python code object, which may then
        be execed or evaled.
        """
        if filename is None:
    ...
    def eval(self, input, glbs=None, locs=None, stacklevel=2,
                transform=True):
        """Evaluates (and returns) tako code."""
