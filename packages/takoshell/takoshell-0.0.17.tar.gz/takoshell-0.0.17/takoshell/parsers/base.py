...
# Copyright (c) 2015-2016 Adam Hartz <hartz@mit.edu>
#
...
# -*- coding: utf-8 -*-
"""Implements the base tako parser."""
import os
import re
import sys
...
from takoshell.ast import has_elts, tako_call
from takoshell.lexer import Lexer, LexToken
...
def tako_pathsearch(pattern, pymode=False, lineno=None, col=None):
    """Creates the AST node for calling the __tako_pathsearch__() function.
    ...
    pattern = ast.Str(s=pattern, lineno=lineno,
                      col_offset=col)
    if searchfunc in {'r', ''}:
        func = '__tako_regexsearch__'
    elif searchfunc == 'g':
        func = '__tako_globsearch__'
    else:
        func = searchfunc[1:]  # remove the '@' character
    func = ast.Name(id=func, ctx=ast.Load(), lineno=lineno,
                    col_offset=col)
    return tako_call('__tako_pathsearch__', args=[func, pattern, pymode],
...
class BaseParser(object):
    """A base class that parses the tako language."""

    def __init__(self,
                 lexer_optimize=True,
                 lexer_table='takoshell.lexer_table',
                 yacc_optimize=True,
                 yacc_table='takoshell.parser_table',
                 yacc_debug=False,
                 outputdir=None):
        """Parameters
        ----------
    ...
    def parse(self, s, filename='<code>', mode='exec', debug_level=0):
        """Returns an abstract syntax tree of tako code.
    ...
    def _lexer_errfunc(self, msg, line, column):
        self._parse_error(msg, self.currloc(line, column), self.tako_code)
    ...
    def p_atom_pathsearch(self, p):
        """atom : SEARCHPATH"""
        p[0] = tako_pathsearch(p[1], pymode=True, lineno=self.lineno,
                                col=self.col)
    ...
    def _dollar_rules(self, p):
        """These handle the special tako $ shell atoms by looking up
        in a special __tako_env__ dictionary injected in the __builtin__.
        """
        lenp = len(p)
        p1, p2 = p[1], p[2]
        ...
        if lenp == 3:  # $NAME
            ...
            p0 = tako_call('__tako_subproc_uncaptured__', p2,
                            lineno=lineno, col=col)
        else:
            assert False
        return p0
    ...
    def _xenv(self, lineno=lineno, col=col):
        """Creates a new tako env reference."""
        return ast.Name(id='__tako_env__', ctx=ast.Load(),
                        lineno=lineno, col_offset=col)
    ...
    def _envvar_by_name(self, var, lineno=None, col=None):
        """Looks up a tako variable by name."""
        xenv = self._xenv(lineno=lineno, col=col)
    ...
    def p_subproc_atom_uncaptured(self, p):
        ...
        p1 = p[1]
        p0 = tako_call('__tako_subproc_uncaptured__', args=p[2],
    ...
    def p_subproc_atom_captured_stdout(self, p):
        ...
        p1 = p[1]
        p0 = tako_call('__tako_subproc_captured_stdout__', args=p[2],
                        lineno=p1.lineno, col=p1.lexpos)
    ...
    def p_subproc_atom_subproc_inject(self, p):
        ...
        p0 = tako_call('__tako_subproc_captured_inject__', p[2],
                        lineno=self.lineno, col=self.col)
        p0._cliarg_action = 'extend'
        p[0] = p0
    ...
    def p_subproc_atom_re(self, p):
        ...
        p0 = tako_pathsearch(p[1], pymode=False, lineno=self.lineno,
                              col=self.col)
        p0._cliarg_action = 'extend'
        p[0] = p0
    ...
    def p_subproc_atom_str(self, p):
        ...
        p0 = tako_call('__tako_expand_path__', args=[p[1]],
                        lineno=self.lineno, col=self.col)
        p0._cliarg_action = 'append'
        p[0] = p0
    ...
    def p_subproc_atom_arg(self, p):
        ...
        p0 = ast.Str(s=p[1], lineno=self.lineno, col_offset=self.col)
        if '*' in p1:
            p0 = tako_call('__tako_glob__', args=[p0],
                            lineno=self.lineno, col=self.col)
            p0._cliarg_action = 'extend'
        else:
            p0 = tako_call('__tako_expand_path__', args=[p0],
                            lineno=self.lineno, col=self.col)
            p0._cliarg_action = 'append'
        p[0] = p0
