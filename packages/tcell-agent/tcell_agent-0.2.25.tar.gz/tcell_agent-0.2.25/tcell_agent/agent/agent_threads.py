...
class TCellAgent(object):
    ...
    def run_event_handler(self):
        ...
        initial_events = [ServerAgentDetailsEvent(
            ...
            TCellAgentSettingEvent("allow_unencrypted_appfirewall_payloads", CONFIGURATION.allow_unencrypted_appsensor_payloads),
        ...
        if CONFIGURATION.logging_options:
            ...
            if logging_enabled:
                ...
                initial_events.append(TCellAgentSettingEvent("logging_level", CONFIGURATION.logging_options.get("level","INFO")))
            else:
        ...
        if CONFIGURATION.reverse_proxy:
            initial_events.append(TCellAgentSettingEvent("reverse_proxy_ip_address_header", CONFIGURATION.reverse_proxy_ip_address_header))
    ...
    def flush_events(self):
        ...
        try:
            ...
            LOGGER.error("Error sending events, will try again laster {e}".format(e=connection_exception))
        ...
        if cls.tCell_agent is not None:
            ...
            try:
                if CONFIGURATION.app_id is None or CONFIGURATION.api_key is None:
                    LOGGER.error("Application ID and API Key must be set by configuration file or environmental variables.")
                    return None
                if cls.tCell_agent is not None:
                    return cls.tCell_agent
    ...
    def request_metric(cls, route_id, request_time, ip, user_agent, session_id=None, user_id=None):
        try:
            if cls.get_agent().in_fork() and cls.get_agent().fork_pipe_broken == False:
                LOGGER.debug("Request metric sending (in fork) {route_id} {request_time}".format(route_id=route_id,request_time=request_time))
                ...
                LOGGER.debug("Request metric sending {route_id} {request_time}".format(route_id=route_id,request_time=request_time))
    ...
    def discover_database_fields(cls, database, schema, table, fields, route_id=None):
        try:
            ...
            if cls.tCell_agent is not None:
                ...
                if dlp_policy is None or dlp_policy.database_discovery_enabled != True:
                    return
                if route_id is None:
                    route_id = ""

                if (cls.get_agent().route_table.routes[route_id].have_fields_been_discovered(database,schema,table,fields)):
