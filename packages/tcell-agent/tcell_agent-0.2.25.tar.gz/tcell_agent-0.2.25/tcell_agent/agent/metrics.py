...
class UserSessionTrackMetric(dict):
    ...
    def add_user_agent_ip_combo(self, user_agent, ip):
        ...
        if ip not in self._track[user_agent]:
            ...
            self["track"] = [[user_agent, self._track[user_agent]] for user_agent in self._track]
...
class AgentMetrics(object):
    ...
    def add_session_track_metric(self, session_id, user_id, user_agent, remote_addr):
        metricsLock.acquire()
        try:
            ...
            self._session_metrics.for_session(session_id).for_user_id(user_id).add_user_agent_ip_combo(user_agent, remote_addr)
    ...
    def add_route_count(self, route_id, request_time):
        ...
        try:
            ...
            self._route_count_table[route_id]["t"] = ((self._route_count_table[route_id]["t"] * (self._route_count_table[route_id]["c"]-1))+request_time)/self._route_count_table[route_id]["c"]
        finally:
            requestCountLock.release()
