...
from tcell_agent.sensor_events import DlpEvent
...
def validate_term(term):
    if term is not None and term != '' and term != DataLossPolicy.B_REDACT_MESSAGE and term != DataLossPolicy.REDACT_MESSAGE:
        if len(str(term)) > 4:
            return True
    return False
...
class TCellInstrumentationContext(object):
    def __init__(self):
        ...
        self.session_id = None # HMAC'd session id
        self.user_id = None
        self.user_agent = None
        self.remote_addr = None
        self.raw_remote_addr = None
    ...
    def add_response_body_filter(self, string):
        ...
        self.filter_from_body.add(cgi.escape(string))
        ...
        if dataloss_policy:
                    ...
                    if send_event:
                        for context_filter in context_filters:
                            if context_filter.type == ContextFilter.DATABASE:
                                dlpe = DlpEvent(self.route_id, self.fullpath, DlpEvent.FOUND_IN_BODY, context_filter.rule.id).for_database(
                                ...
                                dlpe = DlpEvent(self.route_id, self.fullpath, DlpEvent.FOUND_IN_BODY, context_filter.rule.id).for_request(
            ...
            if self.raw_session_id:
                ...
                if session_id_actions:
                    send_event, body = filterx(body, session_id_actions.body_redact, session_id_actions.body_event, self.raw_session_id)
                    if send_event:
                        dlpe = DlpEvent(self.route_id, self.fullpath, DlpEvent.FOUND_IN_BODY).for_framework(DlpEvent.FRAMEWORK_VARIABLE_SESSION_ID)
                        TCellAgent.send(dlpe)
        ...
        if dataloss_policy:
            ...
            for term in terms:
                ...
                if send_event:
                    for context_filter in context_filters:
                        if context_filter.type == ContextFilter.DATABASE:
                            dlpe = DlpEvent(self.route_id, self.fullpath, DlpEvent.FOUND_IN_LOG, context_filter.rule.id).for_database(
                                context_filter.database,
                                context_filter.schema,
                                context_filter.table,
                                context_filter.field)
                            ...
                            dlpe = DlpEvent(self.route_id, self.fullpath, DlpEvent.FOUND_IN_LOG, context_filter.rule.id).for_request(
                                context_filter.context,
                                context_filter.parameter)
                            TCellAgent.send(dlpe)
            session_id_actions = dataloss_policy.get_actions_for_session_id()
            if session_id_actions:
                send_event, msg = filterx(msg, session_id_actions.log_redact, session_id_actions.log_event, self.raw_session_id)
                if send_event:
                    dlpe = DlpEvent(self.route_id, self.fullpath, DlpEvent.FOUND_IN_LOG).for_framework(DlpEvent.FRAMEWORK_VARIABLE_SESSION_ID)
                    TCellAgent.send(dlpe)
        return msg
