...
from tcell_agent.instrumentation.django.routes import get_route_table, make_route_table
...
class TCellLastMiddleware(object):
    ...
    def __init__(self):
        if not tcell_agent.instrumentation.django._route_table_sent:
            tcell_agent.instrumentation.django._route_table_sent = True
            make_route_table()
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if view_kwargs:
            safe_wrap_function("AppSensor Path Parameters", assign_path_dictionary_for_request, request, view_kwargs)
    ...
    def process_response(self, request, response):
        ...
        safe_wrap_function("AppSensor Request/Response", django_request_response_appsensor, HttpResponse, request, response)
        return response
