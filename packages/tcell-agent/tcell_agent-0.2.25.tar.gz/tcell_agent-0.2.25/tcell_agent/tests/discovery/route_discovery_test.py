...
class AppSensorPolicyTest(unittest.TestCase):
    def simple_database_route_test(self):
        ...
        self.assertEqual(TCellAgent.tCell_agent.route_table.routes["routex"].database_fields[hash("databasex,schemax,tablex," + ",".join(["fieldx"]))].discovered, True)
        self.assertEqual(TCellAgent.tCell_agent.route_table.routes["routex"].database_fields[hash("databasex,schemax,tabley," + ",".join(["fieldx"]))].discovered, False)
