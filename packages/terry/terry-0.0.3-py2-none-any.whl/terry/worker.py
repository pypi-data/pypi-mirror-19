...
__all__ = [u'InterruptJob', u'Worker', u'JobChannel']
...
class Worker(object):
    def __init__(self, id_, job_tags, worker_func, controller, **_3to2kwargs):
        if 'interrupt_via_exception' in _3to2kwargs: interrupt_via_exception = _3to2kwargs['interrupt_via_exception']; del _3to2kwargs['interrupt_via_exception']
        else: interrupt_via_exception = False
        self._id = id_
    ...
    def start(self):
        self.logger.info(u'[%s] Listen for tags %s', self._id, self._job_tags)
        self.logger.info(u'[%s] Starting worker...', self._id)
        self._main_loop_thread.start()

    def request_stop(self):
        self.logger.info(u'[%s] Got request to stop...', self._id)
        self._stop.set()
    ...
    def _try_acquire_job(self):
        ...
        if job:
            ...
            self.logger.info(u'[%s] Acquired job %s', self._id, self._job_ctx.job.id)
    ...
    def _try_update_current_job(self):
        ...
        if self._job_ctx.cancelled:
            self.logger.info(u'[%s] It seems job %s was cancelled', self._id, self._job_ctx.job.id)

        if self._job_ctx.revoked:
            self.logger.info(u'[%s] It seems job %s was taken from us', self._id, self._job_ctx.job.id)
    ...
    def _wait_for_worker_thread_and_cleanup(self):
        if self._worker_thread.is_alive():
            if self._interrupt_via_exception and not self._worker_thread.interrupt_requested:
                ...
                self.logger.info(u'[%s] Worker thread with job %s was interrupted via exception',
                                 self._id, self._job_ctx.job.id)

            self._worker_thread.join(math.pi - random.random())
        else:
            self.logger.info(u'[%s] Processing of job %s was terminated', self._id, self._job_ctx.job.id)
            self._job_ctx = None
            self._worker_thread = None
    ...
    def _try_heartbeat_current_job(self):
        ...
        if job:
            ...
            self.logger.info(u'[%s] Failed to heartbeat job %s due to version mismatch',
                             self._id, self._job_ctx.job.id)
    ...
    def _try_requeue_current_job(self):
        ...
        try:
            ...
            self._job_ctx.outdated = True
            self.logger.info(u'[%s] Failed to mark job %s as completed due to version mismatch',
                             self._id, self._job_ctx.job.id)
        else:
            self.logger.info(u'[%s] Job %s has been requeued', self._id, self._job_ctx.job.id)
            self._job_ctx = None
            self._worker_thread = None
    ...
    def _try_finalize_current_job(self):
        ...
        if self._worker_thread.has_failed:
            reason = unicode(self._worker_thread.exc_info[1])
            tback = u''.join(traceback.format_exception(*self._worker_thread.exc_info))
            worker_exception = {u'reason': reason, u'traceback': tback}
        else:
        ...
        if job:
            self.logger.info(u'[%s] Job %s has been processed', self._id, self._job_ctx.job.id)
            self._job_ctx = None
            self._worker_thread = None
        else:
            self._job_ctx.outdated = True
