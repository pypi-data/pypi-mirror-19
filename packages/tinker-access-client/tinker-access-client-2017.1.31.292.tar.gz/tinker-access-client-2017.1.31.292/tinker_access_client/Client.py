...
class Client(Machine):
    def __init__(self):
        ...
        transitions = [
            ...
            # TODO: this needs work, the Login trigger is causing this
            # authoize condition to be called twice as well as triggering the
            # not_authorized code that turns the led red. will have to review the
            # transitions code to see how to return a dynamic state
            {
                'source': State.IDLE,
                'trigger': Trigger.LOGIN,
                'dest': State.IDLE,
    ...
    def authorized(self, *args, **kwargs):
        # TODO: this is getting hit twice... because its
        # specified as a transition condition, will need to change that probably
        ...
        self.__logger.debug('AUTHORIZE...')
        if not self.__user_info:
            badge_code = kwargs.get('badge_code')
            if badge_code:
                try:
                    self.__user_info = self.__tinkerAccessServerApi.login(badge_code)
