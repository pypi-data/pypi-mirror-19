...
class Client(Machine):
    def __init__(self):
        ...
        transitions = [
            ...
            # TODO: handle case where user re-badges in, or another users badges in while the machine is in use
            {
                'source': State.IN_USE,
                'trigger': Trigger.LOGIN,
                'dest': State.IN_USE,
                'conditions': 'is_authorized' #extend time if same user, otherwise go through login process
            },
    ...
    def __do_login(self, *args, **kwargs):
        ...
        if not current_badge_code:
            return self.__do_normal_login(new_badge_code)
    ...
    def __do_normal_login(self, badge_code):
        try:
