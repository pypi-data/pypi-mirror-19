...
class StartScreenUrwidTui(object):
    ...
    def __init__(self):
        ...
        """
        gpl_notice = u"Tok Tokkie Media Manager V" + General.version_number + u"\n"\
    ...
    def generate_menu(self, header, options):
        ...
        for option in sorted(options.keys()):
            ...
            urwid.connect_signal(button, u'click', self.start_tui, user_arg=options[option])
            body.append(urwid.AttrMap(button, None, focus_map=u'reversed'))
        return urwid.ListBox(urwid.SimpleFocusListWalker(body))

    # noinspection PyUnusedLocal,PyMethodMayBeStatic
    def start_tui(self, button, choice = None):  # pragma: no cover
        u"""
