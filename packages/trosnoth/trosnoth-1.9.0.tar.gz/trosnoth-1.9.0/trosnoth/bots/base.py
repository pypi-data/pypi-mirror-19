...
def listAIs(playableOnly=False):
    ...
    for aiName in aiNames:
        try:
            ...
            continue
        if hasattr(c, 'BotClass') and (
                c.BotClass.playable or not playableOnly):
...
class Bot(MessageConsumer):
    ...
    def __init__(self, world, player, agent, *args, **kwargs):
        ...
        self.upgradeOwnStarsOnly = True
        self.upgradeStarBuffer = 0
        self.upgradeDelay = 0
    ...
    def setUpgradePolicy(self, upgrade, ownStars=True, starBuffer=0, delay=0):
        '''
        ...
        ownStars: whether bot should only use its own stars to purchase
        starBuffer: how many surplus stars should there be before purchasing
        delay: how long should the bot wait after there are enough stars
        '''
        self.upgradeChoice = upgrade
        self.upgradeOwnStarsOnly = ownStars
        self.upgradeStarBuffer = starBuffer
    ...
    def _makeUpgradeDecision(self):
        if self.upgradeOwnStarsOnly or self.player.team is None:
            stars = self.player.stars
        else:
            stars = self.world.getTeamStars(self.player.team)

        starThreshold = (
            self.upgradeChoice.requiredStars + self.upgradeStarBuffer)
...
class FutureActionRecord(object):
    ...
    def checkFinalNodeKey(self, player, stepped=False):
        ...
        if error:
            ...
            log.warning(
                '  initial _alreadyJumped: %r',
                self.DEBUG_initialPlayer._alreadyJumped)
            log.warning(
...
class BotFuture(object):
    ...
    def _checkEdgeMatchesCurrentLocation(self, edgeToCheck, exitDir):
        ...
        assert self.futurePlayer.attachedObstacle is not None
