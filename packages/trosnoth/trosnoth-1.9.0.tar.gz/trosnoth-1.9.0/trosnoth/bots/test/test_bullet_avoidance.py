...
class MapSimulator(object):
    ...
    def createPlayer(self, startNodeKey):
        player = Player(self.world, 'Testing', None, None)
...
def test_database_mismatch_1():
    simulator = MapSimulator()
    simulator.loadTransition(
        fromBlock='bckBlockedIslands', fromReversed=True,
        toBlock='bckOpenEmpty', toReversed=False,
        upwards=False,
    )
    simulator.runActions(start=(30, 1), sequence=[
        ([MoveRight, FallPartwayDown, FallLeft, FallRight, FallPartwayDown,
            FallLeft], (7, -2)),
    ])


def test_database_mismatch_2():
    simulator = MapSimulator()
