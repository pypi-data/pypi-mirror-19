...
class PlayerAchievement(Achievement):
    ...
    def achievementTriggered(self):
        gameOn = self.world.gameIsInProgress()
        if (gameOn and not self.inGame) or (not gameOn and not self.afterGame):
            return
...
class WinnerBase(Streak, OnceEverPerPlayer):
    def register(self):
        ...
        self.world.onGameOver.addListener(self.gameOver)

    def unregister(self):
        super(WinnerBase, self).unregister()
        self.world.onGameOver.removeListener(self.gameOver)
...
@availableAchievements.register
class UseStarsEfficiently(OncePerPlayerPerGame):
...
@availableAchievements.register
class KillEnemyWithStars(OncePerPlayerPerGame):
    name = 'Stop Right There, Criminal Scum'
    description = 'Kill an enemy holding five stars or more'
    idstring = 'killEnemyWithStars'

    def register(self):
        super(KillEnemyWithStars, self).register()
        self.player.onKilled.addListener(self.killedPlayer)

    def unregister(self):
        super(KillEnemyWithStars, self).unregister()
        self.player.onKilled.removeListener(self.killedPlayer)
...
@availableAchievements.register
class DestroyStars(Streak, OnceEverPerPlayer):
    ...
    def register(self):
        super(DestroyStars, self).register()
        self.player.onKilled.addListener(self.killedPlayer)

    def unregister(self):
        super(DestroyStars, self).unregister()
        self.player.onKilled.removeListener(self.killedPlayer)

    def killedPlayer(self, target, *args, **kwargs):
        self.increment(target.stars)


@availableAchievements.register
class EarnStars(Streak, OnceEverPerPlayer):
    idstring = 'earnStars'
    ...
    def __init__(self, *args, **kwargs):
        super(EarnStars, self).__init__(*args, **kwargs)
        self.lastStars = self.player.stars

    def register(self):
        super(EarnStars, self).register()
        self.player.onStarsChanged.addListener(self.starsChanged)

    def unregister(self):
        super(EarnStars, self).unregister()
        self.player.onStarsChanged.removeListener(self.starsChanged)

    def starsChanged(self, *args, **kwargs):
        if self.player.stars > self.lastStars:
            self.increment(self.player.stars - self.lastStars)
        self.lastStars = self.player.stars
...
class AbandonEarly(OncePerPlayerPerGame):
    ...
    def register(self):
        super(AbandonEarly, self).register()
        self.player.onAbandonUpgrade.addListener(self.abandonUpgrade)

    def unregister(self):
        super(AbandonEarly, self).unregister()
        self.player.onAbandonUpgrade.removeListener(self.abandonUpgrade)

    def abandonUpgrade(self, upgrade, *args, **kwargs):
        '''
        Don't award if it was within 3 seconds of running out anyway.
        '''
        if upgrade.upgradeType in 'th' and self.player.upgradeGauge >= 3:
            self.achievementTriggered()
...
class UpgradeKillBase(OncePerPlayerPerGame):
    ...
    def killedPlayer(self, target, *args, **kwargs):
        ...
        if upgrade and upgrade.upgradeType in self.upgradeTypes:
            self.achievementTriggered()
...
class MinimapDisruptionTag(OncePerPlayerPerGame):
    ...
    def taggedZone(self, *args, **kwargs):
        for player in self.world.players:
            if (
                    self.player.isFriendsWith(player) and player.upgrade
...
class ChokepointSpam(Streak, OncePerPlayerPerGame):
    ...
    def killedPlayer(self, shooter, deathType, *args, **kwargs):
        if self.player.upgrade is None and deathType != GRENADE_DEATH:
            self.increment()
...
class PhaseShiftAbsorb(Streak, OncePerPlayerPerGame):
    ...
    def hitByShot(self, *args, **kwargs):
        if self.player.upgrade and self.player.upgrade.upgradeType == 'h':
            self.increment()
...
class TurretKill(OncePerPlayerPerGame):
    ...
    def killedPlayer(self, *args, **kwargs):
        if self.player.upgrade and self.player.upgrade.upgradeType == 't':
            self.achievementTriggered()
...
class LastManStanding(OncePerTeamPerGame):
    ...
    def playerKilled(self, killer, *args, **kwargs):
        if not self.world.gameIsInProgress():
            self.check()
