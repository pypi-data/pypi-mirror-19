...
class AuthenticationFactory(Factory):
    ...
    def __init__(self, dataPath=None, gameType=None):
        if dataPath is None:
    ...
    def createGame(self):
        ...
        try:
            game.world.setNextGameType(self.gameType)
        except ValueError:
            log.warning('Unknown game type: %s' % (self.gameType,))
        ...
        def onShutdown():
            ...
            game.world.onGameStateChanged.removeListener(gameStateChanged)
        ...
        gameStateChanged = partial(self.gameStateChanged, game=game)
        game.world.onGameStateChanged.addListener(gameStateChanged)
    ...
    def gameStateChanged(self, game):
        countStats = game.world.state.statsCount

        gameStats = self.gameStats[game]
        if gameStats and not countStats:
            # Stopping game
            gameStats.stopAndSave()
            self.gameStats[game] = None

        elif countStats and not gameStats:
            # Starting game
...
def startServer(
    ...
    pf = AuthenticationFactory(dataPath, gameType)
...
def main():
    ...
    parser.add_option(
        '-t', '--gametype', action='store', dest='gameType', default=None,
        help='the game type to run')
    parser.add_option(
