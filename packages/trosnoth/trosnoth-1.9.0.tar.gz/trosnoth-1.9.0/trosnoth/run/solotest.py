...
class Main(app.MultiWindowApplication):
    def __init__(
            ...
            trosball=False, delay=None, blockRatio=0.5, duration=0,
            level=None):
        '''Initialise the game.'''
        pygame.init()
        self.size = size
        ...
        self.game = None
        self.startGame(level=level, trosball=trosball)
    ...
    def startGame(self, level=None, trosball=False):
        ...
        if not self.lobby:
            game.world.beginGameStartCountdown()
        if self.testMode:
...
class DelayNodeHub(Hub, Node):
    ...
    def connectNewAgent(self):
        result = yield self.hub.connectNewAgent()
