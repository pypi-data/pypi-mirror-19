...
from trosnoth.const import (
    ...
    ENEMY_ZONE_REASON, FROZEN_ZONE_REASON, TICK_PERIOD,
)
...
from trosnoth.model.upgrades import Bomber, Turret
...
from trosnoth.messages import (
    TaggingZoneMsg, ChatFromServerMsg, ChatMsg, GameStartMsg, GameOverMsg,
    StartingSoonMsg, ShotFiredMsg, RespawnMsg, CannotRespawnMsg, TickMsg,
    CannotJoinMsg, AddPlayerMsg, PlayerHasUpgradeMsg, RemovePlayerMsg,
    PlayerStarsSpentMsg, CannotBuyUpgradeMsg, ConnectionLostMsg,
    AchievementUnlockedMsg, SetPlayerTeamMsg, PlaySoundMsg,
    PlayerHasElephantMsg, FireShoxwaveMsg,
    PlayerHasTrosballMsg, TrosballTagMsg, UpgradeChangedMsg,
)
...
class GameInterface(framework.CompoundElement, ConcreteAgent):
    ...
    def __init__(
        ...
        self.world.onSectorNeutralised.addListener(self.sectorNeutralised)
        ...
        self.world.onGameStateChanged.addListener(self.gameStateChanged)
    ...
    def stop(self):
        ...
        self.world.onSectorNeutralised.removeListener(self.sectorNeutralised)
        ...
        self.world.onGameStateChanged.removeListener(self.gameStateChanged)
    ...
    def handle_GameOverMsg(self, msg):
        super(GameInterface, self).handle_GameOverMsg(msg)
        try:
    ...
    def trosballScore(self, msg):
        ...
        if player is not None:
            if player.team == team:
                message = '%s scores for %s!' % (player.nick, team.teamName)
                bigMessage = '%s scored!' % player.nick
                self.playSound('goal', 1)
            else:
                message = '%s scores an own goal!' % (player.nick,)
                if self.detailsInterface.player.id == player.id:
                    bigMessage = '(Psst... wrong goal!)'
                else:
                    bigMessage = 'Goal!'
                self.playSound('ownGoal', 1)
        else:
            message = '%s scores!' % (team.teamName,)
            bigMessage = 'Goal!'
            self.playSound('goal', 1)

        self.winnerMsg.show(
            bigMessage, self.app.theme.colours.chatColour(team))

        self.detailsInterface.newMessage(message)
        teams = list(self.world.teams)
        message = '%s: %d - %s: %d' % (
            teams[0].teamName,
            self.world.trosballScore[teams[0].id],
            teams[1].teamName,
            self.world.trosballScore[teams[1].id],
        )
        self.detailsInterface.newMessage(message)
    ...
    def notEnoughStars(self, msg):
        if msg.reasonId == NOT_ENOUGH_STARS_REASON:
            text = 'Your team does not have enough stars.'
        elif msg.reasonId == HAVE_UPGRADE_REASON:
    ...
    def gotUpgrade(self, msg):
        ...
        if player:
            ...
            if (self.detailsInterface.player is None or
                ...
                if msg.upgradeType == Turret.upgradeType:
                    self.playSound('turret')
                else:
                    self.playSound('buyUpgrade')

            if (
                    self.detailsInterface.player is not None and
                    self.detailsInterface.player.id == msg.playerId):
    ...
    def sectorNeutralised(self, tagger, zoneCount):
        theWordZone = 'zone' if zoneCount == 1 else 'zones'
        if not tagger:
            message = '%d %s neutralised!' % (zoneCount, theWordZone)
        else:
            nick = tagger.nick
            message = '%s neutralised %d %s!' % (
                nick, zoneCount, theWordZone)
        self.detailsInterface.newMessage(message)
    ...
    def playerDied(self, target, killer, deathType):
        if deathType == OFF_MAP_DEATH:
            ...
            if (self.runningPlayerInterface is not None and
                ...
                rect = self.gameViewer.worldgui.getPlayerSprite(
                    target.id).rect
                self.detailsInterface.starGroup.star(rect.center)
        self.detailsInterface.newMessage(message)
...
class GameInfoDisplay(CollapseBox):
    def __init__(self, app, gameInterface, region):
        ...
        self.refreshCall = None
        self.refreshInfo()

    def refreshInfo(self):
        if self.refreshCall:
            self.refreshCall.cancel()
        world = self.interface.world
        player = self.interface.localState.player
        title, lines = world.state.getInfo(world, player)
        self.setInfo(lines, title)

        # Check every 3 seconds
        self.refreshCall = WeakCallLater(3, self, 'doRefreshCall')

    def doRefreshCall(self):
        self.refreshCall = None
        self.refreshInfo()
