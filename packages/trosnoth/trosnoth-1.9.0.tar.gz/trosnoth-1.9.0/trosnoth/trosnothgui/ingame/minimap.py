...
class MiniMap(framework.Element):
    ...
    def draw(self, screen):
        ...
        if self.disrupt and random.random() > 0.05:
            ...
            self.updateFocus()
            if self.universe.uiOptions.shiftingFrontLine:
                self.drawShiftingBg(screen, sRect)
            else:
            ...
            for star in self.universe.collectableStars.values():
                drawCircle(star, colours.miniMapStar, 2)
            ...
            if self.universe.uiOptions.showNets:
                for team in self.universe.teams:
                    ...
                    drawCircle(self.universe.getTrosballTargetZoneDefn(team),
                            colour, 3)
    ...
    def drawShiftingBg(self, screen, sRect):
        try:
            frontLine = self.universe.uiOptions.frontLineGetter()
        except ValueError:
            self.drawZones(screen, sRect)
            return
