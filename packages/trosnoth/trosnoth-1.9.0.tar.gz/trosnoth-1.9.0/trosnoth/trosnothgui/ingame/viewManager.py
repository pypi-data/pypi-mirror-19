...
class ViewManager(framework.Element):
    ...
    def draw(self, screen, drawStars=True):
        ...
        self.backgroundDrawer.draw(screen, self.sRect, self._focus, drawStars)
    ...
    def addSpritesForPlayer(self, player, visPlayers, ntGroup):
        ...
        if showPlayer:
            ...
            if player.rect.colliderect(area):
                ...
                if (
                        player.unit.upgrade
                        and player.unit.upgrade.upgradeType == 'b'):
                ...
                if player.unit.hasProtectiveUpgrade():
                    shieldBar = player.shieldBar
                    shieldBar.setHealth(
                        player.unit.upgrade.protections,
                        player.unit.upgrade.maxProtections)
...
class BackgroundDrawer(object):
    ...
    def draw(self, screen, sRect, focus, drawStars=True):
        if drawStars:
...
class SolidBackgrounds(object):
    ...
    def draw(self, screen, area, focus):
        if self.universe.universe.uiOptions.shiftingFrontLine:
            self.drawShiftingBackground(screen, area, focus)
        else:
    ...
    def drawShiftingBackground(self, screen, area, focus):
        worldRect = viewRectToMap(focus, area)
        uiOptions = self.universe.universe.uiOptions
        trosballLocation = uiOptions.frontLineGetter()
...
class GameViewer(framework.CompoundElement):
    ...
    def makeWidgets(self):
        self.miniMap = MiniMap(self.app, 20, self.worldgui, self.viewManager)
        if self.world.uiOptions.shiftingFrontLine:
    ...
    def tick(self, deltaT):
        ...
        if self.world.state != self.storedState:
            self.storedState = self.world.state
            self.reset()
