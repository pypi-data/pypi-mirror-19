...
/***/ function(module, exports, __webpack_require__) {
	...
	function buildIntegrationFragment(reference) {
	    var _reference$split = reference.split('-'),
	        _reference$split2 = _toArray(_reference$split),
	        kind = _reference$split2[0],
	        ids = _reference$split2.slice(1);
	...
	function easeCopyPasting(content) {
	    ...
	    textarea.focus();
	    textarea.select();
	    var help = content.querySelector('.udata-help');
	    help.firstChild.addEventListener('click', function (e) {
	        e.preventDefault();
	        textarea.select();
	...
	function handleIntegration(event) {
	    ...
	    var aside = paragraph.parentNode;
	    var content = aside.previousElementSibling;
	    var article = aside.parentNode;
	...
	var territories = [].slice.call(document.querySelectorAll('[data-udata-territory-id]'));
	var datasets = [].slice.call(document.querySelectorAll('[data-udata-dataset-id]'));

	var parser = document.createElement('a');
	var scriptURI = document.querySelector('script#udata').src;
	parser.href = scriptURI;
	var baseURL = parser.protocol + '//' + parser.host;

	territories.concat(datasets).chunk(9).forEach(function (elements) {
	    var references = elements.map(function (el) {
	        if (el.hasAttribute('data-udata-territory-id')) {
	            var territoryId = el.getAttribute('data-udata-territory-id');
	            return 'territory-' + territoryId;
	        } else if (el.hasAttribute('data-udata-dataset-id')) {
	            var datasetId = el.getAttribute('data-udata-dataset-id');
	            return 'dataset-' + datasetId;
	        }
	    });
	    var url = baseURL + '/api/1/oembeds/?references=' + references;
	    fetch(url).then(checkStatus).then(function (response) {
	        return response.json();
	    }).then(function (jsonResponse) {
	        zip([elements, jsonResponse, references]).forEach(function (_ref) {
