...
class Org_Golden_Cmd(Cmd, CoreGlobal):
        ...
        def do_list(self, args):
                try:
                        ...
                        table = init_texttable(["Name", "Date", "Version", "Architecture"],
                                               200,
                                               ["l", "l", "l", "l"],
                                               ["a", "a", "t", "a"])
                        ...
                        for distrib in allDist:
                                ...
                                if allGoldens is not None and hasattr(allGoldens, "pkgs"):
                                    allGoldens = order_list_object_by(allGoldens.pkgs.pkg, "name")
                                    nbGoldens = nbGoldens + len(allGoldens)
                                    for item in allGoldens:
                                        table.add_row([item.name, item.pkgBuildDate.strftime("%Y-%m-%d %H:%M:%S"),distrib.version, distrib.arch])
        ...
        def do_create(self, args):
                try:
                        ...
                        result = self.api.Orgs(org.dbId).Distributions(goldenId).Goldens.Add(Language=doArgs.language, Edition=doArgs.edition, Type=doArgs.type, Goldendate=doArgs.goldenDate, Filename=doArgs.goldenName, body=None)
        ...
        def arg_delete(self):
                ...
                mandatory.add_argument('--name', dest='name', type=str, required=True, help="Operating system(s) name (for example CentOS, Debian etc) for which the current command should be executed.")
                mandatory.add_argument('--version', dest='version', type=str, required=True, help="Operating system version (13, 5.6, ...)")
                optional.add_argument('--goldenDate', dest='goldenDate', type=str, required=False, help="The date of the golden image (ex: 2014-04-28). If no date is provided, all golden images are deleted")
                optional.add_argument('--org', dest='org', type=str, required=False, help="The organization name. If no organization is provided, then the default organization is used.")
        ...
        def do_delete(self, args):
                try:
                        ...
                        distId = None
                        profileName = doArgs.edition + "-" + doArgs.type + "-" + doArgs.language;
                        for distrib in allDist:
                                if distrib.name == doArgs.name and distrib.version == doArgs.version and distrib.arch == doArgs.arch:
                                        distId = distrib.dbId
                        ...
                        result = self.api.Orgs(org.dbId).Distributions(distId).Goldens.Delete(Prname=profileName, Goldendate=doArgs.goldenDate, body=None)
                        if doArgs.goldenDate is None:
                            printer.out("Golden image [" + profileName + "] has been deleted", printer.OK)
                        else:
                            printer.out("Golden image [" + profileName + " " + doArgs.goldenDate + "] has been deleted", printer.OK)
                        return 0
