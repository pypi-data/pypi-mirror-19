...
class UpdatableZipFile(ZipFile):
    """
    ...
    def __init__(self, file, mode="r", compression=ZIP_STORED, allowZip64=False):
        # Init base
        super(UpdatableZipFile, self).__init__(file, mode=mode,
    ...
    def writestr(self, zinfo_or_arcname, bytes, compress_type=None):
        ...
        # Otherwise just act normally
            ...
            super(UpdatableZipFile, self).writestr(zinfo_or_arcname,
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            super(UpdatableZipFile, self).__exit__(exc_type, exc_val, exc_tb)
    ...
    def rename_file(self, src, dst):
        with self.open(src) as vf:
            self.writestr(dst, vf.read())
        self.remove_file(src)

    def rename_dir(self, src, dest):
        for file in self.namelist():
            if file.startswith(src):
                self.rename_file(file, os.path.join(dest, file[len(src) + 1:]))
