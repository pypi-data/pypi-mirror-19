...
from velbot.utils.module_loading import import_string
from velbot.engine.slackengine import SlackEngine

sys.dont_write_bytecode = True

class VelBot(SlackEngine):
    def __init__(self, config):
        #Call Slack Engine
        SlackEngine.__init__(self, config)

        '''
            Params:
                - config (dict):
                    - BASE_PATH (optional: defaults to execution directory) VelBot will
                        look in this directory for plugins.
                    - LOGFILE (optional: defaults to velbot.log) The filename for logs, will
                        be stored inside the BASE_PATH directory
                    - DEBUG (optional: defaults to False) with debug enabled, VelBot will
                        break on errors
        '''
        ...
        # establish logging
        log_file = config.get('LOGFILE', 'velbot.log')
        if self.debug:
    ...
    def input(self, data):
        ...
        elif data.has_key('command'):
            function_name = "process_commmand"
        elif data.has_key('action'):
    ...
    def output(self):
        for plugin in self.bot_plugins:
            ...
            for output in plugin.do_output():
                channel = self.find_channels()
                if channel is not None and output[1] is not None:
                    if limiter:
                        time.sleep(.1)
                        limiter = False

                    if "fields" in output[1]:
                        self.send_attachment(output[0],output[1])
                    else:
