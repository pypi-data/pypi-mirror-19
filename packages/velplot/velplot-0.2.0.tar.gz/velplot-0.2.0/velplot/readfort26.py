...
def do26():
    ...
    while line26[i][0:3]=='%% ':
        ...
        headline = line26[i].split('!')[-1] if type(setup.headlist)==str else setup.headlist[i]
    ...
    tempest = np.empty((0,3),dtype=object)
    setup.table2 = []
    idx=1
    for i in range(i,len(line26)):
        ...
        if mode=='turbulent':
            ...
            tempest = np.vstack((tempest,np.array([tie,val0,val2],dtype=object)))
    ...
    for k in range(len(setup.table2)):
        ...
        if id0 not in ['<>','>>','__','<<']:
            if tie0.islower()==True and ord(tie0[0].lower())<ord(setup.lastchtied):
                mass0 = atommass(id0)
