    ...
    def getCrtFieldSQL():
        try:
            ...
            dictTypes = { #mysql类型对照关系
                "c":"char", "vc":"varchar",
                "date":"char", "time":"char", "dt":"datetime",
                "int":"integer",
                "int64":"bigint",
                "byte": "smallint",
                "float": "decimal",
            ...
            for dictF in schemaTable["fields"]:
                sType = dictF["type"]
                def getLens():
                    if sType in ["int","int64","byte","text","idnew","idval","dt"]:
                        return ""  # 这些不需要长度
                    else:
                        if sType == "date":
                            return "(8)"
...
def testCreateTables():
    ...
    GenerateCreateSqlFile(dictSchemaExample,'mysql')
...
def TabCalcAssignMiddleVariable(dictSchemaTab, sEngine):
    ...
    for dictF in dictSchemaTab["fields"]:
        sName = dictF["name"]
        sInitVal = dictF.get("initval","") #dictF.has_key("initval"):
        bGet,bIns=True,True
        if sInitVal!="":
            sFlag = sInitVal[0]
            if sFlag in ['=','*','#']:
                if sFlag=='=':
                    dictPost[sName] = sInitVal
                else:
                    dictPost[sName] = sInitVal[1:]
                bGet = sFlag not in ['*']
                bIns = sFlag not in ['*','#']
        if bGet:
            if sListGet: sListGet += ','+sName
            else: sListGet += sName
        if bIns and dictF["type"]!="idnew":
