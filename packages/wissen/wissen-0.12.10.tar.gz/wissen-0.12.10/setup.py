...
f = open ("wissen/__init__.py", "r")
for line in f:
	if line.startswith ("VERSION"):		
		__VER__ = line [10:].strip ()[1:-1]
		break
f.close ()
...
if os.name == "nt":
	include_dirs = [
		...
		"wissen/win32inclib/pthread2/%s/%s" % (dirv, dira)
	]
...
data_files = [
	...
	"win32inclib/pthread2/2.7/x64/*.*",
	"win32inclib/pthread2/2.7/x86/*.*",
	"win32inclib/pthread2/3.4/x64/*.*",
	"win32inclib/pthread2/3.4/x86/*.*",
...
if os.name == "nt" and ("build" in sys.argv or "install" in sys.argv):
	...
	shutil.copy ("wissen/win32inclib/pthread2/%s/%s/pthreadVC2.dll" % (dirv, dira), "wissen/pthreadVC2.dll")
	
	if os.path.isfile ("wissen/msvcr90.dll"):
		os.remove ("wissen/msvcr90.dll")
	shutil.copy ("wissen/win32inclib/pthread2/%s/msvcr90.dll" % dirv, "wissen/msvcr90.dll")
