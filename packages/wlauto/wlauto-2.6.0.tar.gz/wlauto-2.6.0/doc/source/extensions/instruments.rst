...
install_module : boolean  
    ...
    to be installed by the instrument. (.ko binary must be in /home/marbon02/.workload_automation/dependencies). If this is set
...
labels : list_or_string  
    A list of lables to be used in the CSV output for
...
power_domains : list_of_strs  
    The names of power domains to be monitored by the
...
autostart : boolean  
    Automatically start `servod`. Set to `False` if you want to
    use an already running `servod` instance or a remote servo

    default: ``True``

host : str  
    When `autostart` is set to `False` you can specify the host
...
port : integer  
    When `autostart` is set to false you must provide the port
...
vid : str  
    When more than one servo is plugged in, you must provide
