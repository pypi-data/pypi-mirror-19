...
def basicAnalysis(pgap_dir, annotated_gbk_pat=None):
    ...
    * Returns None; creates a dated figures_tables directory in the PGAP dir
    ...
    orthologs_cluster = pd.read_table(
        os.path.join(pgap_dir, '1.Orthologs_Cluster.txt'),
        index_col=0)
    ...
    plt.savefig(os.path.join(figdir,
                             '1.gene_Distribution_By_Conservation.png'))
    plt.clf()
    ...
    pangenome_data = pd.read_table(os.path.join(pgap_dir,
    ...
    g = sns.violinplot(x="ClusterConservation",
                       y="PanGenome",
                       data=pangenome_data,
                       palette="YlGnBu_d")
    ...
    sns.violinplot(x="ClusterConservation",
                   y="CoreGenome",
                   data=pangenome_data,
                   palette="YlGnBu_d")
    ...
    clustergrid.savefig(os.path.join(
        figdir, 'PGAP-Species_Distance_Clusters_Based.png'))
    ...
    if not os.path.exists(filebase + '.pdf'):
        ...
        cgrid_low = sns.clustermap(accessory,
                                   cmap=cmap,
    ...
    if annotated_gbk_pat:
        ...
        if os.path.exists(seq_df_cache):
            ...
            for fname in files:
                ...
                for i, seq_record in enumerate(SeqIO.parse(
                        wolfeutils.base.openfunc(fname), "genbank")):
                    cds_features = [feat
        ...
        for name, col in ortho_desc.iteritems():
            ortho_nuc.loc[:, name] = col.apply(lookup_desc,
                                               seq_df=seq_df,
                                               key='nuc')
            ortho_pep.loc[:, name] = col.apply(lookup_desc,
                                               seq_df=seq_df,
                                               key='pep')
            ortho_desc.loc[:, name] = col.apply(lookup_desc,
                                                seq_df=seq_df,
                                                key='description')
        ...
        writer = pd.ExcelWriter(os.path.join(
            figdir, 'AccessoryPresenceAbsence_jaccard.xlsx'))
        ...
        table_html = accessory.iloc[
            reordered_row_indices, reordered_col_indices].to_html(
                formatters=[bool2html] * accessory.shape[1],
                escape=False)
...
def cluster_by_group(strains, orthologs_presence):
    ...
    for grp in group_cols.keys():
        count = pd.DataFrame(orthologs_presence[group_cols[grp]].sum(axis=1),
                             columns=['count'])
    ...
    sns.distplot(
        cluster_grp_counts[(cluster_grp_counts['presence_pct'] < 1) & (
            cluster_grp_counts['presence_pct'] > 0)]['presence_pct'],
        rug=True)
    plt.title(
    ...
    groups and counted."""
    cluster_grp_counts_pvt = cluster_grp_counts.pivot(index='clusterid',
                                                      columns='group',
                                                      values='presence_pct')
    ...
    penetrance_sets = ortho_desc.loc[list(flatten([s.tolist(
    ) for s in set_indices])), ordered_strains].copy()
    ...
    hm = sns.heatmap(
        ...
        xticklabels=False,
        cmap=cmap)
    penetrance_sets.loc[:, ordered_strains].T.to_csv(os.path.join(
        figdir, 'AccessoryPenetrance_75.csv'))
    ...
    ) + only_g3.index.tolist()].to_csv(os.path.join(figdir,
                                                    'GroupSpecificGenes.csv'))
...
def discrete_cmap(name, colors):
    ...
    cdict = dict(red=[(0, 0, color1[0]), (1, color2[0], 1)],
...
def get_linkages(df):
    ...
    reordered_row_indices = hierarchy.dendrogram(
        row_linkage,
        no_plot=True,
        color_threshold=-np.inf)['leaves']

    col_linkage = hierarchy.linkage(
        distance.pdist(df.T),
        metric='jaccard',
        method='average')
    reordered_col_indices = hierarchy.dendrogram(
        col_linkage,
        no_plot=True,
        color_threshold=-np.inf)['leaves']
...
def parse_args(args):
    ...
    parser.add_argument('--genbank-pat',
                        default=None,
                        help='Pattern to match genbank files')
...
if __name__ == '__main__':
    ...
    main(pgap_dir, args.genbank_pat)
