...
* Installs packages to ~/.wracker/packages

* Installs "clean" virtualenvs to ~/.wracker/envs
...
def install_dist(requirement):
    ...
    with temp_dir() as tmp_dir:
        ...
        dist = dists[0]
        target = python_path_for(dist.as_requirement())
...
def local_path_for(requirement):
    return os.path.join(
        WRACKER_DIR, 'packages', requirement.key, requirement.specs[0][1])


def python_path_for(requirement):
    return os.path.join(local_path_for(requirement), 'site-packages')
...
def is_installed(requirement):
    try:
        ...
        return False
    return os.path.exists(python_path_for(requirement))


def get_dist(requirement):
    """Get the given requirement, installing it if necessary."""
    if is_installed(requirement):
        dists = list(
            pkg_resources.find_distributions(python_path_for(requirement)))
        assert len(dists) == 1
        assert str(dists[0].as_requirement()) == str(requirement), \
            "{}, {}".format(dists, requirement)
        return dists[0]
    else:
        return install_dist(requirement)
...
def handle_exec(args):
    ...
    if os.environ.get('PYTHONHOME'):
        ...
        print('Overriding existing PYTHONHOME...', file=sys.stderr)

    os.environ['PYTHONHOME'] = env_dir
...
def parse_args():
    global PYTHON
    parser = argparse.ArgumentParser(prog='wracker')
...
def get_platform_info():
    ...
    """
    return dict(
        python_version=get_python_major_minor_version(),
