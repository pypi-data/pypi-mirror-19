...
class XDCCInitiator(MessageSender):
    ...
    def dcc_send_handler(self, ctcp_arguments, connection):
        ...
        if os.path.exists(self.current_pack.get_filepath()) and not self.dcc_resume_requested:
            ...
            if position >= self.filesize:
                ...
                self.logger.log(u"File already completely downloaded. Aborting", LOG.DOWNLOAD_WAS_DONE)
                self.already_downloaded = True
                self.dcc_connection = self.dcc_connect(self.peer_address, self.peer_port, u"raw")
    ...
    def on_privnotice(self, connection, event):
        ...
        try:
            if u"You already requested that pack" in event.arguments[0]:
                self.logger.log(u"Pack already requested, waiting for next Ping", LOG.ALREADY_REQUESTED)
                self.already_requested = True
            elif u"You will have to re-send that, to the bot that transferred the file." in event.arguments[0]:
                connection.privmsg(self.current_pack.get_bot(), self.current_pack.get_request_message())
            elif u"** XDCC SEND denied, you must be on a known channel to request a pack" in event.arguments[0]:
                SentryLogger.sentry.captureMessage(u"Did not join correct channel: Bot=" + self.current_pack.get_bot())
    ...
    def on_ping(self, connection, event):
        ...
        if self.already_requested:
            log_message = u"Resending XDCC Request to " + self.current_pack.get_bot()
