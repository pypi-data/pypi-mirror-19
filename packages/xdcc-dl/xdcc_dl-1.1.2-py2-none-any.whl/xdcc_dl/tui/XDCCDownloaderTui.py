...
class XDCCDownloaderTui(object):
    ...
    def __init__(self):
        ...
        self.gpl_notice = urwid.Text(u"XDCC Downloader V " + General.version_number + u"\n"
                                     u"Copyright (C) 2016 Hermann Krumrey\n\n"
                                     u"This program comes with ABSOLUTELY NO WARRANTY; for details type `show w'.\n"
                                     u"This is free software, and you are welcome to redistribute it\n"
                                     u"under certain conditions; type `show c' for details.")
        self.message_edit = urwid.Edit(caption=u"XDCC Message: ", edit_text=u"")
        self.server_edit = urwid.Edit(caption=u"Server: ", edit_text=u"irc.rizon.net")
        self.add_pack_button = urwid.Button(u"Add Pack")

        self.search_engine_label = urwid.Text(u"Search Engines")
        self.search_engine_options = []
        for search_engine in [u"All"] + PackSearcher.get_available_pack_searchers():
            urwid.RadioButton(self.search_engine_options, search_engine)

        self.search_term_edit = urwid.Edit(caption=u"Search Term: ", edit_text=u"")
        self.search_button = urwid.Button(u"Search")
        self.search_results_label = urwid.Text(u"Search Results:")
        self.add_search_result_button = urwid.Button(u"Add Selected Packs")

        self.download_queue_label = urwid.Text(u"Download_Queue")
        self.remove_pack_button = urwid.Button(u"Remove Selected Packs")

        self.destination_edit = urwid.Edit(caption=u"Destination Directory: ", edit_text=os.getcwdu())
        self.download_button = urwid.Button(u"Download")
        self.single_progress_bar = urwid.ProgressBar(u"Progress", u"Total")
        self.total_progress_bar = urwid.ProgressBar(u"Progress", u"Total")
    ...
    def connect_widgets(self):
        ...
        :return: None
        """
        urwid.connect_signal(self.add_pack_button, u'click', self.add_pack_manually)
        urwid.connect_signal(self.search_button, u'click', self.search)
        urwid.connect_signal(self.add_search_result_button, u'click', self.add_selected_search_result_packs)
        urwid.connect_signal(self.remove_pack_button, u'click', self.remove_selected_packs)
        urwid.connect_signal(self.download_button, u'click', self.download)
    ...
    def add_pack_manually(self, button):
        u"""
    ...
    def add_selected_search_result_packs(self, button):
        u"""
    ...
    def remove_selected_packs(self, button):
        u"""
    ...
    def search(self, button):
        ...
        if not self.searching:
            ...
            search_engine = list(ifilter(lambda x: x.get_state(), self.search_engine_options))[0].get_label()
            searcher = PackSearcher() if search_engine == u"All" else PackSearcher([search_engine])
    ...
    def download(self, button):
        ...
        if not self.downloading:
            ...
            if not os.path.isdir(destination):
                self.downloading = False
                self.show_message_popup(u"The entered directory does not exist.")
                return
    ...
    def update_progress(self, single_precentage, total_percentage):
        u"""
    ...
    def spin_buttons(self, download = False, search = False):
        u"""
        ...
        def spin_thread():
            ...
            while self.downloading or self.searching:
                ...
                if self.downloading and download:
                    new_text = u"Downloading" + (self.download_button.get_label().count(u".") % 3 + 1) * u"."
                    self.download_button.set_label(new_text)

                if self.searching and search:
                    new_text = u"Searching" + (self.search_button.get_label().count(u".") % 3 + 1) * u"."
    ...
    def show_message_popup(self, message):
        ...
        urwid.connect_signal(button, u'click', lambda x: self.refresh_ui())  # pragma: no cover
