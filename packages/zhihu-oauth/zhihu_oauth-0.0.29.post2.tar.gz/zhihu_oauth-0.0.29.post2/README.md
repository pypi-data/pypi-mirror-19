...
目前版本是 0.0.29.post2，没更新的快更新一下，更新说明在[这里][changelog]。

上次更新简要说明：修复/增加了用户动态中的「赞了电子书」动作，暂时使用 `StreamingJSON` 做个兼容。另外 `Activity` 类增加了 `action_text` 作为动作描述，所有类型都可用。

上上次更新简要说明：修复/增加了用户动态中的「参加 Live」，「创建分享」，「点赞分享」三种动作，暂时使用 `StreamingJSON` 做个兼容
