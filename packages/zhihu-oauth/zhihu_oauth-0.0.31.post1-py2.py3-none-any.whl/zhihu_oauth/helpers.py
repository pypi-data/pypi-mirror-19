...
_DEFAULT_ACTIVITY_FORMATTER_ONE_ONE = {
    ...
    ActType.CREATE_PIN: '{act.action_text} {act.target.id}',
    ActType.LIKE_PIN: '{act.action_text} {act.target.id}'
                      ' by 「{act.target.author.name}」',
...
class ActivityFormatter(object):
    ...
    ..  code-block:: python
        ...
        class MyActivityFormatter(ActivityFormatter):
            ...
            def like_pin_formatter(self, act):
                content_summary = act.target.content[0].content[:20]
    ...
    def __check_type(act):
        return isinstance(act, Activity) \
               or (
                   isinstance(act, StreamingJSON)
                   and hasattr(act, 'type')
                   and act.type in ActType
               )
