...
class ActivityGenerator(BaseGenerator):
    ...
    def filter(self, cond):
        ...
        """
        from .activity import ActType

        self._condition_is_func = False
        if isinstance(cond, str) and cond in ActType:
            cond = {cond}
        elif isinstance(cond, set) and all([x in ActType for x in cond]):
            pass
        elif hasattr(cond, '__call__'):
            self._condition_is_func = True
        else:
            raise ValueError('Argument cond can only be '
                             'ActType.xxx, or a set of theme, '
                             'or a callable like func(act: Activity) -> Bool.')
    ...
    def _build_obj(self, data):
        ...
        obj = Activity(data, self._session)
