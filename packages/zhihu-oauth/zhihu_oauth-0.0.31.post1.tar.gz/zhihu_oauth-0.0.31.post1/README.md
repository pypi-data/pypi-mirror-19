...
目前版本是 0.0.31.post1，没更新的快更新一下，更新说明在[这里][changelog]。

注意： **0.0.31** 版本的 `ActivityGenerator.filter` 函数写错了，不要使用这个版本。

本次更新简要说明：对 requests 的依赖上升到 2.10.0 以上，以支持 socks 代理；增加了几个辅助函数，如 `ts2str`, `act2str`, `ActivityGenerator.filter`，和一个辅助类 `ActivityFormatter`，具体请看文档。

上次更新简要说明：更新知乎 API 和 APP 版本，修复 message.py 没有 coding shebang 造成的 Python 2.7 下可能报错的问题。
