...
目前版本是 0.0.33.post2，没更新的快更新一下，更新说明在[这里][changelog]。

0.0.32 版本因为一个疏忽 Python 2 下无法使用，0.0.33 版本未增加任何新功能只是解决了这个问题。

0.0.32 更新简要说明：增加了 `People.is_follower`，`People.is_following` 两个方法用于判断自己和目标用户的关系。增加了 `Badge` 类和 `People.badge` 属性用于获取用户徽章信息。

0.0.31 更新简要说明：对 requests 的依赖上升到 2.10.0 以上，以支持 socks 代理；增加了几个辅助函数，如 `ts2str`, `act2str`, `ActivityGenerator.filter`，和一个辅助类 `ActivityFormatter`，具体请看文档。
