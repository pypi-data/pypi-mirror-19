...
from ztfy.media.interfaces import IMediaConversionUtility, IMediaConversions, IMediaConverter, \
                                  CUSTOM_AUDIO_TYPES, CUSTOM_VIDEO_TYPES, MEDIA_CONVERSION_CMDLINE_KEY
            ...
            if IFile.providedBy(media):
                ...
                if len(conversion_result) > 0:
                    ...
                    for attempt in manager.attempts():
                        with attempt as t:
                            converted = IMediaConversions(media).addConversion(conversion_result,
                                                                               media_format,
                                                                               '.%s' % converter.format)
    ...
    def checkMediaConversion(self, media):
        ...
        if self.audio_formats and \
           (content_type.startswith('audio/') or (content_type in CUSTOM_AUDIO_TYPES)):
            requested_formats = [format for format in self.audio_formats
                                 if self.force_audio_conversion or (format != content_type)]
        elif self.video_formats and \
                (content_type.startswith('video/') or (content_type in CUSTOM_VIDEO_TYPES)):
            requested_formats = [format for format in self.video_formats
                                 if self.force_video_conversion or (format != content_type)]
        else:
            requested_formats = ()
