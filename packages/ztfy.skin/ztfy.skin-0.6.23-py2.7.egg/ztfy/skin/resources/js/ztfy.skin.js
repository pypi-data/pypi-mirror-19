/* global unescape, tinyMCE, jAlert, jConfirm */
(function($) {
	...
	$.ZTFY.skin = {
		...
		getCSS: function(url, id, devmode) {

			var getSource = function(url) {
				return url.replace(/{[^{}]*}/g, function(match) {
					return $.ZTFY.getFunctionByName(match.substr(1, match.length-2));
				});
			};

			var head = $('HEAD');
			var css = $('link[data-ztfy-id="' + id + '"]', head);
			if (css.length === 0) {
	...
	$.ZTFY.dialog = {
		...
		open: function(src, event, callback) {
			...
			/* Stop event ! */
			event = typeof(window.event) !== 'undefined' ? window.event : event;
			...
			if ($.fn.draggable) {
				dialog.draggable({
									 handle: 'DIV[id="osx-container"], H1',
									 containment: 'window'
								 });
			}
		},
			...
			if (status === 'success') {
				...
				switch (output) {
					...
					case 'OK':
						$.ZTFY.dialog.close();
						$('DIV.status').remove();
						message = result.message || $.ZTFY.I18n.DATA_UPDATED;
						$('DIV.required-info').after('<div class="status success"><div class="summary">' + message + '</div></div>');
						break;
					case 'NONE':
						$.ZTFY.dialog.close();
						$('DIV.status').remove();
						message = result.message || $.ZTFY.I18n.NO_UPDATE;
					...
					case 'RELOAD':
						...
						if (window.location.href.indexOf('?') >= 0) {
							var href;
							var ts_value = $.ZTFY.getQueryVar(window.location.href, '_');
							if (ts_value === false) {
								href = window.location.href + '&_=' + ts;
							} else {
								href = window.location.href.replace(ts_value, ts);
							}
							window.location.href = href;
						} else {
							window.location.href = window.location.href + '?_=' + ts;
						}
						break;
					case 'REDIRECT':
